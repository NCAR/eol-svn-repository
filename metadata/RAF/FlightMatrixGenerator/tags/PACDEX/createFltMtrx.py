...
class findRAF(datafile.findFiles):
    ...
    def FlightReport(self,FltNum,Proj,webpath):
	path=webpath+"Projects/"+Proj+"/Flight_Reports/"
	file = FltNum + '.html'
	...
	# Loop through all date subdirs and find all HTML files.
	...
	for i in range(len(subdirlist)):
	    ...
	    #There may be more than one file in the subdir.  
	    # We only want the html
	    #files with names of the form
	    # "report.<platform>.<date>.flight_summary.html"
	    for file in filelist:
		#print "Checking file "+subdir+"/"+file
		if re.search("^report.*.flight_summary.html",file):
		    #print "Found a file!:" + file
		    #We found a html flight summary report.  Now check it to see
		    ...
		    # open for reading only
            	    missionsummary = open(root+path+subdir+"/"+file, 'r')
		    # return a list containing all lines in the file
	    	    lines = missionsummary.readlines()	
	    	    missionsummary.close()

	    	    for line in lines:
			#print line
			match = re.search(r'.*([RrFfTt][Ff][0-9][0-9]).*',line)
			if match:
	    		    flight = match.group(1)
			    if FltNum in str.upper(flight):
			        print "Found file " + file + \
					" for flight " + flight
				#print path + file
				print "Mission Summary " + state + \
					" for flight " + FltNum
				ref = \
    ...
    def QAreport(self,Proj,FltNum,webpath):
        
	path=webpath+"Projects/"+Proj+"/"
        file = "QA.html"
        state = self.find_file(path,file)
...
ProjNum = "506"	#Read this from somewhere - where?
Year = "2007"
PI = "Jeffrey L. Stith"
plat = "Gulfstream V"
platform = string.join(["NSF/NCAR",plat,"(N677F)"]," ")
Proj = "PACDEX"
ProjLongName = "PACific Dust EXperiment"
platform_catalog = "ncar_g5"
webpath = "/net/www/docs/raf/ATD_only/"
platform_image = "/raf/pics/gvtrex.jpg"
Lpath = webpath+"Projects/"+Proj+"/"
Limage = "pics/pacdex_image2_s.jpg"
Rpath = "/net/www/docs"
...
for line in mss_retrieval_lrt:
    ...
    file = findraf.FlightReport(FltNum,Proj,webpath)
    ...
    [starttime,endtime,date] = parsenc.parseFlightTimes(startDate,flightTimes)
    ...
    col5 = findraf.QAreport(Proj,FltNum,webpath)
    cols.append(col5)
	...
	if state is "exists":
	    line = html.href('/raf/pics/'+ProjNum+'/'+FltNum+'/'+file,\
		    plotlabels[index])
            # If the new line is an os file then put it below the previous line 
	    # in the same table box else start a new box
