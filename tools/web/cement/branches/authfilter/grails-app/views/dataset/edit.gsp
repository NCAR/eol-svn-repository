        ...
        <div class="body">
           ...
           <g:form controller="dataset" method="post" >
               ...
               <div class="dialog">
				...
				<tr class='prop'><td valign='top' class='name'><label for='piContact'>Pi Contact:</label></td><td valign='top' class='value ${hasErrors(bean:dataset,field:'piContact','errors')}'><g:select optionKey="id" from="${Contact.list()}" name='piContact.id' value="${dataset?.piContact?.id}"></g:select> <span class="button"><g:link controller="contact" action="create">add contact</g:link></span></td></tr> 
				<tr class='prop'><td valign='top' class='name'><label for='metadataContact'>Metadata Contact:</label></td><td valign='top' class='value ${hasErrors(bean:dataset,field:'metadataContact','errors')}'><g:select optionKey="id" from="${Contact.list()}" name='metadataContact.id' value="${dataset?.metadataContact?.id}"></g:select> <span class="button"><g:link controller="contact" action="create">add contact</g:link></span></td></tr>
                       
				<tr class='prop'><td valign='top' class='name'><label for='datacenterContact'>Datacenter Contact:</label></td><td valign='top' class='value ${hasErrors(bean:dataset,field:'datacenterContact','errors')}'><g:select optionKey="id" from='${Contact.list()}' name='datacenterContact.id' value="${dataset?.datacenterContact?.id}"></g:select> <span class="button"><g:link controller="contact" action="create">add contact</g:link></span></td></tr>
				<!-- noSelection='${[0:"None"]}' -->
               ...
               <div class="buttons">
                     <span class="button"><g:actionSubmit value="Update" />&nbsp;this dataset &nbsp;</span><br>
                     <span class="button"><g:actionSubmit value="Create" />a new dataset</span><br>
                 	 <span class="button"><g:actionSubmit value="Delete" onclick="return warnBeforeDatasetDelete();" />&nbsp;this dataset &nbsp;</span>
               </div>
            </g:form>
        </div>
      </table>
