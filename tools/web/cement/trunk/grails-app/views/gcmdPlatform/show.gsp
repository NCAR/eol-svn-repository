  
<%@ defaultCodec="HTML" %>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
        <meta name="layout" content="ds_publish" />
        <title>Show GcmdPlatform</title>
    </head>
    <body>
        <div class="nav" align="center">
            <span class="menuButton"><g:link class="list" action="list">GcmdPlatform List</g:link></span>
            <span class="menuButton"><g:link class="create" action="create">New GcmdPlatform</g:link></span>
        </div>
        <div class="body">
  			<div class="altsubsubtitle" align="left">Show GCMD Platform</div>
			<br>
            <g:if test="${flash.message}">
            <div class="message">${flash.message}</div>
            </g:if>
            <div class="dialog">
                <table>
                    <tbody>
                    
                        <tr class="prop">
                            <td valign="top" class="name">Id:</td>
                            
                            <td valign="top" class="value">${gcmdPlatform.id}</td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name">Keyword:</td>
                            
                            <td valign="top" class="value">${gcmdPlatform.keyword}</td>
                            
                        </tr>
                    
                    </tbody>
                </table>
            </div>
            <div class="buttons">
                <g:form controller="gcmdPlatform">
                    <input type="hidden" name="id" value="${gcmdPlatform?.id}" />
                    <span class="button"><g:actionSubmit class="edit" value="Edit" /></span>
                    <span class="button"><g:actionSubmit class="delete" onclick="return confirm('Are you sure?');" value="Delete" /></span>
                </g:form>
            </div>
        </div>
    </body>
</html>
