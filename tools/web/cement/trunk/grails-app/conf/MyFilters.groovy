    ...
    authentication(controller:'*', action:'*') {
      before = {
        ...
        if (session.username) return true
          ...
          //   but submit and show are special
        if ('cdp'.equals(controllerName) && !'submit'.equals(actionName) &&
            !'submit'.equals(params?.action) )
            return true
        if ('auth'.equals(controllerName) && !'show'.equals(actionName) &&
            !'show'.equals(params?.action) )
            return true
