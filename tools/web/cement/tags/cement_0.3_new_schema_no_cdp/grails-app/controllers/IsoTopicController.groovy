...
class IsoTopicController extends BaseController {
    ...
    def delete = {
        ...
        if(isoTopic) {
            isoTopic.delete()
            flash.message = "IsoTopic ${params.id} deleted"
            redirect(action:list)
        }
        else {
            flash.message = "IsoTopic not found with id ${params.id}"
            redirect(action:list)
        }
    }
    ...
    def update = {
        ...
        if(isoTopic) {
            ...
            if(!isoTopic.hasErrors() && isoTopic.save()) {
                flash.message = "IsoTopic ${params.id} updated"
