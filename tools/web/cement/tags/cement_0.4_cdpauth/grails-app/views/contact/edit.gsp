...
<g:link controller='dataset' params='["contact.id":contact?.id]' action='create'>Add Dataset</g:link>
				...
				<tr class='prop'><td valign='top' class='name'><label for='piProjects'>PI Projects:</label></td><td valign='top' class='value ${hasErrors(bean:contact,field:'piProjects','errors')}'><ul>
    <g:each var='p' in='${contact?.piProjects?}'>
        <li><g:link controller='project' action='show' id='${p.id}'>${p}</g:link></li>
    </g:each>
</ul>
<g:link controller='project' params='["contact.id":contact?.id]' action='create'>Add Project</g:link>
</td></tr>
