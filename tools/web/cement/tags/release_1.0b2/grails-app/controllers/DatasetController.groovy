...
class DatasetController {
    ...
    def edit = { // views: from=(none), display=edit, next_to=save,update,
        ...
        if (!session.isAdmin && !DatasetAuthorizationService.check(dataset,session.username)) {
    ...
    def uploadto = { // dummy action to simplify view code
        ...
        def uri = 'http://dataportal.ucar.edu/metadata/cadis/' +
            ...
            // XXX need to prepend the context name from CdpController to this url
        redirect(url:'/browse/browse.htm?uri='+uri.encodeAsURL());
    }
