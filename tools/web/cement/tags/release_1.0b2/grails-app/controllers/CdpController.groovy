...
class CdpController {
 ...
 def publish = {
  ...
  if (!d) {
    ...
    redirect(controller:'dataset',action:'review')
    return
    }

    // ugh, security code in the middle of our logic
  if (!session.isAdmin && !DatasetAuthorizationService.check(d,session.username)) {
    flash.message = 'Not authorized to publish dataset.'
    redirect(controller:'dataset',action:'show',id:d.id)
    return
    }

  def xml = g.render(template:"/dataset/metadata", bean:d)
  //log.debug("xml = ${xml}")
  def errs = XmlService.validate(xml)
  if (errs) {
    flash.message = 'Dataset does not produce valid XML. Please edit. ' + errs
    redirect(controller:'dataset',action:'edit',id:d.id)
    return
    }
