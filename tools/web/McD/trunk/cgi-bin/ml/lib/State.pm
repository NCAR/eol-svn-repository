...
#      category_table.
#   "sort" - the method by which to sort the tables
...
sub getUrl
{
	my $self = shift;
	my $sort = shift;
	$sort = $self->getSort() if( !defined( $sort ) );
...
sub setFromParams
{
	...
	foreach my $p ("list_table_proj", "category_table_proj", "category_table_cat", "sort" )
	{
		my $in = param( $p );
		$self->{vals}->{$p} = $in if( defined( $in ) );
	}

	$self->setSort( $DEF_SORT ) if( !defined( $self->{vals}->{sort} ) );
}
