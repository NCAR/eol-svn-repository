	...
	def saveMultiple = {
		...
		theFiles.each {f -> 
			for (fcount = 0; fcount < fileListSize; fcount += 1) {
				...
				def fList = ((MultipartHttpServletRequest)request).getFiles(ffile) //fileMap.get(ffile)
                                if (fList) {
					def fspot = 0
                                        log.info("file list size = ${fList.size()}")
					for (fspot = 0; fspot < fList.size(); fspot += 1) {
						...
						if(!file?.empty) {
							...
							def userDir = retrieveDestDirectory(dataset.author.username) // change to dataset.owner.username next dev-cycle
		...
		//						}
							...
							badFiles += "\""+f.name[fcount]+"\" was empty (file size of 0)"
                                    ...
                                    log.info("failed to get fList!")
                                    log.info("ffile = ${ffile}")          
                                }
				...
				if (!thisFile.hasErrors() && thisFile.save()) {
					log.info "${thisFile} has been saved."
				} else {
