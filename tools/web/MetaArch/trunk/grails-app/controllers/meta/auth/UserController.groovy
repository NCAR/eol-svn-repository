...
//	}
	...
	def save() {
		...
		//log.info "\nUser List: user is admin? ${isAdmin}\n"
		...
		if (curUser) {
			...
			if ( isAdmin == false ) {
				...
				users = currentUserService.currentUserUsers()
			} else {
			...
			//print "${users}!!!!"
			def userTotal = users.count(users)
			
			if (!users) { flash.message = "No users currently available." }
			
			return [ userInstanceList: users, userInstanceTotal: userTotal ]
		}
	}
