...
//				println 'fieldObject: ' << fieldObject.@name.toString() << '\n'
			...
			if (fieldObject != null) {
				...
				if (fieldValue != "") {
                    ...
                    lastXmlFormValue = fieldValue.toString();
				}
			}
		...
		// Add inner contents / children here
        if (node.name().toLowerCase() == "option") {
            def selected = "";
            if (node.text().toLowerCase() == lastXmlFormValue.toLowerCase()) {
                selected = "selected=\"true\""
            }
            xmlNode += "<keyword type=\"ListItem\" " + selected + ">" + node.text()  + "</keyword>"
        }
