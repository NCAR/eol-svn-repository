...
class MetaArchTagLib {
	...
	def ifInternalContact = {attrs, body ->
		...
		if (per) {
			if (isAdmin == true) {
				...
				if (isInternalContact(per, project)) {
					 out << body()
					 return
				}
			}
		...
		if (per) {
			if (isAdmin == true) {
				...
				if (attrs.domain == 'project') {
					...
					//if (dataset.owner == per || dataset.pointOfContact == per) {
	...
	def listProjectEditors = {attrs ->
		...
		if (thisProject) {			
			...
			thisProject.members().each { User per ->
				...
				out << """<li>""" 
				if (isAdmin || (isInternalContact(curUser, thisProject) && mType != MemberType.INTERNAL_CONTACT) ) {
					out << modLink
				}
	...
	def projectBox = {attrs ->
		...
		if (project.summary != null) {
			def snippet = (project.summary).split(/\.\s/)
			sb << """
					${snippet[0]}...
					<br />
				"""
	...
	def selectMemberType = {attrs ->
		...
		def sb = new StringBuilder()
		def per = currentUserService.lookupUser()
		def project = Project.get(attrs.project)
		def isAdmin = currentUserService.isAdmin()
		def isIC = isInternalContact(per, project)
		
		sb << '<select id="memberType-list" name="memberType" class="many-to-one"> \n'
		
		memberTypeList.each { mt ->
			if ( (isAdmin == true && isIC == false) ||
				 (isAdmin == true && isIC == true) ||  
				 (isAdmin == false && isIC == true && mt.name() != 'INTERNAL_CONTACT') ) {
	...
	def selectProject = { attrs ->
		// How can this tag be modified to restrict the available owners to currently logged in user (or all
		// if an admin)?
