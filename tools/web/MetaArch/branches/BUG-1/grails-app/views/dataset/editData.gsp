...
<html>
	<head>
		...
		<title><g:message code="default.create.label" args="[entityName]" /> - Edit Data / Documentation File List</title>
...
<body>
	...
	<div id="edit-dataset-data" class="content scaffold-show" role="main">
		<g:if test="${datasetInstance?.title}">
			<h1>
				<span aria-labelledby="title-label"><g:fieldValue bean="${datasetInstance}" field="title"/>- Edit Data / Documentation File List</span>
			</h1>
		</g:if>
...
<%--		</p>--%>
		...
		<g:if test="${mode != null && mode == 'single'}">
		...
		<g:else>
			<g:form controller="file" action="saveBatch" onSubmit="validateFiles();" enctype="multipart/form-data">
				<fieldset class="form">
				<g:render template="/file/files" model="['datasetInstance':datasetInstance]" />
				</fieldset>
				<br />
				<fieldset class="buttons">
					<g:hiddenField name="dataset.id" value="${datasetInstance?.id}" />
					<g:submitButton class="dataSave" before="validateFiles();" name="saveBatch" value="Save Changes" />
					<g:link class="show" action="show" id="${datasetInstance?.id}">Go Back</g:link>
				</fieldset>
			</g:form>
			<!-- Render the file template as hidden so they can be cloned -->
