...
<%--						<g:sortableColumn property="password" title="${message(code: 'user.password.label', default: 'Password')}" />--%>
				...
				<g:each in="${userInstanceList}" status="i" var="userInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}<g:if test="${(userInstance.getAuthorities()).contains(dmgRole)}"> tr-dmg</g:if>">
						...
						<sec:ifAnyGranted roles="ROLE_ADMIN,ROLE_DEVELOP">
						<g:if test="${(userInstance.getAuthorities()).contains(dmgRole)}"> 
						<td><g:link action="dmg" class="dmg-link del" id="${userInstance.id}" params="[toDo: 'del']">- DMG</g:link></td>
						</g:if>
						<g:else>
						<td><g:link action="dmg" class="dmg-link add" id="${userInstance.id}" params="[toDo: 'add']">+ DMG</g:link></td>
						</g:else>
