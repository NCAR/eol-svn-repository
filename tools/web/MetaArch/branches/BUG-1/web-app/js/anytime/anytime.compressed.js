...
AnyTime.Converter = function(options)
{
	      ...
	      else
	      {
	        ...
	        switch ( ch )
	        {
	          ...
	          case '@': // timezone offset label
		        t = ( _offAl != Number.MIN_VALUE ) ? _offAl :
		        	( _offF == Number.MIN_VALUE ) ? (0-d.getTimezoneOffset()) : _offF;
		        if ( AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]] && AnyTime.utcLabelMinutes[t] )
		        {
				  for (var utcIterator = 0; utcIterator < AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]].length; utcIterator++) {
					str += AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]][utcIterator];
					if (utcIterator < AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]].length-1) {
					  str += ' / ';
					}
				  }
				  
				  /*
		          if ( ( _offFSI > 0 ) && ( _offFSI < AnyTime.utcLabelMinutes[t].length ) ) {
		            str += AnyTime.utcLabelMinutes[t][_offFSI];
		          } else {
		            str += AnyTime.utcLabelMinutes[t][0];
				  }
				  */
	...
	//-------------------------------------------------------------------------
	//  AnyTime.Converter.tzformat() returns a String containing the value
	//  of a specified Date object, using the format string passed to
	//  AnyTime.Converter() and the index of the timezone offset.
	//
	//  Method parameters:
	//
	//    date - the Date object to be converted
	//    tzIndex - the index of the AnyTime.utcLabelMinutes[_offAl] array
	//-------------------------------------------------------------------------
	
	this.tzformat = function( date, tzIndex )
	{
		var d = new Date(date.getTime());
		if ( ( _offAl == Number.MIN_VALUE ) && ( _offF != Number.MIN_VALUE ) )
		  d.setTime( ( d.getTime() + (d.getTimezoneOffset()*60000) ) + (_offF*60000) );
	      ...
	      else
	      {
	        ...
	        switch ( ch )
	        {
	          ...
	          case '@': // timezone offset label
		        t = ( _offAl != Number.MIN_VALUE ) ? _offAl :
		        	( _offF == Number.MIN_VALUE ) ? (0-d.getTimezoneOffset()) : _offF;
		        if ( AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]] && AnyTime.utcLabelMinutes[t] && AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]][tzIndex] )
		        {
				  str += AnyTime.utcLabel[AnyTime.utcLabelMinutes[t]][tzIndex];
		          break;
		        }
		        str += 'UTC';
		        ch = ':'; // drop through for offset formatting
	          ...
	          case '@': // timezone label
	        	...
	        	if ( AnyTime.utcLabel )
	        	{
                  ...
                  if ( ! Array.prototype[tzOff] ) // prototype.js compatibility issue
		              {
		            	  for ( i = 0 ; i < AnyTime.utcLabelMinutes[AnyTime.utcLabel[tzOff]].length ; i++ )
  		            	{
	  	            		sub = AnyTime.utcLabelMinutes[AnyTime.utcLabel[tzOff]][i];
		              		sublen = sub.length;
...
AnyTime.picker = function( id, options )
{
	...
	__pickers[id] =
	{
		...
		oIdx: 0,            // UTC offset selection index (within utcLabel)
      ...
      this.id = 'AnyTime--'+id.replace(/[^-_.A-Za-z0-9]/g,'--AnyTime--');
		...
		askOffset: function( event )
		{
		    if ( ! this.oDiv )
		    {
		      ...
		      if ( AnyTime.utcLabel )
			      for ( var o in AnyTime.utcLabel )
			    	  if ( AnyTime.utcLabel[o] )
			    	  {
			    		var oi = (parseInt(o.substring(1,3))*60 + parseInt(o.substring(3,5)));
			    		if (o.substring(0,1) == "-") { oi*=-1; }
			    		
				        this.oConv.setUtcFormatOffsetAlleged(oi);
				        for ( var i = 0; i < AnyTime.utcLabel[o].length; i++ )
				        {
				          this.oConv.setUtcFormatOffsetSubIndex(i);
				          last = this.tzbtn( cont, this.oConv.tzformat(this.time, i), this.newOPos, ['off-off'], o, i );
				          last[0].AnyTime_offMin = oi;
		...
		//---------------------------------------------------------------------
		//  .tzbtn() is called by AnyTime.picker() to create a <div> element
		//  containing an <a> element.  The elements are given appropriate
		//  classes based on the specified "classes" (an array of strings).
		//	The specified "text" and "title" are used for the <a> element.
		//	The "handler" is bound to click events for the <div>, which will
		...
		tzbtn: function( parent, text, handler, classes, title, tzIndex )
		{
			var tagName = ( (parent[0].nodeName.toLowerCase()=='ul')?'li':'td'); 
			var div$ = '<' + tagName + ' id="tzidx' + tzIndex + '" ' + 
			  				' class="AnyTime-btn';
			for ( var i = 0 ; i < classes.length ; i++ )
				div$ += ' AnyTime-' + classes[i] + '-btn';
			var div = $( div$ + ' ui-state-default">' + text + '</' + tagName + '>' );
      ...
      var _this = this;
        ...
        cmpHi.setHours( this.time.getHours() );
			...
			if ( this.oConv )
		    {
			    ...
			    var tzs = this.oConv.tzformat(this.time, this.offSI);
				this.oCur.html( tzs );
		    }
		...
		updODiv: function(fBtn)
		{
            ...
            this.conv.setUtcFormatOffsetAlleged(this.offMin);
            this.conv.setUtcFormatOffsetSubIndex(this.offSI);
		    this.inp.val(this.conv.tzformat(this.time, this.offSI)).change(); // Changed for revised timezone data structures
		    this.upd(fBtn);
