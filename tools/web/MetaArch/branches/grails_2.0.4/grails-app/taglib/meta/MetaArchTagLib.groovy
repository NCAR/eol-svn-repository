	...
	private String readXmlChild(GPathResult xmlChild) {
		...
		if (xmlChild.@type != null) {
			...
			if (elemOutput != '') {
				...
				sb << "\t</fieldset> <!-- end of children -->\n"
				def isCloneable = "", minClone = "", maxClone = ""
				if (xmlChild.@cloneable) {
					String cloneableField = xmlChild.@cloneable
					if (cloneableField.contains(",")) {
						(isCloneable, minClone, maxClone) = cloneableField.tokenize(',')
					} else {
						isCloneable = cloneableField
						minClone = "1";
						maxClone = "1";
					}
				}
				...
				if (isCloneable == "true") {
					sb << "\t<div class=\"fieldcontain\">\n"
					sb << "\t\t<input type=\"button\" value=\"Clone\" onclick=\"initGlobalVar(\'" + xmlChild.name() + "\', 1); addClone(\'" + xmlChild.name() + "-children\', \'" + xmlChild.name()  + "\', " + maxClone.trim() + ")\"/>\n"
					sb << "\t\t<input type=\"button\" value=\"Remove Last Clone\" onclick=\"removeClone(\'" + xmlChild.name() + "-children\', \'" + xmlChild.name() + "\', " + minClone.trim() + ")\"/>\n"
					sb << "\t</div>\n"
				}
				norecurse = true // Don't call children again since they've been called once
			}
