...
<%--			<h1><g:message code="default.show.label" args="[entityName]" /></h1>--%>
			<g:if test="${datasetInstance?.title}">
				<h1>
					...
					<span class="property-label" style="float: right; font-style: italic; font-size: 16px;">created by <g:link controller="user" action="show" id="${datasetInstance?.author?.id}">${datasetInstance?.author?.realname?.encodeAsHTML()}</g:link></span>
					<br />
					</sec:ifAnyGranted>
				</h1>
			</g:if>
			...
			<g:if test="${datasetInstance?.author && datasetInstance?.pointOfContact}">
				...
				<div class="fieldcontain">
					...
					<br />
					<pre style='white-space: pre-wrap; font-family: "HelveticaNeue-Light","Helvetica Neue Light","Helvetica Neue",Helvetica,Arial,"Lucida Grande",sans-serif;'><g:fieldValue bean="${datasetInstance}" field="summary"/></pre>
				</div>
				</g:if>
...
<%--				<g:if test="${datasetInstance?.author}">--%>
<%--				<li class="fieldcontain">--%>
<%--					<span id="author-label" class="property-label"><g:message code="dataset.author.label" default="Author" /></span>--%>
<%--					--%>
<%--						<span class="property-value" aria-labelledby="author-label"><g:link controller="user" action="show" id="${datasetInstance?.author?.id}">${datasetInstance?.author?.encodeAsHTML()}</g:link></span>--%>
<%--					--%>
<%--				</li>--%>
<%--				</g:if>--%>
				...
				<g:ifPermitted domain="dataset" id="${datasetInstance?.id}">
					<span style="position: relative; top: 5px;">
						<g:link class="dataSave" action="editData" id="${datasetInstance?.id}">Upload Files</g:link>&nbsp;&nbsp;&nbsp;
