...
<%--	<g:hiddenField name="datasetCategories" value="${datasetInstance?.categories()}" />--%>
	<g:select id="categoryList" name="categoryId" size="7" from="${meta.Category.list().sort{it.name.toLowerCase()}}" optionKey="id" multiple="true" value="${datasetInstance?.categories()}" class="many-to-many"/>
</div>
...
<%--	<g:hiddenField name="datasetPlatforms" value="${datasetInstance?.platforms()}" />--%>
	<g:select id="platformList" name="platformId" size="10" from="${meta.Platform.list().sort{it.name.toLowerCase()}}" optionKey="id" multiple="true" value="${datasetInstance?.platforms()}" class="many-to-many"/>
</div>
