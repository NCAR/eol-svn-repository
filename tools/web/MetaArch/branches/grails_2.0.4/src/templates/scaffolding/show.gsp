...
<html>
	...
	<body>
		...
		<div id="show-${domainClass.propertyName}" class="content scaffold-show" role="main">
			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version' << 'datasets' << 'projectMembers'
				allowedNames = domainClass.persistentProperties*.name << 'dateCreated' << 'lastUpdated'
				props = domainClass.properties.findAll { allowedNames.contains(it.name) && !excludedProps.contains(it.name) }
				Collections.sort(props, comparator.constructors[0].newInstance([domainClass] as Object[]))
				props.each { p ->
					if (p.name == 'minLat') {
						p.naturalName = 'Minimum (South) Latitude'
					} else if (p.name == 'maxLat') {
						p.naturalName = 'Maximum (North) Latitude'
					} else if (p.name == 'minLon') {
						p.naturalName = 'Minimum (West) Longitude'
					} else if (p.name == 'maxLon') {
						p.naturalName = 'Maximum (East) Longitude'
					}
			%>
