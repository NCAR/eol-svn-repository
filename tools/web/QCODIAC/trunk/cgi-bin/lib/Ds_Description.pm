...
#  - Eric Dattore
#
#   Add doi, gcmd keywords, version, and instrument to the display - 10/2020
#   J. Scannell
#
...
sub longDesc {
    return cr("       <table border=0 cellpadding=0 cellspacing=0><tr><td>",
        ...
        "               <dd><li>Description Fields (Dataset ID, Title, etc.)</li></dd>",
        "               <dd><li>Contact Fields (Primary and Secondary Contact Associations)</li></dd>",
        "               <dd><li>Data Availability (Notes, Browsable, etc.)</li></dd>",
        "               <dd><li>Listing of Linked Project(s)</li></dd>",
        "       </td></tr></table>" );
}
...
sub queryResult {
    ...
    my @list = split( /,[\s\x20]*/, $list );
    my $result;
    my %contacts = getContacts();

    foreach my $dataset_id (@list) {
    my $eq = EQuery->new();
    $eq->query( "SELECT *,dataset.title as title,frequency.name as frequency,dataset.row_create_time as create_time,dataset.row_create_contact_id as create_contact_id,dataset.row_revise_time as update_time,dataset.row_revise_contact_id as update_contact_id FROM dataset,frequency WHERE dataset.frequency_id=frequency.id AND archive_ident=" . $eq->quote( $dataset_id ) );
    my %ds = $eq->getRow();
    ...
    if( ! %ds ) {
        ...
        my $doi = getDoiField($dataset_id);
        my $gcmdkeywords = getGcmdKeywords($dataset_id);
        my $instruments = getInstrumentField($dataset_id);
        my $versions = getVersionField($dataset_id);
        ...
        $result = $result . cr("<table border=0 width=100% cellpadding=1 cellspacing=1 bgcolor=#e9e9e9>",
            ...
            "<td width=15%>",
            "<a target=_blank href=https://data.eol.ucar.edu/codiac/dss/id?$dataset_id>$dataset_id</a>",
            "</td>",
            "<td width=15% align=left>",
            "<b>Hidden:" . &nbsp(2) . ($ds{visible} ? getNo() : getYes()) . "</b>",
            "</td>",

            "<td align=right nowrap width=5%><b>Grant Code: </b></td>",
            "<td align=left width=10%>$ds{grant_code}</td>",
            ...
            "<td align=right width=15% nowrap><b>Revision Author: </b></td>",
            "<td>$contacts{$ds{update_contact_id}}</td>",
            "<td align=right width=15% nowrap><b>Creation Author: </b></td>",
            "<td>$contacts{$ds{create_contact_id}}</td>",
            "</tr>",
            "<tr>",
            "<td>&nbsp;</td>",
            "<td align=right width=15% nowrap><b>Date last revised: </b></td>",
            "<td>$ds{update_time}</td>",
            "<td align=right width=15% nowrap><b>Date created: </b></td>",
...
sub getDocumentField {
    ...
    while( (my %row = $eq->getRow()) ) {
        if ($row{directory} ne "" && $row{filename} ne "") {
            my $url = sprintf("https://data.eol.ucar.edu/datafile/nph-get/%s/%s",$dataset_id,$row{filename});
...
sub getDoiField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query("SELECT doi.* FROM doi, dataset where doi.dataset_id=dataset.id AND dataset.archive_ident=" . $eq->quote($dataset_id));
    ...
    $ret = cr("<table width=100% border=0 cellpadding=4 cellspacing=1>",
        "<tr>",
        "<td width=8% nowrap><b>DOI: </b></td>" );
    if (scalar(@doi) != 0) {
       $ret = $ret . cr("<td><a href=\"https://doi.org/", substr($doi[0],4), "\" target=\"_blank\">", $doi[0], "</a></td>");
    } else {
       $ret = $ret . cr("<td>", "<b>none</b>", "</td>");
    }
    $ret = $ret . cr ("</table>" );
...
sub getGcmdKeywords {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query("SELECT gcmd_science_keyword.*, dataset_gcmd_science_keyword.* FROM gcmd_science_keyword, dataset_gcmd_science_keyword, dataset where dataset_gcmd_science_keyword.gcmd_science_keyword_id=gcmd_science_keyword.id and dataset_gcmd_science_keyword.dataset_id=dataset.id AND dataset.archive_ident=" . $eq->quote($dataset_id));

    my @gcmd_keyword;
    my $ret;
    while((my %row = $eq->getRow())) {
       push(@gcmd_keyword, $row{name});
    }

    $ret = cr("<table width=100% border=0 cellpadding=4 cellspacing=1>",
        "<tr>",
        "<td width=8% nowrap><b>GCMD Keywords: </b></td>" );
    if (scalar(@gcmd_keyword) != 0) {
       $ret = $ret . cr("<td>", $gcmd_keyword[0], "</td>");
    } else {
       $ret = $ret . cr("<td>", "<b>none</b>", "</td>");
    }
    for( my $x = 1; $x < scalar(@gcmd_keyword); $x++ ){
        $ret = $ret . cr("<tr>",
            "<td width=8%>&nbsp;</td>",
            "<td>$gcmd_keyword[$x]</td></tr>" );
    }
    $ret = $ret . cr ("</table>" );
...
sub getInstrumentField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query("SELECT instrument.*, dataset_instrument.* FROM instrument, dataset_instrument, dataset where dataset_instrument.instrument_id=instrument.id and dataset_instrument.dataset_id=dataset.id AND dataset.archive_ident=" . $eq->quote($dataset_id));

    my @instruments;
    my $ret;
    while((my %row = $eq->getRow())) {
       push(@instruments, $row{name});
    }

    $ret = cr("<table width=100% border=0 cellpadding=4 cellspacing=1>",
        "<tr>",
        "<td width=8% nowrap><b>Instrument: </b></td>" );
    if (scalar(@instruments) != 0) {
       $ret = $ret . cr("<td>", $instruments[0], "</td>");
    } else {
       $ret = $ret . cr("<td>", "<b>none</b>", "</td>");
    }
    for( my $x = 1; $x < scalar(@instruments); $x++ ){
        $ret = $ret . cr("<tr>",
            "<td width=8%>&nbsp;</td>",
            "<td>$instruments[$x]</td></tr>" );
    }
    $ret = $ret . cr ("</table>" );
...
sub getVersionField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query("SELECT dataset_version.* FROM dataset_version, dataset where dataset_version.dataset_id=dataset.id AND dataset.archive_ident=" . $eq->quote($dataset_id) . "order by version_number");

    my @versions;
    my $ret;
    while((my %row = $eq->getRow())) {
       push(@versions, sprintf("%s - %s - %s \n%s", $row{version_number}, $row{quality}, $row{publish_time}, $row{description}));
    }

    $ret = cr("<table width=100% border=0 cellpadding=4 cellspacing=1>",
        "<tr>",
        "<td width=8% nowrap><b>Versions: </b></td>" );
    if (scalar(@versions) != 0) {
       $ret = $ret . cr("<td>", $versions[0], "</td>");
    } else {
       $ret = $ret . cr("<td>", "<b>none</b>", "</td>");
    }
    for( my $x = 1; $x < scalar(@versions); $x++ ){
        $ret = $ret . cr("<tr>",
            "<td width=8%>&nbsp;</td>",
            "<td>$versions[$x]</td></tr>" );
    }
