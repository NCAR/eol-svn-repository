...
#   $connction (optional) - Specify a connection other than the default zith9 database
#   $user (optional) - specify the username of that particular database user
#   $pass (optional) - specify the password of the particular database user
# Returns:
...
sub query
{
    ...
    my ($sql,$connection,$user,$pass) = @_;
    my $database;
    if ($connection eq "") {
        $database = DBI->connect("DBI:mysql:database=zith9;host=emdac.eol.ucar.edu",
            "zithview","look-999", {RaiseError => 1}) or
        die("Unable to establish a datatbase connection.");
    } else {
        $database = DBI->connect($connection, $user, $pass, {RaiseError => 1}) or die ("Unable to establish a database connection.");
    }
#
# Add command to set group_concat_max_len to a longer length.  Needed for the Utils::getIgnoredEmailAddresses() routine.
#  
    my $sth = $database->prepare("set session group_concat_max_len = 5000;");
