...
#  - Eric Dattore
#
#   Fix query, fix hidden field marking, fix error message, change to
#      "only one" dataset_id entry, fix comments, and clean up code - 10/2020
#   J. Scannell
#
#-----------------------------------------------------------------------------#
#                                Subroutines                                  #
#-----------------------------------------------------------------------------#
# new()
#   * Creates and instance of the Tape_Listing Tool
#
# shortDesc()
...
#   * Expected Parameters:
#      -dataset_id - can be either a single dataset_id or comma-delimited list
...
sub longDesc {
    return cr("<table border=0 cellpadding=0 cellspacing=0><tr><td>",
        "Displays the contents of the tape table for one of more datasets, output includes: ",
        "<dd><li>Primary ID Number</li></dd>",
        "<dd><li>Begin/End Date</li></dd>",
        "<dd><li>Primary Media Code</li></dd>",
...
# This query only works with one dataset_id at a time.  This should be fixed.
        "<br><b>Enter <b style=\"color:red;\">only one </b> dataset_id number (e.g. 77.018):</b>" ,
        ...
        "<input type=checkbox name=fields value=size_kb>Data Size<br>" . &nbsp(3),
...
sub queryResult {
    ...
    foreach my $id (@list) {
        ...
        my $sql = "SELECT tape.*,format.full_name,original.name as primary_media_format,backup.name as backup_media_format FROM tape,format,medium as original,medium as backup WHERE format_id=format.id AND original.id=primary_medium_id AND backup.id=backup_medium_id AND dataset_id=(SELECT id from dataset WHERE archive_ident=".$query->quote($dataset_id).")";
