...
#		    $hidden = "<font color=red>Y</font>" if($ds->{hide});
...
#					   "<td bgcolor=white><a href=supervisor?results=ds_description&dataset_id=$ds->{dataset_id}>$ds->{name}</a></td>",
#					   "<td bgcolor=white><a href=/cgi-bin/codiac/dss?$ds->{dataset_id} target=_blank>$ds->{dataset_id}</a></td>",
	                                  "<td bgcolor=white><a href=supervisor?results=ds_description&dataset_id=$ds->{archive_ident}>$ds->{title}</a></td>",
	                                  "<td bgcolor=white><a href=http://data.eol.ucar.edu/codiac/dss/id?$ds->{archive_ident} target=_blank>$ds->{archive_ident}</a></td>",
					   "</tr>" );
			$ds_cnt = $ds_cnt + 1;
		}
    }	
...
sub getAllProjects {
    my $eq = EQuery->new();
#    $eq->query( "SELECT project_id FROM project WHERE project_id NOT LIKE \'COMET_CASE%\' ORDER BY project_id" );
...
sub getAllDatasets {
    ...
    my $eq = EQuery->new();
#    my $sql = "SELECT * FROM dataset JOIN dataset_project ON dataset.dataset_id=dataset_project.dataset_id WHERE dataset_project.project_id=".$eq->quote( $project )." ORDER BY dataset.dataset_id";
