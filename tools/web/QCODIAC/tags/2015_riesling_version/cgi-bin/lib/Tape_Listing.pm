...
#	my $sql = "SELECT tape.*,format.full_name,original.medium_name as primary_media_format,backup.medium_name as backup_media_format FROM tape,format,medium as original,medium as backup WHERE format_id=format_id AND original.medium_id=primary_medium_id AND backup.medium_id=backup_medium_id AND dataset_id=".$query->quote($dataset_id);
	my $sql = "SELECT tape.*,format.full_name,original.name as primary_media_format,backup.name as backup_media_format FROM tape,format,medium as original,medium as backup WHERE format_id=format_id AND original.id=primary_medium_id AND backup.id=backup_medium_id AND dataset_id=(SELECT id from dataset WHERE archive_ident=".$query->quote($dataset_id).")";
