...
sub queryResult {	
    my $self = shift;
    my $params = shift;
    my $title = "Category Types";                         

    my $sort = ( defined( $params ) && $params->exists( "sort" ) )? $params->get( "sort" ) : "category_id";
    
    my $query = EQuery->new();
    
#    my $sql = "SELECT category.*,parent.name as parent FROM category LEFT JOIN category as parent ON category.parent_category_id=parent.category_id ORDER BY \'$sort\'";
##    my $sql = "SELECT category.*,parent.name as parent FROM category LEFT JOIN category as parent ON category.parent_category_id=parent.id ORDER BY \'$sort\'";
    my $sql = "SELECT category.id as category_id,category.*,parent.name as parent FROM category LEFT JOIN category as parent ON category.parent_category_id=parent.id ORDER BY \'$sort\'";
    ...
    my $result = cr("<table border=0 cellpadding=3 cellspacing=2 bgcolor=#e9e9e9 width=$table_width>",
		    ...
		    "<a href=supervisor?qform=$self->{identifier}&sort=category_id><b>Category Id</b></a></td>",
		    "<td align=center bgcolor=#e9e9e9 id=sort_link>",
		    "<a href=supervisor?qform=$self->{identifier}&sort=name><b>Name</b></a></td>",
		    "<td align=center bgcolor=#e9e9e9 id=sort_link>",
		    "<a href=supervisor?qform=$self->{identifier}&sort=parent><b>Parent Category</b></a></td>",
		    "</tr>" );
    
    while( (my %row = $query->getRow() ) ) {
	$result = $result . cr("<tr>",
			       "<td bgcolor=white>$row{category_id}</td>",
			       "<td bgcolor=white nowrap>$row{name}</td>",
			       "<td bgcolor=white nowrap>$row{parent}</td>",
			       "</tr>" );
    }
