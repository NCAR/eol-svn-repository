...
sub longDesc {
    return cr( 
	      ...
	      "				<dd><li>Number of Datasets</li></dd>",
	      "				<dd><li>Number of Files (On-line / Off-line)</li></dd>",
	      "				<dd><li>Total Size of files in bytes (On-line / Off-Line)</li></dd>",
	      "		</td></tr></table>" );
}
...
#    my $sql = "SELECT DISTINCT project_id FROM project WHERE project_id NOT LIKE \'COMET_CASE%\' ORDER BY project_id";
    my $sql = "SELECT DISTINCT name as project_id FROM project WHERE name NOT LIKE \'COMET_CASE%\' ORDER BY project_id";
    $query->query( $sql );
...
#	my $sql = "SELECT COUNT(*) as dataset_count FROM dataset_project WHERE project_id=".$sets->quote($proj);
       my $sql = "SELECT COUNT(*) as dataset_count FROM dataset_project,project WHERE dataset_project.project_id=project.id AND project.name=".$sets->quote($proj);
...
#	$sql = "SELECT COUNT(*) as file_count,SUM(size_kb) as file_size,purpose FROM file WHERE dataset_id IN (SELECT dataset_id FROM dataset_project WHERE project_id=".$sets->quote($proj).") GROUP BY purpose";
	$sql = "SELECT COUNT(*) as file_count,SUM(size_kb) as file_size,purpose FROM file WHERE dataset_id IN (SELECT dataset_id FROM dataset_project,project WHERE dataset_project.project_id=project.id AND project.name=".$sets->quote($proj).") GROUP BY purpose";
...
#	$sql = "SELECT SUM(num_files) as file_count,SUM(size_kb) as file_size FROM tape WHERE dataset_id IN (SELECT dataset_id FROM dataset_project WHERE project_id=".$sets->quote($proj).")";
	$sql = "SELECT SUM(num_files) as file_count,SUM(size_kb) as file_size FROM tape WHERE dataset_id IN (SELECT dataset_id FROM dataset_project,project WHERE dataset_project.project_id=project.id AND project.name=".$sets->quote($proj).")";
