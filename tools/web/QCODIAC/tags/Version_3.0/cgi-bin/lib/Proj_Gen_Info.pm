...
sub queryResult {
	...
	foreach my $proj (@projects) {
	    ...
	    $result = $result . cr(
				   ...
				   "<tr><td bgcolor=white colspan=3 id=tool_link><a target=_blank href=http://data.eol.ucar.edu/cgi-bin/codiac/projs?$proj><b>$proj</b></a></td></tr>",
				   "<tr bgcolor=white>",
				   "<td align=right width=8%><b>Name: </b></td>",
#				   "<td>" . &nbsp(3) . "$hash{full_name}</td>",
                                  "<td>" . &nbsp(3) . "$hash{title}</td>",
				   "<td><b>Hidden: </b>",
#				   $hash{hide} ? "<font color=red>Yes</font>" : "<font color=blue>No</font>",
                                  $hash{visible} ? "<font color=blue>No</font>" : "<font color=red>Yes</font>",
				   "</td>",
				   "</tr>",
				   "<tr bgcolor=white>",
...
#				   "<b>Longitude: </b>" . $hash{minlon} . " to " . $hash{maxlon},
                                  "<b>Latitude: </b>" . $hash{minimum_latitude} . " to " . $hash{maximum_latitude}, &nbsp(3),
                                  "<b>Longitude: </b>" . $hash{minimum_longitude} . " to " . $hash{maximum_longitude},
				   "</tr>",		
				   "<tr bgcolor=white>",
...
sub getProjectHash {
    ...
    my $eq = EQuery->new();
#    $eq->query( "SELECT project.*,parent.project_id as parent,contact.person_name FROM project LEFT JOIN project AS parent ON project.parent_project_id=parent.project_id LEFT JOIN contact ON project.internal_contact_id=contact.contact_id WHERE project.project_id=" . $eq->quote($proj) );
    $eq->query( "SELECT project.*,parent.name as parent,contact.person_name FROM project LEFT JOIN project AS parent ON project.parent_project_id=parent.id LEFT JOIN contact ON project.internal_contact_id=contact.id WHERE project.name=" . $eq->quote($proj) );
    my %row = $eq->getRow();	
...
#    $hash{maxlon} = $row{maxlon};
    $hash{minimum_latitude} = $row{minimum_latitude};
    $hash{maximum_latitude} = $row{maximum_latitude};
    $hash{minimum_longitude} = $row{minimum_longitude};
    $hash{maximum_longitude} = $row{maximum_longitude};
