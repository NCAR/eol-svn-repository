...
#    $form = $form . cr("                   <input type=checkbox checked name=fields value=dataset_id>$field_titles{\"dataset_id\"}", &nbsp(3),"" );
    $form = $form . cr("                   <input type=checkbox checked name=fields value=archive_ident>$field_titles{\"dataset_id\"}", &nbsp(3),"" );
...
#	my $sql = "SELECT target.full_name as target_name,source.full_name as source_name,codiac_web_orders.* from format as target,format as source,codiac_web_orders where source.format_id=source_format_id and target.format_id=target_format_id and dataset_id=".$eq->quote($dataset_id).$dropJoss." ORDER BY ".($params->exists("sort_who_order") ? $params->get("sort_who_order") : "delivery_date");
	my $sql = "SELECT target.full_name as target_name,source.full_name as source_name,codiac_web_order_log.* FROM codiac_web_order_log LEFT JOIN format as source ON source.id=source_format_id LEFT JOIN format as target ON target.id=target_format_id WHERE archive_ident=".$eq->quote($dataset_id).$dropJoss." ORDER BY ".($params->exists("sort_who_order") ? $params->get("sort_who_order") : "delivery_date");
