...
{	
	...
	my $sort = ( defined( $params ) && $params->exists( "sort" ) )? $params->get( "sort" ) : "id";

	my $query = EQuery->new( "data_dict_db" );

	my $sql = "SELECT * FROM platforms ORDER BY \'$sort\'";

	$query->query( $sql );

	my $result = cr(
		"<table border=0 cellpadding=3 cellspacing=2 bgcolor=#e9e9e9 width=$table_width>",
			"<tr>",
				"<td align=center bgcolor=#e9e9e9 id=sort_link>", 
					"<a href=supervisor?qform=$self->{identifier}&sort=id><b>Id</b></a></td>",
				"<td align=center bgcolor=#e9e9e9 id=sort_link>",
					"<a href=supervisor?qform=$self->{identifier}&sort=name><b>Name</b></a></td>",
				"<td align=center bgcolor=#e9e9e9 id=sort_link>",
					"<a href=supervisor?qform=$self->{identifier}&sort=abbrev><b>Abbrev</b></a></td>",
				"<td align=center bgcolor=#e9e9e9 id=sort_link>",
					"<a href=supervisor?qform=$self->{identifier}&sort=desc><b>Description</b></a></td>",
			"</tr>" );
	...
	while( (my %row = $query->getRow() ) )
	{
		$result = $result . cr(
			"<tr>",
				...
				"<td bgcolor=white nowrap>$row{abbrev}</td>",
