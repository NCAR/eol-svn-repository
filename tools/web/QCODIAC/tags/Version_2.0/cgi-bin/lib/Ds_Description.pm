...
sub queryResult {
	...
	my $eq = EQuery->new();
	$eq->query( "SELECT *,dataset.name as title,frequency.name as frequency,dataset.row_revise_time as update_time FROM dataset,frequency WHERE frequency_id=frequency_id_fk AND dataset_id=" . $eq->quote( $dataset_id ) );
	my %ds = $eq->getRow();
	...
	if( ! %ds ) {
	    ...
	    $result = $result . cr("<table border=0 width=100% cellpadding=1 cellspacing=1 bgcolor=#e9e9e9>",
				   ...
				   "<td width=15%>",
				   "<a target=_blank href=http://www.joss.ucar.edu/cgi-bin/codiac/dss?$dataset_id>$dataset_id</a>",
				   "</td>",
				   ...
				   "<tr><td><b>Displayed Contact:</b></td><td>$contacts{$ds{displayed_contact_id_fk}}</td></tr>",
				   "<tr><td><b>Source Contact:</b></td><td>$contacts{$ds{source_contact_id_fk}}</td></tr>",
				   "<tr><td><b>Internal Contact:</b></td><td>$contacts{$ds{internal_contact_id_fk}}</td></tr>",
				   "</table></td>",
				   "</table>",
				   "</tr>",
				   ...
				   "<tr bgcolor=white><td>",
				   &nbsp(30),
				   sprintf("<b>Password Protected: </b>%s",$ds{authentication_reqd} ? getYes() : getNo()), &nbsp(2),
				   ...
				   "<td>$contacts{$ds{row_revise_contact_id_fk}}</td>",
				   "</tr>",
				   "<tr>",
				   "<td>&nbsp;</td>",
...
sub getPiField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query( "SELECT * FROM contact,dataset_pi where contact_id=pi_contact_id_fk and dataset_id_fk=".$eq->quote($dataset_id) );
...
sub getPlatformField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query( "SELECT * FROM platform,dataset_platform where platform_id=platform_id_fk and dataset_id_fk=".$eq->quote($dataset_id) );
...
sub getProjectField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query( "SELECT project_id_fk FROM dataset_project where dataset_id_fk=" . $eq->quote( $dataset_id ) );
    my @projs;
    my $ret;
    while( (my %row = $eq->getRow()) ) {
	push(@projs,$row{project_id_fk});
    }
...
sub getCategoryField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query( "SELECT * FROM category,dataset_category where category_id=category_id_fk AND dataset_id_fk=" . $eq->quote( $dataset_id ) );
...
sub getDocumentField {
    my $dataset_id = shift;
    my $eq = EQuery->new();
    $eq->query( "SELECT * FROM file where purpose!='data' AND dataset_id_fk=" . $eq->quote( $dataset_id ) .
	       "ORDER BY purpose");
	...
	if ($row{directory} ne "" && $row{filename} ne "") {
	    my $url = sprintf("http://www.joss.ucar.edu/%s/%s",substr($row{directory},5),$row{filename});
