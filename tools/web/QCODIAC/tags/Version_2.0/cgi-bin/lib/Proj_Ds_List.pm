...
sub queryResult {
	...
	foreach my $ds (@$dss) {
	    ...
	    $pswd = "<font color=blue>Y</font>" if($ds->{authentication_reqd});
...
sub getAllDatasets {
    ...
    my $eq = EQuery->new();
    my $sql = "SELECT * FROM dataset JOIN dataset_project ON dataset.dataset_id=dataset_project.dataset_id_fk WHERE dataset_project.project_id_fk=".$eq->quote( $project )." ORDER BY dataset.dataset_id";
