...
sub queryResult {
	...
	my $sql = "SELECT COUNT(*) as dataset_count FROM dataset_project WHERE project_id_fk=".$sets->quote($proj);
	while ((my %row = $sets->getRow())) {
	    $datasets += $row{dataset_count};
	}

	$sql = "SELECT COUNT(*) as file_count,SUM(size_kb) as file_size,purpose FROM file WHERE dataset_id_fk IN (SELECT dataset_id_fk FROM dataset_project WHERE project_id_fk=".$sets->quote($proj).") GROUP BY purpose";
	...
	$sql = "SELECT SUM(num_files) as file_count,SUM(size_kb) as file_size FROM tape WHERE dataset_id_fk IN (SELECT dataset_id_fk FROM dataset_project WHERE project_id_fk=".$sets->quote($proj).")";
