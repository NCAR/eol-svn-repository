...
#	    " ORDER BY begin_date, b_hour, b_min";
	my $sql = "SELECT tape.*,format.full_name,original.medium_name as primary_media_format,backup.medium_name as backup_media_format FROM tape,format,medium as original,medium as backup WHERE format_id_fk=format_id AND original.medium_id=primary_medium_id_fk AND backup.medium_id=backup_medium_id_fk AND dataset_id_fk=".$query->quote($dataset_id);
	...
	while( (my %row = $query->getRow() ) ) {
		...
		if( defined( $field_titles{$f} ) ) {
			...
			$row{$f} = sprintf("%s (%d)",$row{$1."_medium_format"},$row{$1."_medium_id_fk"});
		    } elsif ($f eq "hide") {
