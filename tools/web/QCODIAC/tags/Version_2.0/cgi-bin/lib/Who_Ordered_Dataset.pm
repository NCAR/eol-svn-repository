...
my @def_fields = ( "dataset_id_fk", "delivery_date", "email" );

# The field titles to display at the top of the table
my %field_titles = ("dataset_id_fk" => "Dataset ID",
		    "delivery_date" => "Date",
		    "email" => "Email Address",
		    "size_kb" => "Num. Kilobytes",   
		    "source_format_id_fk" => "Source Format",
		    "target_format_id_fk" => "Target Format" );
...
sub queryForm {
    ...
    my $form = cr("		<form action=supervisor method=POST onSubmit=\"return checkText( this.dataset_id_fk, \'Please Enter a Dataset Id.\')\">" , 
    ...
    $form = $form . cr("                   <input type=checkbox checked name=fields value=dataset_id_fk>$field_titles{\"dataset_id_fk\"}", &nbsp(3),"" );
    ...
    $form = $form . cr("                   <input type=checkbox unchecked name=fields value=source_format_id_fk>$field_titles{\"source_format_id_fk\"}", &nbsp(3),"" );
    $form = $form . cr("                   <input type=checkbox unchecked name=fields value=target_format_id_fk>$field_titles{\"target_format_id_fk\"}", &nbsp(3), "" );
...
sub queryResult {
	...
	my $sql = "SELECT target.full_name as target_name,source.full_name as source_name,codiac_web_orders.* from format as target,format as source,codiac_web_orders where source.format_id=source_format_id_fk and target.format_id=target_format_id_fk and dataset_id_fk=".$eq->quote($dataset_id).$dropJoss." ORDER BY ".($params->exists("sort_who_order") ? $params->get("sort_who_order") : "delivery_date");

	if (@def_fields == 1 && $def_fields[0] eq "email") {
	    $sql = "SELECT DISTINCT email from codiac_web_orders where dataset_id_fk=".$eq->quote($dataset_id).$dropJoss." ORDER BY email";
	}
