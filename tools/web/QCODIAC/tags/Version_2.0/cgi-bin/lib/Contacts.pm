...
sub queryResult {
    ...
    my $sort = $params->exists( "sort" )? $params->get( "sort" ) : "contact_short_name";
    
    my $sql = "SELECT * FROM contact WHERE 1=1 ";
    $sql = $sql . "AND contact_short_name LIKE ".$query->quote("%".$contact_short_name."%") if( $contact_short_name ne "" );
    $sql = $sql . "AND person_name LIKE ".$query->quote("%".$person_name."%") if( $person_name ne "" );
    $sql = $sql . "AND org_name LIKE ".$query->quote("%".$org_name."%") if( $org_name ne "" );
    
    $sql = $sql . "ORDER BY $sort";
    $query->query( $sql, "contact_id" );
    ...
    my $result = cr("<table border=0 cellpadding=2 cellspacing=2 bgcolor=#e9e9e9 width=$table_width>",
		    ...
		    "<a href=$sort_url&sort=contact_short_name>",
		    "<b>User Id</a></b>",
		    "</td>",
		    "<td align=center id=sort_link>",
