...
<%
	...
	if (request.getParameter("type").equals("project")) {
		note.loadProject(request.getParameter("projectId"));
	} else {
...
<form action="${pageContext.request.contextPath}/iven/edit/util/proc_note.jsp" method="POST">

<%-- Define unmodifiable parameters needed for processing the form action correctly. --%>
<c:choose>
	<c:when test="${param.type == 'project'}">
...
<div class="headerbar">
    <span style="padding-left: 2px;">
        ...
        (${param.type == 'project' ? param.projectId : param.datasetId})
    </span>
</div>
...
<div class="form">
    <table class="noteForm">
        <tr>
            <th class="required">Note Text:</th>
...
<div class="headerbar">
    <div class="buttons">
        <c:choose>
            <c:when test="${note.noteId == null || note.noteId <= 0}">
                <input type=submit name=action value="Add Note" onClick="return validateNoteForm(this.form);">
                <input type=submit name=action value="Cancel">
            </c:when>
            <c:otherwise>
                <input type=submit name=action value="Update Note" onClick="return validateNoteForm(this.form);">
                <input type=submit name=action value="Cancel">
            </c:otherwise>
        </c:choose>
    </div>
