...
<body class="color">
	...
	<div class="folder" style="padding-left: 30px">
		<a href="${pageContext.request.contextPath}" target="_top">DTS Home</a>
	</div>

    <div class="folder" style="padding-left: 30px">
        <a onClick="javascript: location='menu.jsp'" href="${pageContext.request.contextPath}/dln/body/welcome.jsp" target="main">DLN Home</a>
    </div>
	...
	<c:if test="${param.expand == 'project'}">
	    <c:forEach items="${projects.activeProjects}" var="project">
	        <div class="folderEntry"><a href="${pageContext.request.contextPath}/dln/body/dataset_list.jsp?project=${project.projectId}" target="main">${project.projectId}</a></div>
	    </c:forEach>
	</c:if>
	...
	<c:if test="${param.expand == 'ingest'}">
	    <c:forEach items="${users.activeUsers}" var="user">
	        <div class="folderEntry"><a href="${pageContext.request.contextPath}/dln/body/dataset_list.jsp?ingest=${user.contactId}" target="main">${user.personName}</a></div>
	    </c:forEach>
	</c:if>
    ...
    <c:if test="${param.expand == 'load'}">
        <c:forEach items="${users.activeUsers}" var="user">
            <div class="folderEntry"><a href="${pageContext.request.contextPath}/dln/body/dataset_list.jsp?load=${user.contactId}" target="main">${user.personName}</a></div>
        </c:forEach>
    </c:if>
    ...
    <c:if test="${param.expand == 'check'}">
        <c:forEach items="${users.activeUsers}" var="user">
            <div class="folderEntry"><a href="${pageContext.request.contextPath}/dln/body/dataset_list.jsp?check=${user.contactId}" target="main">${user.personName}</a></div>
        </c:forEach>
    </c:if>
