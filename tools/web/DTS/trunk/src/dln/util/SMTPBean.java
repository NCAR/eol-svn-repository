...
public class SMTPBean implements Serializable {
	...
	private void initializeProcess(DatasetBean dataset, NoteBean note, Map<Integer, UserBean> users, String noteTypes) {
		// Determine who the email is going to be from.
		// 1.  Should be the author of the note if there is a note to be sent with the message. 
		if (note != null && note.getAuthor(users) != null) { from = note.getAuthor(users); }
		// 2.  If there is no note, then try to use the check contact for the data set.
		...
		// if the note does not exist or has a general type, otherwise it will be specific to
		// individual note types.
		if ((noteTypes.equals("") || noteTypes.contains("General") || noteTypes.contains("Process")) && dataset.getProcesser(users) != null) {
			to.add(dataset.getProcesser(users));
		}
	}
	...
	public boolean sendProcessNewVersion(DatasetBean dataset, NoteBean note, String projectId) throws SMTPException, SQLException {
		Map<Integer, UserBean> users = (new UserBean()).getAllUsersMap();
		String noteTypes = note.getTypesAsString(NoteTypeBean.getNoteTypesMap());
		
		// Set up the to and from fields for the email header.
		initializeProcess(dataset, note, users, noteTypes);

		// Define the subject line for the email.
		subject = String.format("Dataset %s has a new Version", dataset.getDatasetId());

		// Define the content of the message.
		message = "Dataset " + dataset.getDatasetId() + " has been given a new version in DTS:IVEN and you are assigned as one of the contacts.  " +
				"Below is a summary of what has been entered into the notes page.  "+
				"For more information refer to DTS:IVEN page at "+
				"http://dmg.eol.ucar.edu/dts/iven.\n\n" +
				"Name:            " + dataset.getName() + "\n" +
				"Project(s):      " + projectId + "\n" +
				"Entry Date:      " + dataset.getEntryDate().toString() + "\n" + 
				"Process Contact: " + (dataset.getProcesser(users) == null ? "unassigned" : dataset.getProcesser(users).getPersonName()) + "\n";

		// Add the note to the message if there is one.
		if (note.getNoteText().trim().equals("")) {
			message += "Please see the dataset page in the DTS:IVEN for more information.";
		} else {
			message += "Note: ("+note.getAuthor(users).getPersonName()+")\n" + note.getNoteText();
		}
	...
	public boolean sendProcessUpdate(DatasetBean dataset, NoteBean note, String projectId) throws SMTPException, SQLException {
		Map<Integer, UserBean> users = (new UserBean()).getAllUsersMap();
		String noteTypes = note.getTypesAsString(NoteTypeBean.getNoteTypesMap());
		...
		// Define the content of the message.
		message = "Dataset " + dataset.getDatasetId() + " has been updated in DTS:IVEN and you are assigned as one of the contacts.  " +
				"Below is a summary of what has been entered into the notes page.  "+
				"For more information refer to DTS:IVEN page at "+
				"http://dmg.eol.ucar.edu/dts/iven.\n\n" +
				"Name:            " + dataset.getName() + "\n" +
	...
	public boolean sendUpdate(DatasetBean dataset, NoteBean note) throws SMTPException, SQLException {
        ...
        /**
	 * Send an email message when a new note has been added to a dataset.
	 * @param datasetId The dataset id where the note was added.
	 * @param note The new note that was added.
	 * @return <code>true</code> if the email was sent successfully, <code>false</code>
	 * if it failed.
	 * @throws SMTPException if there was a problem sending the email.
	 * @throws SQLException if there was a problem loading information from the database
	 * needed to send the email.
	 */
	public boolean sendNewNote(String datasetId, NoteBean note) throws SMTPException, SQLException {
		Map<Integer, UserBean> users = (new UserBean()).getAllUsersMap();
		String noteTypes = note.getTypesAsString(NoteTypeBean.getNoteTypesMap());
		...
		// Define the subject line for the email.
		subject = String.format("Note has been Added to Dataset %s", dataset.getDatasetId());

		// Create the content of the email.
		message = "A note has been added to a dataset and you have been assigned as one of the contacts.  " +
				"Below is a summary of what has been entered into the notes page.  "+
				"For more information refer to the data loading notes page at "+
				"http://dmg.eol.ucar.edu/dts/dln.\n\n" +
				"Name:           " + dataset.getName() + "\n" +
        ...
        /**
	 * Send an email message when a note has been updated in a dataset.
	 * @param datasetId The dataset id where the note was updated.
	 ...
	 * needed to send the email.
	 */
	public boolean sendUpdateNote(String datasetId, NoteBean note) throws SMTPException, SQLException {
		Map<Integer, UserBean> users = (new UserBean()).getAllUsersMap();
		String noteTypes = note.getTypesAsString(NoteTypeBean.getNoteTypesMap());
		...
		// Define the subject line for the email.
		subject = String.format("Note has been Updated in Dataset %s", dataset.getDatasetId());

		// Create the content of the email.
		message = "A note has been updated in a dataset and you have been assigned as one of the contacts.  " +
				"Below is a summary of what has been entered into the notes page.  "+
				"For more information refer to the data loading notes page at "+
				"http://dmg.eol.ucar.edu/dts/dln.\n\n" +
				"Name:           " + dataset.getName() + "\n" +
