...
/**
 * The UserNameComparator is a Comparator to determine the sort order
 * of UserBean instances based on the natural sort order of the UserBeans'
 * full/person names.
 * 
 * @author jclawson
 */
public class UserNameComparator extends UserComparator {

	/**
	 * Create a new instance of a UserNameComparator.
	 * @param reverse if the comparator should use a reverse sort during the comparison.
	 */
	public UserNameComparator(boolean reverse) { super(reverse); }

	/**
	 * Compare two UserBeans for sort order based on their full names.
	...
	public int compare(UserBean first, UserBean second) {
		return isReverseSort() ? second.getPersonName().compareTo(first.getPersonName()) :
			first.getPersonName().compareTo(second.getPersonName());
	}
}
