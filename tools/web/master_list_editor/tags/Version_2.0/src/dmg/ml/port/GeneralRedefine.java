...
public class GeneralRedefine {
    ...
    private TreeMap<String,CategoryBean> categories;
    ...
    private HashMap<DatasetBean,TreeSet<CategoryBean>> datasetCategories;
	...
	categories = new TreeMap<String,CategoryBean>();
	...
	datasetCategories = new HashMap<DatasetBean,TreeSet<CategoryBean>>();
	...
	for (ProjectBean project: projects.values()) {
	    ...
	    Set<CategoryBean> projectCats = new TreeSet<CategoryBean>();
	    while (results.next()) {
	    ...
	    project.setCategories(new ArrayList<CategoryBean>(projectCats));
	}
    }
	...
	for (ProjectBean project: projects.values()) {
		...
		if (datasetCategories.get(stored) == null) {
		    datasetCategories.put(stored,new TreeSet<CategoryBean>());
		}
     ...
     **/
    public CategoryBean reconcileCategory(String heading, String category)
	throws SQLException {
	...
	for (String categoryName: parents.keySet()) {
		...
		CategoryBean category = categories.get(categoryName);
	...
	for (DatasetBean dataset: datasetIds.values()) {
	    dataset.setCategories(new ArrayList<CategoryBean>(datasetCategories.get(dataset)));
	...
	for (ProjectBean project: projectDatasetData.keySet()) {
		...
		/*
		ArrayList<CategoryBean> cats = new ArrayList<CategoryBean>();
		for (CategoryProjectBean category: CategoryProjectBean.getCategoryList(project,dbWrite)) {
