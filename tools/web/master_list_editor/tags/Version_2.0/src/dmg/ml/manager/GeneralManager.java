...
import dmg.ml.manager.state.general.ProjectListState;
...
/**
 * <p>The GeneralManager is a state container for the general Master List.  It
...
public class GeneralManager {
    ...
    private TreeState categoryTreeState,datasetListState;
    
    private List<MasterListException> error;
    
    /**
     * Create a new instance of a GeneralManager.
     **/
    public GeneralManager() {
        currentState = new ProjectListState(this);
        categoryTreeState = new TreeState(false);
     ...
     * @return The currently selected category or <code>null</code> if there is
     * not a selected category.
     **/
    public CategoryBean getCategory() { return currentState.getCategory(); }
    
    /**
     * Get the list of SelectItems to display as potential parent categories for
     * a category.
     * @return The list of potential parent categories for a category as
     * SelectItems.
     **/
    public List<SelectItem> getCategoryParentList() {
        List<SelectItem> categories = new ArrayList<SelectItem>();
        // Create a NULL category
        categories.add(new SelectItem("0","None"));
        
        try {
            List<CategoryBean> list = CategoryBean.getCategories();
            for (Iterator<CategoryBean> itr = list.iterator(); itr.hasNext(); ){
                CategoryBean category = itr.next();
                // Only add a category if it is a top level category and is
                // not the same as the category being editted.
    ...
    /**
     * Get the TreeNode that is to be root node of the category tree.
     * @return The category tree root node.
     **/
    public TreeNode getCategoryTreeRoot() {
        try { return currentState.getCategoryTreeRoot(categoryTreeState); }
        // Store the exception to be displayed to the user.
    ...
    /**
     * Determine if the current state is a state that displays category
     * information.
     * @return <code>true</code> if the current state displays category
     * information, <code>false</code> otherwise.
     **/
    public boolean isCategoryState() { return currentState.isCategoryState(); }
    ...
    /**
     * Change the state to veiw a list of categories.
     * @param evt The event that triggered the change of state.
     **/
    public void viewCategoryList(ActionEvent evt) {
        try {
            currentState = currentState.viewCategoryList(evt);
            // Make sure that the prevous error has been removed after the
            // state change.
            error.clear();
        }
