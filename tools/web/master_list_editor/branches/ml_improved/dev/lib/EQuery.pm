...
#  Date: 2003-06-23
#
# NOTE: this module was not developed for the master list
#  app.  This is used to pull the readme file namd and dir_path
#  from CODIAC.
...
$ENV{PATH} = "";                         # This needs to be uncommented when using -T switch!
$ENV{EMPRESSPATH} = "/usr/empress/";
$ENV{MSQLSELCOLSEP} = "";
$ENV{MSQLSELROWSEP} = "";
$ENV{MSQLSELROWCOLCROSS} = "";
$ENV{MSQLSELGROUPSEP} = "";
$ENV{MSQLSELGROUPCOLCROSS} = "";
#-----------------------------------------------------------------------------#
# EQuery->new( $database ) 
#
...
# Utility function used to parse the data
sub trim
{
	my $str = shift;
	$str =~ s/^\s+//;
...
#-----------------------------------------------------------------------------#
# do( $sql, $database ) or do( $sql )
#
# Executes the given INSERT, UPDATE or DELETE statement 
#
# Arguments:
#   $sql -  statemet to execute
#   $database (optional) - name of database to updated 
#     Name of database must already be set if not given
# Returns:
#   1 
#-----------------------------------------------------------------------------#
sub do
{
	...
	# Execute the query
	system( "$empcmd $db_path/$database \"$sql\""); 
