...
public class DatasetEditState extends DatasetState implements
     ...
     **/
    public DatasetEditState(MasterListManager manager, MasterListState lastState,
    ...
    public DatasetEditState(MasterListManager manager, MasterListState lastState,
        ...
        List<Selectable> allClasses = new ArrayList<Selectable>();
        List<String> types = ClassificationProjectBean.getClassificationTypeList(project);
        allClasses.addAll(ClassificationProjectBean.getClassificationList(project));
        
        List<PhaseBean> phases = PhaseBean.getPhaseList(project);
        if (!phases.isEmpty()) {
            types.add(PhaseTypeBean.TYPE_NAME);
            allClasses.addAll(phases);
        }
        
        // Set up the classification selector for the current dataset.
        selector = new Selector(allClasses,types);
        selector.setIncluded(dataset.getClassifications());
        if (!phases.isEmpty()) { selector.setIncluded(dataset.getPhases()); }
    ...
    public MasterListState accept(ActionEvent evt) throws MasterListException {
        ...
        List<PhaseBean> phases = new ArrayList<PhaseBean>();
        for (Selectable item: selector.getIncludedItems()) {
            if (item instanceof ClassificationBean) { classes.add((ClassificationBean)item); }
            else if (item instanceof PhaseBean) { phases.add((PhaseBean)item); }
        }
        dataset.setClassifications(classes);
        dataset.setPhases(phases);
        
        if (dataset.getOriginalId() == null || dataset.getOriginalId().equals("")) {
            dataset.insert();
        } else {            
            dataset.update();
        }
        ...
        return new DatasetListState(manager,lastState.getProject(),lastState.getClassificationType(),
                lastState.getClassification(),getDataset());
    }
     ...
     * @return The classification from the state the Master List was previously
     * in before this state.
     **/
    public ClassificationBean getClassification() { return lastState.getClassification(); }
     ...
     * @return The classification type of the state that generated this state.
     **/
    public ClassificationTypeBean getClassificationType() { 
        return lastState.getClassificationType();
    }
    ...
    /**
     * Get the selector used for associating classifications with the data set.
     * @return The Selector that associates the data set with classifications.
     **/
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        // When the data set id changes, update the selected classifications.
