...
public class TreeState {
    ...
    /**
     * Determine if the expansion has already been set for the specified node.
     * @param node The node to be tested.
     * @return <code>true</code> if the node has already had its expansion set,
     * <code>false</code> otherwise.
     **/
    public boolean isExpansionSet(TreeNode node) {
        return expansionMap.get(node.getClass().getName()) != null &&
               expansionMap.get(node.getClass().getName()).get(node.getId()) != null;
    }
