...
public class ProjectBean extends MasterListBean<ProjectBean> {
          ...
          try {
            if (!systemDir.mkdirs()) {
                throw new MasterListException("System Directory: "+
                    getSystemDirectory()+" cannot be found or made.");
            }
            systemDir.setReadable(true,false); // o+r
          } catch (SecurityException ex) {
            throw new MasterListException("System Directory: "+
                getSystemDirectory()+": security exception: "+
    ...
    private void checkUrl(String urlString) throws MalformedURLException,IllegalStateException,IOException {
        ...
        try {
            ...
            if (ucon instanceof HttpURLConnection) {
                hcon = (HttpURLConnection)ucon;
                int r = hcon.getResponseCode();
                if ( (r < 200) || (r > 299) ) {
                    throw new IOException("HTTP Status Code ("+r+") does not indicate success (2xx).");
                }
            }
        } finally {
            if (null != hcon) hcon.disconnect();
        }
    }
