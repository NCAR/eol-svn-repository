...
#		fields of a form page.  A field is given
...
#   link - the link the field is supposed to point to
#     when the print_val() subroutine is called, if left
#     undef. link is simply ignored.
#   font_tag - the html <font> tag that is used with
#     the print_val() subroutine
#----------------------------------------------------
...
{
	my $self = shift;
	my $string = "<INPUT TYPE=text NAME=\"$self->{name}\" VALUE=\"" . $self->getValue() . "\"";
...
sub htm_tag_date
{
	...
	$string = "<p><INPUT TYPE=text NAME=\"$self->{name}_year\" VALUE=\"$y\" SIZE=4, MAXLENGTH=4"; 
	if( $self->{tabindex} ) { $string = $string . " TABINDEX=$self->{tabindex}"; }
	if( $self->{onChange} ) { $string = $string . " onChange=\"$self->{onChange}\""; }
	if( $self->{othertagmods} ) 
	{
...
sub htm_tag_textarea
{
	...
	$string = $string . " >" . $self->getValue() . "</TEXTAREA>";
...
sub htm_tag_select
{
	...
	if( $self->{othertagmods} ) 
	{
		...
		{
			if( $arr[$x+1] eq $self->{value} )
			{
				$string = $string . "<option selected value=$arr[$x]>" . escapeHTML( $arr[$x+1] ) . "</option>\n";  
			}
			else
			{
				$string = $string . "<option value=$arr[$x]>" . escapeHTML( $arr[$x+1] ) . "</option>\n";  
			}
		}
	}
	else
	{
		$string = $string . "<option>" . $self->getValue() . "</option>\n";  
	}
...
sub print_text
{
	...
	{ $ret = "<a href=" . escapeHTML( $self->{link} ) . ">"; }

	if( defined( $self->{font_tag} ) )
	{ $ret = $ret . $self->{font_tag}; }

	$ret = $ret . $self->getValue();
...
sub print_checkbox
{
	...
	{
		$ret = $ret . "<b><font color=red>N</font></b>";
	}
...
sub print_date
{
	...
	if( defined( $self->{link} ) )
	{ $ret = "<a href=" . escapeHMTL( $self->{link} ) . ">"; }
