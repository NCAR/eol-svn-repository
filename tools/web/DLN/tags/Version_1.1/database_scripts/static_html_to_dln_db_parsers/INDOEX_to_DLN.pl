...
my @URLS = ("/work/DPG_HTML/INDOEX/INDOEX_notes.html",
	    "/work/DPG_HTML/INDOEX/INDOEX_notes_remote_links.html");
my $PROJECT = "INDOEX";
...
sub createProject {
	...
	execute($PROJECT,22,1) or die($DBI::errstr);
}
...
sub parseHTML {
	...
	while (defined(my $line = <$HTML>)) {
	    ...
	    my ($url,$id);
	    if (cleanHTML($colList[0]) =~ /\<a href=\"([^\"]+)\".*\>(.*)\<\/a\>/) {
		($url,$id) = ($1,$2);
	    } else {
		($url,$id) = ("",cleanHTML($colList[0]));
	    }
...
#	    printf("%d: %s %s (%s)\n",$count,$1,$2,cleanHTML($colList[0]));
	    ...
	    $dataset->{"storm_id"} = defined($id) && ($id =~ /link/i || $id =~ /\d+\.x+/i || $id =~ /\d+\.\d+/) ? trim($id) : "99.999";
	    $dataset->{"remote_url"} = defined($url) && $url =~ /cgi\-bin\/codiac/ ? "" : $url;
	
#	    printf("%d: %s %s (%s)\n",$count,$dataset->{"storm_id"},$dataset->{"remote_url"},cleanHTML($colList[0]));
...
#	    }
	    ...
	    my @loaderList = split("/",cleanHTML($colList[1]));
...
#	printf("\t%s: %s\n",join(",",@loaderList),$dataset->{"loader"});
	    ...
	    if ($dataset->{"date"} =~ /\-99/) { $dataset->{"date"} = "1900-01-01"; }
