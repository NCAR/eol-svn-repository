...
<jsp:useBean id="display" scope="session" class="dln.format.DisplayBean">
	<%
		...
		display.setDisplayId( dataset.getDisplayProject() );
...
//		dataset.setNotes( DatasetBean.formatTextArea( dataset.getNotes(), 74 ) );
		if( DatasetDBA.updateDB(dataset) )
		{

			onLoad = "refresh( top.opener.parent.main, " + dataset.getDatasetId() + "); showStatus( top.opener.parent.top_frame, \'Dataset+Has+Been+Updated\' ); close();";
			msg = "Dataset Updated ";
		...
		{
			onLoad = "";
			msg = "There has been an error trying to update the dataset.  Please contact the developer.";
		}
	}  
...
//		boolean good = DatasetDBA.insertDB( dataset );
		if( DatasetDBA.insertDB(dataset) )
		{
			onLoad = "refresh( top.opener.parent.main, " + dataset.getDatasetId() + "); showStatus( top.opener.parent.top_frame, \'Dataset+Has+Been+Added\' ); close();";
			msg = "Dataset Added";	
		...
		{
			onLoad = "";
			msg = "There has been an error trying to add the dataset.  Please contact the developer.";
		}
	} 
...
//		if( good )
		if (DatasetDBA.deleteFromDB(dataset))
		{
			onLoad = "refresh( top.opener.parent.main, " + dataset.getDatasetId() + "); showStatus( top.opener.parent.top_frame, \'Dataset+Has+Been+Deleted\' ); close();";
			msg = "Dataset Deleted";	
			%><%@ include file="reset_numbers.jsp" %><%
		}
		else
		{
			onLoad = "";
			msg = "There has been an error trying to delete the dataset.  Please contact the developer.";
		}
	}
	else
	{
