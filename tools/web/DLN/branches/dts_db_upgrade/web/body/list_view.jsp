...
<% 
	...
	{
		UserBean loader = UserDBA.getFromDB( Integer.valueOf(display.getDisplayId()).intValue() );
		title = "Datasets by Loader: " + loader.getName();
	}
	else if( display.getDisplayView() == DisplayBean.CHECKERS )
	{
		UserBean checker = UserDBA.getFromDB( Integer.valueOf(display.getDisplayId()).intValue() );
...
<table border=0 width=100% cellpadding=1 cellspacing=1 class=listTable> 
	...
	<tr>
		...
		<td class=<%= fcolor %> width=6% nowrap>
			<a href=set_sort.jsp?field=<% out.print(DisplayBean.STORM_ID); %>>Dataset Id</a>
		</td>
	...
	<%
		...
		while( dss.hasMoreElements() ) 
		{
			...
			if( ds.getDatasetId().equals(hlight))
				trclass="hlightRow";
	%> 
	<tr id=row_<%= count %> class="<%= trclass %>" onmouseover="cursorOver(this);" onmouseout="cursorOut( this );">
		<td width=2%>
			<input type=checkbox name=dscheckbox value="<%= ds.getDatasetId() %>" onClick="selectRow( '<%= count %>', this );">
		</td>
		<td nowrap width=4%>
			...
			<%
				...
				if( ((StatusBean)statusMap.get(new Integer(ds.getLoadedStatusId()))).isDone() )
					out.print( "<td width=25% align=center><img src=../images/loaded.gif alt=\"Dataset Loaded\"></img></td>" );
				else
					out.print( "<td width=25% align=center><img src=../images/blank.gif></td>" );

				if( ((StatusBean)statusMap.get(new Integer(ds.getCheckedStatusId()))).isDone() )
		...
		<td nowrap align=center>
			<%
				if( ds.getEnteredDate() == null )
					out.print( "&nbsp;" );
				else
					out.print( ds.getEnteredDate() );
			%>
		...
		<td nowrap>
			<a 
				href=http://www.joss.ucar.edu/cgi-bin/codiac/dss?<%= ds.getDatasetId() %> 
				target=_blank
		...
		<%
			...
			{
				%>
					<td><%= ((UserBean)userMap.get(new Integer(ds.getLoaderId()))).getName() %></td>
				<%
			}
		%>
		<%
			...
			{
				%>
					<td><%= ((UserBean)userMap.get(new Integer(ds.getCheckerId()))).getName() %></td>
				<%
			}
		%>
		...
		<td align=center valign=center>
			<a
				href="javascript: editWindow( '../edit/edit_dataset.jsp?mode=update&id=<%= ds.getDatasetId() %>')"
