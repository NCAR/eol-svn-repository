...
function changeAuthorFilter(drop, datasetId, entryDate) {
    location.replace("/dln/body/view_dataset.jsp?authorFilterId=" + drop.options[drop.selectedIndex].value +
...
function changeNoteTypeFilter(drop, datasetId, entryDate) {
    location.replace("/dln/body/view_dataset.jsp?noteTypeFilterId=" + drop.options[drop.selectedIndex].value +
            "&datasetId=" + datasetId + "&entryDate=" + entryDate);
}
...
function changeVisibility(sel, visibility) {
    ...
    if (sel.options[sel.selectedIndex].value != "prompt") {
        location.replace("/dln/body/dataset_list.jsp?" + sel.options[sel.selectedIndex].value + "=" + visibility);
    }
}
...
function editDataset( url ) {
	window.open( url, "edit", "width=1200,height=800,menubar=no,toolbar=no,location=no,resizable=yes,directories=no,scrollbars=yes,status=0" ).focus(); 
}
...
function refresh(win, datasetId, entryDate) {
    if (win.location.href.search(/view_dataset/i) >= 0 && datasetId != null) {
       win.location.replace("/dln/body/view_dataset.jsp?datasetId="+datasetId+"&entryDate="+entryDate);
    } else {
       win.location.replace("/dln/body/dataset_list.jsp?datasetId="+datasetId+"&entryDate="+entryDate+"#"+datasetId);
    }
    // Force the page to reload because the page doesn't always want to with the replace.
}
 ...
 */
function rowHover(row, styleClass) {
...
function showStatus(win, msg) {
    win.location.replace("/dln/top.jsp?message=" + msg);
}
...
function validateNoteForm(form) {
    ...
    if (form.noteText.value != '' && (form.author.value <= 0 || checkedCount == 0)) {
       alert("The note must have an author and at least one note style.");
       return false;
    }
