...
my $sat = "G12/RSO";
my $title = "GOES-12 Rapid Scan Schedule";
...
sub getConnection
{
	return DBI->connect( "DBI:mysql:database=sat_query;host=hurricane",
												"stormdba", "codiac", {RaiseError=>1} ) || die( "Unable to Connecto to database" );
}
