...
c***********************************************************************
      include "../inc/ebufr_parm.inc"
      include "../inc/ebufr_vars.inc"
c***********************************************************************
...
c
      ...
      integer numdes,numsub,obsdat,cmpdat,skip_ct
      real ckel2c,rval,elev,rmin
      logical EOF,DSCSCN,FAIL,FSTTIM,gtfils,LOADED,MISSNG,getval
      logical ERR
      character*1 valtyp
      character*50 ifile,ofile,msfile,stnfile,pfile,sfile
      character*32 name1,name2,cval
      character*24 units
      character*20 tstr
...
c     end if
      ...
      call getobs (iunit,msunit,hdr,datum,fmt,numdat,xout,yout,
     +             numdes,numsub,obsdat,cmpdat,FAIL,EOF,DSCSCN,
     +             FSTTIM)
      if (FAIL) then
          goto 2000
      else
...
c
       ...
       q.ll(1) = comlat
       q.ll(2) = comlon
       read(hdr.ebunique,'(I3)') q.occur

c
c Get coop station number out of the E-BUFR data record
c
          MISSNG = getval(datum(1),xout(1),yout(1),fmt(1),rval,cval,
     +                    valtyp,msunit,ERR)
          if (ERR) then
              goto 2000
          end if
...
c
          MISSNG = getval(datum(k),xout(k),yout(k),fmt(k),rval,cval,
     +                    valtyp,msunit,ERR)
          if (ERR) then
              goto 2000
          end if
c          write(msunit,*)k,datum(k),xout(k),yout(k),fmt(k),rval,cval
c
...
c
         ...
         character*32 datum(max)
         character*8 fmt(max)
         integer flag1,flag2


      logical MISSNG,ERR,getval
      real rval
      character*32 cval
      character*1 valtyp
