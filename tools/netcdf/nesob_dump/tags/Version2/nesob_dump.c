/*********************************************************************
 * NESOB_dump - a generic program to take the data from NESOB NetCDF 
 * files and put it out in a readable form. 
 * 
 * Input:  filename of netCDF NESOB file
 * Output: dump of contents to STDOUT
 *
 * 25 jul 97, ds
 *
 * 30 Mar 98,ds
 *	minor fix to usage section
 * 23 Jul 99, ds
 *  Bypassed error checking of getting variable ids;
 *  if variable doesn't exist, it isn't dealt with.
 * 28 Sep 99, jag
 *  Increased maximum line length.  Truncation was occuring.
 * 28 Aug 2003, jag
 *  Increased maximum line length again, to 100000 (max_len = 100000)
 *********************************************************************/
...
#define USAGE   "\
  ...
  [filename]       Where to pipe output\n"

    (void) fprintf(stderr,
           "\nSYNTAX: %s [-c|-h] [-v ...] [[-b|-f] [c|f]] [-l len] [-n name] [-p n[,n]] file > [filename]\n%s\n",
           progname,
           USAGE);
    exit(EXIT_FAILURE);
}
...
/*          NC_CHECK( nc_inq_varid(ncid, specp->lvars[iv], &varid) );  */
            if (nc_inq_varid(ncid, specp->lvars[iv], &varid) == NC_NOERR)
            	varadd(vlist, varid);
        }
    }
    ...
    for (dimid = 0; dimid < ndims; dimid++) {
        ...
        for (varid = 0; varid < nvars; varid++) {
            NC_CHECK( nc_inq_var(ncid, varid, var.name, &var.type, &var.ndims, var.dims, &var.natts) );
        }
    ...
    if (! specp->header_only) {
        ...
        for (varid = 0; varid < nvars; varid++) {
            if (specp->nlvars > 0 && ! varmember(vlist, varid))		/* if var list specified, test for membership */
                continue;
            NC_CHECK( nc_inq_var(ncid, varid, var.name, &var.type, &var.ndims, var.dims, &var.natts) );
            if (specp->coord_vals) {
                is_coord = 0;										/* Find out if this is a coordinate variable */
                for (dimid = 0; dimid < ndims; dimid++) {
            ...
            if (var.ndims == 0 || var.dims[0] != xdimid || dims[xdimid].size != 0) {
                ...
                if(nc_status == NC_NOERR && att.type == var.type && att.len == 1) {
                    if(var.type == NC_CHAR) {
                        char fillc;
                        NC_CHECK( nc_get_att_text(ncid, varid, _FillValue, &fillc ) );
                        var.fillval = fillc;
                    } else {
                        NC_CHECK( nc_get_att_double(ncid, varid, _FillValue, &var.fillval) );
                    }
                } else {
                    switch (var.type) {
                        case NC_BYTE:
...
int main(int argc, char *argv[])
{
    ...
    int c;
    int i;
    int max_len = 100000;     /* default maximum line length */
   ...
   if (argc < 2)
      {
	  usage();
/*      printf ("Usage: process_armsfc <input_file> <input_stn_info_file>\n");
      exit(1); */
      }
