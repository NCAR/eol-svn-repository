...
sub write_final_stats{
   ...
   foreach $height (sort keys %sensor_hts)
      {
      printf "Sensor_Height: %7.2f  Count: %10d\n", 
              $height, $sensor_hts{ $height};
      }
...
sub check_record_metadata {    
   ...
   foreach $meta (sort keys %meta_positions)
      ...
      if ( ($meta_value > $meta_positions{$meta}{MAX_VAL}) ||
         ...
         {
         print "      ERROR META: $meta ($meta_value) outside expected range ($meta_positions{$meta}{MIN_VAL}, $meta_positions{$meta}{MAX_VAL}) at line $number_lines_processed in $input_file_name. Can NOT be Missing!\n";
         $err++; $err_meta++;
         }
      } # foreach meta 
   ...
   if (exists $sensor_hts{$sensor_height} )
      {
      #----------------------------------------
...
";}  
      $sensor_hts{ $sensor_height} = $sensor_hts{ $sensor_height} + 1;
      }
   else
      {
      #--------------------------------
   ...
   if ($record_CSE eq $file_name_CSE )
      ...
      { print "      ERROR META: Record_CSE ($record_CSE) does NOT match File_Name_CSE ($file_name_CSE) at line $number_lines_processed in $input_file_name.\n"; $err_meta++;}
   ...
   if ($record_Ref_Site eq $file_name_Ref_Site)
      ...
      { print "      ERROR META: Record_Ref_Site ($record_Ref_Site) does NOT match File_Name_Ref_Site ($file_name_Ref_Site) at line $number_lines_processed in $input_file_name.\n"; $err_meta++;}
   ...
   if ($record_Stn eq $file_name_Stn )
      ...
      { print "      ERROR META: Record_Stn ($record_Stn) does NOT match File_Name_Stn ($file_name_Stn) at line $number_lines_processed in $input_file_name.\n"; $err_meta++;}  
