...
#----------------------------------------------------------------------
# check_CEOP_flux.pl - This perl program verifies that the input file
#   is in the 29 July 2003 CEOP SSC approved Flux Dataset format.
#   The name of the input file must also be in the specified form.
...
#                                                                
#   Execute: check_CEOP_flux.pl CSE_RefSite_Stn_bdate_edate.flx 0
#
#   Required Inputs: Two input parameters are required as shown below:
#
#      check_CEOP_flux.pl input_file_name info_level
#
...
#
#   E.g., check_CEOP_flux.pl LBA_Pantanal_Pantanal_20020701_20020701.flx 2
#
...
#     - Update to handle Phase II 2005 dates.
# Sep 2007 lec Version 6
#     - Incremented $PROJECT_YEAR_NEXT to include the next 4 years to
#       cover 2005 and 2006 for CEOP/Phase 2.
# Aug 2011 lec Version 7
#     - Updated software to handle data through 2010.
#-------------------------------------------------------
# HARDCODED
#
...
$PROJECT_YEAR_NEXT = $PROJECT_YEAR + 8; # HARDCODED to EOP-3/EOP-4 plus Phase II

$RECORD_LENGTH = 160; # HARDCODED - Includes return for the Flux format
$FILE_SUFFIX = "flx"; # HARDCODED - Set for Flux format.
...
sub write_final_stats{
   ...
   #--------------------------------------------------
   print "\n----------------------------\n";
   print "Extreme Values per Parameter/All Stns/All Records in $ARGV[0]. (Number of Errors for Parm.) (MIN, MAX Limits)\n\n";
   ...
   foreach $height (sort keys %sensor_hts)
      {
      if ($sensor_hts{$height}{SENSIBLE_HEAT_FLUX_COUNT} > 0 &&
          $sensor_hts{$height}{LATENT_HEAT_FLUX_COUNT} > 0 &&
          $sensor_hts{$height}{CO2_FLUX_COUNT} > 0 &&
          $sensor_hts{$height}{SOIL_HEAT_FLUX_COUNT} > 0 &&
          $sensor_hts{$height}{COUNT_RECS} > 0)
         {
         printf "Sensor_Height: %7.2f  Record_Count: %10d Sensible_ht: %10d (%6.1f %) Err_Count: %10d  Latent_ht: %10d (%6.1f %) Err_Count: %10d  CO2_Flux: %10d (%6.1f %) Err_Count: %10d  Soil_ht: %10d (%6.1f %) Err_Count: %10d\n",

              $height, $sensor_hts{$height}{COUNT_RECS},

              $sensor_hts{$height}{SENSIBLE_HEAT_FLUX_COUNT},
              ($sensor_hts{$height}{SENSIBLE_HEAT_FLUX_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{SENSIBLE_HEAT_FLUX_ERROR_COUNT},

              $sensor_hts{$height}{LATENT_HEAT_FLUX_COUNT},
              ($sensor_hts{$height}{LATENT_HEAT_FLUX_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{LATENT_HEAT_FLUX_ERROR_COUNT},

              $sensor_hts{$height}{CO2_FLUX_COUNT},
              ($sensor_hts{$height}{CO2_FLUX_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{CO2_FLUX_ERROR_COUNT},

              $sensor_hts{$height}{SOIL_HEAT_FLUX_COUNT},
              ($sensor_hts{$height}{SOIL_HEAT_FLUX_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{SOIL_HEAT_FLUX_ERROR_COUNT};
         }
      else
         {     
         printf "Sensor_Height: %7.2f  Record_Count: %10d Number_Sensible_Ht_Flux: %10d [%d]  Number_Latent_Ht_Flux: %10d [%d]  Number_CO2_Flux: %10d [%d]  Number_Soil_Ht_flux: %10d [%d]\n",
              $height, $sensor_hts{$height} {COUNT_RECS},
              $sensor_hts{$height}{SENSIBLE_HEAT_FLUX_COUNT},
              $sensor_hts{$height}{SENSIBLE_HEAT_FLUX_ERROR_COUNT},
              $sensor_hts{$height}{LATENT_HEAT_FLUX_COUNT},
...
#  Version 2
   ...
   $total_data = $number_lines_processed*8;  # 8 parms/line
...
#    CSE_ReferenceSite_Station_BeginDate_EndDate.flx
#
# (e.g., LBA_Pantanal_Pantanal_20020701_20020705.flx)
#
...
sub parse_record_metadata {
   ...
   %meta_positions = (
      ...
      SENSOR_HEIGHT =>{BEG_POS=>108, LENGTH=>7,  MIN_VAL=>-999.99, MAX_VAL=>1000.00}  # -999.99=missing
      );
...
sub check_record_metadata {    
   ...
   if (exists $sensor_hts{$sensor_height}{COUNT_RECS} )
      ...
      # *_COUNT - total count of that type value at this height.
      # *_ERROR_COUNT - total count of any type DATA error at height.
      #-------------------------------------------------------------------
      $sensor_hts{ $sensor_height}{COUNT_RECS} = 1;

      $sensor_hts{ $sensor_height}{SENSIBLE_HEAT_FLUX_COUNT} = 0;
      $sensor_hts{ $sensor_height}{SENSIBLE_HEAT_FLUX_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{LATENT_HEAT_FLUX_COUNT} = 0;
      $sensor_hts{ $sensor_height}{LATENT_HEAT_FLUX_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{CO2_FLUX_COUNT} = 0;
      $sensor_hts{ $sensor_height}{CO2_FLUX_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{SOIL_HEAT_FLUX_COUNT} = 0;
      $sensor_hts{ $sensor_height}{SOIL_HEAT_FLUX_ERROR_COUNT} = 0;
...
sub parse_record_data {
   ...
   @space_positions = (
       ...
       107, 115,  124, 126, 135, 137, 146, 148, 157);

   if ($debug) { print  "\nspace_positions:: @space_positions\n"; }


   @decimalPt_positions = (81, 93, 104, 112, 121, 132, 143, 154);
   ...
   #--------------------------------------------------
   %flag_positions = (
        "sensible_heat_flux" => 125, 
        "latent_heat_flux"   => 136, 
        "CO2_flux"           => 147, 
        "soil_heat_flux"     => 158
      );
   ...
   #---------------------------------------------------------------
   %flag_count = (
      "sensible_heat_flux" => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "latent_heat_flux"   => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "CO2_flux"           => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "soil_heat_flux"     => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0}
     );                               
   ...
   #----------------------------------------------------------------------
   %parm_positions = (
      sensible_heat_flux =>{BEG_POS=>116, LENGTH=>8, MIN_VAL=>-150.00,  MAX_VAL=>600.00, 
                            MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},    # Above ground
      latent_heat_flux   =>{BEG_POS=>127, LENGTH=>8, MIN_VAL=>-150.00,  MAX_VAL=>600.00,   
                            MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},    # Above ground
      CO2_flux           =>{BEG_POS=>138, LENGTH=>8, MIN_VAL=>-75.00,  MAX_VAL=>75.00,   
                            MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},    # Above ground
      soil_heat_flux     =>{BEG_POS=>149, LENGTH=>8, MIN_VAL=>-100.00,  MAX_VAL=>300.00,   
                            MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},    # Below ground level
      );
...
sub check_record_data {
   ...
   foreach $parm (keys %flag_positions)
      ...
      if ($parm_flag ne 'U' && $parm_flag ne 'G' && $parm_flag ne 'D' &&
         ...
         if ($parm eq sensible_heat_flux) 
           {$sensor_hts{ $sensor_height}{SENSIBLE_HEAT_FLUX_ERROR_COUNT}++;}  

         if ($parm eq latent_heat_flux)
           {$sensor_hts{ $sensor_height}{LATENT_HEAT_FLUX_ERROR_COUNT}++;}

         if ($parm eq CO2_flux)
           {$sensor_hts{ $sensor_height}{CO2_FLUX_ERROR_COUNT}++;}

         if ($parm eq soil_heat_flux)
           {$sensor_hts{ $sensor_height}{SOIL_HEAT_FLUX_ERROR_COUNT}++;}
   ...
   #
   # Verify that soil ht flux is measured below or at ground level and
   # that latent and sensible heat flux and CO2 Flux are measured
   # above ground.
   #------------------------------------------------------------------
   ...
   foreach $parm (sort keys %parm_positions)
      ...
      if ($parm_value ne "-999.99" && $parm_value ne " -999.99") 
         ...
         if ($parm eq sensible_heat_flux)
            {
            $sensor_hts{ $sensor_height}{SENSIBLE_HEAT_FLUX_COUNT}++;
            if ($debug) {print "Increment (SENSIBLE_HEAT_FLUX_COUNT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{SENSIBLE_HEAT_FLUX_COUNT}\n";}
            }                                       

         if ($parm eq latent_heat_flux)
            {
            $sensor_hts{ $sensor_height}{LATENT_HEAT_FLUX_COUNT}++;
            if ($debug) {print "Increment (LATENT_HEAT_FLUX_COUNT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{LATENT_HEAT_FLUX_COUNT}\n";}
            } 

         if ($parm eq CO2_flux)
            {
            $sensor_hts{ $sensor_height}{CO2_FLUX_COUNT}++;
            if ($debug) {print "Increment (CO2_FLUX_COUNT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{CO2_FLUX_COUNT}\n";}
            } 

         if ($parm eq soil_heat_flux)
            {
            $sensor_hts{ $sensor_height}{SOIL_HEAT_FLUX_COUNT}++;
            if ($debug) {print "Increment (SOIL_HEAT_FLUX_COUNT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{SOIL_HEAT_FLUX_COUNT}\n";}
            } 


         #-------------------------------------------------------
         # Verify that for non-missing values that all readings
         # are taken above ground except for Soil Heat Flux which
         # must be taken at or below ground level.
         #-------------------------------------------------------
         $sensor_ht = substr ($input_record, $meta_positions{$SENSOR_HEIGHT}{BEG_POS},
                             $meta_positions{$SENSOR_HEIGHT}{LENGTH});

         if ( ($parm eq LATENT_HEAT || $parm eq SENSIBLE_HEAT || $parm eq CO2_FLUX) &&
               ($sensor_ht <= 0.00) )
            {
            print "      ERROR DATA: $parm ($parm_value) MISMATCH with Sensor Height of $sensor_ht at line $number_lines_processed in $input_file_name. Should be ABOVE ground level.\n";
            $err++; $err_data++; $parm_positions{$parm}{ERR_CT}++;
            }  
 
         if (($parm eq SOIL_HEAT_FLUX) && ($sensor_ht > 0.00) )
            { 
            print "      ERROR DATA: $parm ($parm_value) MISMATCH with Sensor Height of $sensor_ht at line $number_lines_processed in $input_file_name. Should be BELOW or AT ground level.\n"; 
            $err++; $err_data++; $parm_positions{$parm}{ERR_CT}++;
            }  
...
##################################################
# check_CEOP_flux.pl - MAIN processing.
##################################################
printf "\ncheck_CEOP_flux.pl of file $ARGV[0] began on ";print scalar localtime; printf "\n";
...
write_final_stats(); # Stats for all heights 
...
if ($number_heights > 1 && $number_heights <= 15)
   ...
   foreach $height (sort keys %sensor_hts)
      ...
      $cmd = "check_CEOP_flux.pl ".$ARGV[0].$cmd_height."m > ".$infile_parts[0]."_height".$cmd_height."m.log";
   ...
   {
   print "\n------------------------------------------------\n";
   print "There are $number_heights sensor heights in the input file. If 1 heigh
t, then done. If > 15 heights, then requires manual processing. Processing compl
ete!\n"; 
   }

print "\nTotal number lines processed in file $ARGV[0]: $number_lines_processed\n";

printf "\ncheck_CEOP_flux.pl of file $ARGV[0] ended on ";print scalar localtime;
printf "\n";
