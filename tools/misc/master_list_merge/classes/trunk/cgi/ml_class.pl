...
#-----------------------------------------------------------------------------#
# ml_class_list: This script generates JSON/HTML output of the classifications 
#                in the Master List database on Riesling.
#
# Author: Amanda Orin
# Date: October, 2013
#
...
if ($display == 0) {
	...
	for (my $i = 0; $i < scalar(@roots); $i++) {
		printClass($tableType, \%classes, \@types, $roots[$i], 2, $display);
	}
	...
	if ($display == 0) {
		...
		if (scalar(@children) > 0) {
			...
			for (my $i = 0; $i < scalar(@children); $i++) {
				printClass($tableType, $classesRef, $typeRef, $children[$i], $indents+2, $display);
			}
