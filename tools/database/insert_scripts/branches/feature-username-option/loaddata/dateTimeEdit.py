...
class dateTimeEditField(QtGui.QWidget):
    ...
    def add(self, field_toolTip):
        ...
        # the very earliest date that is valid by QDateTime and set it as min time
        ...
        # the very latest date that is valid by QDateTime and set it as max time
        self.maxDateTime = QtCore.QDateTime(7999,12,31,23,59,59)
        self.dateTimeEdit.setMaximumDateTime(self.maxDateTime)

        # for some reason maxDateTime is not being set correctly. Can investigate further if crucial
    ...
    def load(self, loadData):
        ...
        # convert string dateTimeEdit data to QDateTime data then insert it to QDateTimeEdit widget
        target = self.loadData['dataset'][self.row][self.name]
        if not target:
            self.clear()
        else:
            self.dateTime = QtCore.QDateTime.fromString(str(self.loadData['dataset'][self.row][self.name]), "yyyy-MM-dd HH:mm:ss")
    ...
    def save(self, saveData):
        ...
        # turn QDateTimeEdit data to dateTime.() data then string then save to dataset
        self.dateTimeWidget = str(self.dateTimeEdit.dateTime().toString("yyyy-MM-dd HH:mm:ss"))
    ...
    def check(self):
        ...
        # it's checking to see whether the date has been changed from the default values of min/max dates
        # I remember datasets that used the "beginning/end of time" as their actual dates...
        # if begin date is still min date, send to error
        if str(self.dateTimeEdit.dateTime().toString("yyyy-MM-dd HH:mm")) == self.minDateTime.toString("yyyy-MM-dd HH:mm"):
            return self.labelName
        # if end date is still max date, send to error
        elif str(self.dateTimeEdit.dateTime().toString("yyyy-MM-dd HH:mm")) == self.maxDateTime.toString("yyyy-MM-dd HH:mm"):
            return self.labelName
