...
#                                                                             #
# add function takes the name from the config yaml file then uses it to       #
# create a label and also a lineEdit field.                                   #
#                                                                             #
...
class lineEditField(QWidget):
    ...
    def add(self, field_toolTip):
        ...
        # takes the name from dataset yaml and replaces all under scores with
        # a space then capitalizes all words for label uses.
        self.labelName = self.name.replace("_", " ").title()
        # name in the yaml is actually the project name, so change label
        # in GUI.
    ...
    def load(self, loadData):
        ...
        self.lineEdit.clear()
        # take the respective lineEdit field data from yaml dataset and
        # displays it in QLineEdit
        loadLineData = self.loadData['dataset'][self.row][self.name]
        if loadLineData:
            # if line contains float instead of string, convert to string
            self.lineEdit.setText(str(loadLineData))
        else:
    ...
    # if the dataset you want have an archive ident number that does not fit in
    # these patterns, create a new variable with the pattern then repeat the
    # check steps.
    def arcIdCheck(self):

        # if masking int to the value passes, return error since it needs to
        # be a float
        # pattern1: "999.999"
        pattern1 = re.compile(r"^([0-9][0-9][0-9]\.[0-9][0-9][0-9])$")
        # pattern2: "999.B999"
        pattern2 = re.compile(r"^[0-9][0-9][0-9]\.B[0-9][0-9]-[0-9][0-9][0-9]")
    ...
    def str_arcIdCheck(self):
        ...
        if (self.arcIdCheck() == self.labelName or
                not str(self.lineEdit.displayText())):
            return self.labelName

    # checks for integer with units
    def str_unitCheck(self):
        # value must be an integer followed by a case-insensitive frequency of
        # year, month, day, hour, or minute (any of them can be plural)
    ...
    def versionCheck(self):
        prelimPattern = re.compile(r"^0.*")
        finalPattern = re.compile(r"^[1-9].*")
    ...
    # check for gcmd keyword and xlink lists
    def gcmd_xlinkCheck(self):
