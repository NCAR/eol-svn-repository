...
#----------------------------------------------------------------------
# check_CEOP_MetTwr.pl - This perl program verifies that the input file
#   is in the 29 July 2003 CEOP SSC approved Meteorological Tower
#   Dataset format. The name of the input file must also
#   be in the specified form. This s/w checks the metadata separate
#   from the data.
#
#   WARNING: This software is RECURSIVE. The software will first process
#            the original input file as is. If the original file contains
#            more than one sensor height, the s/w will then divide
#            the data up by sensor height and spawn itself out to process
#            each of those files. The s/w divides the data based on
#            lat/lon/elev and sensor height. It may not work properly
#            if the data contains more than one station. This has not been
#            tested. Since results may be unexpected, this s/w should be
#            closely monitored when running.
#
#   Execute: check_CEOP_MetTwr.pl CSE_RefSite_Stn_bdate_edate.twr 0
#
#   Required Inputs: Two input parameters are required as shown below:
#
#      check_CEOP_MetTwr.pl input_file_name info_level
#
...
#
#   E.g., check_CEOP_MetTwr.pl LBA_Pantanal_Pantanal_20020701_20020701.twr 2
#
#   This s/w also requires a "final_CEOP_sort_height" file (with that exact name)
#   to be located in the directory in which it it executed. The final_CEOP_sort_height
#   file is defined as:
#
...
#   numeric     #       format
#   external    #       value interpretation
#   ascending   # Key 17 direction Sensor Height
#   fixed       #       location
#   109         #       starting column
#   7           #       length
#   numeric     #       format
...
#     dates/times/names/lat/lons/elev/sensor_ht match but possibly not data.)
#
...
#   That if the data value is missing the data flag is either C,M,or I, only.
...
#      record are missing. 
#   That the sensor height is greater than or equal to 0.00.
...
#     - Tracks number of each obs at each depth.
#     - Verify that the Absolute value of the wind components are
#       less than the wind speed.
#
# Sept 2004 lec Version 3
#     - Added debug. Corrected error in debug statement that printed
#       incorrect parm position when there was a Missing flag with a
#       non-missing parm value.
# Oct 2004 lec Version 4 
#     - Incremented $PROJECT_YEAR_NEXT to include the next 2 years to
#       cover EOP4, too.
# May 2007 lec Version 5
#     - Updated dates to handle Phase II 2005 data.
# Sep 2007 lec Version6 
#     - Updated dates to handle Phase II 2006 data.
# Aug 2011 lec Version7
#     - Updated to handle data through 2010. Previously
#       tested. 
#-------------------------------------------------------
# HARDCODED
#
...
$PROJECT_YEAR_NEXT = $PROJECT_YEAR + 8; # HARDCODED to EOP-3/EOP-4 and Phase II

$RECORD_LENGTH = 206; # HARDCODED - Includes return for the Met/Radiation format
$FILE_SUFFIX = "twr"; # HARDCODED - Set for Met/Radiation Surface format.
...
%sensor_hts = (); # Retain uniq list of sensor heights
...
sub write_final_stats{
   ...
   #--------------------------------------------------
   print "\n----------------------------\n";
   print "Extreme Values per Parameter/All Stns/All Records in $ARGV[0]. (Number Errs per Parm) (MIN,MAX Limits)\n\n";
   ...
   foreach $parm (sort keys %parm_positions)
      ...
      if ($err_data != 0) # Don't divide by zero
         ...
         printf "Parm: %-20s  Minimum_Extreme: %8.2f Maximum_Extreme: %8.2f Error_count: %7d (%5.1f%) (%7.2f, %7.2f)\n",
            $parm, $parm_positions{$parm}{MIN_EXT}, $parm_positions{$parm}{MAX_EXT},
            $parm_positions{$parm}{ERR_CT}, (0.0),
            $parm_positions{$parm}{MIN_VAL}, $parm_positions{$parm}{MAX_VAL};
         }                    
   ...
   #-----------------------------------------------
   # Print final counts of sensor heights found.
   #-----------------------------------------------   
   print "\n----------------------------\n";
   print "Counts for Each Different Sensor Height Found for All Parms/All Stns/All Records in $ARGV[0]:\n";  
   print "(%) = Percent of max possible. 100%=every line had this parm.\n";  
   print "[ErrCt] = Number of Data Errors Flagged on this parm.\n\n";  

   foreach $height (sort keys %sensor_hts)
      {
      if ($sensor_hts{$height}{STN_PRESSURE_COUNT}      > 0 &&
          $sensor_hts{$height}{TEMPERATURE_COUNT}       > 0 &&
          $sensor_hts{$height}{DEW_PT_COUNT}            > 0 &&
          $sensor_hts{$height}{RELATIVE_HUMIDITY_COUNT} > 0 &&
          $sensor_hts{$height}{SPECIFIC_HUMIDITY_COUNT} > 0 &&
          $sensor_hts{$height}{WIND_SPEED_COUNT}        > 0 &&
          $sensor_hts{$height}{WIND_DIRECTION_COUNT}    > 0 &&
          $sensor_hts{$height}{U_WIND_COMPONENT_COUNT}  > 0 &&
          $sensor_hts{$height}{V_WIND_COMPONENT_COUNT}  > 0 &&
          $sensor_hts{$height}{COUNT_RECS}              > 0)
         {
         printf "\n  Sensor_Ht   RecCount   StnPress(%) [ErrCt]         Temp(%) [ErrCt]     DewPoint(%) [ErrCt]    Rel_Humid(%) [ErrCt]     SpecHumid(%) [ErrCt]      WindSpeed(%) [ErrCt]      WindDir(%) [ErrCt]       U_Wind(%) [ErrCt]        V_Wind(%) [ErrCt]\n";
         printf " %7.2f  %10d %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d] %10d (%5.1f%) [%d]\n",

              $height, $sensor_hts{$height} {COUNT_RECS},

              $sensor_hts{$height}{STN_PRESSURE_COUNT},
              ($sensor_hts{$height}{STN_PRESSURE_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{STN_PRESSURE_ERROR_COUNT},

              $sensor_hts{$height}{TEMPERATURE_COUNT},
              ($sensor_hts{$height}{TEMPERATURE_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{TEMPERATURE_ERROR_COUNT},

              $sensor_hts{$height}{DEW_PT_COUNT},
              ($sensor_hts{$height}{DEW_PT_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{DEW_PT_ERROR_COUNT},

              $sensor_hts{$height}{RELATIVE_HUMIDITY_COUNT},
              ($sensor_hts{$height}{RELATIVE_HUMIDITY_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{RELATIVE_HUMIDITY_ERROR_COUNT},

              $sensor_hts{$height}{SPECIFIC_HUMIDITY_COUNT},
              ($sensor_hts{$height}{SPECIFIC_HUMIDITY_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{SPECIFIC_HUMIDITY_ERROR_COUNT},

              $sensor_hts{$height}{WIND_SPEED_COUNT},
              ($sensor_hts{$height}{WIND_SPEED_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{WIND_SPEED_ERROR_COUNT},    

              $sensor_hts{$height}{WIND_DIRECTION_COUNT},
              ($sensor_hts{$height}{WIND_DIRECTION_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{WIND_DIRECTION_ERROR_COUNT},

              $sensor_hts{$height}{U_WIND_COMPONENT_COUNT},
              ($sensor_hts{$height}{U_WIND_COMPONENT_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{U_WIND_COMPONENT_ERROR_COUNT},

              $sensor_hts{$height}{V_WIND_COMPONENT_COUNT},
              ($sensor_hts{$height}{V_WIND_COMPONENT_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{V_WIND_COMPONENT_ERROR_COUNT};  

         }
      else
         {
         printf "  Sensor_Ht   RecCount       StnPress           Temp       DewPoint      Rel_Humid      SpecHumid      WindSpeed        WindDir         U_Wind         V_Wind\n";

         printf "    %7.2f %10d %10d [%d] %10d [%d] %10d [%d] %10d [%d] %10d [%d] %10d [%d] %10d [%d] %10d [%d] %10d [%d]\n",
              $height, $sensor_hts{$height} {COUNT_RECS},
   ...
   # Issue File Scores
   #------------------
   $total_meta = $number_lines_processed*14; # 14 meta/line (Not CSE,RefSite,StnID)
...
sub check_general_file_info {
   ...
   if ($number_lines_in_file > 0)
      { $rec_size = $file_info[7]/$number_lines_in_file; }
    else
      {print "ERROR: no data in file $input_file_name!\n";}
   ...
   system "cosort final_CEOP_sort_height";
...
;
   ...
   #---------------------------------------------------------------------
   # Check for inexact Dups. Include all the way up to the sensor height.
   #---------------------------------------------------------------------
   system "cut -c1-115 input.sort.uniq > in1-115chars.sort"; #incld sensor Height
   system "uniq in1-115chars.sort > in1-115chars.uniq";
   system "diff in1-115chars.sort in1-115chars.uniq> INEXACT_DUP_records_$input_file_name.out"; 
   ...
   #-----------------------------------------------
   system "/bin/rm input.dat input.sort.uniq in1-115chars.sort in1-115chars.uniq";
...
#    CSE_ReferenceSite_Station_BeginDate_EndDate.twr
#
# (e.g., LBA_Pantanal_Pantanal_20020701_20020705.twr)
#
...
#    Meta Data are in first 115 chars of record.
...
sub parse_record_metadata {
   ...
   %meta_positions = (
      ...
      SENSOR_HEIGHT =>{BEG_POS=>108, LENGTH=>7,  MIN_VAL=>0.00, MAX_VAL=>1000.00} #Must be above ground level
      );
         ...
         if ($meta_value < -999.90 && $meta ne ELEV && $meta ne LAT && $meta ne LON)
            ...
            if ($meta eq ELEV)
               ...
               if ($err_elev_missing == 0)
                  { print "      ERROR META: (1st Instance) $meta ($meta_value) is MISSING or NEGATIVE ($meta_positions{$meta}{MIN_VAL}, $meta_positions{$meta}{MAX_VAL}) at line $number_lines_processed in $input_file_name. Can NOT be
Missing!\n"; }
   ...
   $sensor_height = substr ($input_record, $meta_positions{SENSOR_HEIGHT}{BEG_POS},
                   $meta_positions{SENSOR_HEIGHT}{LENGTH}); 

   #----------------------------------------------
   # Verify that the sensor height is always above
   # ground level for tower data.
   #----------------------------------------------
   if ($sensor_height <=0.00)
      {
      print "      ERROR META: CSE_RefSite_StnID ($CSE_RefSite_StnID) SENSOR HEIGHT is NOT Above Ground Level ($sensor_height) at line $number_lines_processed in $input_file_name.\n"; $err_meta++;           

      }

   #------------------------------------------------------
   # Form and Save Uniq list of all sensor heights found
   # and how many of each were found.
   #------------------------------------------------------
   if ($debug) {print "\nSensor Height = $sensor_height\n";}

   if (exists $sensor_hts{$sensor_height}{COUNT_RECS})
      {
      #----------------------------------------
      # This height already exists in the hash.
      #----------------------------------------       
      if ($debug) { print "$sensor_height EXISTS in the List of Uniq sensor heights.\n";}  
      $sensor_hts{ $sensor_height}{COUNT_RECS}++;
      }
   else
      {
      #--------------------------------
      # This is a new height to list.
      #--------------------------------
      if ($debug) { print "$sensor_height is NEW. Add to List of Uniq sensor heights.\n";}

      #-------------------------------------------------------------------
      # COUNT_recs - total number of records found at this height.
      # SOIL_*_COUNT - total count of that type value at this height.
      # SOIL_*_ERROR_COUNT - total count of any type DATA error at height.
      #-------------------------------------------------------------------
      $sensor_hts{ $sensor_height}{COUNT_RECS} = 1;

      $sensor_hts{ $sensor_height}{STN_PRESSURE_COUNT} = 0;
      $sensor_hts{ $sensor_height}{STN_PRESSURE_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{TEMPERATURE_COUNT} = 0;
      $sensor_hts{ $sensor_height}{TEMPERATURE_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{DEW_PT_COUNT} = 0;
      $sensor_hts{ $sensor_height}{DEW_PT_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{RELATIVE_HUMIDITY_COUNT} = 0;
      $sensor_hts{ $sensor_height}{RELATIVE_HUMIDITY_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{SPECIFIC_HUMIDITY_COUNT} = 0;
      $sensor_hts{ $sensor_height}{SPECIFIC_HUMIDITY_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{WIND_SPEED_COUNT} = 0;
      $sensor_hts{ $sensor_height}{WIND_SPEED_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{WIND_DIRECTION_COUNT} = 0;
      $sensor_hts{ $sensor_height}{WIND_DIRECTION_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{U_WIND_COMPONENT_COUNT} = 0;
      $sensor_hts{ $sensor_height}{U_WIND_COMPONENT_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{V_WIND_COMPONENT_COUNT} = 0;
      $sensor_hts{ $sensor_height}{V_WIND_COMPONENT_ERROR_COUNT} = 0;

      } # keep list of uniq Sensor Heights
   ...
   if ($record_Ref_Site eq $file_name_Ref_Site)
      ...
      if ($mismatch_Ref_Site == 0)
         {
         print "      ERROR META: (1st Instance) Record_Ref_Site ($record_Ref_Site) does NOT match File_Name_Ref_Site
 ($file_name_Ref_Site) at line $number_lines_processed in $input_file_name.\n";
         }
...
sub parse_record_data {
   ...
   @space_positions = (
       ...
       107, 115, 123, 125, 133, 135, 143, 145, 153, 
       155, 163, 165, 173, 175, 183, 185, 193, 195, 203);

   if ($debug) { print  "\nspace_positions:: @space_positions\n"; }


   @decimalPt_positions = (
        81, 93, 104, 112, 120, 130, 140, 150, 160, 170, 180, 190, 200);
   ...
   %flag_positions = (
        ...
        "specific_humidity"  => 164, 
        "wind_speed"         => 174, 
        "wind_direction"     => 184, 
        "U_wind_component"   => 194, 
        "V_wind_component"   => 204
      );
   ...
   #---------------------------------------------------------------
   %flag_count = (
      "stn_pressure"       => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "temperature"        => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "dew_pt"             => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "relative_humidity"  => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "specific_humidity"  => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "wind_speed"         => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "wind_direction"     => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "U_wind_component"   => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "V_wind_component"   => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
     );                               
   ...
   #----------------------------------------------------------------------
   %parm_positions = (
      stn_pressure       =>{BEG_POS=>116, LENGTH=>7, MIN_VAL=>300.00,  MAX_VAL=>1100.00, 
      ...
      dew_pt             =>{BEG_POS=>136, LENGTH=>7, MIN_VAL=>-90.00,  MAX_VAL=>35.00,   
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      relative_humidity  =>{BEG_POS=>146, LENGTH=>7, MIN_VAL=>0.00,    MAX_VAL=>102.00,  
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      specific_humidity  =>{BEG_POS=>156, LENGTH=>7, MIN_VAL=>0.00,    MAX_VAL=>35.00,   
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      wind_speed         =>{BEG_POS=>166, LENGTH=>7, MIN_VAL=>0.00,    MAX_VAL=>50.00,   
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      wind_direction     =>{BEG_POS=>176, LENGTH=>7, MIN_VAL=>0.00,    MAX_VAL=>360.00,  
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      U_wind_component   =>{BEG_POS=>186, LENGTH=>7, MIN_VAL=>-50.00,  MAX_VAL=>50.00,   
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      V_wind_component   =>{BEG_POS=>196, LENGTH=>7, MIN_VAL=>-50.00,  MAX_VAL=>50.00,   
                                          MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0}
      );
...
sub check_record_data {
   ...
   foreach $parm (keys %flag_positions)
      ...
      if ($parm_flag ne 'U' && $parm_flag ne 'G' && $parm_flag ne 'D' &&
         ...
         if ($parm eq stn_pressure) {$sensor_hts{ $sensor_height}{STN_PRESSURE_ERROR_COUNT}++;}
         if ($parm eq temperature) {$sensor_hts{ $sensor_height}{TEMPERATURE_ERROR_COUNT}++;}
         if ($parm eq dew_pt) {$sensor_hts{ $sensor_height}{DEW_PT_ERROR_COUNT}++;}
         if ($parm eq relative_humidity) {$sensor_hts{ $sensor_height}{RELATIVE_HUMIDITY_ERROR_COUNT}++;}
         if ($parm eq specific_humidity) {$sensor_hts{ $sensor_height}{SPECIFIC_HUMIDITY_ERROR_COUNT}++;}
         if ($parm eq wind_speed) {$sensor_hts{ $sensor_height}{WIND_SPEED_ERROR_COUNT}++;}
         if ($parm eq wind_direction) {$sensor_hts{ $sensor_height}{WIND_DIRECTION_ERROR_COUNT}++;}
         if ($parm eq U_wind_component) {$sensor_hts{ $sensor_height}{U_WIND_COMPONENT_ERROR_COUNT}++;}
         if ($parm eq V_wind_component) {$sensor_hts{ $sensor_height}{V_WIND_COMPONENT_ERROR_COUNT}++;}
   ...
   foreach $parm (sort keys %parm_positions)
      ...
      if ($parm_value ne "-999.99" && $parm_value ne " -999.99") 
         ...
         if ($debug) {print "parm = $parm, sensor_height = $sensor_height\n";}

         if ($parm eq stn_pressure)
            {
            $sensor_hts{ $sensor_height}{STN_PRESSURE_COUNT}++;
            if ($debug) {print "Increment (STN_PRESSURE) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{STN_PRESSURE_COUNT}\n";}
            }

        if ($parm eq temperature)
            {
            $sensor_hts{ $sensor_height}{TEMPERATURE_COUNT}++;
            if ($debug) {print "Increment (TEMPERATURE) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{TEMPERATURE_COUNT}\n";}
            }

        if ($parm eq dew_pt)
            {
            $sensor_hts{ $sensor_height}{DEW_PT_COUNT}++;
            if ($debug) {print "Increment (DEW_PT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{DEW_PT_COUNT}\n";}
            }

        if ($parm eq relative_humidity)
            {
            $sensor_hts{ $sensor_height}{RELATIVE_HUMIDITY_COUNT}++;
            if ($debug) {print "Increment (RELATIVE_HUMIDITY) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{RELATIVE_HUMIDITY_COUNT}\n";}
            }

        if ($parm eq specific_humidity)
            {
            $sensor_hts{ $sensor_height}{SPECIFIC_HUMIDITY_COUNT}++;
            if ($debug) {print "Increment (SPECIFIC_HUMIDITY) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{SPECIFIC_HUMIDITY_COUNT}\n";}
            }

        if ($parm eq wind_speed)
            {
            $sensor_hts{ $sensor_height}{WIND_SPEED_COUNT}++;
            if ($debug) {print "Increment (WIND_SPEED) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{WIND_SPEED_COUNT}\n";}
            }

        if ($parm eq wind_direction)
            {
            $sensor_hts{ $sensor_height}{WIND_DIRECTION_COUNT}++;
            if ($debug) {print "Increment (WIND_DIRECTION) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{WIND_DIRECTION_COUNT}\n";}
            }

        if ($parm eq U_wind_component)
            {
            $sensor_hts{ $sensor_height}{U_WIND_COMPONENT_COUNT}++;
            if ($debug) {print "Increment (U_WIND_COMPONENT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{U_WIND_COMPONENT_COUNT}\n";}
            }

        if ($parm eq V_wind_component)
            {
            $sensor_hts{ $sensor_height}{V_WIND_COMPONENT_COUNT}++;
            if ($debug) {print "Increment (V_WIND_COMPONENT) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{V_WIND_COMPONENT_COUNT}\n";}
            }
         ...
         {
         print "      ERROR DATA: Parameter Flag ($parm_flag) at position $flag_positions{$parm} is NOT a legal (value (i.e., U,G,D,B,I) for a NON-MISSING Value at line $number_lines_processed in $input_file_name.\n";  $err++; $err_data++; $parm_positions{$parm}{ERR_CT}++;
         ...
         if ($parm eq "U_wind_component" || $parm eq "V_wind_component")
            ...
            if (abs($parm_value) > abs($wind_speed) )
               ...
               $sensor_hts{ $sensor_height}{WIND_SPEED_ERROR_COUNT}++;
               $err++; $err_data++; $parm_positions{$parm}{ERR_CT}++;
               } 
...
##################################################
# check_CEOP_MetTwr.pl - MAIN processing.
##################################################
printf "\ncheck_CEOP_MetTwr.pl of file $ARGV[0] began on ";print scalar localtime; printf "\n";
...
while ($input_line = <INPUT_FILE>)
   ...
   chomp($input_line); # 204 chars w/o return
...
#-------------------------------------------------------
# Beware - RECURSIVE software.
#
# If there is more than one height in this dataset, then
# divide the data up into files by depth and rerun this
# checker s/w on that depth. Delete intermediate files
# but keep stat files for each depth.
#-------------------------------------------------------
$number_heights = keys (%sensor_hts);

if ($number_heights > 1 && $number_heights <= 15)
   {
   print "\n------------------------------------------------\n";
   print "There are $number_heights heights in input file.\n";
   print "Generate statistics for each height separately.\n";

   if ($debug) {print "Create height data files.\n";}
   print "Separate statistics (*.log) files will be generated for each of the following heights:\n";

   #---------------------------------------------------------
   #  The following may not work if there are more stns than
   #  just one in input file!
   #  or only one stn will have levels processed!!!
   #---------------------------------------------------------
   @infile_parts = split /\./, $ARGV[0];

   $lat = $stnInfo{ $CSE_RefSite_StnID }{LAT};
   $lon = $stnInfo{ $CSE_RefSite_StnID }{LON};
   $elev = $stnInfo{ $CSE_RefSite_StnID }{ELEV};
   if ($elev < -9999.0){$elev = -999.99;} #earlier reset to printing stationCD.out value.

   $lat =~ s/^\s+//; #remove preceeding spaces.
   $lat =~ s/\-/\./; #remove forward slashes.
   $lon =~ s/^\s+//; #remove preceeding spaces.
   $lon =~ s/\-/\./; #remove forward slashes.
   $elev =~ s/^\s+//; #remove preceeding spaces.
   $elev =~ s/\-/\./; #remove forward slashes.

   if ($debug) { print "lat,lon, elev: $lat, $lon, $elev\n"; }

   foreach $height (sort keys %sensor_hts)
      {
      $cmd_height = $height;

      $height =~ s/^\s+//; #remove preceeding spaces.
      $cmd_height = $height;
      $height =~ s/\-/\./; #remove forward slashes.

      $cmd_part1 = sprintf "grep ' %10s %11s %7s %7s ' ", $lat, $lon, $elev, $height;
      $cmd = $cmd_part1.$ARGV[0]." > ".$ARGV[0].$cmd_height."m";

      if ($debug) { print "Issue the following command: $cmd\n"; }
      system $cmd;

      if ($debug) {print "Spawn process to check height ($height) data.\n";}
      $cmd = "check_CEOP_MetTwr.pl ".$ARGV[0].$cmd_height."m > ".$infile_parts[0]."_height".$cmd_height."m.log";  

      print "Spawn the following command: $cmd\n";
      system $cmd;
   ...
   {
   print "\n------------------------------------------------\n";
   print "There are $number_heights sensor heights in the input file. If 1 height, then done. If > 15 heights, then requires manual processing. Processing complete!\n";
   }


print "\nTotal number lines processed in file $ARGV[0]: $number_lines_processed\n";

printf "\ncheck_CEOP_MetTwr.pl of file $ARGV[0] ended on ";print scalar localtime;
printf "\n";
