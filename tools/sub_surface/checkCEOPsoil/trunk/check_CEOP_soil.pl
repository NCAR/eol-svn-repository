...
#----------------------------------------------------------------------
# check_CEOP_soil.pl - This perl program verifies that the input file
#   is in the 29 July 2003 CEOP SSC approved Soil Temp and Soil Moisture
...
#
#   Execute: check_CEOP_soil.pl CSE_RefSite_Stn_bdate_edate.stm 0
#
#   Required Inputs: Two input parameters are required as shown below:
#
#      check_CEOP_soil.pl input_file_name info_level
#
...
#
#   E.g., check_CEOP_soil.pl LBA_Pantanal_Pantanal_20020701_20020701.stm 2
#
...
#      record are missing. 
#   That the sensor height is less than or equal to 0.00 (i.e., ground level).
#
...
# Aug 2007 lec Version6 
#     - Update to handle Phase II 2006 dates.
# Feb 2009 lec Version7
#     - Update to handle CEOP Phase II dates through 2008.
# Aug 2011 lec Version 8
#     - Update to handle data through 2010. Removed "HERE" comments.
#------------------------------------------------------------------------
# HARDCODED
#
...
$RECORD_LENGTH = 138; # HARDCODED - Includes return for the Met/Radiation format
$FILE_SUFFIX = "stm"; # HARDCODED - Set for soils format.
...
sub write_final_stats{
   ...
   foreach $height (sort keys %sensor_hts)
      {
      if ($sensor_hts{$height}{SOIL_TEMP_COUNT} > 0 && 
          $sensor_hts{$height}{SOIL_MOIST_COUNT} > 0 &&
          $sensor_hts{$height}{COUNT_RECS} > 0)
         {
         printf "Sensor_Height: %7.2f  Record_Count: %10d Soil_Temps: %10d (%6.1f %) [%10d]  Soil_Moistures: %10d (%6.1f %) [%10d]\n", 
              $height, $sensor_hts{$height} {COUNT_RECS},
              $sensor_hts{$height}{SOIL_TEMP_COUNT}, 
              ($sensor_hts{$height}{SOIL_TEMP_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{SOIL_TEMP_ERROR_COUNT},

              $sensor_hts{$height}{SOIL_MOIST_COUNT},
              ($sensor_hts{$height}{SOIL_MOIST_COUNT}/$sensor_hts{$height}{COUNT_RECS})*100.0,
              $sensor_hts{$height}{SOIL_MOIST_ERROR_COUNT};

         }
      else
         {
         printf "Sensor_Height: %7.2f  Record_Count: %10d Number_Soil_Temps: %10d [%d]  Number_Soil_Moisture: %10d [%d]\n",
              $height, $sensor_hts{$height} {COUNT_RECS},
              $sensor_hts{$height}{SOIL_TEMP_COUNT},
              $sensor_hts{$height}{SOIL_TEMP_ERROR_COUNT},
              $sensor_hts{$height}{SOIL_MOIST_COUNT},
...
#    CSE_ReferenceSite_Station_BeginDate_EndDate.stm
#
# (e.g., LBA_Pantanal_Pantanal_20020701_20020705.stm)
#
...
sub parse_and_check_file_name {
   ...
   if (scalar(@name_title) == 5 )
      ...
      #------------------------------------------------------
      # WARNING: THIS MAY NOT WORK IN ALL CASES!!!!! 
      #------------------------------------------------------
...
sub parse_record_metadata {
   ...
   %meta_positions = (
      ...
      SENSOR_HEIGHT =>{BEG_POS=>108, LENGTH=>7,  MIN_VAL=>-999.99, MAX_VAL=>0.00} #Must be at or below ground level
      );
...
";}  
      ...
      $sensor_hts{ $sensor_height}{SOIL_TEMP_COUNT} = 0;
      $sensor_hts{ $sensor_height}{SOIL_TEMP_ERROR_COUNT} = 0;

      $sensor_hts{ $sensor_height}{SOIL_MOIST_COUNT} = 0;
      $sensor_hts{ $sensor_height}{SOIL_MOIST_ERROR_COUNT} = 0;
   ...
   #----------------------------------------------
   # Verify that the sensor height is always below
   # or at ground level for soils data.
   #----------------------------------------------
   if ($sensor_height >0.00)
      {
      print "      ERROR META: CSE_RefSite_StnID ($CSE_RefSite_StnID) SENSOR HEIGHT is NOT At or Below Ground Level ($sensor_height) at line $number_lines_processed in $input_file_name.\n"; $err_meta++;
      }
...
sub parse_record_data {
   ...
   @decimalPt_positions = (81, 93, 104, 112, 121, 132);
   ...
   #---------------------------------------------------------------
   %flag_count = (
      "soil_temperature" => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0},
      "soil_moisture"    => {C=>0,M=>0,B=>0,I=>0,D=>0,G=>0,U=>0,UNKNOWN=>0}
     );                               
   ...
   #----------------------------------------------------------------------
   %parm_positions = (
      soil_temperature =>{BEG_POS=>116, LENGTH=>8, MIN_VAL=>-75.00,  
        MAX_VAL=>75.00,   MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
      soil_moisture    =>{BEG_POS=>127, LENGTH=>8, MIN_VAL=>0.00,  
        MAX_VAL=>100.00,   MIN_EXT=>9999.99,  MAX_EXT=>-999.99, ERR_CT=>0},
...
sub check_record_data {
   ...
   foreach $parm (keys %flag_positions)
      ...
      if ($parm_flag ne 'U' && $parm_flag ne 'G' && $parm_flag ne 'D' &&
         ...
         if ($parm eq soil_temperature) {$sensor_hts{ $sensor_height}{SOIL_TEMP_ERROR_COUNT}++;}
         if ($parm eq soil_moisture) {$sensor_hts{ $sensor_height}{SOIL_MOIST_ERROR_COUNT}++;}
   ...
   foreach $parm (sort keys %parm_positions)
      ...
      if ($parm_value ne "-999.99" && $parm_value ne " -999.99") 
         ...
         if ($parm eq soil_temperature)
            { 
            $sensor_hts{ $sensor_height}{SOIL_TEMP_COUNT}++; 

            if ($debug) {print "Increment (SOIL_TEMP) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{SOIL_TEMP_COUNT}\n";}
            }

         if ($parm eq soil_moisture)
            { 
            $sensor_hts{ $sensor_height}{SOIL_MOIST_COUNT}++; 

            if ($debug) {print "Increment (SOIL_MOIST) sensor_hts at $sensor_height = $sensor_hts{ $sensor_height}{SOIL_MOIST_COUNT}\n";}
            }
...
##################################################
# check_CEOP_soil.pl - MAIN processing.
##################################################
printf "\ncheck_CEOP_soil.pl of file $ARGV[0] began on ";print scalar localtime; printf "\n";
...
if ($number_heights > 1 && $number_heights <= 15)
   ...
   foreach $height (sort keys %sensor_hts)
      ...
      $cmd = "check_CEOP_soil.pl ".$ARGV[0].$cmd_height."m > ".$infile_parts[0]."_height".$cmd_height."m.log";
...
printf "\ncheck_CEOP_soil.pl of file $ARGV[0] ended on ";print scalar localtime;
printf "\n";
