...
struct qcf
    {
    char nomDate[11];       /* UTC nominal date of observation */
    char nomTime[6];       /* UTC nominal time of observation */
    char actualDate[11];    /* UTC actual date of observation */
...
**Date: 06/09/2008
**
**uniqueList created to store a unique list of network,
**station, parameter for each error generated during
**execution. The list is then output to file for use
**with plot_converted_data.pl
**
**each element of the 3 arrays will contain the network,
**station, and parameter for each error.
**---------------------------------------------------*/
struct uniqueList
	{
    char *networkList[10000];
    char *stationList[10000];
    char *parameterList[10000];
	};

typedef struct uniqueList UNIQUE_LIST;
