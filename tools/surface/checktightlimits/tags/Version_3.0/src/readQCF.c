...
** description.
**------------------------------------------------------------------*/
int ReadQCF_Record(FILE* qcfFile)
    {
    char ch;
    int i;
    char datestr[6];
    ...
    fscanf(qcfFile, "%5s", datestr);
    strcpy(qcf_record.nomDate,datestr);
    temp_str = strtok(datestr, "/");

    if (strlen(temp_str) == 4)
       {
       fscanf(qcfFile, "%5s %5s %10s %5s",
                       datestr,
                       qcf_record.nomTime,
    ...
    strcat(qcf_record.nomDate,datestr);
