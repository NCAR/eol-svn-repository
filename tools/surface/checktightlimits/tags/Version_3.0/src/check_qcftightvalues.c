...
**
** 000 15 Aug 2003 jec
**    Modified to stop printing error messages for missing elevation
**    values.  Also made the error messages take only one line.
**
** Edited by: Ben Golden
** Date: 06/09/2008
** Modified to accumulate error details in a UNIQUE_LIST object.
** The network, station, and parameter in error are added if they
** are not a repeat of the previous entry.
**----------------------------------------------------------------*/
...
int Check_Date_Value(char *datevalue,int line);
int Check_Time_Value(char *datevalue,int line);
...
** 
**------------------------------------------------------------------*/
int Check_QCFRecord(QCF_REC_TYPE qcf_record, int line, UNIQUE_LIST *uList, int *uCount, FILE *logFile)
    {

    char uniqueListFill[180];
    int istatus, goodStatus, loop, found;
    ...
    if (!istatus)
      {
      goodStatus = FALSE;
      fprintf (logFile, "# Not Implemented /work/bin/plotters/plot_converted_data.pl ... ERROR (Line %04d): Bad lat (%7.2f) for %-s %-s\n",
             line, qcf_record.lat,
...
** Added by: Ben Golden
** 
** This is the code used to capture network, station, parameter 
** data into the UNIQUE_LIST. The information is not stored if
** the same entry already exists previously. The code is repeated
** for each relevant ERROR message. 
**-------------------------------------------------------------*/


/*--------------------latitude----------------------------------
**UniqueList functionality disabled, plot_converted_data.pl
**does not accept latitude as a parameter so there is no reason
**to include latitude in the UniqueList.
**-------------------------------------------------------------

	//stationId has some trailing whitespcae, so we trim it. 
	strcpy(uniqueListFill, strtok(qcf_record.stationId, " "));

	//If the UNIQUE_LIST is empty we just add this first error info.
	if((*uList).networkList[0] == NULL)
		{
		//Copy the needed info and increment the index tracker.
		(*uList).networkList[(*uCount)] = qcf_record.networkId;
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "latitude";
		(*uCount)++;
		}
	//If the UNIQUE_LIST is not empty.
	else
		{	
		
		found = FALSE;	
		for(loop =0;loop<(*uCount);loop++)
			{
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("latitude", (*uList).parameterList[loop])==0)
				{	
			found = TRUE;
				}
			}
...
-----------UniqueList functionality disabled-------------------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "# Not Implemented /work/bin/plotters/plot_converted_data.pl ... ERROR (Line %04d): Bad lon (%7.2f) for %-s %-s\n",
             line, qcf_record.lon,
             qcf_record.networkId, qcf_record.stationId);
...
**-------------------------------------------------------------
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "longitude";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("longitude", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
-----------UniqueList functionality disabled-------------------*/
    ...
    if (!istatus) 
       {
       goodStatus = FALSE;
       fprintf (logFile, "# Not Implemented /work/bin/plotters/plot_converted_data.pl ... ERROR (Line %04d): Bad elev (%7.2f) for %-s %-s\n",
		line, qcf_record.elevation,
		qcf_record.networkId, qcf_record.stationId); 
...
**-------------------------------------------------------------
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "elevation";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("elevation", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
-----------UniqueList functionality disabled-------------------*/
    ...
    **------------------------------------------------------------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad stn press (%7.2f) for %-s %-s on %-s %-s\n",
		line, qcf_record.pressure,
		qcf_record.networkId, qcf_record.stationId,
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "STN_Press";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("STN_Press", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if (!istatus)  
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad SLP (%7.2f) for %-s %-s on %-s %-s\n", 
		line, qcf_record.sealvlprs, 
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "SL_Press";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("SL_Press", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad Calc SLP (%7.2f) for %-s %-son %-s %-s\n", 
		line, qcf_record.comp_sealvl, 
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Comp_SL_Press";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Comp_SL_Press", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
/*------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad temperature (%7.2f) for %-s %-s on %-s %-s\n",
		line, qcf_record.temperature,
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Air_Temp";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Air_Temp", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad dew_point (%7.2f) for %-s %-s on %-s %-s\n",
		line, qcf_record.dew_point,
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Dew_Pt";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Dew_Pt", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if ((qcf_record.temperature>-900.0) &&
       ...
       {
       fprintf (logFile, "ERROR (Line %04d): dew_point (%7.2f) > temperature (%7.2f) for %-s %-s on %-s %-s\n",
		line, qcf_record.dew_point, qcf_record.temperature, 
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
    ...
    if ( (qcf_record.temperature>-900.0) && 
       ...
       { 
       fprintf (logFile, 
               "WARNING (Line %04d): temperature (%7.2f) - dewpoint (%7.2f) > 50 for %-s %-s on %-s %-s\n",
		line, qcf_record.temperature, qcf_record.dew_point,
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*-------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad Wind Speed (%7.2f) for %-s %-s on %-s %-s\n",
		line, qcf_record.windSpeed,
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Wind_Spd";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Wind_Spd", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (logFile, "ERROR (Line %04d): Bad wind dir (%7.2f) for %-s %-s on %-s %-s\n",
		line, qcf_record.windDir,
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Wind_Dir";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Wind_Dir", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if (!istatus)
       { 
       goodStatus = FALSE; 
       fprintf (logFile, "ERROR (Line %04d): Bad squall Gust (%7.2f) for %-s %-s on %-s %-s\n", 
		line, qcf_record.squallGustSpeed,
		qcf_record.networkId, qcf_record.stationId,
		qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Squall_Gust";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Squall_Gust", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    if ((qcf_record.squallGust != 'G') && (qcf_record.squallGust != 'S') &&
       ...
       { 
       fprintf(logFile, "Invalid symbol for SquallGust (Line %04d); indicator = %c\n",
	       line, qcf_record.squallGust);
       goodStatus = FALSE;
       } 
...
/*-------*/
    ...
    if (!istatus) 
       { 
       goodStatus = FALSE; 
       fprintf (logFile, "ERROR (Line %04d): Bad precip (%7.2f) for %-s %-s on %-s %-s\n", 
             line, qcf_record.precip,
             qcf_record.networkId, qcf_record.stationId,
             qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Precip";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Precip", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
/*-------*/
	...
	{
	goodStatus = FALSE;
	fprintf (logFile, "ERROR (Line %04d): Bad visibility (%7.2f) for %-s %-s on %-s %-s\n", 
        line, qcf_record.visibility,
        qcf_record.networkId, qcf_record.stationId,
        qcf_record.nomDate, qcf_record.nomTime);
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Visibility";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Visibility", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
//}
    ...
    **-----------------------------------------------------------------*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Ceiling_Ht1";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Ceiling_Ht1", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Ceiling_Ht2";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Ceiling_Ht2", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
...
/*---UNIQUE_LIST---*/
	...
	if((*uList).networkList[0] == NULL)
		...
		(*uList).stationList[(*uCount)] = (char*) malloc(sizeof(uniqueListFill));
		strcpy((*uList).stationList[(*uCount)], uniqueListFill);
		(*uList).parameterList[(*uCount)] = "Ceiling_Ht3";
		(*uCount)++;
		...
		for(loop =0;loop<(*uCount);loop++)
			...
			if(strcmp(qcf_record.networkId, (*uList).networkList[loop])==0 &&
				  strcmp(uniqueListFill, (*uList).stationList[loop])==0 &&
				  strcmp("Ceiling_Ht3", (*uList).parameterList[loop])==0)
				{	
				found = TRUE;
				}
			}
    ...
    **---------------------------------*/
    if ((qcf_record.ca1_flag != 'M') && (qcf_record.ca1_flag != 'N'))
        ...
        if (qcf_record.ch1_flag != 'M')
           {
           goodStatus = FALSE;
	   fprintf(logFile, "WARNING (Line %04d) Ceiling Height reported without Cloud Amount.\n", line);
	   }
	}
    ...
    **--------------------------------------------*/
    ...
    istatus = Check_Date_Value(qcf_record.nomDate,line);
    if (!istatus)  goodStatus = FALSE;
        
    assert(qcf_record.actualDate != NULL);
    istatus = Check_Date_Value(qcf_record.actualDate,line);
    if (!istatus)  goodStatus = FALSE;
       
    assert(qcf_record.nomTime != NULL);
    istatus = Check_Time_Value(qcf_record.nomTime,line);
    if (!istatus)  goodStatus = FALSE;
     
    assert(qcf_record.actualTime != NULL);
    istatus = Check_Time_Value(qcf_record.actualTime,line);
    if (!istatus)  goodStatus = FALSE;
...
int Check_Float_tightValue(float value, char qc_flag, DATAVALUE_TYPE value_type)
    ...
    **--------------------------------------------------------*/
    ...
    /*-------------------------------------------------------------
    ** Allows the elevation to have a missing value.
    **-----------------------------------------------------------*/
    if (value_type == elevation && value > -1000.0 && value < -999.98) {
      return TRUE;
    }
...
int Check_Date_Value(char *datevalue, int line)
    ...
    **---------------------------------*/
    ...
    if (year_length == 0)
       ...
       istatus = FALSE;
       fprintf(stderr,
	      "WARNING (Line %04d): YEAR = %s changes from %d char year to %d char year\n",
	       line, temp_str,year_length, strlen(temp_str));
       }
    ...
    **-------------------------------------------------------*/
    ...
    if (((year_length == 2)  && (year < 0 || year > 99)) ||
       ...
       {
       istatus = FALSE;
       fprintf(stderr, "ERROR (Line %04d) Invalid YEAR = %d\n", line, year);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if ((month < 1) || (month > 12))
       {
       istatus = FALSE;
       fprintf(stderr, "ERROR (Line %04d) Invalid MONTH = %d\n", line, month);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if ((day < 1) || (day > NUMDAYS[month]))
       {
       istatus = FALSE;
       fprintf(stderr, "ERROR (Line %04d) Invalid DAY = %d\n", line, day);
       }
...
**------------------------------------------------------------------*/
int Check_Time_Value(char *timevalue, int line)
    {
    int hour, minute;
    int istatus;
    ...
    **-------------------------------------------------------*/
    ...
    if ((hour < 0) || (hour > 23))
       {
       istatus = FALSE;
       fprintf(stderr, "ERROR (Line %04d) Invalid Hour = %d\n", line, hour);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if ((minute < 0) || (minute > 59))
       {
       istatus = FALSE;
       fprintf(stderr, "ERROR (Line %04d) Invalid Minutes = %d\n", line, minute);
       }
