...
** 
**------------------------------------------------------------------*/
int Check_QCFRecord(QCF_REC_TYPE qcf_record, int line)
    {
    ...
    if (!istatus)
      {
      goodStatus = FALSE;
      fprintf (stderr, "ERROR (Line %04d): Bad lat (%7.2f) for %-s %-s\n",
             line, qcf_record.lat,
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad lon (%7.2f) for %-s %-s\n",
             line, qcf_record.lon,
             qcf_record.networkId, qcf_record.stationId); 
       }
    ...
    if (!istatus) 
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad elev (%7.2f) for %-s %-s\n",
    ...
    **------------------------------------------------------------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad stn press (%7.2f) for %-s %-s on %-s %-s\n",
    ...
    if (!istatus)  
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad SLP (%7.2f) for %-s %-s on %-s %-s\n", 
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad Calc SLP (%7.2f) for %-s %-son %-s %-s\n", 
...
/*-------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad temperature (%7.2f) for %-s %-s on %-s %-s\n",
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad dew_point (%7.2f) for %-s %-s on %-s %-s\n",
    ...
    if ((qcf_record.temperature>-900.0) &&
       ...
       {
       fprintf (stderr, "ERROR (Line %04d): dew_point (%7.2f) > temperature (%7.2f) for %-s %-s on %-s %-s\n",
...
/*-------*/
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad Wind Speed (%7.2f) for %-s %-s on %-s %-s\n",
    ...
    if (!istatus)
       {
       goodStatus = FALSE;
       fprintf (stderr, "ERROR (Line %04d): Bad wind dir (%7.2f) for %-s %-s on %-s %-s\n",
    ...
    if (!istatus)
       { 
       goodStatus = FALSE; 
       fprintf (stderr, "ERROR (Line %04d): Bad squall Gust (%7.2f) for %-s %-s on %-s %-s\n", 
    ...
    if ((qcf_record.squallGust != 'G') && (qcf_record.squallGust != 'S') &&
       ...
       { 
       fprintf(stderr, "Invalid symbol for SquallGust (Line %04d); indicator = %c\n",
...
/*-------*/
    ...
    if (!istatus) 
       { 
       goodStatus = FALSE; 
       fprintf (stderr, "ERROR (Line %04d): Bad precip (%7.2f) for %-s %-s on %-s %-s\n", 
    ...
    **---------------------------------*/
    if ((qcf_record.ca1_flag != 'M') && (qcf_record.ca1_flag != 'N'))
        ...
        if (qcf_record.ch1_flag != 'M')
           {
           goodStatus = FALSE;
	   fprintf(stderr, "WARNING (Line %04d) Ceiling Height reported without Cloud Amount.\n", line);
           }
        }
