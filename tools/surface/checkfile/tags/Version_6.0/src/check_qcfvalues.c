...
** 006 21 Mar 2003 jag
**    Increased acceptable visibility to 160000m (just under 100SM).
**    Visibilities of up to 90SM are commonly seen in the West, an
**    area which is now showing up in our projects (i.e. IHOP).
** 007 18 Jul 2005 jag
**    Increased acceptable visibility to 160934.41 (100SM).
**    Visibilities of 100SM can occur in the West, an
**    area which is now showing up in our projects (i.e. NAME).
**----------------------------------------------------------------*/
...
** Function Prototypes
**--------------------*/
int Check_Float_Value(float value, char qc_flag, DATAVALUE_TYPE value_type, int recnum);
int Check_Int_Value(int value, DATAVALUE_TYPE value_type, int recnum);
int Check_Date_Value(char *datevalue, int recnum);
int Check_Time_Value(char *datevalue, int recnum);
...
** 
**------------------------------------------------------------------*/
int Check_QCFRecord(QCF_REC_TYPE qcf_record, int recnum)
    {
    int istatus, goodStatus;

    goodStatus = TRUE;

    istatus = Check_Float_Value(qcf_record.lat, ' ', latitude,recnum);
    if (!istatus)  goodStatus = FALSE;
    istatus = Check_Float_Value(qcf_record.lon, ' ', longitude,recnum);
    if (!istatus)  goodStatus = FALSE;
    istatus = Check_Float_Value(qcf_record.elevation, ' ', elevation,recnum);
    if (!istatus)  goodStatus = FALSE;
    ...
    **------------------------------------------------------------*/
    ...
    istatus = Check_Float_Value(qcf_record.precip, qcf_record.prcflag,
       precipitation,recnum);
    if (!istatus)  goodStatus = FALSE;

    if ((qcf_record.squallGust != 'G') && (qcf_record.squallGust != 'S') &&
        (qcf_record.squallGust != ' '))
       {
       fprintf(stderr, "WARNING: Invalid symbol for SquallGust; ");
       fprintf(stderr, "indicator = %c in record %d\n",
         qcf_record.squallGust, recnum);
       goodStatus = FALSE;
       }

    istatus = Check_Float_Value(qcf_record.squallGustSpeed, 
       qcf_record.squallGustFlag, squallGust,recnum);
    if (!istatus)  goodStatus = FALSE;
    ...
    **-----------------------------------------------------------------*/
    istatus = Check_Float_Value(qcf_record.ceilingHt1, qcf_record.ch1_flag,
       ceilingHeight1,recnum);
    if (!istatus)  goodStatus = FALSE;

    istatus = Check_Int_Value(qcf_record.ch1_code, ceilingFlag1,recnum);
    if (!istatus)  goodStatus = FALSE;

    istatus = Check_Float_Value(qcf_record.ceilingHt2, qcf_record.ch2_flag,
        ceilingHeight2,recnum);
    if (!istatus)  goodStatus = FALSE;

    istatus = Check_Int_Value(qcf_record.ch2_code, ceilingFlag2,recnum);
    if (!istatus)  goodStatus = FALSE;

    istatus = Check_Float_Value(qcf_record.ceilingHt3, qcf_record.ch3_flag,
       ceilingHeight3,recnum);
    if (!istatus)  goodStatus = FALSE;

    istatus = Check_Int_Value(qcf_record.ch3_code, ceilingFlag3,recnum);
    if (!istatus)  goodStatus = FALSE;
    ...
    **---------------------------------*/
    if ((qcf_record.ca1_flag != 'M') && (qcf_record.ca1_flag != 'N'))
        {
        istatus = Check_Int_Value(qcf_record.cloud_amt1, cloudAmount1,recnum);
        if (!istatus)  goodStatus = FALSE;
        }
    ...
    if ((qcf_record.ca2_flag != 'M') && (qcf_record.ca2_flag != 'N'))
        {
        istatus = Check_Int_Value(qcf_record.cloud_amt2, cloudAmount2,recnum);
        if (!istatus)  goodStatus = FALSE;
        }
    ...
    if ((qcf_record.ca3_flag != 'M') && (qcf_record.ca3_flag != 'N'))
        {
        istatus = Check_Int_Value(qcf_record.cloud_amt3, cloudAmount3,recnum);
        if (!istatus)  goodStatus = FALSE;
        }
    ...
    **--------------------------------------------*/
    ...
    istatus = Check_Date_Value(qcf_record.nomDate,recnum);
    if (!istatus)  goodStatus = FALSE;
        
    assert(qcf_record.actualDate != NULL);
    istatus = Check_Date_Value(qcf_record.actualDate,recnum);
    if (!istatus)  goodStatus = FALSE;
       
    assert(qcf_record.nomTime != NULL);
    istatus = Check_Time_Value(qcf_record.nomTime,recnum);
    if (!istatus)  goodStatus = FALSE;
     
    assert(qcf_record.actualTime != NULL);
    istatus = Check_Time_Value(qcf_record.actualTime,recnum);
    if (!istatus)  goodStatus = FALSE;
...
int Check_Float_Value(float value, char qc_flag, DATAVALUE_TYPE value_type, int recnum)
    ...
    switch (value_type)
        ...
        case squallGust: minimum = 0.0; maximum = 9999.99;
           ...
           break;
        case visibility: minimum = 0.0; maximum = 160934.41;
    ...
    **--------------------------------------------------------*/
    if ((value_type == cSeaLvlPressure) && (value <= -999.00) &&
       (qc_flag != 'I'))
       { 
       fprintf(stderr, "WARNING: Bad flag for CSLP. Value Missing. Flag must be I. ");
       fprintf(stderr, "%s = %7.2f, qc_Flag = %1c in record %d\n",
               type_str, value, qc_flag, recnum);
       return FALSE;
       } 
        ...
        switch (qc_flag)
            ...
            case 'C':
               if ((value > -1000.0) && (value < -999.98))
                  ...
                  {
                  fprintf(stderr, "WARNING: Test failed for missing value ");
                  fprintf(stderr, "%s = %7.2f in record\n", type_str, value, recnum);
                  return FALSE;
                  }
               break;
            }
    ...
    **---------------------------------------------------------*/
    ...
    if (strcmp(valuestr,"-0.00") == 0.0)
      {
       fprintf(stderr, "WARNING: Test failed for %s = %7.2f in record %d\n", 
         type_str, value,recnum);
       return FALSE;
      }
...
int Check_Int_Value(int value, DATAVALUE_TYPE value_type,int recnum)
    ...
    switch (value_type)
        ...
        case cloudAmount3: minimum = 0; maximum = 15;
           ...
           break;
        default:
           fprintf(stderr, "WARNING: bad datatype value specified in record %d\n",
             recnum);
           return FALSE;
        }
    ...
    if ((value < minimum) || (value > maximum))
       {
       fprintf(stderr, "WARNING: Test failed for %s = %d in record %d\n", 
          type_str, value,recnum);
       return FALSE;
       }
    else
...
int Check_Date_Value(char *datevalue,int recnum)
    ...
    **---------------------------------*/
    ...
    if (year_length == 0) 
       ...
       istatus = FALSE;
       fprintf(stderr, 
 "WARNING: YEAR = %s changes from %d char year to %d char year at record %d\n",
       temp_str,year_length, strlen(temp_str),recnum);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if (((year_length == 2)  && (year < 0 || year > 99)) ||
       ...
       {
       istatus = FALSE;
       fprintf(stderr, "WARNING: Invalid YEAR = %d in record %d\n", year,recnum);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if ((month < 1) || (month > 12))
       {
       istatus = FALSE;
       fprintf(stderr, "WARNING: Invalid MONTH = %d in record %d\n", month,recnum);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if ((day < 1) || (day > NUMDAYS[month]))
       {
       istatus = FALSE;
       fprintf(stderr, "WARNING: Invalid DAY = %d in record %d\n", day,recnum);
       }
...
**------------------------------------------------------------------*/
int Check_Time_Value(char *timevalue, int recnum)
    {
    int hour, minute;
    int istatus;
    ...
    **-------------------------------------------------------*/
    ...
    if ((hour < 0) || (hour > 23))
       {
       istatus = FALSE;
       fprintf(stderr, "WARNING: Invalid Hour = %d in record %d\n", hour,recnum);
       }
    ...
    **-------------------------------------------------------*/
    ...
    if ((minute < 0) || (minute > 59))
       {
       istatus = FALSE;
       fprintf(stderr, "WARNING: Invalid Minutes = %d in record %d\n", minute,recnum);
       }
