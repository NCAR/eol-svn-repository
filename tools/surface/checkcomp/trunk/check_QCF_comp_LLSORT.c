/*--------------------------------------------------------
 * check_QCF_comp_LLSORT - Main module which controls the processing
 *  that checks a composite made up of QCF records.
 *  This program assumes the following:
 *     - Data records are in QC format (QCF).
 *     - Data is to be sorted by lat/lon then time.
 ...
 *     - If lat/lon are the same, then time in increasing.
 ...
 * 15 Aug 95 lec
 *   Modified this version to handle different sort order.
 *-------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
...
char *argv[];
   ...
   if (argc != 2)
      {
      printf ("Usage: check_QCF_comp_LLSORT <input_file>\n");
      exit(1);
      }
   ...
   while (!feof(data_input_stream))
       ...
       * the data are also sorted by lat/lon then by time.
       */
      if (!strncmp(prev_qnet, qcfptr->qnet, 10) &&
         qcfptr->lat==prev_lat && prev_lon==qcfptr->lon &&
         last_nom_hr < prev_hr )
         printf ("ERROR: Hours are decreasing from %5d at rec %ld to %5d at rec %ld.\n",
                 prev_hr, recs_read-1, last_nom_hr, recs_read);

      if (!strncmp(prev_qnet, qcfptr->qnet, 10) &&
         qcfptr->lat==prev_lat && prev_lon==qcfptr->lon && 
         last_nom_hr == prev_hr && last_nom_min < prev_min) 
         printf ("ERROR: Hours match BUT Minutes are decreasing from %5d at rec %ld to %5d at rec %ld.\n", 
      ...
      if (!strncmp(prev_qnet, qcfptr->qnet, 10) &&
          qcfptr->lat==prev_lat && prev_lon==qcfptr->lon &&
          last_nom_hr > prev_hr && (last_nom_hr - prev_hr) > 1)
