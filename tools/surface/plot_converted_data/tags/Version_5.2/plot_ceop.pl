...
(my $para_ref, my $files_ref, my $stations_ref, my $heights_ref, my $param_file,
 my $out_dir, my $keep, my $verbose, my $multi, my $begindate, my $enddate) 
      = getArguments();
...
sub printUsage
{
    ...
    print( STDERR "  cat data.gz | gunzip | $0 sfc -t 2002/10/01 2003/03/31 -o out_dir -d -\n");
...
sub getArguments()
{
	...
	return(\@params, \@files, \@stations, \@heights, $in_file, $dir, $keep, 
               $verbose, $multi, $begindate, $enddate);
}
#*******************************************************************************
