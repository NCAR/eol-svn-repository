...
# Project was hard-coded into ceop_template.  Modified to be a required input
...
#
# usage:
# plot_ceop.pl <format> -P <project> -t <starttime> <endtime> -d <data files> 
#   -p <parameters> -h <heights> -s <stations> -f <params file> -o <out dir> -k 
#   -m -v
#
...
#-------------------------------------------------------------------------------
my $mtime = -M $files[0]; # Days since last modification of most recently 
                          # modified data file.

#-------------------------------------------------------------------------------
# Append all the data files to plot to a single string containing the list of
# files.
#-------------------------------------------------------------------------------
foreach my $file (@files)
{
...
foreach my $station (@stations)
{
	...
	foreach my $height( @heights )
	{
		...
		close( AWK );
                ...
                {
                        my $time = (-M $infile);
                        print( STDERR "WARNING: Input data modified more ".
