...
# format.  There re some hardcoded cases within the code that may need to be
# changed depending on the particulars of the new format.  This code assumes 
# that the network id/ reference site id is in the column right before the
# station id for all formats.
#-------------------------------------------------------------------------------
...
(my $para_ref, my $files_ref, my $cseids_ref, my $stations_ref, my $heights_ref,
 ...
 my $multi, my $color, my $project, my $begindate, my $enddate) 
      = getArguments();
...
#-------------------------------------------------------------------------------
        ...
        foreach my $stn (@stns)
        {
        	...
        	print( STDERR "\t  ", $stn, "\n" );
                if (!(-e $dirpath."/".$stn))
                        {
                        system ("mkdir $dirpath/$stn");
                        }
                if ($color && !(-e $dirpath."/".$stn."/color"))
                        {
                        system ("mkdir $dirpath/$stn/color");
                        }
                if ($multi && !(-e $dirpath."/".$stn."/multiHt"))
                        {
                        system ("mkdir $dirpath/$stn/multiHt");
                        }
        }
...
foreach my $cseid (@cseids)
{
        ...
        foreach my $station (keys %{$all_stns{$cseid}})
        {
                ...
                if ($multi) {%plot_string = (); my @infiles = undef();}
                #---------------------------------------------------------------
	        ...
	        foreach my $height (sort @heights )
	        {
                        ...
                        #-------------------------------------------------------
        		my $filename = "${out_dir}/${cseid}_${station}";
                        if (defined ($height)) 
                            {$filename .= "_h".$height;}
		        	...
		        	printf( STDERR 
        		        ...
        		        open( INFILE, "<$infile" ) or die
                                ...
                                while (<INFILE>) {
                                        ...
                                        # There are 10 possible flags in a qcf 
                                        # file, and seven possible flags for 
                                        # everything else, so once all ten (or 
	                                ...
	                                my @arr = split( /\s+/, $_ );
                                        ...
                                        elsif (/T($)/ && $format eq "qcf") 
                                                       {$found{T} = 1;}
                                        elsif (/E($)/ && $format eq "qcf") 
                                                       {$found{E} = 1;}
                                        elsif (/N($)/ && $format eq "qcf") 
                                                       {$found{N} = 1;}
                                        elsif (/X($)/ && $format eq "qcf") 
                                                       {$found{X} = 1;}
                                        else {print STDERR 
                        ...
                        # information.
                        	...
                        	$plot_string{$param}{$height} = $plotline;
                                ...
                                if ($color) 
                                {
                                        ...
                                        my $plotline = "";
                                        if ($found{U}) {$plotline .= 
"\\\\\\\"< tr 'U' '1' < ${infile_escaped}\\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"unchecked\\\\\\\" with linespoints 9 7,";}	# grey
                                        if ($found{G}) {$plotline .= 
"\\\\\\\"< tr 'G' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"good\\\\\\\" with linespoints 2 7,";} # green
                                        if ($found{D}) {$plotline .= 
"\\\\\\\"< tr 'D' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"questionable\\\\\\\" with linespoints 6 7,";} # yellow
                                        if ($found{B}) {$plotline .= 
"\\\\\\\"< tr 'B' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"bad\\\\\\\" with linespoints 1 7,";} # red
                                        if ($found{C}) {$plotline .= 
"\\\\\\\"< tr 'C' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"Precip Large or Negative\\\\\\\" with linespoints 4 7,";} # purple
                                        if (($format ne "qcf") && $found{I}) {$plotline .= 
"\\\\\\\"< tr 'I' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"interpolated\\\\\\\" with linespoints 7 7,";} # black
                                        if ($format eq "qcf") {
                                            if ($found{E}) {$plotline .= 
"\\\\\\\"< tr 'E' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"estimated\\\\\\\" with points 7 7,";} # black
                                            if ($found{N}) {$plotline .= 
"\\\\\\\"< tr 'N' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"not available\\\\\\\" with points 3 7,";} # blue
                                            if ($found{I}) {$plotline .= 
"\\\\\\\"< tr 'I' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"incomputable\\\\\\\" with points 3 7,";} # blue
                                            if ($found{M}) {$plotline .= 
"\\\\\\\"< tr 'M' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"missing\\\\\\\" with points 3 7,";} # blue
                                            if ($found{T}) {$plotline .= 
"\\\\\\\"< tr 'T' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"trace\\\\\\\" with points 5 7,";} # cyan
                                            if ($found{X}) {$plotline .= 
"\\\\\\\"< tr 'X' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"glitch\\\\\\\" with points 8 12,";} # orange
                                        }
			                ...
			                system( "sed  \"s:XPROJECTX:$project:g; s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:$theight:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file_escaped:g; s:XPLOTSPECX:$plotspec:g; s:XCOLORX:$color:g; s:XCSEIDX:$cseid_escaped:g; s:XYRANGEX:$Yrange:g; s:XBEGINDATEX:$begindate:g; s:XENDDATEX:$enddate:g; \" $template > \"$gnu_in\"" );
        		...
        		foreach my $param (sort keys %plot_string) 
        		        ...
        		        my $file_name = sprintf( 
                                    ...
                                    "${cseid}_${station}_${param}_multiHt");
                		my $gnu_in = $file_name . ".gnuplot";
                		my $ps_file= $file_name . ".ps";
                                my $ps_file_escaped = $ps_file;
                                $ps_file_escaped =~ s/:/\\:/g;
		                ...
		                foreach my $height 
                                ...
                                system( "sed  \"s:XPROJECTX:$project:g; s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:All Heights:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file_escaped:g; s:XPLOTSPECX:$plotspec:g; s:XCOLORX:$color:g; s:XCSEIDX:$cseid_escaped:g; s:XYRANGEX:$Yrange:g; s:XBEGINDATEX:$begindate:g; s:XENDDATEX:$enddate:g;\" $template > \"$gnu_in\"" );
	...
	foreach my $file (@$files)
	{
		...
		while( <IN> )
		{
			...
			$heights{$arr[$hcol]} = 1 if( defined( $hcol ) && 
                        ...
                        # column just preceeding the station ID.
                        # For qcf data, there are only two identifiers: network
                        ...
                        # we have qcf data.  If not, then cat the two together
                        # and used the combined CSEid_refSiteId.
                        if( defined ( $scol ))
                        {
                            my $id;
                            if ($arr[$scol-1] ne $arr[4])
                                 { $id = $arr[4]."_".$arr[$scol-1]; }
                            else { $id = $arr[$scol-1]; }
