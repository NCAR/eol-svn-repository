...
#-------------------------------------------------------------------------------
        ...
        if (scalar(@stns) != 0)
        {
        	...
        	print( STDERR "  ", $cseid, "\n" );
                ...
                my @dirs;
                if ($netType eq "CSEid") 
                        {@dirs = split ($separatorChar, $cseid );}
                else    {@dirs = $cseid;}
                foreach my $newdir (@dirs)
                        {
                        $dirpath = $dirpath."/".$newdir;
...
"\\\\\\\"< tr 'X' '1' < ${infile_escaped} \\\\\\\" using 1\\\:(\\\$$column{$param}*(\\\$$flag == 1 )) title \\\\\\\"glitch\\\\\\\" with points 8 12,";} # orange
			                ...
			                system( "sed  \"s:XPROJECTX:$project:g; s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:$theight:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file_escaped:g; s:XPLOTSPECX:$plotspec:g; s:XCOLORX:$color:g; s:XCSEIDX:$cseid_escaped:g; s:XYRANGEX:$Yrange:g; s:XBEGINDATEX:$begindate:g; s:XENDDATEX:$enddate:g; s:XKEYX:$legend:g; s:XMISSINGX:$missing:g; \" $template > \"$gnu_in\"" );
        		...
        		foreach my $param (sort keys %plot_string) 
		                ...
		                foreach my $height 
                                ...
                                system( "sed  \"s:XPROJECTX:$project:g; s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:All Heights:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file_escaped:g; s:XPLOTSPECX:$plotspec:g; s:XCOLORX:$color:g; s:XCSEIDX:$cseid_escaped:g; s:XYRANGEX:$Yrange:g; s:XBEGINDATEX:$begindate:g; s:XENDDATEX:$enddate:g; s:XKEYX:$legend:g; s:XMISSINGX:$missing:g; \" $template > \"$gnu_in\"" );
