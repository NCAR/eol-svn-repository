...
# Dan Sullivan
# October, 2003
# Based on the plotit.pl script written for qcf data.  ( Version 1 )
#
	...
	foreach my $height( @heights )
	{
		...
		foreach my $param (keys %units) 
		{
			...
			printf( STDERR 
			        ...
			        system( "sed  \"s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:$theight:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file:g; s:XPLOTSPECX:$plotspec:g;\" ceop_template > \"$gnu_in\"" );
		...
		foreach my $param (keys %plot_string) 
		        ...
		        foreach my $height (keys %{$plot_string{$param}}) 
                        ...
                        system( "sed  \"s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:All Heights:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file:g; s:XPLOTSPECX:$plotspec:g;\" ceop_template > \"$gnu_in\"" );
