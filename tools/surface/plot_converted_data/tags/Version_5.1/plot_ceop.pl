...
#
# Updates 1/21/2004 by Janine Goldstein ( Version 5 )
...
(my $para_ref, my $files_ref, my $stations_ref, my $heights_ref, my $param_file,
 my $out_dir, my $keep, my $verbose, my $multi) 
      = getArguments();
...
if (scalar(@all_CSEids) != 1) {
        print( STDERR "More than one CSEid contained within raw data:\n");
...
foreach my $station (@stations)
{
	...
	foreach my $height( @heights )
	{
		...
		close( AWK );
                ...
                {
                print ( STDERR "WARNING: plottable data file $infile already\n",
		...
		foreach my $param (keys %units) 
		{
			...
			printf( STDERR 
			        ...
			        system( "sed  \"s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:$theight:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file:g; s:XPLOTSPECX:$plotspec:g; s:XCOLORX:$color:g; s:XCSEIDX:$cseid:g; s:XBEGINDATEX:$begindate:g; s:XENDDATEX:$enddate:g; \" ceop_template > \"$gnu_in\"" );
		...
		foreach my $param (keys %plot_string) 
                {
		        my $file_name = sprintf( 
                                "${out_dir}/${station}_${param}_multiHt");
        		my $gnu_in = $file_name . ".gnuplot";
        		my $ps_file= $file_name . ".ps";
        		my $gif_file= $file_name . ".gif";
                        my $plotspec = "plot";
		        foreach my $height (keys %{$plot_string{$param}}) 
                        ...
                        system( "sed  \"s:XPARAMETERX:$param:g; s:XSTATION:$station:g; s:XHEIGHTX:All Heights:g; s:XUNITSX:$units{$param}:g; s:XOUTFILEX:$ps_file:g; s:XPLOTSPECX:$plotspec:g; s:XCOLORX:$color:g; s:XCSEIDX:$cseid:g; s:XBEGINDATEX:$begindate:g; s:XENDDATEX:$enddate:g;\" ceop_template > \"$gnu_in\"" );
...
sub printUsage
{
    ...
    print( STDERR "  -v  Verbose mode, print status messages as the plots\n",
         "      are created.   \n");
    print( STDERR "\nWARNING:\n" );
    print( STDERR "  You will need to modify ceop_template to set\n",
    ...
    print( STDERR "  $0 sfc -o out_dir -s Station1 -m\n" );
    print( STDERR "  $0 sfc -help\n" );
    print( STDERR "  cat data.gz | gunzip | $0 sfc -o out_dir -d -\n");
