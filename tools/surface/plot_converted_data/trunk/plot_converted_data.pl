...
# or station name.
#
# Updated 6/6/2007 by Janine Goldstein ( Version 7.8 )
# The separatorChar is used to create the final filename and the name of the
# .plotdata file, so added change from a ctrl char to an _ for that porpose.
#
# Updated 8/21/2007 by Janine Goldstein ( Version 7.9 )
# Fixed oversight from when added multistn option that was not creating
# CEOP CSEid and Refsite subdirs correctly. Just needed to assing
# $dirpathnet to $dirpath at line 757.
#
# Updated 2/27/2008 by Janine Goldstein ( Version 7.10 )
# Changed call to gnuplot.new to be a call to gnuplot.
# This code should be run on merlot or shiraz from now on, and
# not on tornado!!!
#
# Updated 11/19/2008 by Janine Goldstein ( Version 7.11 )
# Added static yrange for CEOP_ARM.  Other projects may be
# added by creating a yrange hash and adding it to this code.  CEOP_ARM will
# always plot with static yranges.  All other projects will plot with dynamic
# ranges, including project CEOP (without the ARM extension).
# Updated generated directory structure to put static range plots under a static 
# dir in the station dir so that static and dynamic range plots won't overwrite
# each other.
#
#
...
%{$yrange{"CEOP_ARM"}} = (stn_press => [960,1060], air_temp => [-50,50], 
          dew_pt => [-60,35], rel_humid => [0,110], specific_humid => [0,30], 
	  wind_spd => [0,30], wind_dir => [0,360], u_wind_comp => [-30,30], 
	  v_wind_comp => [-30,30], precip => [0,200], snow_depth => [0,120],
	  short_in => [-10,1500], short_out => [0,600], long_in => [100,550], 
	  long_out => [100,650], net_rad => [-100,1300], skin_temp => [-50,70], 
	  sens_heat_flux => [-200,800], lat_heat_flux => [-200,800], 
	  CO2_flux => [-200,200], soil_heat_flux => [-80,120] );
...
#-------------------------------------------------------------------------------
# If these are static range plots, add a subdir "static" so they won't 
# overwrite dynamic range plots.
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
        ...
        foreach my $stn (@stns)
        {
	        ...
	        if ($static)
		    {
                    if (!$multistn && !(-e $dirpathnet."/".$stn."/static"))
                        {
                        system ("mkdir $dirpathnet/$stn/static");
                        }
                    if ($color && !(-e $dirpathnet."/".$stn."/static/color"))
                        {
                        system ("mkdir $dirpathnet/$stn/static/color");
                        }
                    if ($multi && !(-e $dirpathnet."/".$stn."/static/multiHt"))
                        {
                        system ("mkdir $dirpathnet/$stn/static/multiHt");
                        }
                    if ($multistn && !(-e $dirpathnet."/multiStn/static"))
                        {
                        system ("mkdir $dirpathnet/multiStn/static");
                        }
                    }
        }
        ...
        foreach my $station (keys %{$all_stns{$cseid}})
        {
			...
			my $time = (-M $infile);
        		...
        		foreach my $param (keys %units) 
        		{
                                ...
                                my $filename_prefix = $cseid_escaped;
				...
				# char to /.
					...
					{
					    $dir =~ s/$separatorChar/\//;
					    $filename_prefix =~ s/$separatorChar/-/;
					}
		        	my $file_name = "${out_dir}/${dir}/${station}";
				if ($static) {$file_name .= "/static";}
                                if ($color) {$file_name .= "/color";}
                                if ($multi) {$file_name .= "/multiHt";}
                                $file_name .= "/${filename_prefix}_${station}_${param}";
                                if (defined ($height)) 
                                                   {$file_name .= "_h".$height;}
        		...
        		foreach my $param (sort keys %plot_string) 
                                ...
                                if ($netType eq "CSEid") 
					{
        		        ...
        		        my $file_name = sprintf( 
                                    ...
                                    "${filename_prefix}_${station}_${param}_multiHt");
                		my $gnu_in = $file_name . ".gnuplot";
                		my $ps_file= $file_name . ".ps";
                		my $gif_file= $file_name . ".gif";
