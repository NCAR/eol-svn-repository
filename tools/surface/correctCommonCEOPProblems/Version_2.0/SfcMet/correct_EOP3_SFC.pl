...
#-------------------------------------------------------
# correct_EOP3_SFC.pl - This perl program corrects some
#   common problems found in the CEOP EOP3 SFC data 
#   provided by the source in "almost" the right format.
#   This s/w only corrects the Surface/Met/Rad format.
#-------------------------------------------------------
...
printf "\ncorrect_EOP3_SFC.pl began on ";print scalar localtime; print "\n";
...
sub parse_input_line{
   ...
   # Total line length is 306 including return.
   #-------------------------------------------
   $meta_line = substr ($input_line, 0, 107);
   ...
   $data_line = substr ($input_line, 107, 250); # 198 exact
...
sub write_output_line {
    ...
    printf OUTFILE_JOSSCorrected ("%10s %5s %10s %5s %-10s %-15s %-15s %10.5f %11.5f %7.2f", 
    ...
    #-----------------------------
#   printf OUTFILE_JOSSCorrected (" %7.2f %s  %7.2f %s %7.2f %s %7.2f %s %7.2f %s %7.2f %s %7.2f %s %7.2f %s %7.2f %s %7.2f %s %7.2f %s %8.2f %s %8.2f %s %8.2f %s %8.2f %s %8.2f %s %8.2f %s %8.2f %s %8.2f %s\n",

    #---------------------------------------
    # Print Stn Pressure through Snow depth.
    #---------------------------------------
    for ($mm=0; $mm<=21; $mm=$mm+2)  # print 22 vals and flags.
       {
       if ($debug) {print "first Data loop: mm = $mm\n";}

       printf OUTFILE_JOSSCorrected (" %7.2f %s", 
    ...
    #-----------------------------------------------
    for ($mm=22; $mm<=37; $mm=$mm+2)  # print 16 vals and flags.
       {
...
printf "correct_EOP3_SFC.pl ended on ";print scalar localtime; printf "\n";
