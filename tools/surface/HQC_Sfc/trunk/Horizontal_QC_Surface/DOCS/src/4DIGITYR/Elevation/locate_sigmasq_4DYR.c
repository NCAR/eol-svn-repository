/*--------------------------------------------------------
 * locate_sigmasq_4DYR - This module contains s/w that supports
 *   locating the (NEXT) best sigmasq value for the
 *   requested parameter and time. Now handles 4 digit year
 *   this s/w based on V2.0 of locate_sigmasq.c.
 *
 * 10/15 Nov 95 lec
 ...
 * August/September 2002 lec
 *   Updated to handle 4 digit year. Updated locate_sigmasq()
 *   to now handle very short projects with approx. SIGMA PERIOD
 *   days. Note that if the project is SIGMA PERIOD days
 *   long, the main QC program won't even do searching.
 *-------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * 20 Aug 2002 lec
 *   Upgraded to handle situation where project is only
 *   about SIGMA PERIOD length. Check project begin date
 *   and ensure that we don't try to search for times before 
 *   sigmasq files were created.
 *--------------------------------------------------------*/
void locate_sigmasq(/*in*/  char     sigfile_last_searched[NAMELEN_MAX], /* (pathname)YYYYJJJHHMM.sig */
                    ...
                    /*in*/  long int project_begin_YYYYJJJ,      /* Year/jdate that project begin */
   ...
   long int     first_possible_YYYYJJJ; /* first existing Sigfile for this project */
   long int     last_date_searched_YYYYJJJ;
   ...
   int          second_search = 0; /* Only do second search once. */
   ...
   last_date_searched_YYYYJJJ = begin_YYYYJJJ; /* last sigfile searched where no value found. */

   first_possible_YYYYJJJ = add_num_to_YYYYJJJ (project_begin_YYYYJJJ, SIGMA_PERIOD);
  ...
  /* Stop_YYYYJJJ should be SIGMA_PERIOD -1, so that only SIGMA_PERIOD days
     will be searched total. Keep as is for rest of GCIP projects
   ...
   while (continue_search)
     {
#if DEBUG
   printf ("SEARCH from/to: begin_YYYYJJJ, begin_YYYY, stop_YYYYJJJ, project_begin_YYYYJJJ, project_end_YYYYJJJ, first_possible_YYYYJJJ: %ld %d %ld %ld %ld %ld\n",
            begin_YYYYJJJ, begin_YYYY, stop_YYYYJJJ, project_begin_YYYYJJJ, project_end_YYYYJJJ, first_possible_YYYYJJJ);
#endif
      ...
      *     requested time. Or have searched all
      *     possible days - a small project may
      *     have less than SIGMA_PERIOD number of
      *     sigfiles to search. Search all you can.
      *--------------------------------------------*/
     ...
     for( current_YYYYJJJ = begin_YYYYJJJ;  current_YYYYJJJ <= stop_YYYYJJJ;
        ...
        printf ("number_of_days_searched: %d\n", number_of_days_searched);
#endif

        if (current_YYYYJJJ > project_end_YYYYJJJ) break; /* Don't search past end of project!*/
...
/*            continue; if read everything then no broken pipe error issued  - this works*/
...
#endif
              continue; /* assumes numbers always in order. */
              }
           ...
           if (jj == stn_no)
              ...
              while (!feof(sigma_output_stream))
                 ...
                 if (jj > stn_no) break;   /* gone too far  - potential pipe break!!!*/
                 ...
                 if (jj == stn_no && kk >= NUMQCPARMS-1) break;/*done searching- located or not - pipe break*/
        ...
        if (!file_closed)
            ...
            * error is generated by the system if pclose is called
            * before EOF is hit in the file. If the complete file is 
            * read, then no error is generated. Trap this error and
            * discard since to save processing time we want to just
            * close the file without reading the whole file.
            *------------------------------------------------------*/
#if UNIX_ENV
           close_file (&sigma_output_stream, FILE_COMPRESSED);
#else
      ...
      * SIGMA_PERIOD days located between begin and end of
      * orig forward search and sigma_sq was not located,
      * then need to continue searching back at project end
      * minus SIGMA_PERIOD # days (if possible). This second
      * search ends where first search began. As in all other
      * searches, the max number days searched is 
      * SIGMA_PERIOD days. But only do this special second
      * search once.
      *-------------------------------------------------*/
#if DEBUG
     printf ("Consider doing Second Search: number_of_days_searched=%d, second_search=%d, begin_YYYYJJJ=%ld, stop_YYYYJJJ=%ld\n", 
              number_of_days_searched, second_search, begin_YYYYJJJ, stop_YYYYJJJ);
#endif

     if ((number_of_days_searched < SIGMA_PERIOD-1) && 
         (sigma_sq[data_type] < -880.00) && second_search == 0)
       { 
       second_search = 1;
       continue_search = 1;

       begin_YYYYJJJ = subtract_num_from_YYYYJJJ(project_end_YYYYJJJ, SIGMA_PERIOD);

       stop_YYYYJJJ = add_num_to_YYYYJJJ (begin_YYYYJJJ, (SIGMA_PERIOD-number_of_days_searched-1));

#if DEBUG
     printf ("(1) begin_YYYYJJJ=%ld, stop_YYYYJJJ=%ld\n", begin_YYYYJJJ, stop_YYYYJJJ);
#endif

       if (begin_YYYYJJJ < first_possible_YYYYJJJ)
          {
          begin_YYYYJJJ = first_possible_YYYYJJJ;
          }

#if DEBUG
     printf ("(2) begin_YYYYJJJ=%ld, stop_YYYYJJJ=%ld\n", begin_YYYYJJJ, stop_YYYYJJJ);
#endif 

       if (begin_YYYYJJJ >= last_date_searched_YYYYJJJ) 
          {
          continue_search = 0;
          }

#if DEBUG
     printf ("(3) begin_YYYYJJJ=%ld, stop_YYYYJJJ=%ld\n", begin_YYYYJJJ, stop_YYYYJJJ);
#endif 

       if (stop_YYYYJJJ > last_date_searched_YYYYJJJ)
          { 
          stop_YYYYJJJ = subtract_num_from_YYYYJJJ( last_date_searched_YYYYJJJ, 1);
          } 

#if DEBUG
     printf ("(4) begin_YYYYJJJ=%ld, stop_YYYYJJJ=%ld\n", begin_YYYYJJJ, stop_YYYYJJJ);
#endif 

       if (stop_YYYYJJJ < first_possible_YYYYJJJ)
          {
          stop_YYYYJJJ = first_possible_YYYYJJJ;
          }
             
#if DEBUG
     printf ("(5) begin_YYYYJJJ=%ld, stop_YYYYJJJ=%ld\n", begin_YYYYJJJ, stop_YYYYJJJ);
#endif
       ...
       printf ("CONTINUE Search (%d)? NEW begin/stop dates: %ld %ld\n",
               continue_search, begin_YYYYJJJ, stop_YYYYJJJ);
#endif                
       }
     else
       {
...
#endif
       number_of_days_searched++; /* this var doesn't incld first day searched */
       continue_search = 0;
       }
     } /* while (continue_search)
...
/*HERE */
   if (sigma_sq[data_type] < -880.00    && 
       ((number_of_days_searched >= SIGMA_PERIOD-1) || (second_search == 1)) )
      {
