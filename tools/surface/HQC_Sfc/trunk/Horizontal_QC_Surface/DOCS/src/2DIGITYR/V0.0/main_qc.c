 ...
 *   written to the updated sigma sq file fot this stn/time/
 *   parameter set. Note that the number of days searched has
 *   been limited to SIGMA_PERIOD days to prevent crossing
 *   seasons. Applying sigma values computed with summer
 *   data to spring values would be incorrect. See below for
 ...
 *   station A. This means that you can not simply retain
 ...
 *    the sigma_sq file, it will gzip the file.
 ...
 *   lastest update. The use is advised (when it is reasonable)
...
char *argv[];
   ...
   char         compression_cmd [NAMELEN_MAX+10];
   char         junk_char = '\0';
   ...
   printf ("proj_begin_YYYYJJJ, proj_end_YYYYJJJ, begin_YYYYJJJ, end_YYYYJJJ: %ld %ld %ld %ld\n",
...
/*                if (jj == 0)
                  ...
                  if ((theta_obs > -990.00) && (numtheta_o[jj] >1) &&
                     ...
                     if (sigma_sq[stn_no][jj] < -990.00)
                         ...
                         * is recognized by other s/w to mean
                         ...
                         * makes since during the first SIGMA_PERIOD
...
#if DEBUG
         printf ("Completed processing input_stream for %ld. GO TO NEXT DATE.\n", current_date);
#endif
   ...
   printf ("NOTE: Each search may have required searching forward through up to %d files.\n", SIGMA_PERIOD);
