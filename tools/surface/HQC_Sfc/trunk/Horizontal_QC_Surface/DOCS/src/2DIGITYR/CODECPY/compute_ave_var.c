 ...
 *   compute_sigmasq() and computes (for each input file
 *   which is for each hour/min) ave variances and 
 ...
 *   final_ave_var.out. A set of ave stds is output for each
 *   input file. These are the ave stds for each parameter over
 *   all stations in the current input sigmasq file. 
 *
 * 13 Dec 95 lec
 *   Created.
 * 22 May 98 lec
