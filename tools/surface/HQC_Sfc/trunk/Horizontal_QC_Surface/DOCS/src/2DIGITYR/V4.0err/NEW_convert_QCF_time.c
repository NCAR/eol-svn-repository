 ...
 *    in the execution area. See Input for contents of sort.inp.
 ...
 *----------------------------------------------------------------------------------------------*/
   ...
   struct {char  network[11]; int  freq;} network_freq[ MAX_NETWORKS] = { {"ASOS5     \0",  5},
                                                                        ...
                                                                        {  "AWOSH20   \0", 20},
                                                                        {  "AWOSA20   \0", 20},
                                                                        {  "ISWS/ICN  \0",  60},/*was ISWS */
                                                                        {  "ARMSFC1   \0",  60},/*hly ES96*/
                                                                        {  "NPN       \0",  60},/*was WPDN, then WPN*/
                                                                        {  "DATSAV2   \0",  60},/*was NCDC*/
                                                                        {  "DATSAV2M  \0",  60},/*Mobile DATSAV2*/
                                                                        {  "GLERL     \0",  60},
                                                                        {  "GLERLM    \0",  60},/*Mobile GLERLM*/
                                                                        {  "ASOSH     \0",  60},
                                                                        {  "HPLAINS   \0",  60},
                                                                        {  "WI_AWON   \0",  60},
                                                                        {  "COAGMET   \0",  60},
   ...
   printf ("WARNING: This s/w makes assumptions about the century.\n");
   printf ("WARNING: Search for 1900 and 2000.\n");   
   ...
   /*-----------------------------------------
    * Make Assumptions about the century. It's
    * really just a quess, but probably a very
    * good guess.
    *-----------------------------------------*/
   if (begin_YY < 50)
      begin_year = 2000 + begin_YY;
   else
      begin_year = 1900 + begin_YY;

   if (end_YY < 50)         
      end_year = 2000 + end_YY;         
   else
      end_year = 1900 + end_YY;         
				     
#if DEBUG
   printf ("begin_YY, end_YY  = %d  %d\n",
            begin_YY, end_YY);
   ...
   for(current_date=begin_YYYYJJJ; current_date<=end_YYYYJJJ; increment_YYYYJJJ(&current_date))
      ...
      while (!feof(input_stream))
		  ...
		  *-----------------------------------------*/
		 if (qcfptr->year_nom < 50)
		    qcfrec_year = 2000 + qcfptr->year_nom;
         else
   		    qcfrec_year = 1900 + qcfptr->year_nom;
#if DEBUG
