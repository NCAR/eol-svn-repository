...
C 16 Dec 94 lec
C   Modified s/w to work on output from new QC method that performs horizontal
C   surface QC.
C******************************************************************************
...
C	 if(debug) write(0,*) 'in addstats(): '

	 pi = getplatform()
	 si = getstncode()
	 di = qcfday-FIRSTDAY+1
C       write (0,*) 'in addstats(): qcfday, di ', qcfday, di  !here
...
C          write (0,*) 'qcflg = ', qcflg

         if(qcflg .eq. 'F') then
C            write (0,*)'Found F flag'
         ...
         if(qcflg .eq. 'L') then 
C            write (0,*)'Found L flag'
            stats(i,10)=stats(i,10)+1 
            p(pi,i,10)=p(pi,i,10)+1 
            s(si,i,10)=s(si,i,10)+1 
            d(di,i,10)=d(di,i,10)+1 
            h(hi,i,10)=h(hi,i,10)+1 
         endif 
 
         if(qcflg .eq. 'I') then 
C            write (0,*)'Found I flag'
            stats(i,11)=stats(i,11)+1 
            p(pi,i,11)=p(pi,i,11)+1 
            s(si,i,11)=s(si,i,11)+1 
            d(di,i,11)=d(di,i,11)+1 
            h(hi,i,11)=h(hi,i,11)+1 
         endif 
 
         if(qcflg .eq. 'H') then 
C            write (0,*)'Found H flag'
            stats(i,12)=stats(i,12)+1 
            p(pi,i,12)=p(pi,i,12)+1 
            s(si,i,12)=s(si,i,12)+1 
            d(di,i,12)=d(di,i,12)+1 
            h(hi,i,12)=h(hi,i,12)+1 
         endif 
 
         if(qcflg .eq. 'V') then 
C            write (0,*)'Found V flag'
            stats(i,13)=stats(i,13)+1 
            p(pi,i,13)=p(pi,i,13)+1 
            s(si,i,13)=s(si,i,13)+1 
            d(di,i,13)=d(di,i,13)+1 
            h(hi,i,13)=h(hi,i,13)+1 
         endif 

         if(qcflg .eq. 'G') then
C            write (0,*)'Found G flag'
...
C            write (0,*)'Found U flag'
	 ...
	 i=10
      if(qcf.qcpw .eq. 'F') stats(i,9)=stats(i,9)+1
      if(qcf.qcpw .eq. 'L') stats(i,10)=stats(i,10)+1
      if(qcf.qcpw .eq. 'I') stats(i,11)=stats(i,11)+1
      if(qcf.qcpw .eq. 'H') stats(i,12)=stats(i,12)+1
      if(qcf.qcpw .eq. 'V') stats(i,13)=stats(i,13)+1
	 ...
	 i=11
      if(qcf.vis.qc .eq. 'F') stats(i,9)=stats(i,9)+1
      if(qcf.vis.qc .eq. 'L') stats(i,10)=stats(i,10)+1
      if(qcf.vis.qc .eq. 'I') stats(i,11)=stats(i,11)+1
      if(qcf.vis.qc .eq. 'H') stats(i,12)=stats(i,12)+1
      if(qcf.vis.qc .eq. 'V') stats(i,13)=stats(i,13)+1
	 ...
	 j=12
      do i=1,3
         if(qcf.cc(i).ceilqc .eq. 'F') stats(i,9)=stats(i,9)+1
         if(qcf.cc(i).ceilqc .eq. 'L') stats(i,10)=stats(i,10)+1
         if(qcf.cc(i).ceilqc .eq. 'I') stats(i,11)=stats(i,11)+1
         if(qcf.cc(i).ceilqc .eq. 'H') stats(i,12)=stats(i,12)+1
         if(qcf.cc(i).ceilqc .eq. 'V') stats(i,13)=stats(i,13)+1
	    ...
	    j=j+1
         ...
         if(qcf.cc(i).cldqc .eq. 'F') stats(i,9)=stats(i,9)+1
         if(qcf.cc(i).cldqc .eq. 'L') stats(i,10)=stats(i,10)+1
         if(qcf.cc(i).cldqc .eq. 'I') stats(i,11)=stats(i,11)+1
         if(qcf.cc(i).cldqc .eq. 'H') stats(i,12)=stats(i,12)+1
         if(qcf.cc(i).cldqc .eq. 'V') stats(i,13)=stats(i,13)+1
...
C************************************************************************
	 ...
	 integer nobs,nqc, numpsgd
         real    pqc,pg,pg1,pflg,pb,pd,pf,pl,pi,ph,pv
	 integer*2 i,j,k,l
	 integer ndays
	 integer tm,time
	 character*26 ctime
...
C write out final summary statistics
      ...
      do i=1,NQCPARMS
         nobs = 0
         nqc = 0
         nobs=stats(i,1)+stats(i,2)+stats(i,3)+
     +        stats(i,7)+stats(i,8)+
     +        stats(i,9) +stats(i,10) +stats(i,11)+
     +        stats(i,12) +stats(i,13)

         nqc=stats(i,1)+stats(i,2)+stats(i,3)+
     +       stats(i,8)+
     +       stats(i,9) +stats(i,10) +
     +       stats(i,11) +stats(i,12) +stats(i,13)
         ...
         if (nqc .le. 0) then
            ...
            pg = 0             !percent good
            pg1 = 0            !percent good
            pflg = 0           !percent flagged bad or dubious
            pb = 0             !percent flagged bad
            pd = 0             !percent flagged dubious
            pf = 0             !percent flagged as failed
            pl = 0             !percent flagged as passed low
            pi = 0             !percent flagged as passed mid
            ph = 0             !percent flagged as passed high
            pv = 0             !percent flagged as passed very high
         else
            pg1=100.* real(stats(i,1)+stats(i,8))/real(nqc)              !G+E
            pflg=100.* real(stats(i,2)+stats(i,3)+stats(i,9))/real(nqc)  !B+D+F

            pf=100.* real(stats(i,9))/real(nqc)

            pl=100.* real(stats(i,10))/real(nqc)
            pi=100.* real(stats(i,11))/real(nqc)
            ph=100.* real(stats(i,12))/real(nqc)
            pv=100.* real(stats(i,13))/real(nqc)

            pb=100.* real(stats(i,2))/real(nqc)
            pd=100.* real(stats(i,3))/real(nqc)

C  Form total sum of all passed and good!
            pg = pg1 + pl + pi + ph+ pv

         end if

C  Form total sum passed and good

         numpsgd = stats(i,1)+stats(i,8)+stats(i,10)+stats(i,11)
     +             +stats(i,12)+stats(i,13)

Cwas:         write(*,1002) parmnames(i),nobs,nqc,(stats(i,j),j=1,NQCFLAGS),
C     +                 pqc,pg,pflg,pb,pd,pf,pl,pi,ph,pv,pg1

         write(*,1002) parmnames(i),nobs,nqc,numpsgd,
     +                 (stats(i,j),j=2,NQCFLAGS),
     +                 pqc,pg,pflg,pb,pd,pf,pl,pi,ph,pv,pg1
     ...
     +     tstats(9)+tstats(10)+tstats(11)+tstats(12)+tstats(13)
      nqc=tstats(1)+tstats(2)+tstats(3)+tstats(8)+
     +     tstats(9)+tstats(10)+tstats(11)+tstats(12)+tstats(13)

      numpsgd = tstats(1)+tstats(8)+
     +     tstats(10)+tstats(11)+tstats(12)+tstats(13)
      ...
      if (nqc .le. 0) then
         ...
         pg1=100.* real(tstats(1)+tstats(8))/real(nqc)              !G+E
         pflg=100.* real(tstats(2)+tstats(3)+tstats(9))/real(nqc)  !B+D+F

         pf=100.* real(tstats(9))/real(nqc)

         pl=100.* real(tstats(10))/real(nqc)
         pi=100.* real(tstats(11))/real(nqc)
         ph=100.* real(tstats(12))/real(nqc)
         pv=100.* real(tstats(13))/real(nqc)
...
C  Form total sum of all passed and good!
      ...
      write(*,*)
      write(*,1002) 'Grand Totals (all)  ',nobs,nqc,numpsgd,
     +    (tstats(j),j=2,NQCFLAGS),pqc,pg,pflg,pb,pd,pf,pl,
     +    pi,ph,pv,pg1
...
C    it back in!!!!
C
C      write(*,1002) 'Totals (1,2,4-7)    ',nobs,nqc, numpsgd,
C     +    (tstats(j),j=2,NQCFLAGS),pqc,pg,pflg,pb,pd,pg1
C
C write out statistics by platform
     ...
     +      p(i,j,9)+p(i,j,10)+p(i,j,11)+p(i,j,12)+p(i,j,13)

           nqc=p(i,j,1)+p(i,j,2)+p(i,j,3)+p(i,j,8)+
     +      p(i,j,9)+p(i,j,10)+p(i,j,11)+p(i,j,12)+p(i,j,13)

           numpsgd = p(i,j,1)+p(i,j,8)+
     +      p(i,j,10)+p(i,j,11)+p(i,j,12)+p(i,j,13)
           ...
           if (nqc .le. 0) then
              ...
              pg1=100.* real(p(i,j,1)+p(i,j,8))/real(nqc)              !G+E
              pflg=100.* real(p(i,j,2)+p(i,j,3)+p(i,j,9))/real(nqc)   !B+D+F
              pb=100.* real(p(i,j,2))/real(nqc)
              pd=100.* real(p(i,j,3))/real(nqc)

              pf=100.* real(p(i,j,9))/real(nqc)

              pl=100.* real(p(i,j,10))/real(nqc)
              pi=100.* real(p(i,j,11))/real(nqc)
              ph=100.* real(p(i,j,12))/real(nqc)
              pv=100.* real(p(i,j,13))/real(nqc)
...
C  Form total sum of all passed and good!
           ...
           write(*,1002) parmnames(j),nobs,nqc,numpsgd,
     +                   (p(i,j,k),k=2,NQCFLAGS),
     +                   pqc,pg,pflg,pb,pd,pf,pl,pi,ph,pv,pg1
        end do
        do k=1,NQCFLAGS
           tstats(k)=0
...
C  Form total sum of all passed and good!
        ...
        write(*,*)
        write(*,1002) 'Grand Totals        ',nobs,nqc,numpsgd,
     +      (tstats(j),j=2,NQCFLAGS),pqc,pg,pflg,pb,pd,pf,pl,
     +      pi,ph,pv,pg1
      end do
...
C write out statistics by parameter for worst offending stations
      ...
      do j=1,NPRTPARMS
        if (j .ne. 9) then     ! Skip squall printout
     ...
     +        s(i,j,9)+s(i,j,10)+ s(i,j,11)+ s(i,j,12)+ s(i,j,13) 
		 if(nqc .le. 0) goto 50

           pg1=real(s(i,j,1)+s(i,j,8))/real(nqc)
		 if(pg1 .eq. 1.0) goto 50         ! All were good

           pl=real(s(i,j,10))/real(nqc)
                 if(pl .eq. 1.0) goto 50         ! All passed low
 

C           pf=real(s(i,j,9))/real(nqc)
C                 if(pf .eq. 1.0) goto 50         ! All failed
 
C           pl=real(s(i,j,10))/real(nqc)
C                 if(pl .eq. 1.0) goto 50         ! All passed low
 
C           pi=real(s(i,j,11))/real(nqc)
C                 if(pi .eq. 1.0) goto 50         ! All passed mid

C           ph=real(s(i,j,12))/real(nqc)
C                 if(ph .eq. 1.0) goto 50         ! All passed high
 
C           pv=real(s(i,j,13))/real(nqc)
C                 if(pv .eq. 1.0) goto 50         ! All passed very high
 
	      qcf.network = stns(i).network
	      n = getplatform()
C
C          Look at each stn and compare stn percentages to total percentages
C
           do k=1,MAXBADSTN
C             write(*,*) 'MAXBADSTN: ', MAXBADSTN

C             write(*,*) 'j=',j,'; n=',n,'; k=',k,'; np=',np,'; i=',i,
C     +       '; pg1,pl=',pg1, pl,'; s2(n,k).pg,.pl='
C     +       ,s2(n,k).pg, s2(n,k).pl,'; nqc=',nqc

              if(s2(n,k).stn .eq. 0 .or. 
     +          pl .lt. s2(n,k).pl) then   ! identify worst stns.

C                 write(*,*) 'Found bad stn, s2(n,k).stn',s2(n,k).stn
C                 write(*,*) ' pl,  s2(n,k).pl', pl, s2(n,k).pl

                 do m=MAXBADSTN,k+1,-1
	      	     s2(n,m).stn=s2(n,m-1).stn
	      	     s2(n,m).pg=s2(n,m-1).pg
	      	     s2(n,m).pl=s2(n,m-1).pl
                 end do
...
C             write(*,*) 'All bad stn located.'
        ...
        do n=1,np
           do k=1,MAXBADSTN
		    i=s2(n,k).stn
C                    write(*,*) 'Bad stn no (i):', i
		    if(i .le. 0) goto 60
              nobs=s(i,j,1)+s(i,j,2)+s(i,j,3)+
     +             s(i,j,7)+s(i,j,8)+
     +             s(i,j,9)+s(i,j,10)+s(i,j,11)+
     +             s(i,j,12)+s(i,j,13)

              nqc=s(i,j,1)+s(i,j,2)+s(i,j,3)+
     +             s(i,j,8)+
     +             s(i,j,9)+s(i,j,10)+s(i,j,11)+
     +             s(i,j,12)+s(i,j,13)
...
C              write(*,*) 'nobs, nqc for i:', nobs, nqc
...
C              write(*,*) 'pqc for i:', pqc
              ...
              if (nqc .le. 0) then
                 ...
                 pg1=100.* real(s(i,j,1)+s(i,j,8))/real(nqc)

                 pflg=100.* real(s(i,j,2)+s(i,j,3)+s(i,j,9))/real(nqc)
                 pb=100.* real(s(i,j,2))/real(nqc)
                 pd=100.* real(s(i,j,3))/real(nqc)
                 pf=100.* real(s(i,j,9))/real(nqc)

                 pl=100.* real(s(i,j,10))/real(nqc)
                 pi=100.* real(s(i,j,11))/real(nqc)
                 ph=100.* real(s(i,j,12))/real(nqc)
                 pv=100.* real(s(i,j,13))/real(nqc)
...
C  Form total sum of all passed and good!
           ...
           if (nqc .gt. 0) then
C              write(*,*)'Write 2: i,j:', i,j

              write(*,1023) platform(n),stns(i).station,
     +           nobs,nqc,numpsgd,
     +           (s(i,j,m),m=2,13),pqc,pg,pflg,pb,pd,
     +           pf,pl,pi,ph,pv,pg1 ! NQCFLAGS = 13 ?????

C              write(*,*)'End Write 2: i,j', i,j
           endif
...
C       write(*,1061) parmnames(j)
      ...
      do i=1,nstns
        ...
        nobs=tstats(1)+tstats(2)+tstats(3)+
     +       tstats(7)+tstats(8)+
     +       tstats(9)+tstats(10)+tstats(11)+
     +       tstats(12)+tstats(13)

        nqc=tstats(1)+tstats(2)+tstats(3)+
     +       tstats(8)+
     +       tstats(9)+tstats(10)+tstats(11)+
     +       tstats(12)+tstats(13)
     ...
     +       tstats(12)+tstats(13)
        ...
        if (nqc .le. 0) then
            ...
            pg1=100.* real(tstats(1)+tstats(8))/real(nqc)
            pflg=100.* real(tstats(2)+tstats(3)+tstats(9))/real(nqc)
            pb=100.* real(tstats(2))/real(nqc)
            pd=100.* real(tstats(3))/real(nqc)
            pf=100.* real(tstats(9))/real(nqc)
...
C  Form total sum of all passed and good!
        ...
        write(*,1023) stns(i).network,stns(i).station,
     +      nobs,nqc,numpsgd,
     +      (tstats(m),m=2,NQCFLAGS),pqc,pg,pflg,
     +      pb,pd,pf,pl,
     +      pi,ph,pv,pg1
      end do
C       end do
...
C write out statistics by day
      ...
      do j=1,NPRTPARMS
        ...
        if (j .ne. 9) then      ! skip squall amount
...
C  Form total sum of all passed and good!
          ...
          write(*,1033) i+FIRSTDAY-1,nobs,nqc,numpsgd,
     +    (tstats(l),l=2,NQCFLAGS),pqc,pg,pflg,pb,pd,pf,
     +    pl,pi,ph,pv,pg1
        end do
        endif         ! skip squall amount
      end do
...
C          write (*,*) 'nobs, nqc, pqc after check nqc:', nobs, nqc, pqc
          ...
          write(*,1043) i-1,nobs,nqc,numpsgd,
     +      (tstats(l),l=2,NQCFLAGS),pqc,pg,pflg,pb,pd,pf,pl,
     +      pi,ph,pv,pg1
        end do
...
1001  format(/,'QC Statistics:',/,
     ...
     +    '  Parameter',15x,
     +    'NOBS       NQC     PS/GD       BAD     QUEST'
     +    '   MISSNG   NOT MEAS',
     +    '    GLITCH    NOT QC       EST'
     +    '    FAILED    PS LOW    PS MID'
     +    '     PS HI    PS VHI'
     +    '     %QC %PS/GD   %FLG   %BAD   %DUB'
     +    '  %FAIL   %LOW   %MID    %HI   %VHI %GOOD')
1002  format(a20,15(i10),x,11(x,f6.1))       ! was: i8
...
1022  format('Network    Station',14x,
     ...
     +    '     %QC %PS/GD   %FLG   %BAD   %DUB'
     +    '  %FAIL   %LOW   %MID    %HI   %VHI %GOOD')
1023  format(a10,x,a15,15(i10),x,11(x,f6.1)) ! was i8
...
1032  format('Day',9x,
     ...
     +    '     %QC %PS/GD   %FLG   %BAD   %DUB'
     +    '  %FAIL   %LOW   %MID    %HI   %VHI %GOOD')
1033  format(i3,3x,15(i10),x,11(x,f6.1))     ! was i8
...
1042  format('Hr',9x,
     ...
     +    '     %QC %PS/GD   %FLG   %BAD   %DUB'
     +    '  %FAIL   %LOW   %MID    %HI   %VHI %GOOD')
1043  format(i2,3x,15(i10),x,11(x,f6.1))     ! was i8
1151  format(///,
     +'NOTE:  Stations in Worst STATION lists are in random order.'/)
