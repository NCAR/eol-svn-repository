...
echo "Process all 5minute files"
   ls /storage/NEWQC/CASEDAYS/940420.qcf > $IN
   ls /storage/NEWQC/CASEDAYS/940527.qcf >> $IN

#   ls /data10/NEWQC/94*.qcf > $IN
   echo "quit" >> $IN

   $QCPATH/stnstat -nominal < $IN > $OUT

#was:   $QCPATH/stnstat < $IN > $OUT
#   enscript -b$TITLE -r -FTimes-Bold14 -fCourier9 -r $OUT
