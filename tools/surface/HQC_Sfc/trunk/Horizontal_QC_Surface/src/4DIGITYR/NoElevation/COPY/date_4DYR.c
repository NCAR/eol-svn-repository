 ...
 *                  (YYYYMMDD to JJJ) (4 digit yr)
 *-------------------------------------------------------*/
void date_to_julian(/*in*/  char date[9], /* YYYYMMDD */
...
void julian_to_date(/*in*/  int  julian_date,
    ...
    * Form date string.
    */
   sprintf (date, "%4d%02d%02d\0", yr, mo, day);
 
   } /* julian_to_date() */


/*-------------------------------------------------------
 * date_to_YYYYJJJ() - converts YYYYMMDD format to
 *   internal "YYYYJJJ" integer format: YYYYJJJ.
 *   (YYYYMMDD to YYYYJJJ) (4 digit yr)
 *-------------------------------------------------------*/
void date_to_YYYYJJJ(/*in*/  char date_YYYYMMDD[9],
                     /*in*/  int  yr,   /* four places - 1994 */
                     /*out*/ long int  *YYYYJJJ)
   {
   int    date_JJJ;

   date_to_julian (date_YYYYMMDD, yr, &date_JJJ);
 ...
 *   to YYYYMMDD format. (YYYYJJJ to YYYYMMDD) (4 digit yr)
 *-------------------------------------------------------*/ 
void YYYYJJJ_to_date(/*in*/  long int   YYYYJJJ,
                     /*out*/ char  date_YYYYMMDD[9])
   {
   char  YYYYJJJ_str[8] = "\0\0\0\0\0\0\0";
   char  JJJ_str[4] = "\0\0\0";
   char  YYYY_str[5] = "\0\0\0\0\0";
   ...
   int   YYYY = 0; /* Year       */
    ...
    julian_to_date (JJJ, YYYY, date_YYYYMMDD); /* form YYYYMMDD date */
