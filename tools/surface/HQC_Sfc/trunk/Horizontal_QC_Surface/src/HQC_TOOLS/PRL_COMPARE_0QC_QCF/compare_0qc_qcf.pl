...
#-------------------------------------------------------------------
# compare_0qc_qcf.pl - This perl program compares a non-HQC'd 0qc
#   file to the associated HQC'd qcf file.
#
#   Execute: compare_0qc_qcf.pl solar_YYYYMMDD.0qc solar_YYYYMMDD.qcf
#
#   Required Inputs: Two input parameters are required as shown below:
#
#      compare_0qc_qcf.pl input_file_name.0qc  input_file_name.qcf
#
#   where [input_file_name] is the name of either file to be checked.
#
#   E.g.,  compare_0qc_qcf.pl solar_20030704.0qc solar_20030704.qcf
#
#   Output:
...
#
#   That the HQC may perform the following modifications to the 
#   incoming data:
#      * When a data point can be HQC'd, the associated QC flag
#        will be set accordingly. This is true for all flags
#        except the Bad (B) flag. The HQC should never reset the
#        Bad flag.
#
#      * If the wind speed is 0.00, then the HQC will reset the 
#        wind direction to 0.00 according to WMO standard.
#
#      * If the incoming 0qc flag for a parameter is Estimated (E)
#        of Dubious (D), then the HQC may only reset that flag with Bad (B).
#
#      * If the wind speed is light and variable (<10m/s) and the wind
#        direction 0qc flag was set to Bad (B), the HQC will reset the
#        flag to Dubious (D).
#
#      * If precip is > 0.00 and qc flag is G and the first cloud amount
#        is 0 (missing), can't have precip in clear air, so HQC will
#        reset precip QC flag to D.
#
#      * 
...
##################################################
#----------------------------------------------
# check_file_times() - subroutine ensures that
#   data times are not missing records. If records
#   are missing, warning messages are issued.
#   This s/w will add in missing records??????
#   Does this s/w assume there's only one stn? One level?
#----------------------------------------------
sub check_file_times {
   my ($input_file_name, $data_frequency) = @_;

   if (1) {print "\nEnter check_file_times: $input_file_name, $data_frequency\n";}

   @sorted_days_in_month = (31,28,31,30,31,30,31,31,30,31,30,31);

   $sorted_lines_processed = 0;

   #------------------------
   # Open Sorted input file.
   #------------------------
   open (SORTED_INPUT_FILE, "<$input_file_name.sort") ||
     die "Can NOT open first arg $input_file_name.sort input file for reading"; 

   open (SORTED_OUTPUT_FILE, ">$input_file_name.missingfilled") ||
     die "Can NOT open first arg $input_file_name.missingfilled input file for writing";

   while ($sorted_input_line = <SORTED_INPUT_FILE>)
     {
     $sorted_lines_processed++;
     print "Sorted Rec: $sorted_input_line\n";	 

     $nominal_year = substr ($sorted_input_line, $meta_positions{UTC_nominal_year}{BEG_POS},
                             $meta_positions{UTC_nominal_year}{LENGTH});

     $nominal_month = substr ($sorted_input_line, $meta_positions{UTC_nominal_month}{BEG_POS},
                              $meta_positions{UTC_nominal_month}{LENGTH});

     $nominal_day = substr ($sorted_input_line, $meta_positions{UTC_nominal_day}{BEG_POS},
     ...
     #------------------------------------------------------
     # Convert date/time to minutes and diff the minutes = 
     # should be data_freq or else time missing.
     #------------------------------------------------------
     if (($nominal_year%4==0 && $nominal_year%100 !=0 || $nominal_year%400==0) )
      {$sorted_days_in_month[1]++; print "Add leap yr day!\n";} # leap year

     $month_minutes = 0;
     for ($jj=0;$jj<$nominal_month-1;$jj++)
        { 
        $month_minutes = $month_minutes + $sorted_days_in_month[$jj]*1440; 
        print "jj, month_minutes: $jj, $month_minutes\n";
        }

     if (($nominal_year%4==0 && $nominal_year%100 !=0 || $nominal_year%400==0) )
      {$sorted_days_in_month[1]--; print "Remove leap yr day!\n";} #reset because don't know what type of gap exists!

     $date_minutes = $nominal_minutes + $nominal_hour*60 + ($nominal_date-1)*1440 + $month_minutes;
     $year_minutes = $nominal_year.$date_minutes; # 200430 =30 minutes into 1 Jan 2004
                                                  # 2003   = 0830 on 14 Feb 2003

     if (1) 
       { 
       print "nominal_year, nominal_month, nominal_day: $nominal_year, $nominal_month, $nominal_day; ";
       print "nominal_minute, nominal_hour, year_minutes: $nominal_minute, $nominal_hour, $year_minutes\n"; 
       }
    
     if ($sorted_lines_processed == 1)
       {
       $prev_year_minutes  = $year_minutes;
       }
     else
       {
       if (($year_minutes%$data_frequency) != 0)
          {
          print "      ERROR DATA: Wrong Data Frequency! Should be $data_frequency minute data at sorted file line $sorted_lines_processed in $input_file_name.sort.\n";  $err++; $err_data++;
          }

       if ($year_minute > $prev_year_minute &&
          ($year_minute - $prev_year_minute) > $data_frequency)
          {
          print "      ERROR DATA: Missing > one $data_frequency minute period at sorted file line $sorted_lines_processed in $input_file_name.sort.\n";  $err++; $err_data++;

          $new_minute = prev_year_minute;

          # Write out completely missing records to fill
          while ($new_minute < $year_minute)
             {
             # Convert to year, month, day, hour, minute;
             printf SORTED_OUTPUT_FILE  "$new_minute -999.99 M -999.99 M.......\n";




             $new_minute=+$data_frequency;
             # Convert to year, month, day, hour, minute;
       ...
       #----------------------------------
       # Save off date/times for next pass
       #----------------------------------
       $prev_year_minutes  = $year_minutes;
       }
     }

   close (SORTED_INPUT_FILE);
   close (SORTED_OUTPUT_FILE);
...
##################################################
# compare_0qc_qcf.pl - MAIN processing.
##################################################
printf "\ncompare_0qc_qcf.pl of file $ARGV[0] began on ";print scalar localtime; printf "\n";
...
#HERE
$data_frequency = $ARGV[2]; # Data Freq in Minutes
...
check_file_times ($ARGV[0].sort, $data_frequency);
...
printf "\ncompare_0qc_qcf.pl of file $ARGV[0] ended on ";print scalar localtime;
printf "\n";
