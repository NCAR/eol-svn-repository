...
#------------------------------------------------------------
# set_LKICE_20min_occ.pl - This perl script/program reads QCF
...
printf "\nset_LKICE_20min_occ.pl began at $year/$mon/$mday $hour:$min:$sec\n";

#---------------------------------------
# Open the input and output files.
#
# Input names are LK20_YYMMDD.qcf.
...
while ($line = <INFILE>) 
   ...
   #-------------------------------------------------
   if ( $stn_info eq "AWOSH20    HCDa              44.86000   -94.38000   0")
      {
      if ($debug)
         {
...
($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
printf "set_LKICE_20min_occ.pl ended at $year/$mon/$mday $hour:$min:$sec\n";
