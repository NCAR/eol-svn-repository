	...
	while (read_hrly(in_fptr, &curr_hour))
	{
		if (!(comp_recs(&curr_hour, &curr_daily)))
		{
			...
			case 1 :
				break;
			case 0 :
				fprintf(stderr, "Error in Quality Checking output record.\n");
				break;
			case -1 :
...
struct daily_rec *daily;
{
	...
	if (printres == EOF)
	{
		fprintf(stderr, "Error writing daily record.\n");
		return 0;
	}
		...
		switch (hrly->obs[i].code_flag)
		{
			case 0:
				if (accum || static_accum)
				{
					...
					fprintf(stderr, "Found an hourly reading in the middle of an accumulation period.\n");
			...
			case 7:
				...
				break;
			default :
				fprintf(stderr, "Unknown code flag %d for observation number %d in record number %d\n", hrly->obs[i].code_flag, i, recno);
				return 0;
		}
	}
...
*			are considered bad.
* Return:	1 if the record is good, 0 if the record is bad, -1 if there
