...
/*----------------------------------------------------------------------
 * build_CDstnlist.c - This code builds the stn list which is placed on
 *   the CD. It replaces the internal stn id with the external stn id
 *   shown in the data. Also, the First few characters which indicate
 *   project are stripped.
 *
 * OUTPUT: a stn list
 *
...
int main()
   ...
   if (( output_stream = fopen("./stn_list.txt", "w")) == NULL)
      perror ("Error: Can't open stn_list.txt");

   /* 
    * Both files must be sorted in same order.
    */
   if (( stations_in_stream = fopen("GIDS1_master_stn_list_best", "r")) == NULL)
      perror ("Error: Can't open GIDS1_master_stn_list_best");

   if (( stn_id_in_stream = fopen("GIDS1_master_stn_id_best", "r")) == NULL)
      perror ("Error: Can't open GIDS1_master_stn_id_best");
      ...
      while (!feof(stations_in_stream) && !feof(stn_id_in_stream))
         ...
         strncpy (junk_str1, &newline[26], MAX_CHARS);
          ...
          * Write out rec.
          */ 
         fprintf( output_stream, "%-15s %-s\n",
                  stn_id_ext, junk_str1); 
