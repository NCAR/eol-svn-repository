...
/*----------------------------------------------------------------------
 * postproc_hplains.c - This code updates the Wichita, KS station to 
 *   have the same lat and lon as shown in the STORMFEST data.
 *
 * INPUT : List of files to be processed in a file named :
 *         'filelist.hplains'. This should be a list of files
 *         with the qcf format (i.e., *.0qc). Only the records
 *         with the network shown as "HPLAINS" will be updated.
 *
...
int main()
   ...
   /* 
    * Read each record in. Only update HPLAINS. 
    */
   if (( data_stream3 = fopen("./filelist.hplains", "r")) == NULL)
      perror ("Error: Can't open filelist.hplains for reading");
   ...
   while (!feof(data_stream3))
      ...
      while (!feof(data_stream1))
         ...
         /* printf ("Check for HPLAINS!! network = %-sxxx\n", network);  */
        
         if (!strncmp("HPLAINS\0", network, 7))
            {
            /* printf ("It's HPLAINS!!\n");  */

            /*
             * Update only Wichita HPLAINS stn records accordingly.
             */
            if ( !strncmp("Wichita\0", &new_line[41], 7) ) 
               {
               /* printf ("stnid = Wichita\n"); */
               strncpy( &updated_line[54], "37.87000", 8);
               strncpy( &updated_line[65], "-97.50000", 9);
               }

            if ( !strncmp("De Soto\0", &new_line[41], 7) )
               {
               strncpy( &updated_line[54], "38.98000", 8);
               strncpy( &updated_line[65], "-94.93000", 9);
               strncpy( &updated_line[79], " 286.00", 7);
               }
 
            if ( !strncmp("RedCloud\0", &new_line[41], 8) )
               {
               strncpy( &updated_line[41], "Red Cloud\0",9);
               strncpy( &updated_line[54], "40.17000", 8);
               strncpy( &updated_line[65], "-98.87000", 9);
               strncpy( &updated_line[79], " 524.00", 7);
               }

            if ( !strncmp("Rockport\0", &new_line[41], 8) )
               {
               strncpy( &updated_line[54], "40.47000", 8);
               strncpy( &updated_line[65], "-95.87000", 9);
               strncpy( &updated_line[79], " 268.00", 7);
               }
 
            if ( !strncmp("Scottsbluf\0", &new_line[41], 10) )
               {
               strncpy( &updated_line[54], "41.85000", 8);
               strncpy( &updated_line[64], "-103.68000", 10);
               strncpy( &updated_line[79], "1208.00", 7);
               }

            if ( !strncmp("Shelton\0", &new_line[41], 7) )
               {
               strncpy( &updated_line[54], "40.77000", 8);
               strncpy( &updated_line[65], "-98.75000", 9);
               strncpy( &updated_line[79], " 614.00", 7);
               }
            ...
            {
            /*
             * This is not an HPLAINS record. Write line to output unchanged.
             */
            fprintf (data_stream2, "%-s\n", new_line);
            }
