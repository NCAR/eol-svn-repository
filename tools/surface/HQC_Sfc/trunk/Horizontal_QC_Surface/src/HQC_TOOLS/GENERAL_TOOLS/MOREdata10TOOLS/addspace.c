...
/*----------------------------------------------------------------------
 * addspace.c - This code adds a space between the the network ID and
 *   the station ID for the daily precip composite data.
 *
 * INPUT : Name of file to be processed.
 *         This should be a list of files
 *         with the pqcf format. 
 *
 * OUTPUT: File with space added. (*.pqcf.out)
 *
...
int main()
   ...
   while (!feof(data_stream3))
      ...
      while (!feof(data_stream1))
         ...
         /*
          * Add a space to the output record between the 
          * network ID and the station ID.
          */
         strncpy ( junk_str1, new_line, 18);
         strncpy ( junk_str2, &new_line[18], 1000);

         fprintf (data_stream2, "%-s %-s\n", junk_str1, junk_str2); 
