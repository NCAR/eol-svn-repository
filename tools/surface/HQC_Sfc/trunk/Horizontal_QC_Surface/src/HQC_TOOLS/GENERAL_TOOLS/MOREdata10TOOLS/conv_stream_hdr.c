...
/*----------------------------------------------------------------------
 * conv_stream_hdr.c - This code converts the usgs stream hdr file to
 *   to a specific format so that process_usgs_data.c can use the
 *   output file to extract the time zone information.
 *
 * INPUT : 
 *     Name of usgs.q.hdr file to be converted. 
 *
 * OUTPUT: 
 *
 * 000  01 Dec 93 lec
...
int main()
   ...
   printf ("\nBegin processing of USGS hdr file!\n");
   ...
   while (!feof(input_stream))
      ...
      strncpy (station_id, &new_line[2],9);
      printf ("station_id: %-sxxx\n", station_id); 

      strncpy (latdms, &new_line[16],6);
      strncpy (londms, &new_line[23],7);
      printf ("latdms, londms: %-sxxx %-sxxx\n", latdms, londms); 

      strncpy (name, &new_line[76], 80);
      /* printf ("name: %-sxxx\n", name); */
      ...
      /*
       * Automatically id those time zones which are
       * obvious. Rest must be hand identified.
       */       
      zone = '\0';

      if (lon > -84)
         zone = 'E';
      else if (lon < -89 && lon > -100)
         zone = 'C';
      else if (lon < -106 && lon > -114)
         zone = 'M';
      else if (lon < -118 && lon > -125)
         zone = 'P';
      else
         zone = 'x';

      /*
       * Initialize all sites to show Daylight Savings Time.
       * User must hand correct DCP's in WY; all sites in ND;
       * All sites in Indiana; all sites in Maryland; all sites
       * in Georgia to 'N' for non-DST.
       */      
      DST = 'D';


      fprintf (output_stream, "%-12s %9.5f  %9.5f  %1c  %1c        %-s\n",
               station_id, lat, lon, zone, DST, name);
   ...
   printf ("\nProcessing of USGS hdr file is complete!\n");
