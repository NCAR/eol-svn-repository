/*--------------------------------------------------------
 * remove_net_from_sigsqfiles - Removes the specified network
...
char *argv[];
   ...
   if (argc != 4)
      {  
      printf ("Usage: remove_net_from_sigsqfiles <network> <infile> <outfile>\n");
      exit(1);
      }  
    ...
    * for searching.
    *------------------------------------*/
   sprintf (network_to_remove, "%-10s\0", input_network);
   ...
   for (ii=0;ii<numstns_inp;ii++)
      ...
      if (!strncmp (stn_list[jjj], network_to_remove, 10))
         {
#if DEBUG
   ...
   if (stn_to_remove > -1)
      ...
      for (ii=0;ii<numstns_inp;ii++) 
         {
         if (strncmp (stn_list[ii], network_to_remove,10)) /* print all except stn to be removed */
            {
            final_ct ++;
