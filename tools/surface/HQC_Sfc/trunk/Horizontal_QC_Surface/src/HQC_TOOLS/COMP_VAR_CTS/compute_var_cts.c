 ...
 * 15 Oct 2003 lec
 *   Added to check when max bin counts are in first or
 *   last bin to also ensure that all bins are not zero.
 *   That is there are some counts in those bins. If all
 *   bins are zero (no counts), then the parm is missing.
 *-------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*Wind Direction */
    {0,0.5,1,1.5,2,3,4,5, 6,7,8,9,
     ...
     100,150,200,300,500,750,1000,1250,1500,1750,
     2000,2250,2500,2750,
     3000,3250,3500,3750,
     4000,4250,4500,4750,
     5000,5250,5500,5750,
     ...
     20000,20250,20400,20600,20800,21000,21250,21500,21750,22000,22250,22500,22750,
     23000,23250,23500,23750,24000,25000,28000,
     30000,35000,100000}}; 
...
/*orig: fprintf (output_stream, "%7d %7ld %7ld %8ld\n", jj, mm, limit[jj][mm], var_ct[jj][mm]); */
   ...
   * areas. Also ensure that there were
   * at least some counts (i.e., >0).
   *------------------------------------*/
  for (jj=0; jj<NUMQCPARMS; jj++)
    {
    if ((var_max[jj][0] ==0 || var_max[jj][0] ==(MAX_NUM_BINS-1))  && (var_max[jj][1] != 0.0))
       {
       fprintf (output_stream, "MAX num cts occurred in bin# %ld for parm= %ld. Redo bin values!\n",
                jj, var_max[jj][0]);
