...
#-------------------------------------------------------------------
# form_webPage_output_elev.pl - This perl program creates a colorized
...
#   compute distances.
#
#   Execute: form_webPage_output.pl input_station_list [AOIs] [delta_elev]
#
#   Required Inputs:
#
#     input_station_list - station to process and must be in stationCD.out form.
#     AOI - Area of Interest. Only stations within the AOI are listed in output.
#     delta_elev - Only stations within delta_elev (higher or lower) are listed in output.
#
#   Output:
#      A web page containing colorized levels for the distances
#      comparing each station in the input station list.
#
# Execute: form_webPage_output.pl input_station_list AOI_distance delta_elev
# Example: form_webPage_output.pl TREX_stationCD.out 200 50
#--------------------------------------------------------------------
#
# May 2004 lec
#   Created Version 1.0.
# May 2007 lec
#   Updated to include elevation consideration.
#-------------------------------------------------------
$number_lines_in_file = 0;
$number_lines_processed = 0;

$NUMBER_STNS_TO_PRINT = 100;  # Plus all colocated stns are printed, too.
...
#----------------------------------------------
sub create_web_page {
   my ($station_info_file, $distanceKM, $delta_elevM) = @_;

   if ($debug) { print "Enter create_web_page with station_info_file = $station_info_file; distanceKM = $distanceKM; delta_elevM = $delta_elevM\n";}
   ...
   printf DIST_OUTPUT_FILE "<H1>and within +/- $delta_elevM Meters of Current_Station elevation</H1>\n";
   printf DIST_OUTPUT_FILE "</center>\n";
   ...
   while ($input_line = <DIST_INPUT_FILE>)
      ...
      if ( $number_lines_processed != 1 && $number_lines_processed != 2 && $number_lines_processed != 3 &&
         ...
         if (1) { print "current_stn = $current_stn\n";}

         if($current_stn eq "1170010006     ")  ### HERE
           { $debug=1}
         else
           { $debug=0}
         ...
         $current_elev = $CD_elev;

         $stations_within_VAOI = 0; # HERE
...
###was:  printf DIST_OUTPUT_FILE "<b><font color=blue>StnFileLineNum.</font>Current_Station (lat/lon location): &nbsp;&nbsp;&nbsp;<font color=blue>$current_stn</font></b><br>\n";
         ...
         #-------------------------------
         # Don't process Mobile stations.
         #-------------------------------
         if (($CD_lat <= -990.0) || ($CD_lon <= -990.0) || ($CD_elev <= -9990.0))
            {
            if ($debug) {print "Current_Station is mobile OR not enough info to compare. SKIP!\n"; }
            printf DIST_OUTPUT_FILE "<b>Current_Station is Mobile OR not enough info to compare.</b><br><hr><br>\n";
            }
         else
            {
            #-------------------------
...
####           if ($stn_index[0] ne "[")
               ...
               #----------------------------------------------------------------------------------
               # Don't put in array if the stations are off in elevation more than delta_elevM.
               # This way only stations that would be used to determine current station's HQC flag
               # will be listed in output. Don't list stations that are not in HQC's AOI or 
               # Vertical AOI. If the elevation is missing just print based on horizontal distance.
               #----------------------------------------------------------------------------------
               if (($stn_index[0] ne "[") && ( (abs($current_elev - $CD_elev) <= $delta_elevM) || ($current_elev < -9990.0)) )
                  {
                  if ($debug) { print "Found stn within AOI and less than delta_elevM. Before Increment stations_within_VAOI = $stations_within_VAOI\n"; }
                  $stations_within_VAOI++;
            ...
            foreach $DIST (sort keys %stnInfo)
               ...
               if ($num == 1) #First is always "Found xx stations within...."
                  { 
                  if ($debug) {print "stnInfo{num==1}:: $stnInfo{$DIST}\n";}

                  printf DIST_OUTPUT_FILE "&nbsp;&nbsp;&nbsp;<font color=black> $stn_index KM.]</font><br><br>\n";

                  printf DIST_OUTPUT_FILE "&nbsp;&nbsp;&nbsp;<font color=black> [Found $stations_within_VAOI stations within Elevation limit of $delta_elevM meters.]</font><br><br>\n"; 
                  }
               else
                  {
...
###                        printf DIST_OUTPUT_FILE "<th>Platform</th>\n";
            ...
            } # Not mobile (lat/lon/elev > missing)
         } # Not a comment line
...
#---------------------------------------------
print "Call create_web_page( $station_info_file, $distanceKM $delta_elevM ).\n";
create_web_page ($station_info_file, $distanceKM, $delta_elevM);
