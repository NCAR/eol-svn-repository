...
/* Must choose an array size of MAXDISTSTNS (times) MAXPROCESS that fits in memory.*/
/* Was 2000, Then 7000, Then 39000 for NAME 2004 */
#define  MAXDISTSTNS 39000
/* Process matrix in MAXPROCESS hunks. was 100 then was 50*/
#define  MAXPROCESS 5
...
char *argv[];
   ...
   for (xx=0;xx<MAXDISTSTNS;xx++)
      { 
      stn_list[xx][23] = '\0'; /*was 15*//*Now add on Unique Internal Stn Number (.0->.99999) *//*HARDCODED*/
   ...
   while (!feof(input_stream))
     ...
     fgets(junk_str, 16, input_stream); /*was 14*/
     if (feof(input_stream)) break;
     junk_str[15] = '\0'; /*was 15*/

#if DEBUG2
     printf ("junk_str: xxx%-sxxx\n", junk_str);
#endif

     /* Place a uniq station number into station ID. */
     sprintf (stn_list[numstns], "%d.%-s", numstns, junk_str);
