 ...
 * handle the transtion except in the US (west longitude,  north latitude).
...
void ll2xydrv(/*in*/ double PLAT, 
  ...
  if ((ORLON > 0.0) && (PLON < 0.0))
     ...
     if(ICROSS!=0) 
        ...
        printf("(1) ORLAT, ORLON; PLAT, PLON: %f %f %f %f\n", ORLAT, ORLON, PLAT, PLON);
        exit(1); 
        }
     else
        ICROSS=3;
     ...
     if(ICROSS!=0)
        {
        printf("+++ CANNOT HANDLE DUAL HEMISPHERE CROSSOVER \n");
        printf("(2) ORLAT, ORLON; PLAT, PLON: %f %f %f %f\n", ORLAT, ORLON, PLAT, PLON);
        exit(1);
        }
     else
       ICROSS=4;
