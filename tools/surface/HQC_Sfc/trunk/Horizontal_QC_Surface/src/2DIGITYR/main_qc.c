 ...
 *              (See WARNINGS below.)
 *
 *   Usage: main_qc <initialization_input_file>
 *
 *   Or:  nohup (runQC > runQC.log) >& /dev/null&
 *
 *      where runQC contains: 
 *          /usr/bin/nice -3 main_qc QCinit.inp
 *
 ...
 *   main_qc closes a file that was not scanned completely.
 ...
 *   unlikely (B).  In the final processing step, main_qc
 ...
 *   cause main_qc to perform HQC on the station
...
int main( int argc, char *argv[])
   ...
   char         date[7]      = "\0\0\0\0\0\0"; /* YYMMDD */
 
   char         proj_begin_date[7] ="\0\0\0\0\0\0"; /* Date project began. This is the date
                                                         ...
                                                         recomputed variances. YYMMDD - input value */

   char         proj_end_date[7] ="\0\0\0\0\0\0";   /* Date that project ends. YYMMDD - input value */
   char         begin_date[7] ="\0\0\0\0\0\0"; /* Input date to begin (QC) processing. YYMMDD - input*/
   char         end_date[7]   ="\0\0\0\0\0\0"; /* Input date to end processing. YYMMDD - input value */
    ...
    * next year.
    */
   char         end_YY_str[3]   = "\0\0";
   int          end_YY   = 0;
   char         end_MMDD[5]   = "\0\0\0\0";
   char         begin_YY_str[3] = "\0\0";
   int          begin_YY = 0;
   char         begin_MMDD[5] = "\0\0\0\0";

   int          proj_begin_YY = 0;
   char         proj_begin_YY_str[3] = "\0\0";
   int          proj_end_YY = 0;
   char         proj_end_YY_str[3] = "\0\0";
    ...
    *------------------------------------------------------------*/
   date_to_YYYYJJJ (proj_begin_date, current_yr, &proj_begin_YYYYJJJ); /* given by user */

   /*------------------------------------------------------------
    * Pick out the Year the PROJECT begins. User provided century.
    *------------------------------------------------------------*/
   strncpy (proj_begin_YY_str, &proj_begin_date[0], 2);
   proj_begin_YY_str[2]  = '\0';
   proj_begin_YY = atoi (proj_begin_YY_str);
    ...
    *-----------------------------------*/
   strncpy (proj_end_YY_str, &proj_end_date[0], 2);
   proj_end_YY_str[2]  = '\0';
   proj_end_YY = atoi (proj_end_YY_str);
    ...
    *-----------------------------------------------------------*/
   if (begin_YY != proj_begin_YY)
      {
      date_to_YYYYJJJ (begin_date, current_yr+1, &begin_YYYYJJJ);
      }
   else
      {  
    ...
    *--------------------------------------------------------*/
    if (end_YY != proj_begin_YY)
      {  
      /*------------------------------------
       * Data rolls from one year into next.
       *------------------------------------*/
#if DEBUG 
      printf ("--------End rolls to next year!!! end_YY, begin_YY: %d %d\n", end_YY, begin_YY);
#endif
       ...
       *----------------------------------*/
#if DEBUG
      printf ("--------Data all in same year: end_YY, begin_YY: %d %d\n",  end_YY, begin_YY);
#endif
     ...
     *---------------------------------------------*/
    if (proj_end_YY != proj_begin_YY)
      {
      /*---------------------------------------
       * Project rolls from one year into next.
       *---------------------------------------*/
#if DEBUG 
      printf ("-PROJECT rolls to next year: proj_end_YY, proj_begin_YY: %d %d\n",  proj_end_YY, proj_begin_YY);
#endif
      date_to_YYYYJJJ (proj_end_date, current_yr+1, &proj_end_YYYYJJJ);
      }
    else
      {
      /*------------------------------------
       ...
       *------------------------------------*/
#if DEBUG 
      printf ("-PROJECT all in same year: proj_end_YY, proj_begin_YY: %d %d\n",  proj_end_YY, proj_begin_YY);
#endif
      date_to_YYYYJJJ (proj_end_date, current_yr, &proj_end_YYYYJJJ);
      }  
...
/*                if (jj == 0)
                   ...
                   * to running HQC (main_qc), but this might cause lots
...
/* #if DEBUG */
#if ALWAYS_PRINT
                        printf ("SEARCHING for sigmasq PAST file %-s at hr,min: %d %d \n",
                                sigma_output_file_name, current_hr, current_min);
...
/* #if ALWAYS_PRINT  */
   ...
   printf ("\nHQC - main_qc() completed with %ld searches for sigmasq values.\n",
