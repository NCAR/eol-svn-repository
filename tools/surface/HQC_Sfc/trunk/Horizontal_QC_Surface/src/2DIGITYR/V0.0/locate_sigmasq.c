   ...
   for( current_YYYYJJJ = begin_YYYYJJJ;  current_YYYYJJJ <= stop_YYYYJJJ;
      ...
      while (!feof(sigma_output_stream))
         ...
         if (jj == stn_no)
            ...
            if (sigma_sq[kk] < -880.00) /* Don't drop the first val located */
               {
            ...
            while (!feof(sigma_output_stream))
...
#endif
               if (sigma_sq[kk] < -880.00) /* only save first good val encountered */
                  {
                  sigma_sq[kk] = best_sigma_sq;
#if DEBUG 
               ...
               if (jj == stn_no && kk >= NUMQCPARMS-1) break; /* done searching- located or not*/

               } /* collecting this stn's sigmasq vals */
#if DEBUG            
