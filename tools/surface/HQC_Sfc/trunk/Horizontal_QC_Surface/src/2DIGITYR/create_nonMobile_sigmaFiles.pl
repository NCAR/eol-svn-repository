...
#-------------------------------------------------------
# create_nonMobile_sigmaFiles.pl - This perl script/program takes
#   input sigmasq files and changes all variances for
#   mobile stations to -888.88 (basically, missing).
#   This creates files from which total variances for
#   non-mobile stns can be computed.
# 
# 99 Mar lec
#   Created.
#-------------------------------------------------------*/
...
printf "\ncreate_nonMobile_sigmaFiles.pl began at $year/$mon/$mday $hour:$min:$sec\n";
...
####system "mkdir NOMOBILE"; # User needs to create this dir in advance.

$file_name = substr ($ARGV[0], 0, 15);

open (INFILE, "$file_name") || die "Can NOT open input file ($file_name) for reading";
open (OUTFILE, ">./NOMOBILE/$file_name") || die "Can NOT open input file ($file_name) for reading";

####open (OUTFILE, ">JUNK") || die "Can NOT open input file ($file_name) for reading";
      ...
      if ($lines_read <= $num_stns+1)
         ...
         if ($parts[0] ne "stn_no")  # skip the title line
            ...
            if ( ($stnIDs{ $index } eq "DATSAV2M") ||
               ...
               #----------------------------------------
               # Reset variance to basically be missing.
               #----------------------------------------
               $parts[2] = -888.88;

##             printf "(HERE) %5d %5d %f\n", $parts[0], $parts[1], $parts[2];

               printf OUTFILE "%5s%6s%8s\n", $parts[0], $parts[1], $parts[2];
...
($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
printf "create_nonMobile_sigmaFiles.pl ended at $year/$mon/$mday $hour:$min:$sec\n";

system "gzip ./$file_name";
system "gzip ./NOMOBILE/$file_name";
