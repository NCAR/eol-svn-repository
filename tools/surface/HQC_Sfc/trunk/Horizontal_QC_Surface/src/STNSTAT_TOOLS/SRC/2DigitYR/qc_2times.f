...
C 27 Mar 95 lec
C   Updated s/w to handle new 'I' flag which means there was insufficient
C   info to compute a derived parameter. In this case the derived parameter
C   will be "-999.99 I ". Treat as Missing. Also, no longer call IEEE error./
C   standard arithmetic fn unless a known error has occurred.
C
C******************************************************************************
      ...
      real	div1,div2,div3, std1, std2, std3,answ1
C
...
C
      ...
      do i=1,NANALPARMS+1
         ...
         if (stats(i,1) .eq. 0) then
            div1 = -999.99
         else
            div1 = dev(i,2)/stats(i,1)
         endif

         if (stats(i,2) .eq. 0) then
            div2 = -999.99
         else 
            div2 = dev(i,3)/stats(i,2)
         endif

         if (stats(i,3) .eq. 0) then
            div3 = -999.99
         else 
            div3 = dev(i,4)/stats(i,3)
         endif 
         
Cwas:         write(*,1002) parmnames(i),answ1,
C     +    (dev(i,j)/stats(i,j-1),j=2,4),(dev(i,j),j=5,8)

         write(*,1002) parmnames(i),answ1,
     +     div1, div2, div2, (dev(i,j),j=5,8)
     ...
     +    (dev(i,j),j=13,16),(dev(i,j),j=17,20)
      ...
      do i=1,NANALPARMS+1
         ...
         div1 = stats(i,1)*(stats(i,1)-1)
         div2 = stats(i,2)*(stats(i,2)-1)
         div3 = stats(i,3)*(stats(i,3)-1)

         if (n .ne. 0 .and. n .gt. 1) then
           answ1 = sqrt( (dev(i,9)*n-dev(i,1)**2) / (n*(n-1)) )
         else
           answ1 =  -999.99
         end if

         if (div1 .ne. 0) then
           std1 = sqrt( (dev(i,10)*stats(i,1)-dev(i,2)**2) /
     +        div1)
         else
           std1 = -999.99
         end if
         ...
         if (div2 .ne. 0) then
           std2 = sqrt( (dev(i,11)*stats(i,2)-dev(i,3)**2) /
     +        div2)
         else
           std2 = -999.99 
         end if 
         ...
         if (div3 .ne. 0) then
           std3 = sqrt( (dev(i,12)*stats(i,3)-dev(i,4)**2) /
     +        div3)
         else
           std3 = -999.99 
         end if 
 
CWAS:       write(*,1004) parmnames(i),(dev(i,j)+dev(i,j+4),j=13,16),
C     +        sqrt( (dev(i,9)*n-dev(i,1)**2) / (n*(n-1)) ),
C     +       (sqrt( (dev(i,j)*stats(i,j-9)-dev(i,j-8)**2) /
C     +              (stats(i,j-9)*(stats(i,j-9)-1)) ),j=10,12)

            write(*,1004) parmnames(i),(dev(i,j)+dev(i,j+4),j=13,16),
     +        answ1, std1, std2, std3 
...
CWAS: 999   call standard_arithmetic()

999   stop


1001  format(a20,9(3x,i5))
...
C A variance of -999.99 means NOT measured and, therefore, 
C not applicable.
C
C     do i=1,NANALPARMS
...
C   Now check if complete field for a parameter is zero. If all zero,
C   then assume bad MAPS data and replace with missing.
C************************************************************************
...
C
      ...
      parameter (x=81, y=62, rec=1)        ! dimension sizes; Currently only 
                                           ! one rec per file.
      integer   id, valtid
      integer   ncid, rcode
...
C     Most values come directly from Netcdf header info.
C     Order is same as varname2. Some values have been
C     tightened to help catch bad MAPS data. Note that the
C     max,min temps ever recorded on earth are [136F,-127F]
C     which correspond to approx. [334K,184K]. Better gross
C     limits would be [400K, 100K]. Left 600K on Potential Temp.
C
      data range_min / 0.0,      80000.0,  80000.0,
     +                 100.0, 100.0, -200.0, -200.0 /

      data range_max / 110000.0, 110000.0, 110000.0,
     +                 600.0, 400.0,  200.0,  200.0 /
...
C
      do i=1,4 
         ...
         readmaps=0
CCC WAS  analname='../MAPS/'//fnameanal//'0000' !!!! USE THIS ONE!!!!
CCC      analname='/data8/vortex/QC/MAPS/'//fnameanal//'0000' ! Used this for other testing.
CCC      analname='/data10/MAPS2/'//fnameanal//'0000' ! Used this for QC (2nd half of) GIST data.

         analname='/raid/MAPS2/'//fnameanal//'0000' ! Use for rest of GIST data.
...
C
C        Apparently, something in the conversion of MAPS to
C        NetCDF files can sometimes yield a complete field
C        of zeros (e.g., pressures, dew pts, etc). The following
C        loop checks for a COMPLETE field of zeros and resets
C        any complete parameter field of zeros to missing for all
C        elements of that particular parameter, only. The whole
C        field must be zero for this reset to be done.
C
         allzero = .TRUE.
         ...
         do j=1, GRIDY
            do i=1,GRIDX
              ...
              if (vals(i,j,1) .ne. 0.0) then
                 allzero = .FALSE.         ! at least one non-zero value.
                 go to 1212
              end if 
...
1212     continue
         ...
         if (allzero) then
            ...
            write(*,*)'Field ', varname2(K),' in file ', 
     +        fnameanal,' is all zero! Resetting to missing!'

            do j=1, GRIDY
               do i=1,GRIDX
                 vals(i,j,1) = -99999.0 ! reset complete field of zeros to missing
               end do
            end do
         end if
...
C*************************************************************************
      ...
      varqb7_reset = .FALSE.            ! initialize each time
...
C                 made in GIDS1 and VORTEX/GIST. This oversight has
...
C leave M (missing), N (not measured), X (PAM glitch) qc codes
C and I (insufficient data to compute derived parameter) untouched.
         ...
         if (qcf.parms(i).qc .eq. 'I') then
            stats(i,9)=stats(i,9)+1
            goto 100
         end if

C
C If the qcf value is missing, but flag was
C not set to M,N,X, or I then don't QC it and
C reset the qcf flag to not available!
C
...
C handle. Note that if the QCF value is missing
C then a check above prevents QCing the value.
C
         if ( analvals(i) .gt. -999.99 ) then
            var(i) = qcf.parms(i).val - analvals(i)
         end if

C         write(*,*)'i, qcf.parms(i).val, analvals(i), var(i): ',
C     +   i, qcf.parms(i).val, analvals(i), var(i)
...
C
     ...
     +             .and. qcf.parms(3).qc .ne. 'I'              !Don't reset CSLP if it's QC flag='I'
     +             .and. qcf.parms(1).qc .ne. 'U') then
            qcf.parms(3).qc = qcf.parms(1).qc                  !GOOD
...
C
C JM check: if wind speed < 10m/s then use 90deg for Questionable Wind Dir
C tolerance and 180 for Bad. If wind speed >= 10m/s then use 50deg for 
C Quest. tolerance and 90deg for Bad tolerance on wind direction.
C

C         write(*,*)'Input wind sp, dir:', qcf.parms(6).val, 
C     +           qcf.parms(7).val

         if (i.eq.7 .and. qcf.parms(6).val .ge. 10.0) then

C           write(*,*)
C     +  ' Wind sp is greater than 10.0 m/s - use tight limits!!!'
C           write(*,*)'varb(7), varq(7): ', varb(i),varq(i)
...
C            write(*,*)'speed, analvals(6), dir, analvals(7), varq(7): ', 
C     +      qcf.parms(6).val, analvals(6), qcf.parms(7).val, 
C     +      analvals(7), varq(7)
...
C
C        write(*,*)'i, var,varb: ', i, var(i), varb(i)
...
C
C   For JM check above, reset wind dir questionable value.
C
         if (varqb7_reset) then
            varq(7) = varq_sv
            varb(7) = varb_sv
C           write(*,*)'Reset wind tol!!!!!!'
         end if

C        write(*,*)'varq(7), varb(7): ', varq(7), varb(7)
...
C and E (Estimated) qc codes untouched. Note
C that precip is not a derived value and should
C never have it's qc flag set to I.  Note that if
C it is...the qc flag will be reset to M.
C
      ...
      if(stns(stncode).val(8) .eq. -999.99) then
	    ...
	    rocdt="        0"
         ...
         if (roctm .ne. 0) then            ! s/w use to allow division by 0
...
C 
C 27 Sept 94 lec
C   It has been decided to not perform
C   gross limit checks on squall amt. Don't
C   forget I flag, if this section is ever
C   used again.
C
