...
C     This program tests double precision gridswap routines

      IMPLICIT NONE

      DOUBLE PRECISION lat,lon,rlat,rlon,x,y
      DOUBLE PRECISION x_grid,y_grid,lat_xy,lon_xy
      character*2 t
...
C
      ...
      DOUBLE PRECISION LAT_XY,LON_XY,X_GRID,Y_GRID
      DOUBLE PRECISION DSIND,DCOSD,DATAN2D,DATAND
      DOUBLE PRECISION LAT_TRUE_P,LON_XX_P,X_POLE_P,Y_POLE_P,EARTH_RAD_P
...
C
      ...
      X_GRID=(X_POLE_P+((1.+DSIND(LAT_TRUE_P))/
     +(1.+DSIND(LAT)))*EARTH_RAD_P*DCOSD(LAT)*DCOSD(LON-LON_XX_P))
      RETURN
C
      ENTRY Y_GRID(LAT,LON)
      Y_GRID=(Y_POLE_P+((1.+DSIND(LAT_TRUE_P))/
     +(1.+DSIND(LAT)))*EARTH_RAD_P*DCOSD(LAT)*DSIND(LON-LON_XX_P))
      RETURN
C
C	LAT_XY and LON_XY give (LAT,LON) as a function of (X,Y)
C
      ENTRY LAT_XY(X,Y)
      LAT_XY=90.-2.*DATAND(SQRT((X-X_POLE_P)*(X-X_POLE_P)
     + +(Y-Y_POLE_P)*(Y-Y_POLE_P))/(EARTH_RAD_P*(1.+DSIND(LAT_TRUE_P))))
      RETURN
C
      ENTRY LON_XY(X,Y)
      LON_XY=LON_XX_P+DATAN2D((Y-Y_POLE_P),(X-X_POLE_P))
      RETURN
      END
