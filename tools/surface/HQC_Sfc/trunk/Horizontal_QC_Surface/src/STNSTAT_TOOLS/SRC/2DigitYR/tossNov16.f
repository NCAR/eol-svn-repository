...
C
C usage:   toss [-h | -m] [-b | -d | -bd] [-debug] -sXXXX
C        
...
C get command line arguments
      ...
      if(narg .eq.1) then
         call getarg(1,arg1)
	    call setparms(arg1,stnid,prth,prtm,prtb,prtd)
      else if(narg .eq.2) then
         call getarg(1,arg1)
         call getarg(2,arg2)
	    call setparms(arg1,stnid,prth,prtm,prtb,prtd)
	    call setparms(arg2,stnid,prth,prtm,prtb,prtd)
	    ...
	    call setparms(arg1,stnid,prth,prtm,prtb,prtd)
	    call setparms(arg2,stnid,prth,prtm,prtb,prtd)
	    call setparms(arg3,stnid,prth,prtm,prtb,prtd)
	    ...
	    call setparms(arg2,stnid,prth,prtm,prtb,prtd)
	    call setparms(arg3,stnid,prth,prtm,prtb,prtd)
	    call setparms(arg4,stnid,prth,prtm,prtb,prtd)
      else
...
C loop through the file of input records
      ...
      do while (rcv.eq.0 .and. rct.eq.0)
         ...
         call prttoss(prtm,prtb,prtd)	!print the toss info for this stn,day
      end do
...
C************************************************************************
      ...
      subroutine setparms(arg,stnid,prth,prtm,prtb,prtd)
...
C************************************************************************
      ...
      integer function prttoss(prtm,prtb,prtd)
...
C print pressure info
     ...
     +      stns(tossstn).elev,stns(tossstn).x,stns(tossstn).y
       ...
       if (.not.prtm) then    ! Don't print hrly if data is 5min
...
C print temperature info
     ...
     +      stns(tossstn).elev,stns(tossstn).x,stns(tossstn).y
	    ...
	    write(unit_temp,1042)
          ...
          if (.not.prtm) then    !Print hours for Hrly data not 5 min. data
...
1043  format(' ',8x,'          ************* Wind Speed ************',
     +          '      *********** Wind Direction **********',/,
     +'  day hr ',2('           maps      qcf     var    chg  cd  qc'))
1044  format(' ',8x,'*********** Precipitation (Not Compared to MAPS) ***********',/,
