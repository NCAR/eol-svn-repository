...
QCPATH="."
IN="$QCPATH/stnstat_4DYR20.in"

OUT="./stnstat_4DYR20.out"
...
# Pathnames must be verified.
  ...
  $QCPATH/stnstat_4DYR20 -nominal < $IN > $OUT
...
then
   if [ $HOURLY = "y" ]
   then
      $QCPATH/stnstat_4DYR20 -nominal > $OUT
   else
      $QCPATH/stnstat_4DYR20 > $OUT
   fi
#   enscript -b$TITLE -r -FTimes-Bold14 -fCourier9 -r $OUT
elif [ $PROMPT = "n" ]
then
   ...
   if [ $HOURLY = "y" ]
   then
echo "running: $QCPATH/stnstat_4DYR20 -nominal < $IN > $OUT"
      $QCPATH/stnstat_4DYR20 -nominal < $IN > $OUT
   else
      $QCPATH/stnstat_4DYR20 < $IN > $OUT
   fi
