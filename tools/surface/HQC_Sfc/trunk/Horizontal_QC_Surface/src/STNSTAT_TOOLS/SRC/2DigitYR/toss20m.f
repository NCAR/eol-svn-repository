...
C
C usage:   toss [-h | -m | -20m] [-b | -d | -bd] [-debug] -sXXXX
C        
C              where -debug     option will print variable info as processing
C                               is done.
C                    -h         print hourly report
C                    -m         print 5 minute report
C                    -20m       print 20 minute report
...
C 16 Sep 94 lec
C   Updated s/w to print 20 minute reports.
C******************************************************************************
...
C print flags
	 ...
	 logical prtm		!flag=true to print 5 minute report
...
C get command line arguments
      ...
      if(narg .eq.1) then
         call getarg(1,arg1)
	    call setparms(arg1,stnid,prth,prtm,prt20m,prtb,prtd)
      else if(narg .eq.2) then
         call getarg(1,arg1)
         call getarg(2,arg2)
	    call setparms(arg1,stnid,prth,prtm,prt20m,prtb,prtd)
	    call setparms(arg2,stnid,prth,prtm,prt20m,prtb,prtd)
	    ...
	    call setparms(arg1,stnid,prth,prtm,prt20m,prtb,prtd)
	    call setparms(arg2,stnid,prth,prtm,prt20m,prtb,prtd)
	    call setparms(arg3,stnid,prth,prtm,prt20m,prtb,prtd)
	    ...
	    call setparms(arg2,stnid,prth,prtm,prt20m,prtb,prtd)
	    call setparms(arg3,stnid,prth,prtm,prt20m,prtb,prtd)
	    call setparms(arg4,stnid,prth,prtm,prt20m,prtb,prtd)
      else
      ...
      if(debug) then
         ...
         if(prtm) write(*,*) 'Printing 5-minute report'
...
C loop through the file of input records
      ...
      do while (rcv.eq.0 .and. rct.eq.0)
         rct=gettoss(stnid,prtb,prtd,prtm) !get next toss stn & fill in toss qc codes
         rcv=getvar(stnid,prtm)	           !get variance records for current toss stn
         call prttoss(prth,prtm,prt20m,prtb,prtd)   !print the toss info for this stn,day
      end do
...
C************************************************************************
      ...
      subroutine setparms(arg,stnid,prth,prtm,prt20m,prtb,prtd)
...
C************************************************************************
      ...
      integer function gettoss(stnid,prtb,prtd,prtm)
...
10       read(unit_toss,1001,iostat=ios,err=901,end=900) stn,
     +      day,hr,min,code,qc
...
C if same station, update toss code in the variance array
	   if(stn .eq. tossstn) then
           ...
           if(code.le.8) i=code
           if(code.eq.13) i=8
           if(code.gt.8 .and. mod(code,NQCPARMS).le.8)
     +         i=mod(code,NQCPARMS)
           if(i.eq.0) i=17
           if(code.eq.35) i=5
		 h=hr+1
		 ...
		 vm(m,i).code=code
		 vm(m,i).qc=qc
		 if(.not.prtm) then  ! hrly
		    vh(h,i).code=code
		    vh(h,i).qc=qc
		 end if
		 if(prtm .and. min.eq.55) then
		    vh(h,i).code=code
		    vh(h,i).qc=qc
		 end if
         else
...
20       read(unit_toss,1001,iostat=ios,err=901,end=900) stn,
     +      day,hr,min,code,qc
...
C************************************************************************
      ...
      integer*2 function getvar(stnid,prtm)
...
C
C************************************************************************
      integer function prttoss(prth,prtm,prt20m,prtb,prtd)
...
C print flags
	 logical prtb		!flag=true to print "B" qc flag info
	 logical prtd		!flag=true to print "D" qc flag info
	 logical prth		!flag=true to print hrly report
	 logical prtm		!flag=true to print 5 minute report
	 logical prt20m		!flag=true to print 20 minute report
...
C print pressure info
     ...
     +      stns(tossstn).elev,stns(tossstn).x,stns(tossstn).y
       ...
       if (prth) then    ! Don't print hrly if data is 5min or 20min
     ...
     +         vh(i,3).code,vh(i,3).qc
	    ...
	    if(prtm .OR. prt20m) then    !5 or 20min data
...
C print temperature info
     ...
     +      stns(tossstn).elev,stns(tossstn).x,stns(tossstn).y
	    ...
	    write(unit_temp,1042)
          ...
          if (prth) then    !Print hours for Hrly data
