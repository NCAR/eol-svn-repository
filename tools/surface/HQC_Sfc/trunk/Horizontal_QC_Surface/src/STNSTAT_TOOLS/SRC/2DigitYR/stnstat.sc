...
#   Modified for GIDS
#
USAGE="Usage:  $0 [-debug] [-title "REPORT_TITLE_TEXT"] [-hrly | -5min] [ [-hall | -mall] | [FILE1.qcf] [FILE2.qcf] [FILE3.qcf] ... ]"
...
do
	case $1 in
		-debug)	DEBUG="y";
				shift;;
		-hrly|-hourly|-nom|-nominal)	HOURLY="y";
				shift;;
...
QCPATH="/spare1/gids/QC/exe/qc"
IN="$QCPATH/stnstat.in"
OUT="$QCPATH/stnstat.out"
if [ -r $IN ]
then
...
#  ls /spare1/gids/ilmeso/out/final/isws60_*.qcf >> $IN
...
then
   ...
   if [ $HOURLY = "y" ]
   then
echo "running: $QCPATH/stnstat -hrly < $IN > $OUT"
      $QCPATH/stnstat -hrly < $IN > $OUT
   else
      $QCPATH/stnstat < $IN > $OUT
   fi
echo  "running: enscript -b$TITLE -r -FTimes-Bold14 -fCourier9 -r $OUT"
   enscript -b$TITLE -r -FTimes-Bold14 -fCourier9 -r $OUT
fi
