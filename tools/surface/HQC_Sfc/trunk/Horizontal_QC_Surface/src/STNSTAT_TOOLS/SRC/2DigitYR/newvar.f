...
C
C usage:   var [-debug] variance file
C        
...
C
C 25 Aug 94 lec
C  Update s/w to handle 15 char station IDs which were 10 chars.
C******************************************************************************
...
C************************** end variable definitions *********************
      ...
      write(*,*)  'Running var program: Begin: ',ctime(tm)

      debug = .false.
C get command line arguments
      narg = iargc()
      if(narg .eq. 1) then
         call getarg(1,qc_var)
      else if(narg .eq.2) then
         call getarg(1,debugarg)
         call getarg(2,qc_var)
...
C initialize some variables
	 ...
	 do j=1,NPARMS
         ...
         if(j.eq.5) step(j)=.5
         if(j.eq.6) step(j)=.5
         if(j.eq.7) step(j)=5.0
C        if(j.eq.8) step(j)=.25
      end do
...
C accumulate statistics by day
	       tobs(i)=tobs(i)+1
	       d(di,i).nobs=d(di,i).nobs+1
		  if(v.var(i) .lt. 0.0) d(di,i).lvar=d(di,i).lvar+v.var(i)
		  if(v.var(i) .gt. 0.0) d(di,i).hvar=d(di,i).hvar+v.var(i)
		  if(abs(v.var(i)) .gt. abs(d(di,i).max)) d(di,i).max=v.var(i)
		  d(di,i).var2=v.var(i) ** 2

C accumulate statistics by hour
	       h(hi,i).nobs=h(hi,i).nobs+1
		  if(v.var(i) .lt. 0.0) h(hi,i).lvar=h(hi,i).lvar+v.var(i)
		  if(v.var(i) .gt. 0.0) h(hi,i).hvar=h(hi,i).hvar+v.var(i)
		  if(abs(v.var(i)) .gt. abs(h(hi,i).max)) h(hi,i).max=v.var(i)
		  h(hi,i).var2=v.var(i) ** 2

C accumulate statistics by station
	       s(si,i).nobs=s(si,i).nobs+1
		  if(v.var(i) .lt. 0.0) s(si,i).lvar=s(si,i).lvar+v.var(i)
		  if(v.var(i) .gt. 0.0) s(si,i).hvar=s(si,i).hvar+v.var(i)
		  if(abs(v.var(i)) .gt. abs(s(si,i).max)) s(si,i).max=v.var(i)
		  s(si,i).var2=v.var(i) ** 2

C accumulate grand total statistics
	       t(i).nobs=t(i).nobs+1
		  if(v.var(i) .lt. 0.0) t(i).lvar=t(i).lvar+v.var(i)
		  if(v.var(i) .gt. 0.0) t(i).hvar=t(i).hvar+v.var(i)
		  if(abs(v.var(i)) .gt. abs(t(i).max)) t(i).max=v.var(i)
		  t(i).var2=v.var(i) ** 2

C accumulate statistics by variance limit
            do j=1,20
               if(abs(v.var(i)) .gt. real(j)*step(i))
     +               nobs(j,i)=nobs(j,i)+1
            end do
...
C print statistics for station pressure
      ...
      write(*,1000)
      write(*,1006) 'day'
      do 141 i=1,47 ! was 45
         do 161 j=1,1
            if(d(i,j).nobs .eq. 0) goto 161
            p(j).n=d(i,j).nobs
            p(j).ave=(abs(d(i,j).lvar)+d(i,j).hvar)/d(i,j).nobs
            p(j).lvar=d(i,j).lvar
            p(j).hvar=d(i,j).hvar
            p(j).tvar=abs(d(i,j).lvar)+d(i,j).hvar
            p(j).max=d(i,j).max
C           p(j).dev=sqrt( abs((d(i,j).var2-p(j).tvar**2) /
C    +                     (p(j).n*(p(j).n-1))) )
161      continue
         if(d(i,1).nobs .eq.0) goto 141
         write(*,1023) i+firstday-1,(p(j).n,p(j).ave,p(j).max,
...
141   continue
      ...
      do 142 i=1,24
         do 162 j=1,1
            if(h(i,j).nobs .eq. 0) goto 162
            p(j).n=h(i,j).nobs
            p(j).ave=abs((h(i,j).lvar)+h(i,j).hvar)/h(i,j).nobs
            p(j).lvar=h(i,j).lvar
            p(j).hvar=h(i,j).hvar
            p(j).tvar=abs(h(i,j).lvar)+h(i,j).hvar
            p(j).max=h(i,j).max
C           p(j).dev=sqrt( abs((h(i,j).var2-p(j).tvar**2) /
C    +                     (p(j).n*(p(j).n-1))) )
162      continue
         if(h(i,1).nobs .eq.0) goto 142
         write(*,1023) i-1,(p(j).n,p(j).ave,p(j).max,
     +        p(j).lvar,p(j).hvar,p(j).tvar,j=1,1)
C    +        p(j).lvar,p(j).hvar,p(j).tvar,p(j).dev,j=1,1)
142   continue
...
C print statistics for sea level pressures
      write(*,1001)
      write(*,1005) 'day'
	 do 101 i=1,47    ! was 45
	    do 121 j=2,3
            if(d(i,j).nobs .eq. 0) goto 121
            p(j).n=d(i,j).nobs
            p(j).ave=(abs(d(i,j).lvar)+d(i,j).hvar)/d(i,j).nobs
            p(j).lvar=d(i,j).lvar
            p(j).hvar=d(i,j).hvar
...
C    +                     (p(j).n*(p(j).n-1))) )
121      continue
         if(d(i,2).nobs .eq.0 .and. d(i,3).nobs .eq.0) goto 101
         write(*,1022) i+firstday-1,(p(j).n,p(j).ave,p(j).max,
...
C    +                     (p(j).n*(p(j).n-1))) )
122      continue
         if(h(i,2).nobs .eq.0 .and. h(i,3).nobs .eq.0) goto 102
         write(*,1022) i-1,(p(j).n,p(j).ave,p(j).max,
     +        p(j).lvar,p(j).hvar,p(j).tvar,j=2,3)
C    +        p(j).lvar,p(j).hvar,p(j).tvar,p(j).dev,j=2,3)
102   continue
...
C print statistics for temperatures
      ...
      write(*,1002)
      write(*,1005) 'day'
	 do 104 i=1,47   ! was 45
	    do 124 j=4,5
            if(d(i,j).nobs .eq. 0) goto 124
            p(j).n=d(i,j).nobs
            p(j).ave=(abs(d(i,j).lvar)+d(i,j).hvar)/d(i,j).nobs
            p(j).lvar=d(i,j).lvar
            p(j).hvar=d(i,j).hvar
...
C    +                     (p(j).n*(p(j).n-1))) )
124      continue
         if(d(i,4).nobs .eq.0 .and. d(i,5).nobs .eq.0) goto 104
         write(*,1022) i+firstday-1,(p(j).n,p(j).ave,p(j).max,
...
C    +                     (p(j).n*(p(j).n-1))) )
125      continue
         if(h(i,4).nobs .eq.0 .and. h(i,5).nobs .eq.0) goto 105
         write(*,1022) i-1,(p(j).n,p(j).ave,p(j).max,
     +        p(j).lvar,p(j).hvar,p(j).tvar,j=4,5)
C    +        p(j).lvar,p(j).hvar,p(j).tvar,p(j).dev,j=4,5)
105   continue
...
C print statistics for winds
      ...
      write(*,1003)
      write(*,1005) 'day'
	 do 107 i=1,47 ! was 45
	    do 127 j=6,7
            if(d(i,j).nobs .eq. 0) goto 127
            p(j).n=d(i,j).nobs
            p(j).ave=(abs(d(i,j).lvar)+d(i,j).hvar)/d(i,j).nobs
            p(j).lvar=d(i,j).lvar
            p(j).hvar=d(i,j).hvar
...
C    +                     (p(j).n*(p(j).n-1))) )
127      continue
         if(d(i,6).nobs .eq.0 .and. d(i,7).nobs .eq.0) goto 107
         write(*,1022) i+firstday-1,(p(j).n,p(j).ave,p(j).max,
...
C    +                     (p(j).n*(p(j).n-1))) )
128      continue
         if(h(i,6).nobs .eq.0 .and. h(i,7).nobs .eq.0) goto 108
         write(*,1022) i-1,(p(j).n,p(j).ave,p(j).max,
     +        p(j).lvar,p(j).hvar,p(j).tvar,j=6,7)
C    +        p(j).lvar,p(j).hvar,p(j).tvar,p(j).dev,j=6,7)
108   continue
...
C     write(*,1004)
C     write(*,1005) 'day'
C     do 110 i=1,47 !was 45
C        do 130 j=8,8
C           if(d(i,j).nobs .eq. 0) goto 130
C           p(j).n=d(i,j).nobs
C           p(j).ave=(abs(d(i,j).lvar)+d(i,j).hvar)/d(i,j).nobs
C           p(j).lvar=d(i,j).lvar
C           p(j).hvar=d(i,j).hvar
C           p(j).tvar=abs(d(i,j).lvar)+d(i,j).hvar
C           p(j).max=d(i,j).max
C           p(j).dev=sqrt( abs((d(i,j).var2-p(j).tvar**2) /
C    +                     (p(j).n*(p(j).n-1))) )
C130      continue
C        if(d(i,8).nobs .eq.0) goto 110
C        write(*,1023) i+firstday-1,(p(j).n,p(j).ave,p(j).max,
...
C     do 111 i=1,24
C        do 131 j=8,8
C           if(h(i,j).nobs .eq. 0) goto 131
C           p(j).n=h(i,j).nobs
C           p(j).ave=(abs(h(i,j).lvar)+h(i,j).hvar)/h(i,j).nobs
C           p(j).lvar=h(i,j).lvar
C           p(j).hvar=h(i,j).hvar
C           p(j).tvar=abs(h(i,j).lvar)+h(i,j).hvar
C           p(j).max=h(i,j).max
C           p(j).dev=sqrt( abs((h(i,j).var2-p(j).tvar**2) /
C    +                     (p(j).n*(p(j).n-1))) )
C131      continue
C        if(h(i,8).nobs .eq.0) goto 111
...
C print grand total statistics
      write(*,1050)
      write(*,1008)
	 do 113 j=1,NPARMS
            if(t(j).nobs .eq. 0) goto 113
            p(j).n=t(j).nobs
            p(j).ave=(abs(t(j).lvar)+t(j).hvar)/t(j).nobs
            p(j).lvar=t(j).lvar
            p(j).hvar=t(j).hvar
            p(j).tvar=abs(t(j).lvar)+t(j).hvar
            p(j).max=t(j).max
C           p(j).dev=sqrt( abs((t(j).var2-p(j).tvar**2) /
C    +                     (p(j).n*(p(j).n-1))) )
         write(*,1025) parmnames(j),p(j).n,p(j).ave,p(j).max,
     +          p(j).lvar,p(j).hvar,p(j).tvar
C    +          p(j).lvar,p(j).hvar,p(j).tvar,p(j).dev
113   continue

C print statistics for kick outs by variance step
      write(*,1007)
	 do i=1,20
            write(*,1024) (real(i)*step(j),nobs(i,j),
     +         real(nobs(i,j))*100/tobs(j),j=1,NPARMS)
      end do

      close(unit_var)
      goto 999
901	 errstr=gerror(errstr)
      write(0,*) 'qc(): I/O Error: ',ios,' ',errstr
999   continue
      write(*,*) 'In newvar.f'
      call standard_arithmetic()
      stop
1050  format(/////,'GRAND TOTAL Variance statistics')
...
1006  format(4x,
     ...
     +  '|   obs     var     var       var       var       var')
1008  format(///,22x,
     +    '************* ALL stations; ALL time **************',
     + /,25x,
     + 'no.     ave     max       low      high     total',
     + /,'parameter',15x,
     + 'obs     var     var       var       var       var',/)
1007  format(///,'Misc Variance statistics',/,
     +   '**** stn pres ***| ',
     +   '**** sl pres ****| ','** calc sl pres *| ',
     +   '***** temp ******| ','***** dew pt ****| ',
     +   '*** wind speed **| ','**** wind dir ***| ',/,
C    +   '***** precip ****| ',/,
     + 7('  var # obs % tot| '),/,)
...
1024  format(7(f5.2,x,i5,f6.2,'|',x))
...
C write the variance record
     ...
     +  v.stn,v.day,v.hr,min,(maps(i),v.var(i),i=1,NPARMS)
      if(firstday.eq.-999) firstday = v.day
