C************************************************************************
C
C function: getqcfrec2_4DYR()
C description: get the qcf records with nominal date/time from the qcf file
C
C 26 Aug 94 lec
C  Updated s/w to handle 15 char station ids instead of 10 chars.
C 9 Sept 2002 lec
C   Updated s/w to handle 4 Digit YR.     
C************************************************************************

      integer*2 function getqcfrec2_4DYR()
      ...
      include 'debug.h'
      include 'ioerr.h'
      character*259 qcfbuf

C loop throught the data records
      getqcfrec2_4DYR = 0
      if (debug) write (0,*)'Before read unit_qcf 1002, yn::', yn

100   continue
      if (debug) write (0,*)'read()'
      read(unit_qcf,1002,iostat=ios,err=901,end=900) qcfbuf
      if (debug) write (0,*)'After unit_qcf 1002, yn, qcfbuf::', 
     + yn, qcfbuf

      if(qcfbuf(1:8) .eq. '********' .or.
     +   qcfbuf(1:13) .eq. 'Nominal  Time' .or.
     +   qcfbuf(1:13) .eq. 'Date     UTC ') goto 100
     ...
     +qcf.cc(3).cldamt,qcf.cc(3).cldqc
      if(debug) then
         write (0,*)'Before debug=false - yn::', yn
     ...
     +     qcf.cc(3).ceilhgt,qcf.cc(3).ceilflg,qcf.cc(3).ceilqc,
     +     qcf.cc(3).cldamt,qcf.cc(3).cldqc
	    write(0,*) ' getqcfrec2_4DYR: Continue?'
         accept 1005, yn
         write (0,*)'Before end if: yn::', yn
      end if
      if (debug) write (0,*)'goto 999 - return'

      goto 999

900   getqcfrec2_4DYR = -2
      if (debug) write (0,*)'900 getqcfrec2_4DYR = -2 - goto 999'
      goto 999
901   errstr=gerror(errstr)
      write (0,*)'901 Grabbed errstr; then return'
      write(0,*) 'getqcfrec2_4DYR(): I/O Error: ',ios,' ',errstr
      write(0,*) 'qcfbuf=',qcfbuf
999   return
1001  format(a4,'/',a2,'/',a2,1x,a2,':',a2,1x,
     *       a4,'/',a2,'/',a2,1x,a2,':',a2,1x,a10,1x,a15,1x,f10.5,
     *          1x,f11.5,1x,i3,1x,f7.2,1x,
     *          8(f7.2,1x,a1,1x),a1,1x,f7.2,1x,a1,
     *          1x,i4,1x,a1,1x,f8.2,1x,a1,1x,
     *          3(f7.2,1x,i2,1x,a1,1x,i2,1x,a1,1x))
1002  format(a259)	!This must be equal to length of the qcf record
1006  format(a31)	!This must be equal to length of the eof indicator
1003  format('Record read:',/,
     +'Nominal Date/Time: ',a4,'/',a2,'/',a2,x,a2,':',a2,/,
     +'Date/Time: ',a4,'/',a2,'/',a2,x,a2,':',a2,/,
