...
#ifdef ALASKA
#include "qc_BNL_ARM_NWS_ALASKA_limits.h"
#else
#include "qc_BNL_ARM_NWS_PLAINS_limits.h"
#endif
...
void check_ascension_rate_range(class_node& node, ofstream& err_file) {
  if (node.asc_rate > QUEST_MAX_ASC_RATE) { 
    char outline[100];
...
/**
 * Check to see if the time is increasing like expected.
 *
 * @param err_file The output stream where errors are to be written.
