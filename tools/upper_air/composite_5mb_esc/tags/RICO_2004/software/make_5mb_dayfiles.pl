#! /usr/bin/perl -w
##Module------------------------------------------------------------------------------------
# <p>The make_5mb_dayfiles.pl script is used for merging individual CLASS formatted
# 5 mb sounding files into day files.  These files are the invidual soundings stacked next to
...
sub get_source_directory { return "../5mb"; }
...
sub create_day_files {
	...
	$count += @{ $days->{$day}};
	system(sprintf("cat %s > %s/%s_5mb_%s.cls",join(" ",sort(@{ $days->{$day}})),
