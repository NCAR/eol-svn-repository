...
/**
 * <p>The NominalTimeCheckState class is a CheckState that checks the ESC header
 * line that contains the nominal time the sounding was released.  The checks 
 ...
 * @author Joel Clawson
 */
public class NominalTimeCheckState extends DateCheckState {

	/**
	 * Create a new instance of a NominalTimeCheckState.
	 ...
	 */
	public NominalTimeCheckState(BufferedReader in, String currentLine, int lineNumber) {
	...
	@Override public void executeLineCheck(String line, int lineNumber, DataStore store, PrintWriter log) {
		...
		if (!line.startsWith("Nominal Release Time (y,m,d,h,m,s):")) {
			log.printf("The 'Nominal Release' label is not a 35 char string starting with 'Nominal Release Time (y,m,d,h,m,s):' on line %d.\n", lineNumber);
		}
		
		// 2.  Make sure there is a valid date after the label.
		Calendar nominal = parseDate(line.substring(35));
		if (nominal == null) {
			log.printf("The 'Nominal Release' line does not have a valid date '%s' after the label on line %d.\n", line.substring(35), lineNumber);
		} else {
			store.setNominalDate(nominal);
		}
	...
	/**
	 * Perform the check to ensure that the nominal time header line is on the 12th 
	 * line of the file.
	 * @param lineNumber The number of the line in the file.
	 * @param log The output stream where failures are to be written.
	 */
	@Override public void executeLineNumberCheck(int lineNumber, PrintWriter log) {
		if (lineNumber != 12) {
			log.printf("The 'Nominal Release' header line is expected on line 12.  It is on line %d.\n", lineNumber);
			log.flush();
		}
	}
}
