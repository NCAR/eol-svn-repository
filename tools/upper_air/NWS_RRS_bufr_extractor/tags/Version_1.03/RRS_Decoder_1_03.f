...
c    NCDC ADDED VARIABLES...
      character bufrin*80 
c     character of19*80, of20*80,
c    &  of21*80, of22*80, of23*80, of24*80, of25*80
      character opt*1, rslash*1, tconv*1
c
      integer narg, iargc, idate, ireadsb, ireadmg, ii,
     & ix, lenout
      integer*1 numrsl /92/
      logical gotbfr, doloop
c
      equivalence (rslash, numrsl)  ! Reverse Slash (DOS Usages)
c
...
c    NCDC ADDED CODE...
      write(6, 20) 
 20   format(/' Program: RRS_Decoder.exe   Ver. 1.03')
c
...
c     &     /' Usage:  RRS_Decoder UNIXBUFRfile'
...
c
      ...
      do while (tconv .eq. ' ')
       write(6, 60)
 60    format(/' CHOOSE TEMPERATURE UNITS:  K = Kelvin,',
     &   ' C = Celcius')
       read(5, '(a1)') tconv
       ...
       elseif (tconv .eq. 'c' .or. tconv .eq. 'C') then
        ...
        write(6, 62) tconv
 62     format(//' NOTE:  UNITS CODE NOT ACCEPTABLE, ',
     &    'Please Re-enter the UNITS')
        tconv = ' '
       endif
      enddo
c
...
C  -----------------------------------------------
      ...
      OPEN( 8, FILE=bufrin, FORM='UNFORMATTED')
      CALL OPENBF(8,'IN',8)
c
...
C
      ...
      SUBROUTINE def_fname(wban,iyy,imm,idd,ihh,imin)
c
      implicit none
c
      common /fninfo/ lendir, op19, op20, op21, op22, op23, op24,
     &  op25, fname, outdir, stname
      ...
      logical op19, op20, op21, op22, op23, op24, op25
c
      integer wban,iyy,imm,idd,ihh,imin
...
c
     ...
     &  'tbpb 11501 78954 Barbados, Christchurch',
     &  'mdsd 11629 78486 Santo Domingo, Dom. Rep.',
     &  'ttpp 11634 78970 Trinidad, Piarco',
     &  'tjsj 11641 78526 San Juan, PR',
     &  'tncc 11643 78988 Curacao, N. A.',
     &  'tncm 11645 78866 St Maarten, N.A.',
     &  'mkjp 11715 78397 Kingston, Jamaica',
     &  'mwcg 11813 78384 Grand Caymen Is.',
     &  'sksp 11814 80001 San Andres Is, Columbia',
     &  'mzbz 11818 78583 Belize, Ladyville',
     &  'mynn 12717 78073 Bahamas, Nassau',
     &  'ktbw 12842 72210 Tampa Bay, FL',
     &  'keyw 12850 72201 Key West, FL',
     &  'kccv 12868 74794 Cape Canaveral, FL',
     ...
     &  'kwaj 40604 91366 Kwajalein Atoll',
     &  'pkmr 40710 91376 Majuro',
     &  'pgum 41406 91212 Agana, Guam',
     &  'kasi 50101 61902 Ascension Island',
     ...
     &  'spol 90001 98009 South Pole (Amundsen-Scott)',
     &  'kmfl 92803 72202 Miami, FL',
     &  'knid 93104 72384 China Lake, CA',
     &  'ptmu 93111 72391 Point Mugu, CA',
     &  'snic 93116 72291 San Nicholas Is, CA',
