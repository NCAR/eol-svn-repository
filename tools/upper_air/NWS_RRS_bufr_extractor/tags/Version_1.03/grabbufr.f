...
C
C   INPUT FILES:
c     unit  5  - Size of input bufr file in bytes (ie from wc)
...
C  ------------------------------------------------
      ...
      ENDIF
C
C  READ BYTE LENGTH OF BUFR INPUT FILE FROM STANDARD INPUT UNIT 5
C  --------------------------------------------------------------
C     READ(5,*,END=900,ERR=900) KBYTES
...
C  ------------------------------
      ...
      if (.not.exists) then
       write(6, *)'ERROR:  Input File NOT Found'
       stop
      endif
c
...
C  Search for AND EXTRACT THE next BUFR message
C  --------------------------------------------
...
C  UNPACK THE BUFR edition number
C  ------------------------------
        call gbyte(cbuf(ibeg),ied,56,8)

C  UNPACK THE Length of BUFR message IN BYTES
C  -------------------------------------------
        if (ied.le.1) then
...
C  WRITE OUT THE EXTRACTED MESSAGE
C  -------------------------------
        if ( ctemp.eq.'7777') then ! THE MESSAGE ENDS WELL
          if (ied.le.1) then ! IF EDITION 0 OR 1 CONVERT TO EDITION 2
            ilen = ilen+4
            call sbyte(ctemp,ilen,0,24)
            call sbyte(ctemp,newed,24,8)
            write(i2) bufr,ctemp,(cbuf(j),j=ibeg+4,iend),
     .      (byte(j),j=1,8-mod(ilen,8))
          else               ! WRITE THE MESSAGE AS IS
            write(i2) (cbuf(j),j=ibeg,iend),
     .      (byte(j),j=1,8-mod(ilen,8))
          endif
          ibeg=iend
        else ! BAD BUFR - MISSING THE 7777
...
C  END OF THE LOOP AND THE PROGRAM
C  -------------------------------
