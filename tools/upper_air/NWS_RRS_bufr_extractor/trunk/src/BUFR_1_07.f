...
C.KRK       
*
*  TO DO: remove all of the remaining PRINT lines
*  TO DO: provide a better abort than CALL EXIT in BORT
*
*  Version:     1.07
*   09/09/05/KRK -- fix HHMMSS to be *10 in BORT
*   09/07/05/KRK -- fix LEN3 uninitialized in CMSGINI
*   09/06/05/KRK -- fix guard condition order-of-eval in UFBSEQ
*  Version:     1.06
*   07/11/03/JA  -- Jeff: change STANDARD to not expand subset 
*  Version:     1.05
*   04/29/03/JSW -- correct bug in SAVE'ing WRCMPS info
*  Version:     1.04
*   04/14/03/JSW -- added WRITST to simplify using STANDARD
*   04/14/03/JSW -- correct small bug in READLC and WRITLC
*   04/14/03/JSW -- remodel READLC enable for standard messages
*   04/14/03/JSW -- correct small bug in RDTREE for new READLC
*   04/14/03/JSW -- some small improvements to UFDUMP
*  Version:     1.03
*   03/27/03/KRK -- Jeff: change CMSGINI and DXMINI as well
*   03/14/03/KRK -- Jeff: change MSGINI octet  6 to be '9' (other orig)
*   03/14/03/KRK -- Jeff: change MSGINI octet 11 to be '10' (version)
*  Version:     1.02
*   02/22/02/KRK -- patch to compile cleanly
*   03/18/02/JSW -- correction to WRCMPS,READLC; added MAXOUT
*   03/12/02/JSW -- small correction/addition to WRCMPS
*   02/25/02/JSW -- add WRITCP WRCMPS CMSGINI READLC UFDUMP
*  Version:     1.01
*   02/11/02/KRK -- add Jack Woollen's WRITLC
*   02/04/02/KRK -- patch BORT to dump to log file
*   01/14/02/KRK -- patch to compile cleanly
*  Version:     1.0
*   04/11/01/KRK -- Jack Woollen version from NCEP
*
C.KRK    
C---------------------------------------------------------------------- 
...
902   CALL BORT('ADN30 - CHARACTER LENGTH L30 <> 5 OR 6')              
      ...
      COMMON /BITBUF/ MAXBYT,IBIT,IBAY(5000),MBYT(NF),MBAY(5000,NF)     
...
C.KRK      PRINT*
      ...
      OPEN (51,FILE='BUFR-err.log',ACTION='WRITE',POSITION='APPEND')
      WRITE(51,*) CCYYMMDD//' '//HHMMSS(1:6)//' '//STR
      CLOSE(51)
      CALLEXIT(49)                                                          
      END                                                               
...
C-----------------------------------------------------------------------
      ...
      IF(IL.LT.0) GOTO 901
      IF(IM.NE.0) THEN
         IF(NSUB(LUN).GT.0) CALL MSGWRT(LUNIT,MBAY(1,LUN),MBYT(LUN))
         IF(NSUB(LUN).LT.0) CALL WRCMPS(-LUNIT)
      ENDIF
...
C  ----------------------------------
      ...
      IF(MCEN.EQ.1) CALL BORT('CMSGINI - IDATE = 0!')
...
C  ---------
      ...
      CALL PKB(   3 ,  8 , MESG,MBIT)
C.KRK CALL PKB(   7 ,  8 , MESG,MBIT)
      CALL PKB(   9 ,  8 , MESG,MBIT)
      CALL PKB(   0 ,  8 , MESG,MBIT)
      CALL PKB(   0 ,  8 , MESG,MBIT)
      CALL PKB(MTYP ,  8 , MESG,MBIT)
      CALL PKB(MSBT ,  8 , MESG,MBIT)
C.KRK CALL PKB(   4 ,  8 , MESG,MBIT)
      CALL PKB(  10 ,  8 , MESG,MBIT)
      CALL PKB(   0 ,  8 , MESG,MBIT)
...
900   CALL BORT('CMSGINI - CANT FIND SUBSET IDN: '//SUBSET)
      END
...
c     print*,adn30(idnr(i,2)),':',idnr(i,2)
      else
      IDND(N,LUN) = ifxy('360000')                                      
      TABD(N,LUN)(1: 6) = '360000'
      TABD(N,LUN)(7:70) = INID(I)                                       
      endif
      NTBD(LUN) = N                                                     
      ENDDO                                                             
...
C  ---------                                                            
      ...
      CALL PKB(     3 ,  8 , MBAY,MBIT)                                 
C.KRK CALL PKB(     7 ,  8 , MBAY,MBIT)                                 
      CALL PKB(     9 ,  8 , MBAY,MBIT)                                 
      CALL PKB(     0 ,  8 , MBAY,MBIT)                                 
      CALL PKB(     0 ,  8 , MBAY,MBIT)                                 
      CALL PKB(  MTYP ,  8 , MBAY,MBIT)                                 
      CALL PKB(  MSBT ,  8 , MBAY,MBIT)                                 
C.KRK CALL PKB(     4 ,  8 , MBAY,MBIT)                                 
      CALL PKB(    10 ,  8 , MBAY,MBIT)                                 
...
C     ENTRY IREADMM(IMSG,SUBSET,IDATE)                                 
C     CALL READMM(IMSG,SUBSET,IDATE,IRET)                         
C     IREADMM = IRET
C     RETURN                                                            
...
C  ---------                                                            
      ...
      CALL PKB(   3 ,  8 , MBAY(1,LUN),MBIT)                            
C.KRK CALL PKB(   7 ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(   9 ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(   0 ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(   0 ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(MTYP ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(MSBT ,  8 , MBAY(1,LUN),MBIT)                            
C.KRK CALL PKB(   4 ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(  10 ,  8 , MBAY(1,LUN),MBIT)                            
      CALL PKB(   0 ,  8 , MBAY(1,LUN),MBIT)                            
...
C  --------------------------------------------------------             
      ...
      IF(IO.EQ.'IN' .AND. LUNIT.EQ.LUNDX) THEN                          
         ...
         IBIT = 0                                                       
C.KRK    CALL UPC(BUFR,4,MSTR,IBIT)                                     
         CALL UPC(BUFR,4,ISTR,IBIT)                                     
...
900   CALL BORT('POSAPX - IO ERR REREADING A MESSAGE')                 
     ...
     .                ISEQ(MAXJL,2),JSEQ(MAXJL)                         
      ...
      CHARACTER*10 TAG                                                  
      CHARACTER*3  TYP                                                  
      DIMENSION ITMP(MAXJL,MAXRC),VTMP(MAXJL,MAXRC)                 
      DIMENSION NBMP(2,MAXRC),NEWN(2,MAXRC)                           
      DIMENSION KNX(MAXRC)                                             
      REAL*8    VAL,VTMP                                                
...
C  -------                                                              
      ...
      IDN = IDN30(CIDN,L30)                                             
c     print*,n,idn,':',numb,':',nemo,':',cidn,':',adn30(idn,l30)
...
C.KRK CHARACTER*8   SUBSET,CVAL,CTAG
      CHARACTER*8   SUBSET,CREF,CVAL,CTAG
      CHARACTER*3   TYP
      REAL*8        VAL
...
C  ---------------------
      ...
      CALL STATUS(LUNIT,LUN,IL,IM)
      IF(IL.EQ.0) CALL BORT('READLC - FILE IS CLOSED          ')
      IF(IL.GT.0) CALL BORT('READLC - FILE IS OPEN FOR OUTPUT ')
      IF(IM.EQ.0) CALL BORT('READLC - NO MESSAGE OPEN         ')

C  CHECK FOR TAGS IN THE INPUT STRING
C  ----------------------------------

      CALL PARSEQ(STR,TGS,MAXTG,NTG)
      IF(NTG.GT.1) CALL BORT('READLC - MORE THAN ONE TAG')
      CTAG = TGS(1)
      CHR = ' '
...
C  ---------------------------------------------------
      ...
      ENDDO
      RETURN
1     IF(ITP(NOD).NE.3) CALL BORT('READLC - TAG NOT CHARACTER ELEMENT')
...
C  ---------------------------
      ...
      ELSEIF(MSGUNP(LUN).EQ.2) THEN
         CALL BORT('READLC - NOT ENABLED FOR COMPRESSED DATA')
      ELSE
         CALL BORT('READLC - UNKNOWN MSGUNP TYPE')
      ENDIF
...
C  -----------------------------------------------------------          
      ...
      CALL PKB(ISUB,16,MSGOT,IBIT)                                      
      CALL PKB(0,8,MSGOT,IBIT)                            
                                                                        
C  COPY EACH DESCRIPTOR IN THE SUBSET SEQUENCE INTO THE NEW SECTION 3   
C  ------------------------------------------------------------------   

C     CALL UPTDD(ITAB,LUN,0,NSEQ)                                       

C     DO N=1,NSEQ                                                       
C     CALL UPTDD(ITAB,LUN,N,IDSC)                                       
C     CALL PKB(IDSC,16,MSGOT,IBIT)                                      
C     IF(N.EQ.NSEQ) CALL PKB(0,8,MSGOT,IBIT)                            
C     ENDDO                                                             
...
902   CALL BORT('UFBEVN - I-NODE MISMATCH                    ')        
     ...
     .                ISEQ(MAXJL,2),JSEQ(MAXJL)                         
      ...
      DIMENSION     TAB(I1)                                             
      EQUIVALENCE   (CVAL,RVAL)
      REAL*8        VAL,RVAL,TAB,BMISS,UPS,TEN
...
C.KRK fix guard condition order-of-eval (test INS1 before VAL)
      IF(INS1.GT.0.AND.TYP(NODE).EQ.'RPC') THEN
         IF(VAL(INS1,LUN).EQ.0.) THEN
            INS1 = INS1+1
            GOTO 1
         ENDIF
      ENDIF
...
903   CALL BORT('UFBSTP - INCOMPLETE WRITE                   ')        
     ...
     .                ISEQ(MAXJL,2),JSEQ(MAXJL)                         
      ...
      EQUIVALENCE   (CVAL,RVAL)                                         
      LOGICAL       OPENIT                                              
      REAL*8        VAL,TAB,RVAL,BMISS,UPS,TEN
...
C  ---------------------------------------------------                  
      ...
      IF(ITYP.GE.1.AND.ITYP.LE.3) THEN                      
         ...
         IF(TAB.NE.'B') CALL BORT('UFBDMP - BAD ITYP!')
         NUMB = TABB(N,LUN)(1:6)
      ...
      IF(ITYP.EQ.1.OR.ITYP.EQ.2) THEN                      
         IF(RVAL.LT.BMISS) THEN
            FMT = '(A6,2X,A10,2X,F20.00,2X,A24)'
            WRITE(FMT(19:20),'(I2)') MAX(1,ISC(NODE))
            WRITE(LUOUT,FMT) NUMB,NEMO,RVAL,UNIT     
         ELSE
            FMT = '(A6,2X,A10,2X,A20,2X,A24)'
            WRITE(LUOUT,FMT) NUMB,NEMO,PMISS,UNIT
         ENDIF
      ELSEIF(ITYP.EQ.3) THEN                                            
         LCHR = CVAL
         NCHR = IBT(NODE)/8
         IF(NCHR.GT.8.AND.IL.LT.0) CALL READLC(LUNIT,LCHR,NEMO)
         IF(RVAL.EQ.BMISS) LCHR = PMISS
         IRET = RJUST(LCHR)
         FMT = '(A6,2X,A10,2X,A20,2X,"(",i2,")",A24)'
         WRITE(LUOUT,FMT) NUMB,NEMO,LCHR,NCHR,UNIT    
      ENDIF                                                             
      ENDDO                                                             
...
C  ---------------------------------------                              
     ...
     .                ISEQ(MAXJL,2),JSEQ(MAXJL)                         
      ...
      CHARACTER*10 TAG                                                  
      CHARACTER*3  TYP                                                  
      DIMENSION    ITMP(MAXJL),VTMP(MAXJL)                            
...
C  -------------------------------                                      
      ...
      IF(NVAL(LUN)+NEWN*NBMP.GT.MAXJL) print*,'@:',nval(lun)+newn*nbmp 
      IF(NVAL(LUN)+NEWN*NBMP.GT.MAXJL) GOTO 907                        
...
900   VALX = BMISS                                                      
     ...
     .                ISEQ(MAXJL,2),JSEQ(MAXJL)
      COMMON /USRINT/ NVAL(NF),INV(MAXJL,NF),VAL(MAXJL,NF)
      COMMON /USRBIT/ NBIT(MAXJL),MBIT(MAXJL)
      COMMON /COMPRS/ MATX(MROW,MCOL),CATX(MROW,MCOL)
      COMMON /COMPRS/ KMIN(MROW),KMAX(MROW),KMIS(MROW),KBIT(MROW)
      COMMON /COMPRS/ ITYP(MROW),IWID(MROW)
 
      CHARACTER*10 TAG
      CHARACTER*8  CATX,SUBSET
      CHARACTER*3  TYP
      DIMENSION    MESG(5000)
      LOGICAL      FIRST,FLUSH,WRIT1,KMIS
      REAL*8       VAL
 
      DATA FIRST/.TRUE./

      SAVE FIRST,NROW,NCOL,LUNC,KBYT,WRIT1,FLUSH
...
C  IF THIS IS A FIRST CALL THEN RESET THE BUFFER AND PROCEED
C  ---------------------------------------------------------
...
1     IF(FIRST) THEN
      ...
      IF(LUN.NE.LUNC) CALL BORT('WRCMPS - CALLED WITH CONFLICTED UNIT')
 
C  IF THIS IS A FLUSH CALL - THEN CLEAR OUT THE BUFFER
C  ---------------------------------------------------
...
20    LDATA = 32
      IF(NCOL.LE.0) CALL BORT('WRCMPS - NCOL LE 0!')
      DO I=1,NROW
      IMISS = 2**IWID(I)-1
      IF(ITYP(I).EQ.1 .OR. ITYP(I).EQ.2) THEN
         IF(ICOL.EQ.1) THEN
         ...
         ENDIF
         ENDDO
         KMIS(I) = KMIS(I).AND.KMIN(I).LT.IMISS
         RANGE = MAX(1,KMAX(I)-KMIN(I)+1)
         IF(ITYP(I).EQ.1.AND.RANGE.GT.1) THEN
            IF(KMIS(I)) CALL BORT('WRCMPS - MISSING DRF')
            WRIT1 = .TRUE.
            NCOL = NCOL-1
            ICOL = 1
            GOTO 20
         ELSEIF(ITYP(I).EQ.2.AND.(RANGE.GT.1..OR.KMIS(I))) THEN
...
900   PRINT*,'WRCMPS - MROW(',MROW,')<(',NVAL(LUN),') FOR ',SUBSET
      CALL BORT('WRCMPS - MROW TOO SMALL')
901   CALL BORT('WRCMPS - BIT COUNT CHECK')
C.KRK PRINT*,'MESG LEN FROM SEC0=',LBYTE,' LEN FROM COUNT=',NBYT
902   PRINT*,'MESG LEN FROM SEC0=????????? LEN FROM COUNT=',NBYT
      CALL BORT('WRCMPS - MESG LENGTHS DO NOT AGREE')
      END
      SUBROUTINE WRDLEN
...
C  ----------------------------------------------
      ...
      RETURN
900   CALL BORT('WRITCP - FILE IS CLOSED                     ')
901   CALL BORT('WRITCP - FILE IS OPEN FOR INPUT             ')
902   CALL BORT('WRITCP - NO MESSAGE OPEN                    ')
      END
...
C  ----------------------------------
      ...
      CALL PARSEQ(STR,TGS,MAXTG,NTG)
      IF(NTG.GT.1) CALL BORT('WRITLC - MORE THAN ONE TAG')
      CTAG = TGS(1)
 
C  CHECK THE INPUT TAG IS A CHARACTER STRING
C  ----------------------------------------
      ...
      IF(CTAG.EQ.TAG(NOD)) GOTO 1
      ENDDO
      CALL BORT('WRITLC - STRING TAG NOT FOUND')
1     IF(TYP(NOD).NE.'CHR') THEN
         CALL BORT('WRITLC - INPUT TAG NOT CHARACTER ELEMENT')
      ENDIF
 
C  LOCATE THE BEGINNING OF THE DATA IN SECTION 4
C  ---------------------------------------------
...
C  ----------------------------------------------------
      ...
      IF(NSUBS.NE.NSUB(LUN)) CALL BORT('WRITLC - NSUB PROBLEM')
 
C  LOCATE THE STRING ELEMENT TO WRIT
C  ---------------------------------
...
C  ERROR IF CANNOT FIND CTAG BY THIS POINT
C  ---------------------------------------
 
      CALL BORT('WRITLC - UNABLE TO FIND '//CTAG//' IN SUBSET')
 
C  SOME OTHER POSSIBLE ERRORS
C  --------------------------
 
900   CALL BORT('WRITLC - FILE IS CLOSED                     ')
901   CALL BORT('WRITLC - FILE IS OPEN FOR INPUT             ')
902   CALL BORT('WRITLC - NO MESSAGE OPEN                    ')
      END
...
C  -------------------------------------
      ...
      CALL STATUS(LUNIT,LUN,IL,IM)
      IF(IL.LE.0) CALL BORT('WRITST - NO OPEN OUTPUT BUFR FILE!!')

C  UPDATE THE SECTION 0 BYTE COUNT AND INSTALL 7777 TO HUMOR STANDARD
C  ------------------------------------------------------------------

      IBIT = 32
      CALL PKB(MBYT(LUN),24,MBAY(1,LUN),IBIT)
      IBIT = (MBYT(LUN)-4)*8
      CALL PKC('7777',4,MBAY(1,LUN),IBIT)
...
C  TRANSFER STANDARD MESSAGE INTO MESSAGE OUTPUT BUFFER
C  ----------------------------------------------------
