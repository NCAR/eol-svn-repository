...
C  18 Aug 1997 by Darren R. Gallant
C   + increased RSIZE to 90002 from 45002
C   + increased ISIZE to 50014 from 25007
C   increase RSIZE should allow class files almost 10000 lines long!!
C  
C  December 1998 by Darren R. Gallant
C     Commented out all sun specific lines inorder to port code to 
C     SGI machine running IRIX
C  May 21, 1999 by Darren R. Gallant
C  added two fields RDATA(12),RDATA(13) two hold meteorological 
C  variables ranging from -99.9 to 998.9. For NWS soundings these fields
C  will be the Elevation and Azimuth angles used in the wind calculations
C  other possible parameters include Range(km) and Azimuth angle
C  mixing ratio, and virtual temperatures
C  14 Oct 1999
C   + increased RSIZE to 180000 from 90002
C   + increased ISIZE to 90000 from 50014
C
C Description:
...
C  and are used in computing the proper index to IDATA,RDATA
	...
	PARAMETER (ISIZ=90002,RSIZ=180000,YEAR=1992,IMUL=5,RMUL=12)
...
C	WRITE (0,'(A)') 'SCF filename: ',SCFFIL
C	WRITE (0,'(A)') 'EBUFR output filename: ',OUTFIL
C	WRITE (0,'(A)') 'Control filename: ',CTLFIL
C	WRITE (0,'(A)') 'Class descripton filename: ',CLAFIL
C	WRITE (0,'(A)') 'Parameter description filename: ',PRMFIL

C Open Files

C	WRITE (0,*) ' Made it to right before open statements'
...
D	WRITE (0,*) ' Made it to CALL WRTHDR'
...
C set network ID appropriately
	...
	ELSE IF (HDRSTR(36:44) .EQ. 'ETA MOLTS') THEN
	  CDATA(1:10) = 'ETA MOLTS '
        ELSE IF (HDRSTR(36:43) .EQ. 'ARM-CART') THEN
	  CDATA(1:10) = 'Sounding  '
        ELSE IF (HDRSTR(36:46) .EQ. 'Atmospheric') THEN
	  CDATA(1:10) = 'Sounding  '
        ELSE IF (HDRSTR(36:40) .EQ. '10sec') THEN
	  CDATA(1:10) = 'Sounding  '
        ELSE IF (HDRSTR(36:39) .EQ. 'GEM ') THEN
	  CDATA(1:10) = 'GEM MOLTS  '
        ELSE IF(INDEX(HDRSTR,'Profile')) THEN
	  CDATA(1:10) = 'Profile    '
	ELSE
	  CDATA(1:10) = 'Sounding  '
	END IF
	...
	READ (SCFUNI,'(A130)') HDRSTR
	READ (SCFUNI,'(A130)') HDRSTR
D        write(0,*)'made it past first header line'
C Set Site ID
	CDATA(11:45) = '          '
&	 // '                         '
...
C        write(0,*)'made it past latitude and logitude line'
	READ (SCFUNI,'(A130)') HDRSTR
	J = 0
	K = 130
	...
	IF (K .LE. 0) THEN
	  ...
	  TMPSTR(1:2) = '0 '
	END IF
D        write(0,*)'made it past latitude and logitude line'
...
D        write(0,*)'made it to section finding position'
	K=K+2
	J=K
	DO WHILE (HDRSTR(J:J) .NE. ',')
D	  write(0,*)'made it to while loop looking for numbers'
	  J=J+1
	END DO
	J=J-1
	IF (INDEX(HDRSTR(K:J),'.') .EQ. 0) THEN
	  ...
	  LATLON(2) = REAL(ITMP)
D          write(0,*)'made it beyond latlon(2) = real(itmp)'
	ELSE
	  READ (HDRSTR(K:J),'(F)') LATLON(2)
D          write(0,*)'made it beyond to else loop'
	END IF
	K=J+2
	J=K
C        write(0,*) hdrstr," ",K," ",J," ",LATLON(2)
	DO WHILE (HDRSTR(J:J) .NE. ',')
D          write(0,*)'made it to second while loop'
	  J=J+1
	END DO
	J=J-1
	IF (INDEX(HDRSTR(K:J),'.') .EQ. 0) THEN
	  READ (HDRSTR(K:J),'(I)') ITMP
	  LATLON(1) = REAL(ITMP)
D         write(0,*)'made it beyond latlon(1) = real(itmp)'
	ELSE
	  READ (HDRSTR(K:J),'(F)') LATLON(1)
D          write(0,*)'made it beyond to 2nd else loop'
	END IF
	K=J+2
 	J=K
C        write(0,*) hdrstr," ",K," ",J," ",LATLON(1)
	DO WHILE (HDRSTR(J:J) .NE. ',' .AND. J .LT. LEN(HDRSTR))
D          write(0,*)'made it to third while loop'
	  J=J+1
	END DO
	J=J-1
	IF (INDEX(HDRSTR(K:J),'.') .EQ. 0) THEN
	  READ (HDRSTR(K:J),'(I)') ITMP
	  RDATA(1) = REAL(ITMP)
D          write(0,*)'made it beyond rdata(1) = real(itmp)'
	ELSE
	  READ (HDRSTR(K:J),'(F)') RDATA(1)
D          write(0,*)'made it beyond to 3rd else loop'
	END IF
...
C            write(0,*)'made it beyond to if'
...
C Error reading nominal time so use Launch time
     ...
     .     ' stn/hr: ',CDATA(11:13),IDATA(4)
          ...
          TIME(6) = IDATA(6)
        END IF
D        write(0,*)'made it beyond nominal time section'
...
C Elevation Angle(NWS only) (deg deg) or Mixing Ratio(kg/kg) or Virtual Temp(K)
	IF (SDATA(13) .EQ. S3MISS) THEN
	  RDATA(CTR*RMUL+12) = RMISS
	ELSE
	  RDATA(CTR*RMUL+12) = SDATA(13)
	END IF

C Azimuth Angle(NWS only) (deg deg)
	IF (SDATA(14) .EQ. S3MISS) THEN
	  RDATA(CTR*RMUL+13) = RMISS
	ELSE
	  RDATA(CTR*RMUL+13) = SDATA(14)
	END IF
