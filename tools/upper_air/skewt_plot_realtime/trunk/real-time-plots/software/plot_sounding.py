#!/opt/local/anaconda3/envs/skewt-env/bin/python

#get_ipython().magic(u'matplotlib inline')

##Module--------------------------------------------------------------------------------
## <p>The plot_sounding.py script takes the SHARPpy formatted sounding with one
## sounding per file, and creates a skew-t plot for the field catalog.
## The argument is:
## <ol>
##   <li> Input file: The SHARPpy sounding file to plot.
##
## The following file is created:
##   <li> Output file: The skew-t plot is created and named appropriately
...
#
# For a new project, change the PLOTDIR variable below.
# May need to update the plotfile_name variable.
#
# 
...
def decode(filename):
    ...
    for k in list(profs._profs.keys()):
        ...
        for i in range(len(all_prof)):
            prof = all_prof[i]
            new_prof = profile.create_profile(pres=prof.pres, hght=prof.hght, tmpc=prof.tmpc, dwpc=prof.dwpc, wspd=prof.wspd,                                               wdir=prof.wdir, strictQC=False, profile='convective', date=dates[i])
            return new_prof, dates[i], stn_id 

PLOTDIR = '/net/work/Projects/test_GTS_BUFR_NearRT_Plot_Generation/real-time-plots/data_processing/output_plots/'

# Get filename to be processed from the command line.

parser = argparse.ArgumentParser()
parser.add_argument("filename", help="file to be plotted")
...
title = time.strftime('%Y/%m/%d %H:%M') + ' UTC  ' + location.partition('/')[0]
plotfile_name = 'upperair.Research_SkewT.' + time.strftime('%Y%m%d%H%M.') + location.partition(',')[0] 
...
ax.set_xlim(-50,50)
# Set x axis labels, so plot is not squished when values between -100 and -60 are shown 
# This issue happened when upgrading from python 2 to python 3
...
# Plot every 200 wind barb
# For high res data, plotting every wind barb makes it very hard to read.
skew.plot_wind_barbs(ax2, prof.pres[::200], prof.u[::200], prof.v[::200])
...
for key in keys:
    ...
    if counter < 7:
        ...
        counter = 0
        ax3.text(x, 1, string, verticalalignment='top', transform=ax3.transAxes, fontsize=6)
        string = ''
        x += 0.3
ax3.text(x, 1, string, verticalalignment='top', transform=ax3.transAxes, fontsize=6)
ax3.set_axis_off()
