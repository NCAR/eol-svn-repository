...
##Module-----------------------------------------------------------------------
## <p>The Real-time-plots.pl script is a cron script that does the following
## tasks:</p>
## <ol>
...
##   <li>Run the python script (call_plot_sounding.py) which sets up the python environment and
##          then calls python script  plot_sounding.py to create skew-t plots from the 
##          .cls.sharppy format.</li> 
...
## </ol>
##
## This program ran for OTREC 2019 and HIGHWAY 2019.
##
## @author Janet Scannell 
## @version 1.0 July 2019
##
## Add more error checking.  Remove name of project from name of program.
##
## @author Janet Scannell
## @version 1.1 February 2020
##
## This program ran for SWEX in 2022.
##
## @author Janet Scannell 
## @version 1.2 April 2022
## 
## Change the software to run an additional python script to set up the python environment
...
#
# For a new project, change the ftp_dir, proj_name and catalog_dir variables.
# Also, change the lines that have this comment above them: CHANGE HERE
# May need to be concerned about incoming bfr file names.  For the OTREC project,
# Greg renamed the Graw GTS-BUFR files, since his script checks more often than
# this script.
# For the ACCLIP project, the bufr files were scp'ed over from zephyr.
#

my $proj_name = "test_GTS_BUFR_NearRT_Plot_Generation";
#my $ftp_dir = "/net/ftp/pub/data/incoming/swex/radiosonde";

# For testing purposes
my $ftp_dir = "/net/work/Projects/$proj_name/real-time-plots/data_processing/test-incoming-ftp";

#my $ftp_dir2 = "/net/work/Projects/$proj_name/data_ingest/upperair/radiosonde/GTS/HKLO/BUFR";
#my $catalog_dir = "/net/iftp2/pub/incoming/catalog/swex";

# For testing purposes
my $catalog_dir = "/net/iftp2/pub/incoming/catalog/operations";

my $project_dir = "/net/work/Projects/$proj_name/real-time-plots";
#
# Used for ACCLIP project which was never run
#
#my $ingest_dir = "/export/ldm/data/bufr_sndg";
#my $ftp_dir = "$project_dir/bufr_files";
...
my $to_skewt_pgm = "$software_dir/call_plot_sounding.py";
my $datafiles_listing_short = "raw-data-cross-reference.txt";

# CHANGE HERE - May need to update the users monitoring the project.
my @monitors = ('anstett@ucar.edu'); # For testing
...
my $log_file = "$log_dir/real-time-log-$current_time.log";
my $err_file = "$log_dir/real-time-err-$current_time.err";
my $ftp_listing = "$log_dir/real-time-ftp-listing-$current_time.txt";
...
# CHANGE HERE - ACCLIP used scp to get a copy of the bufr files from zephyr LDM.
#system("scp -p anstett\@zephyr:$ingest_dir/BABJ\* $ftp_dir/. >> $err_file");
#print LOGFILE "scp -p anstett\@zephyr:$ingest_dir/BABJ\* $ftp_dir/. >> $err_file\n";

# Save a long listing of the ftp directory to debug problems that happen.
# CHANGE HERE - HIGHWAY used bufr files from two different directories.
system("ls -l $ftp_dir >>$ftp_listing\n");
...
# The following 4 lines of comments apply to the HIGHWAY project.
...
#
# CHANGE HERE - Need to change find command to correct bfr file names.

my $age_of_last_run_time = -M $last_run_time;
my @bufr_files;
find(sub {
   push @bufr_files, $File::Find::name if (-f $File::Find::name and /\.bfr$/ and (-M $_) <= $age_of_last_run_time);
   }, $ftp_dir);
...
print LOGFILE "Copied *.bfr files from $ftp_dir that are newer than $last_run_time file.\n";

# Zero out array before find next station.  The HIGHWAY project used soundings from two different directories.
# CHANGE HERE - Most projects only use one ftp directory, so comment out this (second) find command if necessary.
#@bufr_files = ();
#find(sub {
...
# CHANGE HERE - May need to change the extension for the bfr file name.
my @raw_files = grep(/^.*\.bfr$/,readdir(PDIR));
closedir(PDIR);
if ($#raw_files == -1) {
...
# Append to the file that lists the snapshot file and which raw bufr files were processed.
# This assumes that the file already exists as a zero length file.  The file is in svn, so 
# it should be created in the Projects directory when the software is checked out.
...
system("$to_ascii_pgm $raw_gts_bufr_dir $output_ascii_dir $output_preproc_dir >& $log_dir/preproc_$current_time.log");
print LOGFILE "$to_ascii_pgm $raw_gts_bufr_dir $output_ascii_dir $output_preproc_dir >& $log_dir/preproc_$current_time.log\n";
print LOGFILE "Ran $to_ascii_pgm.\n";
system("$to_esc_pgm $proj_name $output_preproc_dir $output_esc_dir 0 >& $log_dir/convert_to_esc_$current_time.log");
print LOGFILE "$to_esc_pgm $proj_name $output_preproc_dir $output_esc_dir 0 >& $log_dir/convert_to_esc_$current_time.log\n";
...
foreach my $file (@esc_files) {
   my $line_count = `wc -l < $output_esc_dir/$file`;
   if (-z "$output_esc_dir/$file") {
      print LOGFILE "Zero length cls file: $output_esc_dir/$file\n";
   } elsif ($line_count <= 16) {
# Bad cls file if the number of lines in the file is <= 16 (15 header lines and 1 data line)
      print LOGFILE "Less than 17 lines in cls file (Not processed): $output_esc_dir/$file\n";
   } else {
      print LOGFILE "before to_sharpy_pgm command\n";
      ...
      system("$to_skewt_pgm $output_esc_dir/$file.sharppy $log_dir/runSkewTplot_$current_time.log $log_dir/runSkewTplot_$current_time.err");
      print LOGFILE "$to_skewt_pgm $output_esc_dir/$file.sharppy $log_dir/runSkewTplot_$current_time.log $log_dir/runSkewTplot_$current_time.err\n";
...
{
   my ($body) = @_;
   MAIL::send_mail("$proj_name Real-time Plots Output - ERROR", $0 . "\n\n" . $body, @monitors);
}
