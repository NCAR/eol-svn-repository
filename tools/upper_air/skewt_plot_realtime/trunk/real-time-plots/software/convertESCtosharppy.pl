...
## read. This was used for the OTREC and HIGHWAY projects to plot skew t's for the field
## catalog.</p>
## The argument is:
## <ol>
...
#
# For a new project, change the proj_name variable.
# May need to update the monitors variable.
#
...
# Create log file
my $log_file = "$log_dir/convertESCtosharppy-$current_time.log";
...
if ($#ARGV != 0) {
   print LOGFILE "Usage: convertESCtosharppy.pl input-ESC-file\n";
   close(LOGFILE);
   exit(1);
}

my $infile = $ARGV[0];
print LOGFILE "Input file to convert: $infile\n";
my $outfile = $infile . ".sharppy";
print LOGFILE "Output file to create: $outfile\n";
...
if (-z $infile) {
   print LOGFILE "Zero length cls file: $infile\n";
   close(LOGFILE);
   exit(1);
}
...
if (! open(FILE1,"<$infile")) {
   sendMail(sprintf("ERROR: Could not open $infile for reading.\n"));
   print LOGFILE "ERROR: Could not open $infile for reading.\n";
   close(LOGFILE);
   exit(1);
}
my @infile = <FILE1>;
...
if (! open(FILE2,">$outfile")) {
   sendMail(sprintf("ERROR: Could not open $outfile for writing.\n"));
   print LOGFILE "ERROR: Could not open $outfile for writing.\n";
   close(LOGFILE);
   exit(1);
}
...
print LOGFILE "Location: $location   Release Date: $releasedate\n";
...
sub sendMail {
   my ($body) = @_;
   MAIL::send_mail("$proj_name - Convert ESC file - ERROR", $0 . "\n\n" . $body, @monitors);
}
