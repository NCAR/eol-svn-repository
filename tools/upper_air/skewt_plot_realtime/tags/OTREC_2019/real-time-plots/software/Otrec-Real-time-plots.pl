...
##Module-----------------------------------------------------------------------
## <p>The Otrec-Real-time-plots.pl script is a cron script that does the following
## tasks:</p>
## <ol>
...
my $ftp_dir = "/net/ftp/pub/data/incoming/otrec/radiosonde";
my $project_dir = "/net/work/Projects/OTREC/real-time-plots";
...
my $catalog_dir = "/net/iftp2/pub/incoming/catalog/otrec";
...
my $log_file = "$log_dir/otrec-rt-log-" . $current_time . ".log";
my $err_file = "$log_dir/otrec-rt-err-" . $current_time . ".err";
my $ftp_listing = "$log_dir/otrec-rt-ftp-listing-" . $current_time . ".txt";
...
# Greg will rename the Graw GTS-BUFR files, since his script checks things more often than this script.
...
closedir(PDIR);
if ($#raw_files == -1) {
   print LOGFILE "No *.bfr files found to process. Exiting...\n";
...
# Process all *.bfr files, including sorting the files.
system("$to_ascii_pgm $raw_gts_bufr_dir $output_ascii_dir $output_preproc_dir >& $log_dir/preproc_OTREC_$current_time.log");
print LOGFILE "$to_ascii_pgm $raw_gts_bufr_dir $output_ascii_dir $output_preproc_dir >& $log_dir/preproc_OTREC_$current_time.log\n";
print LOGFILE "Ran $to_ascii_pgm.\n";
system("$to_esc_pgm OTREC $output_preproc_dir $output_esc_dir 0 >& $log_dir/runOTREC_rt_$current_time.log");
print LOGFILE "$to_esc_pgm OTREC $output_preproc_dir $output_esc_dir 0 >& $log_dir/runOTREC_rt_$current_time.log\n";
...
{
   my ($body) = @_;
   MAIL::send_mail("OTREC Real-time Plots Output - ERROR", $0 . "\n\n" . $body, @monitors);
}
