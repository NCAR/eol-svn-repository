...
##Module------------------------------------------------------------------
# <p>The create_5mb_ebufr_files.pl script is a script to generate EBUFR'ed
# day files for a 5 mb Level Sounding Composite.  It assumes that all of the 
# files in the INPUT_DIR are to be included in the composite and that they are 
# either .05mb or .05mb.gz files.</p>
...
# named ID_YYYYMMDDHHMM.05mb).  This will put all of the soundings for a
...
sub main {
	...
	foreach my $file (grep(/\.05mb/,readdir($DIR))) {
	...
	my $out_file = sprintf("%s/%s.05mb",$OUTPUT_DIR,$date);
...
sub create_ebufr_file {
    ...
    move($file,sprintf("%s_5mb_%s",$PROJECT,$file));
