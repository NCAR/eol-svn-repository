...
# reading the nominal release time in the data files.  All files that are copied and compressed
# in gzip, will be gunzipped.</p>
...
sub main {
	...
	foreach my $file (grep(/\.cls(\.gz)?$/,readdir($DATA))) {
		...
		# Gunzip the file if it is not already
		gunzip(sprintf("%s/%s/%s",$DATA_DIR,$network,$file)) if ($file =~ /\.gz$/);
	    }
	}
	closedir($DATA);
    }
...
##---------------------------------------------------------------------------------
# @signature void gunzip(String file)
# <p>Uncompress the specified file using gunzip.</p>
#
# @input $file The file to be uncompressed.
# @warning This function will die if the file cannot be gunzipped.
##---------------------------------------------------------------------------------
sub gunzip {
    my ($file) = @_;

    system("gunzip $file") == 0 or die("Can't gunzip $file\n");
}
