/** @ Monica Jacobs- July 2011
 ...
 SoundingGraph.java- This java program starts a GUI which allows a user to select multiple datasets and plot them 
on five plots. X11 forwarding must be enabled in order for this software to work. gnuTemplate.gnu, DataFilter.java 
and quit.sh must also be included in the working directory in order for this software to run properly. This software requires java, gnuplot and /bin/bash.
...
This software is run assuming that the data files selected are sounding files and their datatables are organized according to the NCAR EOL Sounding Composite (ESC) format. Additionally, the files opened SHOULD NOT BE IN THE SAME DIRECTORY AS THIS FILE. This program 
also operates under the assumption that an opened file is not empty or incomplete. In these situations, the plots 
...
/**
This method sets the items in zoomBox to those in list
*/
...
yMax is the maximum y-value in the previously processed data files
return the new yMax value
*/
	private double  writeData(File f,int index,int y,double yMax){
...
/**
This method returns the number of files in File[] f before the first null
*/
...
yvar is the y variable being plotted
maxY is the maximum y-value in the plot
*/
	...
	if(yvar==2){
		writer.write("set yrange[0:"+(yPresMax+100)+"]reverse\n");
		}
	else if((yvar==1)&&(yAxisSelection==2)){
		writer.write("set yrange[0:"+(yTimeMax+100)+"]reverse\n");
		}
...
*/
	...
	//	zoomFrame.setSize(java.awt.Toolkit.getDefaultToolkit().getScreenSize().width,java.awt.Toolkit.getDefaultToolkit().getScreenSize().height);
	...
	/**
	Returns the index of the array at which the input String is
	...
	*/
	...
	/** Returns the boolean indicating if the file is in the array
	f is the name of the file being searched for
	fArr is the array of files
	*/
		...
		plot.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				for (int k=0; k<getLengthFiles(files); k++){
					max=writeData(files[k],k,yvarNums[yAxisSelection],max);
				}
	...
	private void quitActions(){
                               ...
                               //removes all generated files
                                for (int i=0; i<getLengthFiles(rewriteFiles); i++){
                                       rewriteFiles[i].delete();
                                }
