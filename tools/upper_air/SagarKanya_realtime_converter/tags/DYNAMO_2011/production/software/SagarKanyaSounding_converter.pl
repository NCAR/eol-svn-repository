...
##Module------------------------------------------------------------------------
# <p>The SagarKanyaSounding_converter.pl script is used for converting high
...
#</p> 
#
# @author Linda Cully 2011-10-06
# @version DYNAMO_2011
#    - Data source has send files with junk, binary, and partial emails
#      embedded. This resulted in odd errors occuring and being written
#      to the errlog. The size of the errlog is now checked along with
#      whether or not one of the expected errors occurred. This should
#      help prevent junk data being sent to the GTS.
#
# @author Linda Cully 2011-10-03
# @version DYNAMO_2011
#    - Data source has send files with dates in the file names that
#      are future (UTC time). Add code so that if this occurs, don't
#      process the file. Issue an error and move file and err log to archive.
#    - Add "- ERROR" to the end of the email subject message if any type
#      of error occurs. This will make it easier for scientific staff to
#      know which dates/files to examine. 
#
# @author Linda Cully 2011-09-16
# @version DYNAMO_2011 created from the VORTEX2_2010 West Texas Mesonet converter.
...
#      Removed s/w that creates station list. Used the West Tx Mesonet
#      code created for VORTEX2 since is processed Vaisala Digicora 3 
#      formatted data. Added requested checks to verify if input sounding
#      converted "properly". Added code to then archive input and output
#      files in archive area. 
#
...
#         /net/ingest/dynamo/upper_air/RV_Sagar_Kanya/realtime/highres directory. 
#  - That the following directories exist: ../output, ../archive.
#
# Note the following:
#  - Code was added from the Ron Brown Converter to derive the ascension rate. 
#      This was from the VORTEX2 West Texas Mesonet converter.
#  - All samples received contained a blank line at the end of the data. These
#      blank lines are ignored.
#
# BEWARE:  Search for "HARDCODED" to find project-specific items that may
#          need to updated.
#
# Cron info: This task is run as a cron script. As of 22 Sept 2011, SL will add
#     to his merlot/local crontab to run this script frequently (every 10 mins
...
#
#     */10 * * * * /net/work/Projects/dynamo/upper_air/RV_Sagar_Kanya/production/
#     software/SagarKanyaSounding_converter.pl > /dev/null 2>&1
#
...
#
##Module------------------------------------------------------------------------
package SagarKanyaSounding_converter;
use strict;
...
if ($debug2) {printf "\nSagarKanyaSounding_converter.pl began on ";print scalar localtime;printf "\n";}
&main();
if ($debug2) {printf "\nSagarKanyaSounding_converter.pl ended on ";print scalar localtime;printf "\n";}

##------------------------------------------------------------------------------
# @signature void main()
# <p>Process the RV Sagar Kanya radiosonde data by converting it from 
# the native ASCII format into the ESC format.</p>
##------------------------------------------------------------------------------
sub main {
    my $converter = SagarKanyaSounding_converter->new();
    $converter->convert();
} # main

##------------------------------------------------------------------------------
# @signature SagarKanyaSounding_converter new()
# <p>Create a new instance of a SagarKanyaSounding_converter.</p>
#
# @output $self A new SagarKanyaSounding_converter object.
##------------------------------------------------------------------------------
sub new {
    my $invocant = shift;
    my $self = {};
    ...
    # HARDCODED
    $self->{"PROJECT"} = "DYNAMO";
    $self->{"NETWORK"} = "SagarKanya";
   
    #----------------------------------------
    # Input/ingest directory. Data to convert
    # WAS:  $self->{"RAW_DIR"} = "../raw_data"; # test area
    #----------------------------------------
    $rawDirName = "/net/ingest/dynamo/upper_air/RV_Sagar_Kanya/realtime/highres";
###   $rawDirName = "/net/work/Projects/dynamo/upper_air/RV_Sagar_Kanya/processing/raw_data";      # test area
    ...
    # Output directories. 
    #---------------------
    $outputDirName = "/net/work/Projects/dynamo/upper_air/RV_Sagar_Kanya/production/output";     # working dir
    $archiveDirName = "/net/work/Projects/dynamo/upper_air/RV_Sagar_Kanya/production/archive";   # archive copy of all CLS and log files

    $gtsDirName =  "/h/eol/iss/project/dynamo/data/vtjr/cls";  # Final GTS area to place converted *.cls file
####  $gtsDirName =  "/net/work/Projects/dynamo/upper_air/RV_Sagar_Kanya/processing/gts";          # test area
...
sub parseHeader {
    ...
    # Expect file name structure: VTJR_yyyymmddhhmm.tsv .
    # ----------------------------------------------------------
...
sub parseRawFile {
    ...
    if (defined($header)) 
       ...
       foreach my $line (@lines)  
          ...
          #-------------------------------------------------------------
          # Check for blank line particularly at end of file in SK data.
          ...
          #-------------------------------------------------------
          # Following calc of asc rate not required for Sagar Kanya,
   ...
   if ($allData == 0)
    ...
    #------------------------------------------------------
    # Get current UTC time and compare to date/time in
    # file name. If date/time in file name is in the
    # future, issue an error. File Name Form: 201012081200
    # that is YYYYMMDDHHMM.
    #------------------------------------------------------
    my $sec; my $min; my $hour; my $mday;my $mon;my $year;my $wday;my $yday;my $isdst;
    my $currentUTC_time; my $fileName_time;

    ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = gmtime(); #get current UTC time
    $mon = $mon + 1;                    # gmtime Fn returns 0-11 value
    $year = $year + 1900;               # gmtime Fn returns number of years since 1900

    $currentUTC_time = sprintf ("%4d%02d%02d%02d%02d", $year,$mon,$mday,$hour,$min);

    $fileName_time = substr($file,5,12);

    if ($debug2) { print "fileName_time:: $fileName_time; currentUTC_time:: $currentUTC_time\n"; }

    if ($fileName_time >  $currentUTC_time)
       {
       if ($debug2) { print "ERROR on filename:: $fileName_time is greater than $currentUTC_time\n"; }
       $errText =  $errText."ERROR:: File Name Date-Time is in the FUTURE. Current UTC Time is $currentUTC_time\n";
       print  $ERROUT "ERROR:: File Name Date-Time is in the FUTURE. Current UTC Time is $currentUTC_time\n";
       }
...
# ESC formatted file. If the date in the file name is a UTC time in the future,
# don't process the file. Issue an error and move file and err log to archive.</p>
##------------------------------------------------------------------------------
sub readDataFiles {
    my ($self) = @_;
    ...
    #------------------------------------------------------------
    # Input file names must be of the form: VTJR_yyyymmddhhmm.tsv
    # where VTJR is the call sign for the R/V Sagar Kanya, yyyy = year,
...
##     my $addressto = sprintf "%s", "cully\@ucar.edu";   #HARDCODED EMAIL
       ...
       if ($errlength > 0 || $number_lines_in_ERROR_file > 0)
          { $message = "R/V Sagar Kanya Sounding $input_file Processed - ERROR"; }
       else
          { $message = "R/V Sagar Kanya Sounding $input_file Processed"; }
