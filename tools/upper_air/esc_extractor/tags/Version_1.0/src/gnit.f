C
C $Id: gnit.f,v 1.1 1992/12/07 22:29:07 john Exp $
C
...
C Local vars,initialization
	INTEGER GDHI,GDLO,ESHI,ESLO,DUHI,DULO,BAHI,BALO,LOGUNI
	GDHI = HINDX
	ESHI = HINDX
	DUHI = HINDX
        BAHI = HINDX
	...
	DULO = LONDX
        BALO = LONDX
        LOGUNI = 13
C This is the "gnit.f" version --- the one that ignores time values.
C  Algorithm: get good values, if they're not present, then get est, dub, bad
C
C The original algorithm when we used time values:
C Algorithm:
...
C  (G,D)<=B and (E,D)<=B in 5; etc.
        ...
        IF (GDHI .LE. CTR) THEN
         IF ((LOG(SDATA(2,GDLO)) - LOG(SDATA(2,GDHI))) .LE. ARANGE) THEN
            HINDX = GDHI
            LONDX = GDLO
        ...
        IF (ESHI .LE. CTR) THEN
         IF ((LOG(SDATA(2,ESLO)) - LOG(SDATA(2,ESHI))) .LE. ARANGE) THEN
        ...
        IF (GDHI .LE. CTR) THEN
         IF ((LOG(SDATA(2,GDLO)) - LOG(SDATA(2,GDHI))) .LE. BRANGE) THEN
...
2001    CONTINUE
        IF (ESHI .LE. CTR) THEN
         IF ((LOG(SDATA(2,ESLO)) - LOG(SDATA(2,ESHI))) .LE. BRANGE) THEN
...
2002    CONTINUE
        ...
        IF (DUHI .LE. CTR) THEN
         IF ((LOG(SDATA(2,DULO)) - LOG(SDATA(2,DUHI))) .LE. BRANGE) THEN
 
            HINDX = DUHI
            LONDX = DULO
            QCFLG = QCWRST(SDATA(FIELD,DUHI),SDATA(FIELD,DULO))
            IF ((LOG(SDATA(2,DULO))-LOG(SDATA(2,DUHI))).GT.ARANGE) THEN
        ...
        HINDX = BAHI
        LONDX = BALO   
        if (sdata(2,HINDX) .eq. 0.0) then
              go to 9999
           endif
        IF ((LOG(SDATA(2,LONDX))-LOG(SDATA(2,HINDX))).LE.BRANGE) THEN
           QCFLG = QCDEG(QCFLG)
        END IF
        
        IF ((LOG(SDATA(2,LONDX))-LOG(SDATA(2,HINDX))).GT.BRANGE) THEN
           QCFLG = QCDEG(QCFLG)
        END IF
