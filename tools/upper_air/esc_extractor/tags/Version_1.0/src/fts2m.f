...
C PRSINT - Interval on which to interpolate
	REAL MINPRS,PRSINT
c	PARAMETER (MINPRS=100.0,PRSINT=10.0)
	PARAMETER (MINPRS=100.0,PRSINT=5.0)
...
50	CONTINUE
	...
	WRITE (LOGUNI,*) 'Files open.'

C A 'D' in the 1st column means only compile this statement
C  when the -xld flag is given to f77. For debugging purposes.
	WRITE (LOGUNI,'(A20)') 'SCF filename: ',SCFFIL
	WRITE (LOGUNI,'(A20)') 'Output filename: ',OUTFIL
	WRITE (LOGUNI,'(A20)') 'Log filename: ',LOGFIL
...
C Number of records seen
	CTR = J - 1
	WRITE (LOGUNI,*) 'CTR = ',CTR

C Write first record as is
C	WRITE (OUTUNI,800) SDATA(1:21,1)
	CALL WRTSCF (OUTUNI,SDATA,1,IOS)
	OUTCT = OUTCT + 1
	WRITE (LOGUNI,*) 'First record output:'
	WRITE (LOGUNI,'(A,F6.1)') ' Pressure ',SDATA(2,1)
	WRITE (LOGUNI,'(A,F6.1)') ' time ',SDATA(1,1)
...
C23456789012345678901234567890123456789012345678901234567890123456789012
        ...
        if((sdata(6,iwhi) .ne. 999.0) .and. (sdata(6,iwhi) .ne. 9999.0)
     &  .and. (sdata(6,iwlo).ne.999.0).and.(sdata(6,iwlo) .ne. 9999.0))
     &  then
...
C	WRITE (OUTUNI,800) IDATA
	OUTCT = OUTCT + 1
	WRITE (LOGUNI,'(A,F6.1,A)') 'Level ',RMB,' mb interpolated.'
	WRITE (LOGUNI,'(A,I,A)') 'Record ',OUTCT,' output.'
