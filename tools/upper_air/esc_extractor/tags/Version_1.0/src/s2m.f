...
D       WRITE (LOGUNI,'(A,F6.1)') ' time difference ',DIFF
	TIME = SDATA(1,IPLO) + DIFF * LOG(RMB/SDATA(2,IPLO)) /
&	       LOG(SDATA(2,IPHI)/SDATA(2,IPLO))
	IDATA(1) = TIME
D       WRITE (LOGUNI,'(A,F6.1)') ' calc time using LOG diff ',IDATA(1)
D	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',IPHI,', time ',SDATA(1,IPHI)
D	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',IPLO,', time ',SDATA(1,IPLO)
...
D       WRITE (LOGUNI,'(A,F6.1)') ' Ascension rate ',IDATA(10)
D	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',IPHI,', alt ',SDATA(15,IPHI)
D	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',IPLO,', alt ',SDATA(15,IPLO)          
...
C	IF (TIME .GE. SDATA(1,ITHI)) THEN
...
C	IF (TIME .GE. SDATA(1,IRHI)) THEN
...
C	IF (TIME .GE. SDATA(1,IWHI)) THEN
