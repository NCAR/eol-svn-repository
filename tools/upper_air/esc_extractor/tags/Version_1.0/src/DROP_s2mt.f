C
C $Id: s2mt.f,v 1.4 1992/12/10 18:44:26 john Exp $
C
	PROGRAM S2MT
C
C S2MT CONVERTS A CLASS FILE TO INTERPOLATED y-MB INTERVALS, IGNORING TIME.
C
C MODIFIED FROM S2M
C
C Synopsis: s2mt [<class_data_file>]
C
...
C Modification history:
C 19 Feb 2003 by Darren R. Gallant
C  adapted for Dropsonde soundings which have undergone auto qc but not visual
...
C  changed logic preventing interpolation of missing latlon 
C 02 Feb 1996 by Darren R. Gallant
C  added checks for less than -99.9 dew points and for greater than 1000.0
C  wind velocities
C 29 Jan 1996 by Scot M. Loehrer and Darren Gallant
C  ensured that if first data point in file has missing pressure, it
C  is not used in interpolation (problem with GTS data).
C  Added section to gnit.f to use logarithmic pressure changes rather
C  than time as basis for degrading QC flags (used for missing time
C  data, primarily GTS).
C 04 Dec 1992 by John J. Allison
C  change s2m into s2mt
...
C
C  S2MT is used for SCF files with missing times. It ignores any time
C field on the input and outputs missing for the time field.
C Instead of using the times to interpolate (like S2M does because
C it bases the QC flags on time), S2MT uses the log of the pressure to
C interpolate.
C
C Input:
...
C   (want 2 values <= A apart before <= B apart before > B apart)
	...
	PARAMETER (PAR=0.07,PBR=0.14)
	PARAMETER (TAR=0.035,TBR=0.07)
	PARAMETER (HAR=0.035,HBR=0.07)
	PARAMETER (WAR=0.035,WBR=0.07)
...
C LRMB = LOG(RMB)
	REAL SDATA(21,MAXREC),IDATA(21),RMB,LRMB
...
C Last altitude and time for recalulating ascendsion rate.
        real lasttime, lastalt
C File units,names
...
30	CONTINUE
	...
	IF (J .EQ. 1) THEN
	  WRITE (LOGUNI,'(A,A20)')
&	    's2mt: No data records found in ',SCFFIL
	  GO TO 300
	END IF
...
C Interpolation loop
	...
	WRITE (LOGUNI,'(A,F6.1,A)') 'RMB = ',RMB,', using (Pressure):'
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',IPHI,', time ',SDATA(1,IPHI)
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',IPLO,', time ',SDATA(1,IPLO)
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',IPHI,', press ',SDATA(2,IPHI)
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',IPLO,', press ',SDATA(2,IPLO)
...
C S2MT note : using pressure to interpolate
C  but kept the same order in subtractions, etc.
C  so intermediate values all have wrong signs
C  but it works out because wrong/wrong or wrong*wrong gives right
C  (?)

C below is the original S2M method
C
C If a time value is missing, then skip this level
C	IF ((SDATA(1,IPHI) .GE. 9999.0) .OR.
C&	    (SDATA(1,IPLO) .GE. 9999.0)) THEN
C	  WRITE (0,'(A,F6.1)')
C&	    'TIME VALUE MISSING FOR RMB = ',RMB
C	  WRITE (LOGUNI,'(A,F6.1)')
C&	    'TIME VALUE MISSING FOR RMB = ',RMB
C	  GO TO 250
C	END IF

C Interpolate time
        if((sdata(1,iphi) .ne. 9999.0) .and.
     &     (sdata(1,iplo) .ne. 9999.0)) then
	...
	LRMB = LOG(RMB)
	DIFF = LOG(SDATA(2,IPHI)) - LOG(SDATA(2,IPLO))
...
&	    (SDATA(15,IPLO) .GE. 99999.0)) THEN
	  ...
	  IDATA(21) = 9.0
	ELSE
	  HICO = (LRMB - LOG(SDATA(2,IPLO))) / DIFF
	  LOCO = (LOG(SDATA(2,IPHI)) - LRMB) / DIFF
	  IDATA(15) = SDATA(15,IPHI)*HICO + SDATA(15,IPLO)*LOCO
        endif
C Compute Ascension rate
        if((time .ne. 9999.0) .and.
     &     (lasttime .ne. 9999.0) .and.
     &     (idata(15) .ne. 99999.0) .and.
     &     (lastalt .ne. 99999.0)) then
	   IDATA(10) = (idata(15) - lastalt) / (idata(1)-lasttime)
	   IDATA(21) = 99.0
...
C if RMB is not w/in Temp interp pts, then get some new Temp interp pts
	IF (RMB .LE. SDATA(2,ITHI)) THEN
	  CALL GTNXIN (RMB,17,ITHI,ITLO,QCTMP,TAR,TBR,SDATA,CTR)
C   if Temp interp pts are still bad, then error
	IF ((SDATA(2,ITHI) .GT. RMB).OR.(SDATA(2,ITHI).LT. 0.1)) THEN
	  WRITE (LOGUNI,*) 'Warning: ITHI.pressure > RMB.'
	  WRITE (LOGUNI,*) ' RMB = ',RMB
C force quit by setting RMB = 0.0
	  IDATA(3) = 999.0
	  IDATA(17) = 9.0
	  GO TO 103
	END IF
	WRITE (LOGUNI,'(A,F6.1,A)') 'RMB = ',RMB,', using (Temperature):'
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',ITHI,', time ',SDATA(1,ITHI)
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',ITLO,', time ',SDATA(1,ITLO)
	END IF
C If we can't find a temperature to use, then set temp to missing
        IF(SDATA(2,ITHI).LT. 0.1) THEN
	  IDATA(3) = 999.0
	  IDATA(17) = 9.0
	  GO TO 103
        END IF
C If hi,lo pts are the same, then just use that value
...
C Compute temperature
        if((sdata(3,ithi) .ne. 999.0) .and.
     &     (sdata(3,itlo) .ne. 999.0)) then
 	  DIFF = LOG(SDATA(2,ITHI)) - LOG(SDATA(2,ITLO))
	  HICO = (LRMB - LOG(SDATA(2,ITLO))) / DIFF
	  LOCO = (LOG(SDATA(2,ITHI)) - LRMB) / DIFF
...
C   then get some new Hum. interp. pts.
	IF (RMB .LE. SDATA(2,IRHI)) THEN
	  CALL GTNXIN (RMB,18,IRHI,IRLO,QCHUM,HAR,HBR,SDATA,CTR)
        write(16,*)'sdata =',sdata(2,irhi)
	IF ((SDATA(2,IRHI) .GT. RMB) .OR. (SDATA(2,IRHI) .LT. 0.1)) THEN
	  WRITE (LOGUNI,*) 'Warning: IRHI.pressure > RMB.'
	  WRITE (LOGUNI,*) ' RMB = ',RMB
	  IDATA(5) = 999.0
          IDATA(18) = 9.0
	  GO TO 105
	...
	END IF
	WRITE (LOGUNI,'(A,F6.1,A)') 'RMB = ',RMB,', using (Humidity):'
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',IRHI,', time ',SDATA(1,IRHI)
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',IRLO,', time ',SDATA(1,IRLO)
	END IF
C If we can't find a rel hum to use, then set hum to missing
	  IF(SDATA(2,IRHI).LT. 0.1) THEN
            IDATA(5) = 999.0
            IDATA(18) = 9.0
	    GO TO 105
...
C Interpolate relative humidity
        if((sdata(5,irhi) .ne. 999.0) .and.
     &     (sdata(5,irlo) .ne. 999.0)) then
	DIFF = LOG(SDATA(2,IRHI)) - LOG(SDATA(2,IRLO))
        
	HICO = (LRMB - LOG(SDATA(2,IRLO))) / DIFF
	LOCO = (LOG(SDATA(2,IRHI)) - LRMB) / DIFF
...
C  Interpolation of Wind comp.
	IF (RMB .LE. SDATA(2,IWHI)) THEN
C QC flags for u,v components are always supposed to be the same
	  CALL GTNXIN (RMB,19,IWHI,IWLO,QCWND,WAR,WBR,SDATA,CTR)
	IF ((SDATA(2,IWHI) .GT. RMB).OR.(SDATA(2,IWHI).LT. 0.1)) THEN
	  WRITE (LOGUNI,*) 'Warning: IWHI.pressure < RMB.'
	  WRITE (LOGUNI,*) ' RMB = ',RMB
	...
	END IF
	WRITE (LOGUNI,'(A,F6.1,A)') 'RMB = ',RMB,', using (Wind):'
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' HI = ',IWHI,', time ',SDATA(1,IWHI)
	WRITE (LOGUNI,'(A,I,A,F6.1)') ' LO = ',IWLO,', time ',SDATA(1,IWLO)
	END IF
C If we can't find a wind to use, then set all winds and latlon to missing
        IF(SDATA(2,IWHI).LT. 0.1) THEN
	  IDATA(6) = 9999.0
	  IDATA(7) = 9999.0
	  IDATA(8) = 999.0
	  IDATA(9) = 999.0
...
C  then interpolate latlon
	DIFF = LOG(SDATA(2,IWHI)) - LOG(SDATA(2,IWLO))
	HICO = (LRMB - LOG(SDATA(2,IWLO))) / DIFF
	LOCO = (LOG(SDATA(2,IWHI)) - LRMB) / DIFF
        if((sdata(6,iwhi) .ne. 9999.0) .and. 
     &     (sdata(6,iwlo) .ne. 9999.0) .and.
     &      (sdata(7,iwhi) .ne. 9999.0) .and. 
     &     (sdata(7,iwlo) .ne. 9999.0)) then
     ...
     &       (sdata(11,iwlo) .ne. 9999.0) .and.
     &       (sdata(12,iwhi) .ne. 999.0) .and. 
...
990	WRITE (0,*) 's2mt: Can''t open file.'
	WRITE (LOGUNI,*) 's2mt: Can''t open file.'
	GO TO 999

991	WRITE (0,*) 's2mt: Error reading SCF file.'
	WRITE (LOGUNI,*) 's2mt: Error reading SCF file.'
	GO TO 999
...
&	  SDATA(21,INDEX)
C Adaptation for unchecked Dropsonde soundings
        DO I = 16,20
...
C
	DO I = 16,20
          IF(SDATA(I,INDEX).EQ. 1.0 .OR. SDATA(I,INDEX) .EQ. 2.0) THEN
            SDATA(I,INDEX) = 99.0
          END IF
        END DO
