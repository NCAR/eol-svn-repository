...
/**
 * The ESCExtractor class is the generic class for extracting a pressure or height level frequency
 * from a sounding.  It is designed to be extended by individual frequencies.
 * <br><br>
 * April 2019 <br><br>
 * This class was modified to allow for either 5 mb or 50 m level frequencies in a sounding.
 * Basically, most of the routines were duplicated and the ones that are used for the 50 m level
 * frequencies have Height as the last part of the name of the routine.<br><br>
 *
 * This web page, https://dmg.eol.ucar.edu/software/tools/upper_air/esc_extractor/howto_build.html,
 * has information on how the Extraction Algorithm works.  For the 50 m level frequency, the first
 * record is kept as is in the output file.  The next level calculated for is the next highest height
 * evenly divisible by 50.  For example, if the first record has a height of 0, then the next record 
 * to interpolate for, would be 50 m.  Another example, if the first record is 587 m then the next
 * height to interpolate for, would be 600 m.  The time for this height 
 * is interpolated and records within 10 seconds (unless this value is changed)
 * on either side of this time are passed to the interpolation routine.  The interpolation routine
 * looks for the closest record on either side of the interpolated height that has a "good" flag.
 * If that is not found, then it looks for an "estimated" flag, then "maybe", then "bad", then
 * "missing", then "unchecked".  As long as there is a data value for the variable that it is
 * interpolating for, it will create an interpolated value.  If all the records on one side of 
 * the height value have a missing value for the data value, then a missing value is put in the
 * interpolated record with a missing value for the flag.  The resulting quality control flag is
 * set to the worst of the two flags from the two records that are used in the interpolation.
 * Note that since there is not a flag for the height (or altitude) value, the pressure flag is
 * used for the height flag.<br><br>
 * There will be an interpolated record added for every 
 * 50 m until the maximum height, in the original file, has been reached. 
 *
 * @author Joel Clawson - original author, J. Scannell - April 2019 modifications
 */
public abstract class ESCExtractor {
    ...
    /**
     * Create the interpolated record for the specified height.
     * @param interpolationPressure The height the record is being interpolated for.
     * @param previousInterpolatedRecord The last record that was previously interpolated.
     * @param sounding The sounding being interpolated.
     * @param log The output stream where errors/warning are to be written.
     * @return The interpolated record for the specified height or <code>null</code> if the
     * record could not be interpolated.
     * @throws ExtractionException when there is a major problem that prevents the record from
     * being interpolated.
     */
    private ESCSoundingRecord buildInterpolatedRecordHeight(int interpolationHeight, ESCSoundingRecord previousInterpolatedRecord, ESCSounding sounding, PrintWriter log) throws ExtractionException {		
	// Determine the set of records that can be used in the interpolation.
	List<ESCSoundingRecord> secondRangeRecords = findRecordsWithinSecondRangeHeight(interpolationHeight, previousInterpolatedRecord, sounding, log);
	
	Double height = new Double(interpolationHeight);
               
	// Create the interpolated record.
	ESCSoundingRecord interpolated = new ESCSoundingRecord();
	// Interpolate the height related values.
	interpolateAltitudeValuesHeight(interpolated, height, secondRangeRecords, log);
	
	// If the height could not be set, the record is useless, so return null and don't try
	// to interpolate any other values.
	if (interpolated.getAltitude() == null) { return null; }	
        
	// Interpolate the rest of the record's values.
        interpolatePressureValuesHeight(interpolated, height, secondRangeRecords, log);
	interpolateTemperatureValuesHeight(interpolated, height, secondRangeRecords, log);
	interpolateRelativeHumidityValuesHeight(interpolated, height, secondRangeRecords, log);
	interpolateWindValuesHeight(interpolated, height, secondRangeRecords, log);
     ...
     * @param lower The lower pressure or height record to use in the calculation.
     * @param upper The upper pressure or height record to use in the calculation.
    ...
    /**
     * Determine the first height value that should be interpolated for the sounding.  This
     * makes sure that the first interpolation height is not the same as the first height
     * record since this will already be output as the surface data line.
     * @param sounding The sounding to be interpolated.
     * @return The initial interpolation height for the extracted sounding or <code>-1</code>
     * if the initial height cannot be determined.
     */
     private int calculateInitialInterpolationHeight(ESCSounding sounding) {
	int height = -1;
	for (ESCSoundingRecord record: sounding.getRecords()) {
	    if (record.getAltitude() != null) {
		// Determine the initial interpolation height.
		height = ((int)(record.getAltitude() / getInterpolationInterval())) * getInterpolationInterval();
                // The above calculation will give you an interpolation height, below the first height,
                // so add the interpolation interval to the result.  i.e. if initial height is 1196.6, then the above
                // calculation will be 1150, but want to start interpolating at 1200.
                height += getInterpolationInterval();
		return height;
	    }
	}
	return height;
	...
	absolutePath = absolutePath.replaceFirst(INPUT_DIRECTORY.getAbsolutePath(), OUTPUT_DIRECTORY.getAbsolutePath());
        if (getInterpolationType() == 'P') {
	   absolutePath = absolutePath.replaceFirst(PATTERN, String.format(".%02dmb%s", getInterpolationInterval(), GZIP ? ".gz" : ""));
        } else {
           absolutePath = absolutePath.replaceFirst(PATTERN, String.format(".%02dm%s", getInterpolationInterval(), GZIP ? ".gz" : ""));
        }
	return new File(absolutePath);
    }
    
    /**
     * Create the extracted/interpolated sounding from the specified sounding for pressure.
     * @param sounding The sounding to be interpolated.
	...
	while (interpolationPressure >= getMinimumInterpolationValue()) {
    ...
    /**
     * Create the extracted/interpolated sounding from the specified sounding for height.
     * @param sounding The sounding to be interpolated.
     * @param log The output stream where errors are to be written.
     * @return The interpolated/extracted sounding.
     */
    public ESCSounding extractHeight(ESCSounding sounding, PrintWriter log) {
	replaceUncheckedFlags(sounding);
	...
	// Determine the first height value to be interpolated.
	int interpolationHeight = calculateInitialInterpolationHeight(sounding);
	
	// Special case when the interpolation height could not be determined.
	if (interpolationHeight < 0) { return extract; }
	
        // Find highest altitude in sounding
        Double highestAltitude = sounding.getRecords().get(sounding.getRecords().size()-1).getAltitude();
        
	// Continue to loop through the set of interpolation height increments
	// until the interpolation height passes the highest height in the sounding.
	while (interpolationHeight <= highestAltitude) {
	    // Create the record for the current interpolation height and
	    // add it to the extracted sounding.
	    try {
		if (DEBUG) {
		    log.printf("Building Intp Record for %04d\n", interpolationHeight);
		...
		ESCSoundingRecord record = buildInterpolatedRecordHeight(interpolationHeight, extract.getRecords().get(extract.getRecords().size() - 1), sounding, log);
		if (record != null) { 
		    extract.add(record);
		}
	    } catch (ExtractionException e) {
		log.printf("The record at height %d could not be interpolated.\n", interpolationHeight);
		log.println("\t"+e.getMessage());
		log.flush();
	    }
	    
	    // Adjust the interpolation height to the next height in the incrementation.
	    interpolationHeight += getInterpolationInterval();
	}         

	// Add all of the missing height records.
	Double nextInterpHeight = new Double(calculateInitialInterpolationHeight(sounding));
	// Loop through all of the known records.
	for (int i = 1; i < extract.getRecords().size(); i++) {

	    // If the next interpolation height is less than the current record's height, there
	    // is a missing height record.  Add the new record to the current location in the record
	    // list.  This will push the current record to be the next record in the list (and making
	    // the new record the current one).  This will make the size of the record list grow by one
	    // so the next iteration of the for loop will make the original current record the new
	    // current record allowing for multiple missing records to be added before the original
	    // current record as necessary.  
	    if (nextInterpHeight.compareTo(extract.getRecords().get(i).getAltitude()) < 0) {

		// Build the new record and add it to the extracted sounding.
		ESCSoundingRecord record = new ESCSoundingRecord();
		try {
		    // Assign the height to a new record.
		    record.setAltitude(nextInterpHeight, METERS);
		    // Add the new missing record to the current location in the list.
		    extract.getRecords().add(i, record);
		}
		// This can be ignored because it is caused by a sea level pressure calculation.		
		catch (CalculationWarning e) {}
		// This exception should only occur if the program/library changes to not allow METERS
		// as a valid altitude unit.  This is a programming issue, not a data issue, so blow up.
		catch (ConversionException e) {
		    e.printStackTrace();
		    System.exit(1);
		}
		// This occurs if the value being assigned to the height is invalid.  This is a major problem
		// since the value is the basis of the extraction.  This is such a potentially big
		// problem that the program is going to blow up since it is probably a programming error.
		catch (InvalidValueWarning e) {
		    e.printStackTrace();
	    ...
	    // Finally, the next interpolated pressure needs be incremented by the interval for the next
	    // iteration since the current interpolated already existed or has just been created as a
	    // missing record.
	    nextInterpHeight += getInterpolationInterval();
	}
	...
	while (time != null && secondRangeRecords.isEmpty() && index >= 0) {
    ...
    /**
     * Determine the set of records (in increasing height order) that are allowed to be included
     * as potential end points for the interpolation of record values.
     * @param interpolatedHeight The height value that is being interpolated.
     * @param sounding The sounding that contains the high resolution data records.
     * @return The list of high resolution data records that can be used in the interpolation of values.
     */
    private List<ESCSoundingRecord> findRecordsWithinSecondRangeHeight(int interpolatedHeight, ESCSoundingRecord previousInterpolatedRecord, ESCSounding sounding, PrintWriter log) {
	// Define the record lists used.
	List<ESCSoundingRecord> allRecords = sounding.getRecords();		
	List<ESCSoundingRecord> secondRangeRecords = new ArrayList<ESCSoundingRecord>();
	...
	int index = soundingIndex;
	Double time = null;
	Double height = new Double(interpolatedHeight);
	
	//System.out.printf("Attempting height: %.1f\n", height);
	
	// Determine the time of the record at the interpolation height.
	while (index < allRecords.size() && time == null) {
	    if (index + 1 < allRecords.size()) {
		Double ascentRate = calculateAscentRate(previousInterpolatedRecord, allRecords.get(index));
		//System.out.printf("\tAscent Rate at Height %.1f: %s (%d)\n", height, ascentRate, index);
		//System.out.printf("\t\t%s\n\t\t%s\n\n", previousInterpolatedRecord, allRecords.get(index));
		if ((ascentRate == null && previousInterpolatedRecord.equals(allRecords.get(index))) || 
		    (ascentRate != null && (ascentRate.compareTo(20.0) > 0 || ascentRate.compareTo(-40.0) < 0))) {
		    
		}		
		else if (allRecords.get(index).getAltitude() != null && (allRecords.get(index).getAltitude().compareTo(height) == 0 ||
									 (allRecords.get(index+1).getAltitude() != null && allRecords.get(index).getAltitude().compareTo(height) <= 0 &&
									  allRecords.get(index+1).getAltitude().compareTo(height) > 0))) {
		    if (allRecords.get(index).getAltitude().compareTo(height) == 0) {
			time = allRecords.get(index).getTime();
		    }
		    else {
			time = interpolateTimeHeight(height, allRecords.get(index), allRecords.get(index + 1));
		    }
		    soundingIndex = index;
		    
		    if (time == null) {
			int lowerIndex = index;
			int upperIndex = allRecords.get(index).getAltitude().compareTo(height) == 0 ? index : index + 1;
			
			while (lowerIndex >= 0 && (allRecords.get(lowerIndex).getAltitude() == null || allRecords.get(lowerIndex).getTime() == null)) { lowerIndex--; }
			while (upperIndex < allRecords.size() && (allRecords.get(upperIndex).getAltitude() == null || allRecords.get(upperIndex).getTime() == null)) { upperIndex++; }
			
			if (lowerIndex >= 0 && upperIndex < allRecords.size()) {
			    time = interpolateTimeHeight(height, allRecords.get(lowerIndex), allRecords.get(upperIndex));
			    soundingIndex = lowerIndex;
			}
		    }				
		}
	    } else {
		if (allRecords.get(index).getAltitude() != null && allRecords.get(index).getAltitude().compareTo(height) == 0) {
		    time = allRecords.get(index).getTime();
		    soundingIndex = index;
		}
	    }
	...
	if (DEBUG) {
	    log.printf("Time of Interpolated Height %04d = %.1f\n", interpolatedHeight, time);
	    log.flush();
	}
	
	// Back up the index to the first record within the second range of the interpolation height.
	// This is needed to back up the index if the height interval record is the last record of the sounding.
	while (index >= allRecords.size()) { index--; }
	while (time != null && index > 0 && (allRecords.get(index).getTime() == null || Math.abs(allRecords.get(index).getTime() - time) <= getSecondRange())) { index--; }
	// Need to increment this index once to get it back into line with the first allowed time.
	index++;
	if (DEBUG) {
	    if (index < allRecords.size()) {
		log.printf("Index of First Record for Height %04d = %d (Time = %.1f)\n", interpolatedHeight, index, allRecords.get(index).getTime());
	    } else {
		log.printf("Index of First Record for Height %04d = null\n", interpolatedHeight);
	    }
	    log.flush();
	}
	...
	// distance has passed.
	while (time != null && index < allRecords.size() && (allRecords.get(index).getTime() == null || Math.abs(allRecords.get(index).getTime() - time) <= getSecondRange())) {
	    if (allRecords.get(index).getAltitude() != null && allRecords.get(index).getTime() != null) {
		secondRangeRecords.add(allRecords.get(index));
	    }
	    index++;
	}
	...
	// any other records within the allowed time range.
	while (time != null && secondRangeRecords.isEmpty() && index >= 0) {
	    if (index < allRecords.size() && allRecords.get(index).getAltitude() != null && height.compareTo(allRecords.get(index).getAltitude()) == 0) {
		secondRangeRecords.add(allRecords.get(index));
	    }
	    index--;
	}
	...
	if (DEBUG) {
	    if (secondRangeRecords.size() > 0) {
		log.printf("Index of Final Record for Height %04d = %d (Time = %.1f)\n", interpolatedHeight, allRecords.indexOf(secondRangeRecords.get(secondRangeRecords.size() - 1)), allRecords.get(allRecords.indexOf(secondRangeRecords.get(secondRangeRecords.size() - 1))).getTime());
		log.flush();
	    } else {
		log.printf("Index of Final Record for Height %04d = null\n", interpolatedHeight);
		log.flush();
	    }
	}
	
	// Now return the list of records within second range of the interpolation height.
	return secondRangeRecords;
    }
    
    /**
     * Get the difference in pressure or height between interpolated pressure or height values.
     * @return The pressure interpolation interval in millibars for pressure or meters for height. 
     */
    public abstract int getInterpolationInterval();
    
    /**
     * Get the pressure or height value that is that smallest allowed pressure or height to be interpolated for the 
     * sounding.
     * @return The minimum pressure or height that can be interpolated.
     */
    public abstract int getMinimumInterpolationValue();
    
    /**
     * Get the number of seconds on either side of the interpolated pressure or height a record is allowed
     * to be from the interpolated pressure's or height's time to be included as a potential end point in the
     * interpolation of record values.
     * @return The number of seconds from the interpolation pressure's or height's time.
     */
    public abstract int getSecondRange();
    
    /**
     * Get the type of interpolation that is being calculated.  Either H for height or P for pressure.
     * @return The type of interpolation that is being calculated (H or P). 
     */
    public abstract char getInterpolationType();
     ...
     * @param sounding
     * @return The initial interpolated sounding.
     */
...
// Add the surface record as is to the interpolated record for either height or pressure.
	int i = 0;
        if (getInterpolationType() == 'P') {
	   ...
	   while (extract.getRecords().isEmpty() && i < sounding.getRecords().size()) {
	      if (sounding.getRecords().get(i).getAltitude() != null) {
		 extract.add(sounding.getRecords().get(i));
	      }
	      i++;
           }
    ...
    /**
     * Interpolate the values related to the height for the specified record.  Since there
     * is no Altitude flag, use the Pressure flag instead.
     * @param interpolated The record being interpolated.
     * @param height The height of the record being interpolated.
     ...
     */
    private void interpolateAltitudeValuesHeight(ESCSoundingRecord interpolated, Double height, List<ESCSoundingRecord> records, PrintWriter log) {
	ESCSoundingRecord lower = null, upper = null;
	for (ESCSoundingRecord record: records) {
	    // Determine the lower height end point for the height related values.
	    if (record.getAltitude().compareTo(height) <= 0) {
		// Lower hasn't been defined, so define it.
		if (lower == null || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getPressureFlag()))) {
		    lower = record;
		}
		// Lower has the same pressure flag as the record, so choose the closest record to the interpolation height.
		else if (lower.getPressureFlag().compareTo(record.getPressureFlag()) == 0) {
		    lower = lower.getAltitude().compareTo(record.getAltitude()) >= 0 ? lower : record;
		}
		// Use the record between the lower and current record that has the best pressure flag.
		else {
		    lower = lower.getPressureFlag().isWorseFlag(record.getPressureFlag())  && height.compareTo(lower.getAltitude()) != 0 ? record : lower;
		}
	    }
	    // Determine the upper height end point for the height related values.
	    if (record.getAltitude().compareTo(height) >= 0) {
		// Upper hasn't been defined, so define it.
		if ((upper == null && !(height.compareTo(record.getAltitude()) == 0 && !BAD_FLAG.isWorseFlag(record.getPressureFlag()))) || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getPressureFlag()))) {
		    upper = record;
		}
		// Upper has the same pressure flag as the record, so choose the record closest to the interpolation height.
		else if (upper != null && upper.getPressureFlag().compareTo(record.getPressureFlag()) == 0) {
		    upper = upper.getAltitude().compareTo(record.getAltitude()) <= 0 ? upper : record;
		}
		// Use the record between the upper and current records that has the best pressure flag.
		else {
		    upper = upper != null && upper.getPressureFlag().isWorseFlag(record.getPressureFlag()) && height.compareTo(upper.getAltitude()) != 0 ? record : upper;
		}
	    }
	}
	
	if (DEBUG) {
	    log.printf("Height End Points Used:\n\t%s\n\t%s\n", lower, upper);
	}
	
	// At least one end point could not be determined for the interpolation height,
	// so the remaining height values can not be interpolated.
	if (lower == null || upper == null) { return; }
	
	// Assign the interpolation height to the record.
	try { interpolated.setAltitude(height, METERS); }
	// This can be ignored because it is caused by a sea level pressure calculation.		
	catch (CalculationWarning e) {}
	// This exception should only occur if the program/library changes to not allow METERS
	// as a valid pressure unit.  This is a programming issue, not a data issue, so blow up.
	...
	// Interpolate the time from the height end points.
	try { interpolated.setTime(interpolateTimeHeight(height, lower, upper)); } 
	...
	// Calculate the ascent rate from the height end points
	try { 
    ...
    /**
     * Interpolate the latitude value using the specified records as end points.
     * @param height The height the latitude is being interpolated to.
     * @param lower The lower pressure end point to use in the interpolation.
     * @param upper The upper pressure end point to use in the interpolation.
     * @return The interpolated latitude value for the specified height.
     */
    private Double interpolateLatitudeHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (upper.getLatitude().compareTo(lower.getLatitude()) == 0) { return lower.getLatitude(); }
	return lower.getLatitude() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getLatitude() - lower.getLatitude());
    }    
    ...
    /**
     * Interpolate the longitude value using the specified records as end points.
     * @param height The height the longitude is being interpolated to.
     * @param lower The lower height end point to use in the interpolation.
     * @param upper The upper height end point to use in the interpolation.
     * @return The interpolated longitude value for the specified height.
     */
    private Double interpolateLongitudeHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (upper.getLongitude().compareTo(lower.getLongitude()) == 0) { return lower.getLongitude(); }
	...
	// Calculate the weighted average of the longitude.
	double avg = lower.getLongitude() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (delta);
    ...
    /**
     * Interpolate the pressure value using the specified records as end points.
     * @param height The height the altitude is being interpolated to.
     * @param lower The lower height end point to use in the interpolation.
     * @param upper The upper height end point to use in the interpolation.
     * @return The interpolated pressure value for the specified height.
     */
    private Double interpolatePressureHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (lower.getPressure() == null || upper.getPressure() == null) { return null; }
	else if (upper.getPressure().compareTo(lower.getPressure()) == 0) { return lower.getPressure(); }
	else {
	    return lower.getPressure() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getPressure() - lower.getPressure());
	}
    }
     ...
     * @param log The output stream where errors/warnings are to be written.
     */
    private void interpolatePressureValuesHeight(ESCSoundingRecord interpolated, Double height, List<ESCSoundingRecord> records, PrintWriter log) {
	ESCSoundingRecord lower = null, upper = null;
	for (ESCSoundingRecord record: records) {
	    // Determine the lower height end point for the pressure.
	    if (record.getAltitude().compareTo(height) <= 0 && record.getPressure() != null) {
		// Lower hasn't been defined, so define it.
		if (lower == null || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getPressureFlag()))) { lower = record; }
		// Lower has the same flag as the record, so choose the record closest to the interpolation height.
		else if (lower.getPressureFlag().compareTo(record.getPressureFlag()) == 0) {
		    lower = lower.getAltitude().compareTo(record.getAltitude()) >= 0 ? lower : record;
		}
		// Use the record between the lower and current records that has the best pressure flag.
		else {
		    lower = lower.getPressureFlag().isWorseFlag(record.getPressureFlag()) && height.compareTo(lower.getAltitude()) != 0 ? record : lower;
		}
	    }
	    // Determine the upper height end point for the pressure.
	    if (record.getAltitude().compareTo(height) >= 0 && record.getPressure() != null) {
		// Upper hasn't been defined, so define it.
		if ((upper == null && !(height.compareTo(record.getAltitude()) == 0 && !BAD_FLAG.isWorseFlag(record.getPressureFlag()))) || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getPressureFlag()))) { upper = record; }
		// Upper has the same flag as the record, so choose the record closest to the interpolation height.
		else if (upper != null && upper.getPressureFlag().compareTo(record.getPressureFlag()) == 0) {
		    upper = upper.getAltitude().compareTo(record.getAltitude()) <= 0 ? upper : record;
		} 
		// Use the record between the upper and current records that has the best pressure flag.
	...
	// At least one end point could not be determined for the interpolation height,
	// so the pressure cannot be interpolated.
	if (lower == null || upper == null) { return; }
	
	try {
	    // Handle the case where the interpolated height has a high resolution record.
	    // The values and flag are only kept if the pressure flag is not bad or missing.
	    if (lower.getAltitude().compareTo(height) == 0 && !lower.getPressureFlag().isWorseFlag(BAD_FLAG) && MISSING_FLAG.compareTo(lower.getPressureFlag()) != 0) {
		interpolated.setPressure(lower.getPressure(), MILLIBARS);
		interpolated.setPressureFlag(lower.getPressureFlag());
	    }
	    // All other cases will cause the pressure to be interpolated, so calculate the value and assign it to the record.
	    else {
		interpolated.setPressure(interpolatePressureHeight(height, lower, upper), MILLIBARS);
		interpolated.setPressureFlag(lower.getPressureFlag().isWorseFlag(upper.getPressureFlag()) ? lower.getPressureFlag() : upper.getPressureFlag());
	    }
	    if (DEBUG) {
                log.printf("Setting Pressure %.1f Pressure Flag = %.1f\n", interpolated.getPressure(), interpolated.getPressureFlag().getValue());
 	...
 	// This can be ignored because it is caused by a sea level pressure calculation.           
	    ...
	    // never be passed an invalid flag because the flag sources should have caused a problem earlier.  This will
	    // blow up as a programming error and not a data issue.
	    e.printStackTrace();
	    System.exit(1);
	} catch (InvalidValueWarning e) {
	    // This exception occurs if an invalid pressure is being assigned to the record.
	    log.printf("InvalidValueWarning from pressure %.1f m: %s\n", height, e.getMessage());
	    log.flush();
	}
    }
    ...
    /**
     * Interpolate the relative humidity value using the specified records as end points.
     * @param height The height the rh is being interpolated to.
     * @param lower The lower pressure end point to use in the interpolation.
     * @param upper The upper pressure end point to use in the interpolation.
     * @return The interpolated RH value for the specified height.
     */
    private Double interpolateRelativeHumidityHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (lower.getRelativeHumidity().compareTo(upper.getRelativeHumidity()) == 0) { return lower.getRelativeHumidity(); }
	return lower.getRelativeHumidity() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getRelativeHumidity() - lower.getRelativeHumidity());
    }
          
    /**
     * Interpolate the relative humidity for the specified sounding record.
     ...
     * Interpolate the relative humidity for the specified sounding record.
     * @param interpolated The sounding record being interpolated.
     * @param height The height of the interpolated record.
     * @param records The high resolution data records available to use as end points for the interpolation.
     * @param log The output stream where errors/warnings are to be recorded.
     */
    private void interpolateRelativeHumidityValuesHeight(ESCSoundingRecord interpolated, Double height, List<ESCSoundingRecord> records, PrintWriter log) {
	ESCSoundingRecord lower = null, upper = null;
	for (ESCSoundingRecord record: records) {
	    // Determine the lower height end point for the RH.
	    if (record.getAltitude().compareTo(height) <= 0 && record.getRelativeHumidity() != null) {
		// Lower hasn't been defined, so define it.
		if (lower == null || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getRelativeHumidityFlag()))) { lower = record; }
		// Lower has the same flag as the record, so choose the record closest to the interpolation height.
		else if (lower.getRelativeHumidityFlag().compareTo(record.getRelativeHumidityFlag()) == 0) {
		    lower = lower.getAltitude().compareTo(record.getAltitude()) >= 0 ? lower : record;
		}
		// Use the record between the lower and current records that has the best RH flag.
		    ...
		    lower = lower.getRelativeHumidityFlag().isWorseFlag(record.getRelativeHumidityFlag()) && height.compareTo(lower.getAltitude()) != 0 ? record : lower;
		}
	    }
	    // Determine the upper height end point for the RH.
	    if (record.getAltitude().compareTo(height) >= 0 && record.getRelativeHumidity() != null) {
		// Upper hasn't been defined, so define it.
		if ((upper == null && !(height.compareTo(record.getAltitude()) == 0 && !BAD_FLAG.isWorseFlag(record.getRelativeHumidityFlag()))) || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getRelativeHumidityFlag()))) { upper = record; }
		// Upper has the same flag as the record, so choose the record closest to the interpolation height.
		else if (upper != null && upper.getRelativeHumidityFlag().compareTo(record.getRelativeHumidityFlag()) == 0) {
		    upper = upper.getAltitude().compareTo(record.getAltitude()) <= 0 ? upper : record;
		} 
		// Use the record between the upper and current records that has the best RH flag.
		    ...
		    upper = upper != null && upper.getRelativeHumidityFlag().isWorseFlag(record.getRelativeHumidityFlag()) && height.compareTo(upper.getAltitude()) != 0 ? record : upper;
		}
	    }
	}
	...
	try {
	    // Handle the case where the interpolated height has a high resolution record.
	    // The values and flag are only kept if the RH flag is not bad or missing.
	    if (lower.getAltitude().compareTo(height) == 0 && !lower.getRelativeHumidityFlag().isWorseFlag(BAD_FLAG) && MISSING_FLAG.compareTo(lower.getRelativeHumidityFlag()) != 0) {
		interpolated.setRelativeHumidity(lower.getRelativeHumidity());
		interpolated.setRelativeHumidityFlag(lower.getRelativeHumidityFlag());
	    }
	    // All other cases will cause the RH to be interpolated, so calculate the value and assign it to the record.
	    else {
		interpolated.setRelativeHumidity(interpolateRelativeHumidityHeight(height, lower, upper));
		interpolated.setRelativeHumidityFlag(lower.getRelativeHumidityFlag().isWorseFlag(upper.getRelativeHumidityFlag()) ? lower.getRelativeHumidityFlag() : upper.getRelativeHumidityFlag());
	    }
	    if (DEBUG) {
                log.printf("Setting RH %.1f RH Flag = %.1f\n", interpolated.getRelativeHumidity(), interpolated.getRelativeHumidityFlag().getValue());
    ...
    /**
     * Interpolate the temperature value using the specified records as end points.
     * @param pressure The pressure the temperature is being interpolated to.
     * @param lower The lower pressure end point to use in the interpolation.
     * @param upper The upper pressure end point to use in the interpolation.
    ...
    /**
     * Interpolate the temperature value using the specified records as end points.
     * @param height The height the temperature is being interpolated to.
     * @param lower The lower height end point to use in the interpolation.
     * @param upper The upper height end point to use in the interpolation.
     * @return The interpolated temperature for the specified height.
     */
    private Double interpolateTemperatureHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (lower.getTemperature().compareTo(upper.getTemperature()) == 0) { return lower.getTemperature(); }
	return lower.getTemperature() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getTemperature() - lower.getTemperature());
    }
     ...
     * @param log The output stream where errors/warnings are to be written.
     */
    private void interpolateTemperatureValuesHeight(ESCSoundingRecord interpolated, Double height, List<ESCSoundingRecord> records, PrintWriter log) {
	ESCSoundingRecord lower = null, upper = null;
	for (ESCSoundingRecord record: records) {
	    // Determine the lower height end point for the temperature.
	    if (record.getAltitude().compareTo(height) <= 0 && record.getTemperature() != null) {
		// Lower hasn't been defined, so define it.
		if (lower == null || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getTemperatureFlag()))) { lower = record; }
		// Lower has the same flag as the record, so choose the closest record to the interpolation height.
		else if (lower.getTemperatureFlag().compareTo(record.getTemperatureFlag()) == 0) {
		    lower = lower.getAltitude().compareTo(record.getAltitude()) >= 0 ? lower : record;
		}
		// Use the record between the lower and current record that has the best temperature flag.
		else {
		    lower = lower.getTemperatureFlag().isWorseFlag(record.getTemperatureFlag()) && height.compareTo(lower.getAltitude()) != 0 ? record : lower;
		}
	    }
	    // Determine the upper height end point for the temperature.
	    if (record.getAltitude().compareTo(height) >= 0 && record.getTemperature() != null) {
		// Upper hasn't been defined, so define it.
		if ((upper == null && !(height.compareTo(record.getAltitude()) == 0 && !BAD_FLAG.isWorseFlag(record.getTemperatureFlag()))) || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getTemperatureFlag()))) { upper = record; }
		// Upper has the same flag as the record, so choose the closest record to the interpolated height.
		else if (upper != null && upper.getTemperatureFlag().compareTo(record.getTemperatureFlag()) == 0) {
		    upper = upper.getAltitude().compareTo(record.getAltitude()) <= 0 ? upper : record;
		}
		// Use the record between the upper and current record that has the best temperature flag.
		else {
		    upper = upper != null && upper.getTemperatureFlag().isWorseFlag(record.getTemperatureFlag()) && height.compareTo(upper.getAltitude()) != 0 ? record : upper;
		}
	    }
	}
	...
	try {
	    // Handle the case where the interpolated height has a high resolution record.
	    // The value and flag are only kept if the temperature flag is not bad or missing.
	    if (lower.getAltitude().compareTo(height) == 0 && !lower.getTemperatureFlag().isWorseFlag(BAD_FLAG) && MISSING_FLAG.compareTo(lower.getTemperatureFlag()) != 0) {
		interpolated.setTemperature(lower.getTemperature(), CELCIUS);
		interpolated.setTemperatureFlag(lower.getTemperatureFlag());
	    }
	    // All other cases will cause the temperature to be interpolated, so calculate the
		...
		interpolated.setTemperature(interpolateTemperatureHeight(height, lower, upper), CELCIUS);
		interpolated.setTemperatureFlag(lower.getTemperatureFlag().isWorseFlag(upper.getTemperatureFlag()) ? lower.getTemperatureFlag() : upper.getTemperatureFlag());
	    }
            if (DEBUG) {
                log.printf("Setting Temperature %.1f Temp Flag = %.1f\n", interpolated.getTemperature(), interpolated.getTemperatureFlag().getValue());
    ...
    /**
     * Interpolate the time using the specified records as end points.
     * @param height The height the time is being interpolated to.
     * @param lower The lower height end point to use in the interpolation.
     * @param upper The upper height end point to use in the interpolation.
     * @return The interpolated time for the specified height.
     */
    private Double interpolateTimeHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (lower.getTime() == null || upper.getTime() == null) { return null; }
	else if (lower.getTime().compareTo(upper.getTime()) == 0) { return lower.getTime(); }
	else {
	    return lower.getTime() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getTime() - lower.getTime());	
	}
    }        
    ...
    /**
     * Interpolate the U component wind value using the specified records as end points.
     * @param height The height the u component is being interpolated to.
     * @param lower The lower height end point to use in the interpolation.
     * @param upper The upper height end point to use in the interpolation.
     * @return The interpolated U component for the specified height.
     */
    private Double interpolateUComponentHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (lower.getUComponent().compareTo(upper.getUComponent()) == 0) { return lower.getUComponent(); }
	return lower.getUComponent() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getUComponent() - lower.getUComponent());
    }
    ...
    /**
     * Interpolate the V component wind value using the specified records as end points.
     * @param height The height the v component is being interpolated to.
     * @param lower The lower height end point to use in the interpolation.
     * @param upper The upper height end point to use in the interpolation.
     * @return The interpolated V component for the specified height.
     */
    private Double interpolateVComponentHeight(Double height, ESCSoundingRecord lower, ESCSoundingRecord upper) {
	if (lower.getVComponent().compareTo(upper.getVComponent()) == 0) { return lower.getVComponent(); }
	return lower.getVComponent() + ((height - lower.getAltitude()) / (upper.getAltitude() - lower.getAltitude())) * (upper.getVComponent() - lower.getVComponent());
    }
     ...
     * usually caused by an improper combination of wind values and flags.
     */
    private void interpolateWindValuesHeight(ESCSoundingRecord interpolated, Double height, List<ESCSoundingRecord> records, PrintWriter log) throws ExtractionException {
	ESCSoundingRecord lower = null, upper = null;
	for (ESCSoundingRecord record: records) {
	    // Make sure that the both the U and V components are missing or have a value.
	    if ((record.getUComponent() == null && record.getVComponent() != null) ||
	    ...
	    // Determine the Lower height end point for the wind components.
	    if (record.getAltitude().compareTo(height) <= 0 && record.getUComponent() != null) {
		// Lower hasn't been defined, so define it.
		if (lower == null || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getUComponentFlag()))) { lower = record; }
		// Lower has the same flag as the record, so choose the closest record to the interpolation height.
		else if (lower.getUComponentFlag().compareTo(record.getUComponentFlag()) == 0) {
		    lower = lower.getAltitude().compareTo(record.getAltitude()) >= 0 ? lower : record;
		}
		// Use the record between the lower and current record that has the best wind flag.
		    ...
		    lower = lower.getUComponentFlag().isWorseFlag(record.getUComponentFlag()) && height.compareTo(lower.getAltitude()) != 0 ? record : lower;
		}
	    }
	    // Determine the Upper height end point for the wind components.
	    if (record.getAltitude().compareTo(height) >= 0 && record.getUComponent() != null) {
		// Upper hasn't been defined, so define it.
		if ((upper == null && !(height.compareTo(record.getAltitude()) == 0 && !BAD_FLAG.isWorseFlag(record.getUComponentFlag()))) || (height.equals(record.getAltitude()) && BAD_FLAG.isWorseFlag(record.getUComponentFlag()))) { upper = record; }
		// Upper has the same flag as the record, so choose the closest record to the interpolated height.
		else if (upper != null && upper.getUComponentFlag().compareTo(record.getUComponentFlag()) == 0) {
		    upper = upper.getAltitude().compareTo(record.getAltitude()) <= 0 ? upper : record;
		}
		// Use the record between the upper and current record that has the best wind flag.
		    ...
		    upper = upper != null && upper.getUComponentFlag().isWorseFlag(record.getUComponentFlag()) && height.compareTo(upper.getAltitude()) != 0 ? record : upper;
		}
	    }
	}
	...
	try {
	    // Handle the case where the interpolated height has a high resolution record.
	    // The values are only kept if the wind flag is not bad.
	    if (lower.getAltitude().compareTo(height) == 0 && !lower.getUComponentFlag().isWorseFlag(BAD_FLAG) && MISSING_FLAG.compareTo(lower.getUComponentFlag()) != 0) {
		interpolated.setUComponent(lower.getUComponent(), METERS_PER_SECOND);
		interpolated.setVComponent(lower.getVComponent(), METERS_PER_SECOND);
		interpolated.setUComponentFlag(lower.getUComponentFlag());
		interpolated.setVComponentFlag(lower.getVComponentFlag());
		...
		interpolated.setUComponent(interpolateUComponentHeight(height, lower, upper), METERS_PER_SECOND);
		interpolated.setVComponent(interpolateVComponentHeight(height, lower, upper), METERS_PER_SECOND);
		interpolated.setUComponentFlag(lower.getUComponentFlag().isWorseFlag(upper.getUComponentFlag()) ? lower.getUComponentFlag() : upper.getUComponentFlag());
		interpolated.setVComponentFlag(lower.getVComponentFlag().isWorseFlag(upper.getVComponentFlag()) ? lower.getVComponentFlag() : upper.getUComponentFlag());
	    }
            if (DEBUG) {
	    ...
	    // This exception should only occur if the program/library changes to not allow METERS_PER_SECOND to
	    // be a valid unit to one of the components.  This will blow up since this is a programming issue
	    // and not a data issue.
	    e.printStackTrace();
	    System.exit(1);
	...
	try {
	    if (lower.getLatitude() != null && upper.getLatitude() != null) {
		interpolated.setLatitude(interpolateLatitudeHeight(height, lower, upper));
	    }
	} catch (InvalidValueWarning e) {
	    // This exception should never occur because averaging two valid latitudes should result in
	    // another valid latitude.
	...
	try {
	    if (lower.getLongitude() != null && upper.getLongitude() != null) {
		interpolated.setLongitude(interpolateLongitudeHeight(height, lower, upper));
	    }
	} catch (InvalidValueWarning e) {
	    // This exception should never occur because averaging two valid longitudes should
	    // result in another valid longitude.
    ...
    /**
     * Open the specified file to a writable stream.  This is used to open ASCII files and
     * gzipped ASCII files.
	...
	ESCSoundingParser parser = new ESCSoundingParser(false);
        ...
        char interpolationType = getInterpolationType();
	...
	for (File file: files) {
	    ...
	    for (ESCSounding sounding: soundings) {
		    ...
		    // Extract the interpolation data from the sounding.
                    if (interpolationType == 'P') {
		       extract = extract(sounding, log);                   
                    } else {
                       extract = extractHeight(sounding, log);                     
                    }
