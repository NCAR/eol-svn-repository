...
##Module-------------------------------------------------------------
# This s/w is used to generate skew-T plot from ESC formatted
# soundings. It was created by Joel Clawson and modified by 
# Scot Loehrer. The skew-T images generated may be placed a project's
# Field Catalog. This header was added by L. Cully on 27 Oct 2008.
#
# @author Joel Clawson
##Module-------------------------------------------------------------
use strict;
...
sub create_plots {
	...
	printf($FILE "#! /bin/csh\n");
	printf($FILE "/opt/local/bin/suds << EOF\n");
	printf($FILE "file %s class NCAR/EOL\n", $temp_class);
	printf($FILE "color data2 rgb 0 0 1\n");
	printf($FILE "output ./%s psc\n", $new_file);
	printf($FILE "skewt plimits %d %d\n",@PRESS_LIMITS) if (@PRESS_LIMITS);
	printf($FILE "skewt tlimits %d %d\n",@TEMP_LIMITS) if (@TEMP_LIMITS);
	printf($FILE "skewt NCAR/EOL\n");
	printf($FILE "quit\nEOF\n");
	close($FILE);
	...
	my $gif_file = sprintf("%s/%s_%04d%02d%02d%02d%02d.skewt.gif",
			       $OUT_DIR,$id,$year,$month,$day,$hour,$min);

	# Create the GIF file from the post script file.
	system(sprintf("/usr/bin/convert -rotate 90 -crop 0x0 -fill black -draw \"text 45,30 '%s %s'\" %s %s",$data[4],$data[5],$new_file,$gif_file));
