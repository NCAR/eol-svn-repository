...
for (;fscanf(stdin,"%d %d",&ex,&ey)>0;) {
 ei=(ex-start_col)+ease_cols*(ey-start_row);
 findnn(ei);
 printnn(ei,ex,ey);
 }
}
...
int mi;
 ...
 printf("E%d %3d,%3d %f,%f\n",ei,ex,ey,ease_lat[ei],ease_lon[ei]);
 for (j=0; j<NUM_NBORS; j++)
  printf("  N%d idx=%d %d,%d %f,%f dist=%f km\n",
   j,mi=nbor_idx[ei][j],
   mi%MODEL_COLS+MODEL_START_COL,mi/MODEL_COLS+MODEL_START_ROW,
   model_lat[mi],model_lon[mi],
   nbor_dist[ei][j]
   );
}
...
fprintf(stderr,"Model %s using file %s\n",MODEL_NAME,MODEL_FILE);
