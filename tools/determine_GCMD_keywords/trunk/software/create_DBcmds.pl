...
sub main 
      ...
      if ($has_GCMD)
         ...
         foreach my $Gkey (@record_yyy)
           ...
           if (exists ($Input_UUID_GCMDID{$GCMD_code}) )
              ...
              if (($parm_keyword_string eq " ") || ($parm_keyword_string eq "")) {print "WARNING: GCMD_code NOT FOUND in HASH. Input_UUID_GCMDID{$GCMD_code} = zzz $Input_UUID_GCMDID{$GCMD_code} zzz\n"; }
              }
           else
              {
   ...
   foreach my $file (sort(@files))
      ...
      if ($debug) {print "file = zzz $file zzz, file_parts = zzz @file_parts zzz\n";}
      ...
      if ($debug) {print "archIdent = zzz $archIdent zzz, ID = zzz $ID zzz, Input_DatasetID_archID = zzz $Input_DatasetID_archID{$ID} zzz  \n";}

      if (($archIdent eq " ") || ($archIdent eq "")) {$archIdent = 9999; print "WARNING: archIdent NOT FOUND in HASH! archIdent = zzz $archIdent zzz\n";}
      ...
      foreach my $line (@PARM_lines)
        ...
        ### Do not write sql command if the GCMD code is null!
         ...
         $GCMD_codes_for_parm = $Input_parm_match_GCMD{$parm};
         if ($debug) {print "(44-C)GCMD_codes_for_parm zzz $GCMD_codes_for_parm zzz parm = zzz $parm zzz\n";}

         if (($GCMD_codes_for_parm eq " ") || ($GCMD_codes_for_parm eq "")) 
            { 
            print "WARNING: parm = zzz $parm zzz NOT FOUND in Input_parm_match_GCMD HASH! RESET $GCMD_codes_for_parm to null;\n";
            $GCMD_codes_for_parm = "null";    # NEW
            } # Filter_CFDC results in "" or blank value.   # NEW HERE

         if ($GCMD_codes_for_parm ne "null")
            {
            if ($debug) {print "Hash $GCMD_codes_for_parm DOES NOT EQUAL null.\n";}
...
#
            ...
            if ($debug) {print "(66B)GCMD_codes_for_parm zzz $GCMD_codes_for_parm zzz parm = zzz $parm zzz\n";}
      ...
      foreach my $ins_line (@ins_lines)
         ...
         my $ins_gcmd = $ins1[1]; 
         if ($debug) {print "(001 after split ins_line)ins_gcmd:: $ins_gcmd\n";}
