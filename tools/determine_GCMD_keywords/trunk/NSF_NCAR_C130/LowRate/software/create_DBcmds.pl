...
##         if ((index($all_gcmd, $ins_gcmd)) == -1){$all_gcmd = $ins_gcmd.",".$all_gcmd;}
         ...
         if ($ins_index == -1){$all_gcmd = $ins_gcmd.",".$all_gcmd;}

         $ins_gcmd =~ s/,//g;  # rm off trailing comma
      ...
      my $ins_sql_cmd = "ALL-HERE: INSERT IGNORE INTO dataset_gcmd_science_keyword (dataset_id, gcmd_science_keyword_id) SELECT ".$ins_datasetID. ", g.id FROM gcmd_science_keyword g WHERE g.id IN (" .$all_gcmd. ")\n";  # HARDCODED
