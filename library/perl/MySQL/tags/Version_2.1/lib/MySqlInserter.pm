...
sub create_directory_list {
	...
	my @results = grep(/^d.+\s+$next_segment$/,split('\n',`msls -l $base_directory`));
...
##--------------------------------------------------------------------------
sub create_files {
    my ($self,$block_number,$directory,$files) = @_;
	...
	$self->process_directory($block_number,$directory,$found_directory,
                                 $self->{"cfg"}->{$block_number}->{"directory"},\@block_files);
    }
...
# <p>Determine the start and dates for a file.</p>
#
...
sub determine_dates {
    ...
    for (my $i = 0; $i < @date_patterns; $i++) {
        ...
        if ($date_patterns[$i] =~ s/\\([YMyJHmV])/$1/g) {
            #printf("Matched Group: %s -> %s\n",$date_patterns[$i],$captures[$i]);
			...
			if (defined($end->{"month_abrv"})) {
                ...
                # Only handle date/time characters within a group.
                    ...
                    printf("Unknown character %s in date/time grouping %d.  All characters within a date/time group must be a combination of [YMyJHmV] date/time characters.  If there are characters between parts of the date/time, use multiple groups.\n",$char,$i+1);
                    exit(1);
                }
...
sub determine_end_date {
    ...
    if ($filelength =~ /^(\d+)\s+(years?|months?|days?|hours?|minutes?)$/i) {
...
sub insert {
    ...
    foreach my $index (keys(%{ $self->{"cfg"}})) {
        $self->create_files($index,$self->{"cfg"}->{$index}->{"dir_pattern"},\@files);
    }
    ...
    if (scalar(@files) > 0) {
        ...
        foreach my $file (@files) {
            # Handle the use of a non-tsunami file system.
            my $directory = $file->getDirectory();
            $directory =~ s/^\/net/\/export/;
            $directory = sprintf("/export%s",$directory) if ($directory =~ /^\/archive/);

            # Reset the directory to make sure it uses /export
            $file->setDirectory($directory);

            # Display the file information to the user.
            printf("%s %s %s %4s %02d %05d %10s %s/%s\n",$file->getDatasetId(),$file->getBeginDate(),$file->getEndDate(),$file->getPurpose(),$file->getFormatId(),$file->getSize(),$file->getHost(),$file->getDirectory(),$file->getFilename());
...
sub is_mass_store_block {
    ...
    return ($self->{"cfg"}->{$block_number}->{"host"} eq "mass_store");
}
...
sub process_directory {
    my ($self,$block_number,$directory_pattern,$directory,$directory_date_pattern,$files) = @_;
	...
	# Process files that match the file pattern
	    ...
	    my ($begin_date,$end_date) = $self->determine_dates($block_number,
								$directory,
								$file,
								$directory_pattern,
								$file_pattern,
            ...
            my $mysql = $self->is_mass_store_block($block_number) ? MySqlMSSFile->new() : MySqlFile->new();
            $mysql->setDatasetId($self->{"cfg"}->{$block_number}->{"dataset_id"});
...
sub read_config_file {
	...
	if ($line =~ /^\{/) {
		...
		$self->{"cfg"}->{$blockCount}->{"host"} =~ s/(mss|mass_?store)/mass_store/i;
	    }
