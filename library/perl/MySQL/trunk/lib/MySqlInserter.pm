...
# insert of the files.  This means that if any error occurs with the inserting of
...
# @version 2.8.0
# Added functionality to load files from the Campaign Storage host.
# The script should be run as user eoldata. 
#
# @author Janine Aquino
# # @version 2.7.2
# .yml files can include multiple configuration blocks
#
# @author Janine Aquino
# @version 2.7.1: Added ability to ready .yml config files in addition to cfg- files. 
# .yml files can include one or two configuration blocks and they contain all metadata 
# for codiac, dts and cfg in a single file. 
#
# @author Janine Aquino
# @version 2.7: Added ability to ready .yml config files in addition to cfg- files. .yml files can only
# include a single configuration block, but they contain all metadata for codiac, dts and cfg in a single
# file. 
# A side benefit is that now, instead of putting the codiac_id in the cfg- filename, it can optionally be put
# in the configuration block instead as "dataset_id: xxx.xxx"
#
# @author Amanda Orin
...
# user and host for Campaign Storage access
...
sub create_directory_list {
    ...
    if ($self->is_campaign_store_block($block_number)) {
        @results = grep(/^d.+\s+$next_segment$/,`ssh $CS_HOST ls -l $base_directory`);
    foreach my $result (@results) {
        print "create_directory_list: Campaign Store result = ${result}\n" if ($self->{'debug'});
        push(@found_directories,(split(' ',$result))[8]);
    }
    } elsif ($self->is_mass_store_block($block_number)) {
        print "HPSS is no longer a valid host\n";
    } else {
	...
	if ((!$self->is_mass_store_block($block_number) && -d $next_directory) ||
        (!$self->is_campaign_store_block($block_number) && -d $next_directory) ||
        ($self->is_campaign_store_block($block_number)) ||
...
sub create_files {
    ...
    # Remove the empty piece from the directory pattern starting with a /.
    shift(@path_segments); 
    ...
    my @directories = $self->create_directory_list($block_number,
			   sprintf("/%s",shift(@path_segments)), @path_segments);
    my @block_files = (); 
...
sub insert {
    ...
    # TODO: any required MySqlDatabase args should be passed to this class constructor
    ...
    if (scalar(@files) > 0) {
        print "Inserting files into database at " .
          ($database->getHost() ? $database->getHost() : "(host from my.cnf file)") .
          "\n\n";
	...
	my $numberOfPrints = 0;
        ...
        my $vers_where = ' and ';
        my $cfg_vers = $file->getDesiredDatasetVersion();
	    printf("config version: %s\n",$cfg_vers ? $cfg_vers : 'undef') if ($self->{'debug'});
        $cfg_vers = '*' if (!defined($cfg_vers)); # use existing version
        my $noVersIsError = 0;

        if ( $cfg_vers =~ m/^\d+$/ ) { # a primary key (id)
          $vers_where .= "id=$cfg_vers";
          $noVersIsError = 1;
        } elsif ( $cfg_vers =~ m/\./ ) { # a version number string
          $vers_where .= "version_number='$cfg_vers'";
          $noVersIsError = 1;
        } elsif ( $cfg_vers eq 'NONE' ) { # do not hook up to version
          $vers_where .= 'false'; # forces version not to be found
        } elsif ( $cfg_vers eq '*' ) { # use existing version
          $vers_where = '';
          $noVersIsError = 1;
        } else {
          $err .= "Odd configured dataset_version $cfg_vers : should be NONE, a version_number string or * to mean any existing version.";
          $vers_where .= 'false';
        }

        print("version where: $vers_where\n") if ($self->{'debug'});
        my @datasetVersionId = $database->select("dataset_version", 'id', 'dataset_id='.$file->getDatasetId().$vers_where);
        print 'dataset_version err=',$datasetVersionId[0],' id=',$datasetVersionId[1],"\n" if ($self->{'debug'});
        if ($database->{'rows'} == 0 and $noVersIsError) {
          # we looked for a dataset_version but didn't find one
          $err .= "No dataset_version found\n";
        }
        if ($database->{'rows'} > 1) {
          $err .= "Multiple dataset versions found in database. Please specify which to use in the configuration blocks.\n";
          $datasetVersionId[1]=undef;
        }

        #link the file to the dataset version
        if ($datasetVersionId[1]) {
            my @fileId = $database->select("file", 'id',
            'dataset_id='.$file->getDatasetId().' and directory="'.$file->getDirectory().'" and filename="'.$file->getFilename().'"');
            $err .=
            $database->insert("dataset_version_file","dataset_version_id,file_id","$datasetVersionId[1],$fileId[1]");
        }
	    ...
	    if($numberOfPrints < 200){
                ...
                my $verstr = $file->getVersion();
                $verstr = '' if (!defined($verstr));

            	printf("%s %s %s %7s %02d %05d %10s %s/%s %s %s\n",
...
sub is_campaign_store_block {
    my ($self,$block_number) = @_;
    return ($self->{"cfg"}->{$block_number}->{"host"} eq "campaign");
}
...
sub process_directory {
	...
	} elsif ($self->is_campaign_store_block($block_number)) {
    my @results = grep(/^(\-|\d)/,`ssh $CS_HOST ls -l $directory`);
    ...
    if ((
           ( (!$self->is_mass_store_block($block_number) || !$self->is_campaign_store_block($block_number)) &&
             (-d sprintf("%s/%s",$directory,$file))
           ) ||
           ( ($self->is_mass_store_block($block_number) || $self->is_campaign_store_block($block_number)) &&
             (shift(@mss_type_list) =~ /^d/)
           ) ) &&
	...
	# Process files that match the file pattern
	    ...
	    my ($begin_date,$end_date,$event) 
	       = $self->determine_dates($block_number,
					$directory,
					$file,
					$directory_pattern,
					$file_pattern,
	    ...
	    next if ( $self->{"cfg"}->{$block_number}->{"mode"} eq "add" &&
               $mysql->load($self->{"cfg"}->{$block_number}->{"dataset_id"},
               $directory, $file, $database)
            );

	    if ( $self->{"cfg"}->{$block_number}->{"mode"} eq "update" &&
            $mysql->load($self->{"cfg"}->{$block_number}->{"dataset_id"},
                         $directory, $file, $database)
            ) { 
	    ...
	    # Set the values to be added to the SQL database
	    ...
	    if (defined $size && $size > 0) {
	        ...
	        printf("\n\nConfig dataset_version: %s\n",$self->{"cfg"}->{$block_number}->{"dataset_version"}) if ($self->{'debug'});
            $mysql->setDesiredDatasetVersion($self->{"cfg"}->{$block_number}->{"dataset_version"});
	        printf("\n\nDesired dataset_version: %s\n",$mysql->getDesiredDatasetVersion()) if ($self->{'debug'});

	    if (defined $self->{"cfg"}->{$block_number}->{"file_version"}){
                $mysql->setVersion($self->{"cfg"}->{$block_number}->{"file_version"});
	    }
		...
		if ($timeSec >= $fileBegin && $timeSec >= $fileEnd) {
		    # It is important to expand the end time first otherwise the 
		    # end time will get double incremented.
		    # Technically could just increment the Begin time and let MySql 
		    # file handle the end time but it more confusing that way.
...
sub read_config_file {
    ...
    if ($basename =~ /^cfg/) {
        @data = grep(/^(\{|\})|^(\s*[a-z_]+:)/,<$CFG>);
    } elsif ($basename =~ /.yml$/) {
        @data = grep(/^(- [a-z0-9_]+:)/,<$CFG>);
    } else {
	print "File on command line must be either a cfg- or .yml file\n";
	exit(1);
    }
    if ($file =~ /.yml$/) {

        # YAML file doesn't have brackets, so add them here.
        unshift @data,"{\n";

	my $id;
	my $blocks=2;
	my $foundBlock = $blocks;
        for (@data) { 
	    s/^-//; 
	    s/"//g;
	    s/'//g;
	    ...
	    if (/^.*: $/) { 
	        my $index = first { $data[$_] =~ m/^.*: $/ } 0 .. $#data;
		$_ = "";
		splice @data, $index, 1, $_;
	    }
	    ...
	    # Additional blocks, if exists.
	    if (m/archive_location_$blocks:/) {
	        $foundBlock=$blocks;
	        my $index = first { $data[$_] =~ m/archive_location_$blocks:/ } 0 .. $#data;
                # YAML file doesn't have brackets, so add them here.
		splice @data, $index, 0, $id;
		splice @data, $index, 0, "{\n";
		splice @data, $index, 0, "}\n";

	       # Count to break into more blocks if necessary.
	       $blocks++;

	    }
	    s/_$foundBlock//;

	    # codiac and dts have different database fields for the same parameter.
	    # Convert from codiac to DTS here.
	    s/_pattern:/:/;
	    s/archive_location:/directory:/;
	    s/archive_ident:/dataset_id:/;
	    s/version_number:/dataset_version:/;
	...
	my $line = $data[$count];
	    ...
	    # If dataset_id is not defined within the config file, read it
	    # from the filename here.
	    if (!defined($self->{"cfg"}->{$blockCount}->{"dataset_id"})) {
	        $basename =~ /^cfg\-([^\-]+)\-?(.+)?$/;
	        $self->{"cfg"}->{$blockCount}->{"dataset_id"} = $1;
	    }
	    $file= $self->{"cfg"}->{$blockCount}->{"dataset_id"};
	    $file =~ /([^\-]+)\-?(.+)?$/;
	    if (defined($2)) {
