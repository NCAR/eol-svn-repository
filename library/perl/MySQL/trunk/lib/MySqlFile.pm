...
sub setFile {
    ...
    elsif ($self->getHost() eq "campaign") {
        my @result = `ssh $CS_HOST ls -l $_[0]/$_[1]`;
...
# <p>Set the host machine for the file.  It must be localhost or campaign.</p>
#
...
sub setHost {
    ...
    if ($new_host eq 'mass_store') {
        warnings::warnif("deprecated", "mass_store is deprecated, use campaign instead (changing for you)");
        $new_host='campaign';
        }
    elsif ($new_host eq 'hpss') {
        warnings::warnif("deprecated", "hpss is deprecated, use campaign instead (changing for you)");
        $new_host='campaign';
        }
	die("Unknown host: $new_host\n") if ($new_host !~ /^(localhost|hpss|campaign)$/);
    $self->{"host"} = $new_host;
}
