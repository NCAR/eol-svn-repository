...
my $CS_PREFIX = '/glade/campaign/eol/archive';
...
sub create_directory_list {
	...
	foreach my $result (@results) {
	    ...
	    push(@found_directories,(split(' ',$result))[8]);
            ...
            my @results = grep(/^d.+\s+$next_segment$/,`ssh $CS_HOST ls -l $CS_PREFIX$base_directory`);
...
sub process_directory {
	...
	foreach my $result (@results) {
        ...
        my @results = grep(/^(\-|\d)/,`ssh $CS_HOST ls -l $CS_PREFIX$directory`);
	...
	# Process files that match the file pattern
        ...
        # change host setting because 'campaign' not in db yet XXX
        if ($self->{"cfg"}->{$block_number}->{"host"} eq "campaign") {
            $mysql->setHost('hpss');
        }
