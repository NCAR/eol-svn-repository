...
##----------------------------------------------------------------------------
# @signature MySqlDatabase new([String user, String pass, String host, String db, String contact, String my_default_file, String my_default_group])
...
# <ul>
# <li><blockquote><code>
#   $mysql = $MySqlDatabase->new(); # get all connection info from "perl" section of ~/my.cnf
# </code></blockquote></li>
# <li><blockquote><code>
#   $mysql = $MySqlDatabase->new("myuser","mypass",undef,"mydb"); # connect to "localhost"
# </code></blockquote></li>
# <li><blockquote><code>
#   $mysql = $MySqlDatabase->new(undef,undef,undef,undef,"foo"); # no user,pass so use ~/my.cnf plus override contact
# </code></blockquote></li>
# </ul>
# </p>
#
...
# @input $my_default_file Optional filepath for the mysql defaults file ($DMG_MYCNF_PATH or ~/.my.cnf)
# @input $my_default_group Optional group name in the mysql defaults file ($DMG_MYCNF_GROUP or perl)
...
sub new {
    ...
    if (!($self->{"user"} && $self->{"pass"}) || $self->{"mycnf"} || $self->{"mygroup"}) {
        if (!$self->{"mycnf"}) {
            $self->{"mycnf"} = $ENV{'DMG_MYCNF_PATH'} ? $ENV{'DMG_MYCNF_PATH'} : "~/.my.cnf";
        }
        if (!$self->{"mygroup"}) {
