...
##Module---------------------------------------------------------------
package QCFSurfaceRecord;

use strict;
use lib ".";
use QCFConstants;
use SurfaceRecord;
our @ISA = ("QCFConstants", "SurfaceRecord");
...
our $getDewPointFlagPrecedence = sub {
    ...
    $flag = $flag_precedence{$flag} > $flag_precedence{$self->getIncalcuableFlag()} ? 
	$self->getIncalcuableFlag() : $flag;

    # Make sure that if there is a value that it has a correct flag.
    return (defined($self->{"dew_pt"}) && $flag eq $self->getIncalcuableFlag()) &&
	$self->{"dew_pt"} ne $self->getMissing() ? $self->getUncheckedFlag() : $flag;
};
...
our $checkParameter = sub {
    ...
    if (length($_[2]) != 1 ||
	($_[2] ne $self->getMissingFlag()         && 
	 $_[2] ne $self->getIncalcuableFlag()     &&
	 $_[2] ne $self->getGlitchFlag()          && 
	 $_[2] ne $self->getNegativePrecipFlag()  &&
	 $_[2] ne $self->getDubiousFlag()         && 
	 $_[2] ne $self->getValueDoesNotFitFlag() &&
	 $_[2] ne $self->getBadFlag()             && 
	 $_[2] ne $self->getNoReadingFlag()       &&
         $_[2] ne $self->getEstimateFlag()        && 
	 $_[2] ne $self->getTracePrecipFlag()     &&
	 $_[2] ne $self->getUncheckedFlag())) {
	...
	$warning .= sprintf("\t%s has a missing value (%s) but not a missing flag (%s).\n", $_[0], $string, $_[2]);
	     ...
	     ($_[2] eq $self->getBadFlag()         &&
	      $_[2] eq $self->getTracePrecipFlag() &&
	      $_[2] eq $self->getDubiousFlag()     && 
	      $_[2] eq $self->getUncheckedFlag()   &&
	      $_[2] eq $self->getEstimateFlag())) {
...
sub getFlag {
    ...
    elsif ($_[0] eq $self->getMissing()) { return $self->getMissingFlag(); }
    else { return $self->getUncheckedFlag(); }
}
...
sub getCalcSeaLevelPressureFlag {
	...
	$flag_precedence{$self->getIncalcuableFlag()} ?
	    $self->getIncalcuableFlag() : $flag;
}
...
sub setCloudAmount {
    ...
    $self->{"cloud_".$_[1]} = $_[0];
    if ($_[0] != 15) { 
	$self->{"cloud_".$_[1]."_flag"} = $self->getUncheckedFlag();
    }
}
