...
sub getStateCodeFromFile {
    ...
    if (-e "/work/software/conversion_modules/Version6/") {
        open(FILE, sprintf("/work/software/conversion_modules/Version6/%s.states",lc($country)))
	    || die(sprintf("Cannot open the %s.states file\n",lc($country)));
    } elsif (-e "/net/work/software/conversion_modules/Version6/") {
        open(FILE, sprintf("/net/work/software/conversion_modules/Version6/%s.states",lc($country)))
            || die(sprintf("Cannot open the %s.states files\n",lc($country)));
    } else {
        die("Cannot find the (/net)/work/software/conversion_modules/Version6 directory\n");
    }
    while(<FILE>) {
	$_ =~ m/^(.*),(.*)$/;
	my $code = $1;
