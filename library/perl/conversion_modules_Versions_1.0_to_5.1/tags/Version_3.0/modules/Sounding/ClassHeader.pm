...
sub getLatitude {
	...
	$self->{"station"}->getLatitude() <= -999 ? 99.0 : 
...
sub new {
    my $invocant = shift;
    if (scalar(@_) > 1) { die("Invalid parameters to ClassHeader->new\n"); }

    my $self = {};
    my $class = ref($invocant) || $invocant;
    bless($self,$class);

    $self->{"station"} = defined($_[0]) ? $_[0] : Station::Station->new();
...
sub setLine {
    my $self = shift;
    if (scalar(@_) != 2) { die("Invalid parameters to setLine\n"); }
