...
package Station::TestPositionedStationMap;
use strict;
use lib "..";
use TestModule;
...
sub main {
    my $tester = Station::TestPositionedStationMap->new();

    my @stations = buildStations();

    my $psm = Station::PositionedStationMap->new();
    ...
    $tester->assertString($stations[0]->toString(),
			 $psm->getStation("ID 1","NET A",0,0)->toString(), 
			  "Get 1 A");
