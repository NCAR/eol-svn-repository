...
##Module--------------------------------------------------------------------------
# <p>The PrecipHourlyRecord is a PrecipRecord that contains hourly precipitation
...
##Module--------------------------------------------------------------------------
package Precip::PrecipHourlyRecord;
use strict;
use lib "..";
...
##--------------------------------------------------------------------------------
sub getDate {
    if (@_ != 1) { die("Invalid parameters to PrecipHourlyRecord->getDate\n"); }
    my ($self) = @_;
...
##--------------------------------------------------------------------------------
sub getPrecip {
    if (@_ != 3) { die("Invalid parameters to PrecipHourlyRecord->getPrecip\n"); }
    my ($self,$time,$fmt) = @_;
...
##--------------------------------------------------------------------------------
sub getPrecipFlag {
    if (@_ != 3) { die("Invalid parameters to PrecipHourlyRecord->getPrecipFlag\n"); }
    my ($self,$time,$fmt) = @_;
...
##--------------------------------------------------------------------------------
sub getQCFFlag {
    if (@_ != 3) { die("Invalid parameters to PrecipHourlyRecord->getQCFFlag\n"); }
    my ($self,$time,$fmt) = @_;
...
##--------------------------------------------------------------------------------
sub getTime {
    if (@_ != 1) { die("Invalid parameters to PrecipHourlyRecord->getTime\n"); }
    my ($self) = @_;
...
# <p>Determine if the specified time is a nominal hourly time.</p>
#
...
##--------------------------------------------------------------------------------
sub isNominalTime {
    if (@_ != 3) { die("Invalid parameters to PrecipHourlyRecord->isNominal\n"); }
    my ($self,$time,$fmt) = @_;
...
##--------------------------------------------------------------------------------
sub setReadingTime {
    if (@_ != 3 && @_ != 6) { die("Invalid Parameters to PrecipHourlyRecord->setReadingTime\n"); }
    my ($self,$date,$fmt,$hour_off,$min_off,$sec_off) = (@_,0,0,0);
...
##--------------------------------------------------------------------------------
sub setPrecip {
    if (@_ != 5) { die("Invalid parameters to PrecipHourlyRecord->setPrecip\n"); }
	...
	printf({$self->{"warn"}} "Precip %s for time %s %s cannot be set.  Time is not a nominal hour.\n",$precip,$self->getDate(),$time);
    }
}
...
##--------------------------------------------------------------------------------
sub setPrecipFlag {
    if (@_ != 4) { die("Invalid parameters to PrecipHourlyRecord->setPrecip\n"); }
    my ($self,$time,$fmt,$flag) = @_;

    if (!$self->isNominalTime($time,$fmt)) {
	printf({ $self->{"warn"}} "Precip flag %s for time %s %s cannot be set.  Time is not a nominal hour.\n",$flag,$self->getDate(),$time);
...
sub setQCFFlag {
    ...
    if (!$self->isNominalTime($time,$fmt)) {
	printf({$self->{"warn"}} "QCF Flag %s for time %s %s cannot be set.  Time is not a nomianl hour.\n",$flag,$self->getDate(),$time);
...
##--------------------------------------------------------------------------------
sub toString {
    if (@_ != 1) { die("Invalid parameters to PrecipHourlyRecord->toString\n"); }
    my ($self) = @_;
