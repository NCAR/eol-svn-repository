...
sub getStateCodeFromFile {
    ...
    if (-e "/work/lib/perl/Utilities") {
        open(FILE, sprintf("/work/lib/perl/Utilities/%s.states",lc($country)))
	    || die(sprintf("Cannot open the /work/lib/perl/Utilities/%s.states file\n",lc($country)));
    } elsif (-e "/net/work/lib/perl/Utilities") {
        open(FILE, sprintf("/net/work/lib/perl/Utilities/%s.states",lc($country)))
            || die(sprintf("Cannot open the /net/work/lib/perl/Utilities/%s.states file\n",lc($country)));
    } else {
        die("Cannot find the (/net)/work/lib/perl/Utilities directory\n");
    }
    while(<FILE>) {
	$_ =~ m/^(.*),(.*)$/;
	my $code = $1;
