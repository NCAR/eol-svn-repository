...
# @version 5.0 <p>1 September 2011 - Adding back the checks for dewpoints less
#   than -99.9 that were removed in Version 4.0 at Scot Loehrer's request.  
#   Else line lengths are greater than allowable.</p>
#
# @author L. Cully
# @vserion 4.0 <p> 17 Oct 2008 Cully - Per Loehrer's request, removed check that set
#   Dew Points less than -99.99 to -99.99. Now any value that can be fit
#   in the output format is allowed. </p>
#
# @author Joel Clawson
...
#
# <p> 1 September 2011 Echo-Hawk - Adding back checks removed in Oct 2008 to
#   check for dew points less than -99.9 and converting them to -99.9,
#   at Scot Loehrer's request.</p>
#
# <p> 17 Oct 2008 Cully - Per Loehrer's request, removed check that set
#   Dew Points less than -99.99 to -99.99. Now any value that can be fit
#   in the output format is allowed. </p>
#
...
sub check_values {
	...
	printf($WARN
           "%s: Dew Point value %s is less than -99.9 at time %s (temp=%s RH=%s).  Setting to -99.9 when ClassRecord.pm toString calls check_values().\n",
           $self->{"filename"},$self->getDewPoint(),$self->getTime(), $self->getTemperature(),$self->getRelativeHumidity());
