...
#
# @author L. Echo-Hawk
# Changed toString to check actual latitude/longitude values for negative
# values (to apply "S" or "N") rather than checking the degrees portion. This
# is due to a problem with convertLatLong (DpgConversions.pm) which multiplies
# the degrees portion of a negative value by -1.  When the degrees value equals
# zero, this results in a value of zero rather than a negative value. The
# code in convertLatLong needs to be fixed after a complete investigation of
# the effects of any change.  In the meantime this change to toString solves
# the problem of incorrect header values in sounding conversions.
#
# @author L. Cully
...
#
# <p>See the note in the code about the change to patch the
# problem with convertLatLong (DpgConversion.pm) which causes
# the degrees portion of negative zero latitudes to be zero (not
# negative) and thus results in appending "N" rather than "S" 
# to the value.</p>
#
...
sub toString {
    ...
    # -------------------------------------------------------------------------------
	# NOTE: This is a fix for the call to convertLatLong (DpgConversion.pm) 
	# which checks to see if there is a minus sign for the latitude (or longitude)
	# value, and if so, multiplies the degrees by "-1", which works for all values
	# except negative zero degrees. With negative zero values, after calling
	# convertLatLong, the minutes will never be negative, and so the call
	# has been changed to check the actual latitude rather than the degrees
	# to see if the value is negative.  That is why we were seeing values such 
	# as "-0.400" with "N" instead of "S".  This patch is in place in lieu of a
	# permanent fix to convertLatLong which will be made at a future time.
    # -------------------------------------------------------------------------------
	my $lat_dir = $self->getLatitude() < 0.000 ? "S" : "N";
    # my $lat_dir = $lat_deg < 0.000 ? "S" : "N";
    my $lon_dir = $self->getLongitude() < 0 ? "W" : "E";
    # my $lon_dir = $lon_deg < 0.0 ? "W" : "E";
	#
    # ---------------------------------------------------    
