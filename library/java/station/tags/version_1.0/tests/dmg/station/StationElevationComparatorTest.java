...
public class StationElevationComparatorTest {

	private static StationElevationComparator comparator;
	
	@BeforeClass public static void setup() { 
		comparator = new StationElevationComparator();
	}
	
	@Test public void elevationComparisons() throws ConversionException, 
	InvalidValueException, RestrictedOperationException {
		...
		assertTrue("Elevation: null, null",
				comparator.compare(first, second) == 0);
		assertTrue("Elevation: null, null",
				comparator.compare(second, first) == 0);
		
		// Test when one latitude is null
		first.setElevation(10.0, METERS);
		assertTrue("Elevation: value, null",
				comparator.compare(first, second) > 0);
		assertTrue("Elevation: null, value",
				comparator.compare(second, first) < 0);
		
		// Test when the latitudes have different values.
		second.setElevation(-10.0, METERS);
		assertTrue("Elevation: big, little",
				comparator.compare(first, second) > 0);
		assertTrue("Elevation: little, big",
				comparator.compare(second, first) < 0);
		
		// Test when the latitudes have the same value.
		second.setElevation(10.0, METERS);
		assertTrue("Elevation: value, value",
				comparator.compare(first, second) == 0);
		assertTrue("Elevation: value, value",
				comparator.compare(second, first) == 0);
	}
