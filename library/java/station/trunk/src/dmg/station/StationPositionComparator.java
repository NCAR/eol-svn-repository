...
/**
 * <p>The StationPositionComparator is a StationComparator used for comparing
 * two Stations for sort order using the latitude, longitude, station identifier,
 * and network name.  It is to be used with a StationList when it does not make
 * sense to use the elevation, but requires more than just the identifier and
 * network name to differentiation between the Stations.</p>
 * 
 * @author Joel Clawson
 */
public class StationPositionComparator extends StationNameComparator {

    /**
     * Compare two Stations for sort order.  A Station is ordered by latitude,
     * longitude, station identifer and network name.
