C
C $Id: dp2e.f,v 1.6 1995/12/06 21:16:30 john Exp $
C

	PROGRAM DP2E
C
C DP2E CONVERTS A DAILY PQCF FILE TO AN EBUFR FILE.
C
C Synopsis: dp2e [<pqcf_data_file>]
C
...
C  06 Jul 1994 by John J. Allison
C    converted p2e into dp2e; didn't include daily as an option of p2e
C    due to reading/writing differences that would require too many
C    cumbersome if statements
...
C
C Description:
C  This program converts a DAILY PQCF (Precip QCF) file into an EBUFR file.
C It was modified from P2E, which
C was modified from Q2E, which converts QCF to EBUFR. DP2E uses the
...
C    assumed to be named "control_dpqcf.txt"
...
C Get QCF filename from command line
	...
	QCFFIL = ARGV
C QCF filename is in form of file.pqc or file
...
&	    (QCF(1) .EQ. 'M')) THEN
	  ...
	  IDATA(4) = 15
	ELSE
C	  IF (QCF(1) .EQ. 'T') THEN
C BUFR says -1.0 means Trace
C  (see p.I-Bi-73 of WMO Manual, 1988 edition, Suppl No. 3 (VIII.1991))
C EXCEPT THAT THE BUFR MANUAL IS BROKEN SO -1.0 WILL NOT WORK
C	    RDATA(1) = -1.0
C	  ELSE
	    RDATA(1) = PRECIP(1)
C	  END IF
...
&	      (QUAL(J) .EQ. IDATA(3+(K-1)*4)) .AND.
&	      (QCFLAG .EQ. IDATA(4+(K-1)*4)) .AND.
&	      (OBSHR(J) .EQ. IDATA(5+(K-1)*4))) THEN
	    CT = CT + 1
	  ELSE
	    IF (OBSHR(J) .GT. 24) THEN
	      IDATA(5+K*4) = IMISS
	    ELSE
	      IDATA(5+K*4) = OBSHR(J)
	    END IF
...
&	  PRECIP,QUAL,QCF,OBSHR,IOS)
	...
	IF (IOS .NE. 0) THEN
	  RETURN
	END IF
	READ (BUFFER,1002) TIME(1),TIME(2),NETSTR,STNSTR,LATLON,CLIBYT,
&	  (PRECIP(K), QUAL(K), QCF(K), OBSHR(K), K=1,31)
1002	FORMAT (I4,'/',I2,
&	    X,A10,X,A15,
&	    X,F10.5,X,F11.5,X,I3,31(: X,F7.2,X,I1,X,A1,X,I2))
