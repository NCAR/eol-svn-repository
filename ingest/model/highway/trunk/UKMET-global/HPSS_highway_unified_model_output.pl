...
##Module-----------------------------------------------------------------------
## <p>The HPSS_highway_unified_model_output.pl script is a daily cron script that does the following
## tasks:</p>
## <ol>
##   <li>Create a daily tar.gz file for 572.006: UK Met Office Unified Model Output (Global 
##   Configuration) over the Lake Victoria Region.</li>  
...
my $dataset_id = "572.006";
my $ingest = "/net/work/Projects/HIGHWAY/data_ingest/model/UKMET/data/FieldCampaign";
my $temp = "/scr/tmp/joss/highway/global";
my $hpss = "/FS/EOL/2019/highway/model/ukmet/global";
...
# Verify that there are files to tar
   opendir(PDIR,"$ingest");
   my @files = grep(/^$year$month$day.+_n1280_ga6/,readdir(PDIR));
   closedir(PDIR);
   if ($#files < 0) {
      sendMailAndDie("There are no data files to tar for $month/$day/$year.\n");
   } 
   my $tar_file = "HIGHWAY_UKMET_n1280_ga6_$year$month$day.tar.gz";
   my $result = system("tar czf $temp/$tar_file $year$month$day*_n1280_ga6_*");
   if ($result != 0) {
...
{
   my ($body) = @_;
   MAIL::send_mail("HIGHWAY UKMET Unified Model Output- ERROR",$0."\n\n".$body, @monitors);
   exit(1);
}
