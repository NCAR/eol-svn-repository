...
##
##----------------------------------------------------------------------
## RUN THIS SCRIPT MANUALLY AS THE JOSS USER
##----------------------------------------------------------------------
## <p>On "Power Down" days this script doesn't run and you will see a missing log 
## file from 9 days earlier here: /net/work/operational/precip/NCEP_EMC/ingest-NOMADS. 
## You can modify this script find the correct day to be downloaded, then run it 
## manually as the joss user. You will only have a couple of days to do this before 
## the data is scrubbed and no longer available. Janet's comments are below:
##
## In order to rerun the NCEP process, you will need to make one change in the script.
## This line needs to change:
##
##    my $day_to_download = $dt - DateTime::Duration->new( days => 9 );
##
##    If you run one day later, then change the 9 to 10, two days later, then change 9
##    to 11, etc.  Note that these files are no longer available on their web site
##    after 13 days, so there is not much time to rerun this process.  
##
##    What I would suggest is make a copy of the script in the same directory, then
##    change the line in the copy and run that script from the command line as the joss
##    user. 
##
...
sub main {
    my $dt = DateTime->now;
    my $day_to_download = $dt - DateTime::Duration->new( days => 10 );
