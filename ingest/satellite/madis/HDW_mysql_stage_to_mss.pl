...
#my $ingest = "/ingest/ldm/data/madis/point/HDW/netcdf";
my $ingest = "/scr/satellite/ldm/data/madis/point/HDW/netcdf";
my $temp = "/net/ingest/tmp/madis/HDW";
my $mss = "/JOSS/DATA/RAW/SATELLITE/HDW";
...
sub main {
    ...
    if ($report ne "") { send_mail("LDM MADIS HDW Satellite Winds MSS/CODIAC Script Error\n",$report); }
    #else { send_mail("LDM MADIS HDW Satellite Winds MSS/CODIAC Notification\n","There are no errors to report"); }
}
...
sub sendMailAndDie {
    my ($body) = @_;
    send_mail("LDM MADIS HDW Satellite Winds MSS/CODIAC Error",$body);
    exit(1);
}
