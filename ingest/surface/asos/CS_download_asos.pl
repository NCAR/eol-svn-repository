...
#
# Download all the data files by file type
#
for (my $i = 0; $i < 3; $i++) {
   download_files($year, $month, $FTP_DIR[$i]);
#
# Create the tar/zip files for each data type
#
   $tarfiles[$i] = create_tar_file($year, $month, $INFILENAME[$i], $OUTFILENAME[$i]);
}
#
# Remove extra files from the download directory.  Don't save
# the log file unless there has been an error.
#
unlink glob("$DOWNLOAD_DIR/robots.txt*");
#unlink glob("$DOWNLOAD_DIR/logs/log*");
#
...
#
# Create checksums for all the new tarfiles
#
my $checksum_res = system("ssh $CS_USER python3 eoldata-utils/cksum/checksum_utility.py -n -d $CS_SUBDIR[$i] >/dev/null");
if ($checksum_res != 0) {
