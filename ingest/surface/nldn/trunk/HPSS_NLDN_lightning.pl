...
##Module-----------------------------------------------------------------------
# <p>The HPSS_NLDN_lighning script is a daily cron script that does the following
# tasks:</p>
# <ol>
#   <li>Copy the local NLDN  to the HPSS (aka mass store).  (This removes the 
#   tar file from the local system if it was copied correctly.)</li>
#   <li>Insert the HPSS files into the database.</li>
# </ol>
#
...
#        /export/ldm/data/lightning/nldn. Daily tar files 
...
my $ingest = "/export/ldm/data/lightning/nldn";
...
sub main {
    ...
    my $year_cs = substr($tar_files[0],5,4);
    my $mkdir_out = mkdir_CS($year_cs);
    ...
    my $my_cs_chmod = `ssh $cs_host chmod 440 $cs_archive/$year_cs/* 2>&1`;
    $report .= $cs_msg;
    $report .= $my_cs_chmod;
 	...
 	if ($msg eq "") {
	    ...
	    if ($msg eq "") {
		my $size = -s "$temp/$file";
		# $report .= "HPSS filesize for $file: $hpss_size eq? ".($size/1024)."\n";
		if ($hpss_size == (-s "$temp/$file")/1024) { 
		    unlink "$temp/$file"; 
		}
	    }
...
sub create_tar_files {
    ...
    my $julian_today = sprintf("%03d", convertJulian($year+1900,$day,$mon+1));
	...
	push(@files,$file) if ($file =~ /^nldn_\d{13}.bin$/ && substr($file,9,3) < $julian_today);
    }
    closedir($INGEST);
	...
	# NOTE: Temporarily hard-coded the year
	# my $file_year = "2019";
	# changed to use the year from the first file name
...
sub insert_file {
    ...
    $mysql->setFile(sprintf("%s/%04d",$hpss,substr($file,5,4)),$file);
...
sub sendMailAndDie {
    ...
    print "LDM NLDN Lightning HPSS/CODIAC Error ",$0."\n\n".$body, @monitors;
    exit(1);
}
