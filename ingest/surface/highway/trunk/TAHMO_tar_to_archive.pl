...
##Module-----------------------------------------------------------------------
# <p>The HIGHWAY TAHMO_tar_to_archive.pl script is a daily cron script 
...
# Constants
my $dataset_id = "572.004";
my $archive = "/net/archive/data/highway/surface/TAHMO";
...
sub create_tar_files {
    ...
    # TAHMO data directories for 572.004
    my $tahmo_dir = "TAHMO/data/$odate/";
    
    # SYNOP TAHMO archive directory
    my $tahmo_arch_dir = "/net/archive/data/highway/surface/TAHMO";

    # ----------------------------------------------
    # create and move TAHMO file to archive location
    # ----------------------------------------------
    # print "tar'ing TAHMO directories\n";
    system("/bin/tar cf HIGHWAY_TAHMO_$odate.tar $tahmo_dir");
    # system("/bin/tar cvf HIGHWAY_TAHMO_$odate.TEST.tar $tahmo_dir");

    system("/bin/mv HIGHWAY_TAHMO*tar $tahmo_arch_dir");

    my $file = "HIGHWAY_TAHMO_$odate.tar";
    # my $file = "HIGHWAY_TAHMO_$odate.TEST.tar";
    push(@tar_files, $file);
...
sub sendMailAndDie {
    my ($body) = @_;
    send_mail("HIGHWAY TAHMO Error Report",$0."\n\n".$body);
    exit(1);
}
