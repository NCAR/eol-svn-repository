...
##Module-----------------------------------------------------------------------
# <p>The HIGHWAY SYNOP_BUFR_tar_to_archive.pl script is a daily cron script 
...
# Constants
my $dataset_id = "572.003";
my $archive = "/net/archive/data/highway/surface/SYNOP/BUFR";
...
sub main {
    ...
    foreach my $file (sort(@tar_files)) {
		...
		if ($msg eq "") {
			...
			# $msg .= "File $file inserted into DB $dataset_id\n";
			$report .= $msg;
		}
		else {
			$report .= $msg;
...
sub create_tar_files {
    ...
    #SYNOP BUFR data directories for 572.003
    my $hknc_synop_bufr_dir = "SYNOP/HKNC/BUFR/$date/";
    
    #SYNOP BUFR archive directory
    my $synop_bufr_dir = "/net/archive/data/highway/surface/SYNOP/BUFR";

    # ---------------------------------------------------
    # create and move SYNOP BUFR file to archive location
    # ---------------------------------------------------
    # print "tar'ing SYNOP BUFR directories\n";
    system("/bin/tar cf HIGHWAY_SYNOP_BUFR_$date.tar $hknc_synop_bufr_dir");
    # system("/bin/tar cvf HIGHWAY_SYNOP_BUFR_$date.TEST.tar $hknc_synop_bufr_dir");
  
    system("/bin/mv HIGHWAY_SYNOP_BUFR*tar $synop_bufr_dir");
	
    my $file = "HIGHWAY_SYNOP_BUFR_$date.tar";
    # my $file = "HIGHWAY_SYNOP_BUFR_$date.TEST.tar";
    push(@tar_files, $file);
...
sub insert_file {
	...
	# HIGHWAY_SYNOP_BUFR_20190409.tar
    ...
    $mysql->setBeginDate(substr($file,19,4),substr($file,23,2),substr($file,25,2),0,0,0);
    $mysql->setEndDate(substr($file,19,4),substr($file,23,2),substr($file,25,2),23,59,59);
...
sub sendMailAndDie {
    my ($body) = @_;
    send_mail("HIGHWAY SYNOP BUFR Error Report",$0."\n\n".$body);
    exit(1);
}
