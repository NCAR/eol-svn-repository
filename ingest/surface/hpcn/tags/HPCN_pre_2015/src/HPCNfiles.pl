...
my $recipients = "orin\@ucar.edu,cully\@ucar.edu,loehrer\@ucar.edu";
#my $recipients = "orin\@ucar.edu";	# For testing purposes
my $report = "";
		...
		my $oldYear = $year;
   		for (my $i = 2; $i >= 0; $i--)
   		{
			...
			if ( $halfYear ne $monthRange.$curYear ) {
				#print "Backing up HTMLInventory for half-year $monthRange of $curYear...";
				printAndLog("Backing up HTMLInventory for half-year $monthRange of $curYear...");
...
##		}
		...
		if ( $halfYear ne $monthRange.$year ) {
			#print "Backing up HTMLInventory for half-year $monthRange of $year...";
			printAndLog("Backing up HTMLInventory for half-year $monthRange of $year...");
...
sub createMonthlyInventoryFile {
	...
	if (-e $fileName) {
		...
		print INVENTORY	 "\t<li> Cells with a <font color=\"purple\"><b>purple</b></font> background in the ratio column have a compression ratio (from the compressed and uncompressed file sizes) greater than 10</li>\n";	
